...
local function runOnTreadmill(part,seconds,pet,rarity,targetTotal)
	...
	if part.Size.Z >= part.Size.X then
		baseDir=part.CFrame.LookVector
	else
		baseDir=part.CFrame.RightVector
	end
	...
	local edge=math.max(3,length*0.36)
	local center=Vector3.new(part.Position.X,part.Position.Y+3.15,part.Position.Z)

	local oldSpeed=hum.WalkSpeed
	local oldAutoRotate=hum.AutoRotate

	-- v17: без постоянных телепортов и без жёсткой Velocity.
	-- Бежит туда-сюда параллельно дорожке, чтобы игра видела нормальный Humanoid-бег.
	hum.WalkSpeed=speed
	hum.AutoRotate=true

	local runDir=baseDir
	local startPos=center-runDir*edge
	r.CFrame=CFrame.lookAt(startPos,startPos+runDir)
	...
	while not _G.PetAlignStop and os.clock()-started<seconds do
		...
		local flatPos=Vector3.new(r.Position.X,center.Y,r.Position.Z)
		local offset=flatPos-center
		local along=offset:Dot(baseDir)

		-- Пинг-понг без телепорта: дошёл до края -> разворот.
		if along>edge then
			runDir=-baseDir
		elseif along<-edge then
			runDir=baseDir
		end

		-- Мягко тянем к центру ширины дорожки, чтобы не съезжал боком.
		local projected=baseDir*along
		local lateral=offset-projected
		local correction=Vector3.zero
		if lateral.Magnitude>0.15 then
			correction=-lateral.Unit*math.min(lateral.Magnitude*0.22,0.65)
		end

		local move=(runDir+correction)
		if move.Magnitude<0.05 then move=runDir end
		move=move.Unit

		hum:Move(move,false)

		-- Только аварийный фикс если его реально выбросило с дорожки.
		if offset.Magnitude>edge+8 and os.clock()-lastHardFix>1.2 then
			lastHardFix=os.clock()
			local safe=center-runDir*edge
			r.CFrame=CFrame.lookAt(safe,safe+runDir)
		end
...
close.MouseButton1Click:Connect(function() _G.PetAlignStop=true running=false gui:Destroy() end)
...
status.Size=UDim2.new(1,-112,0,30)
status.Position=UDim2.new(0,14,1,-36)
status.BackgroundTransparency=1
status.Text="Клик = постоянный бег по дорожке, потом камень"
...
stopBtn.Size=UDim2.new(0,76,0,26)
stopBtn.Position=UDim2.new(1,-90,1,-36)
stopBtn.Text="STOP"
...
local function useTreadmill(gain,count)
	...
	-- один заход на дорожку + постоянный бег со скоростью 30.
	local seconds=math.max(1.5,count*(_G.PetAlignRunSecondsPerUse or 0.45))
	status.Text="Постоянный бег +"..gain..fallbackInfo.." ~"..string.format("%.1f",seconds).."с"
...
local function runAlign()
	...
	status.Text="Ровняю стабильным бегом: "..planText(plan)
	_G.PetAlignStop=false
