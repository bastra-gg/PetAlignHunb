-- PetAlignHub v29_ROCK_EXACT_RUN_FIX
...
local function chooseLargestPartNear(modelOrObj,nearPos)
	...
	local function consider(p)
		...
		local score=s-d*2
		if p.Anchored then score+=100 end
...
local function findRockByLabel(rockId)
	...
	for _,obj in ipairs(workspace:GetDescendants())do
		if obj:IsA("TextLabel")or obj:IsA("TextButton")or obj:IsA("TextBox")then
			...
			for _,name in ipairs(names)do
				if txt:find(tostring(name):lower(),1,true)then hit=true break end
			end

			if hit and not hasBadRockWord(txt)then
				...
				local model=parent and (parent:FindFirstAncestorWhichIsA("Model") or parent) or obj.Parent
				local nearPos=(adornee and adornee:IsA("BasePart") and adornee.Position) or (hrp and hrp.Position) or nil
				local part=nil

				if adornee and adornee:IsA("BasePart")then
					part=adornee
				elseif model then
					part=chooseLargestPartNear(model,nearPos)
				end

				if part then
					local allText=(objectPath(part).." "..objectText(model or part).." "..txt):lower()
					if not hasBadRockWord(allText)then
						local score=500
						if part.Anchored then score+=50 end
						if hrp then score-=math.clamp((part.Position-hrp.Position).Magnitude/500,0,40)end
						if score>bestScore then
							bestScore=score
							best=part
						end
...
local function findRockPart(rockId)
	-- 1) Сначала ищем по надписи над скалой: "Большой камень", "Golden Rock" и т.д.
	local byLabel=findRockByLabel(rockId)
	if byLabel then return byLabel end

	-- 2) Потом ищем по имени/пути объекта, но жёстко режем тренажёры/бросалки.
	local names=ROCK_EXACT[rockId] or {}
	local keys=nil
	...
	for _,obj in ipairs(workspace:GetDescendants())do
		...
		if part and part:IsA("BasePart")then
			local m=obj:IsA("Model") and obj or obj:FindFirstAncestorWhichIsA("Model") or obj
			local txt=(objectPath(part).." "..objectText(m).." "..objectText(obj)):lower()

			if not hasBadRockWord(txt)then
				local score=0
				for _,name in ipairs(names)do
					if txt:find(tostring(name):lower(),1,true)then score+=260 end
				end
				for _,w in ipairs(keys)do
					if txt:find(tostring(w):lower(),1,true)then score+=55 end
				end

				if txt:find("rock",1,true)or txt:find("rocks",1,true)then score+=100 end
				if txt:find("камень",1,true)or txt:find("скал",1,true)then score+=100 end
				if txt:find("workspace/rocks",1,true)or txt:find("/rocks/",1,true)then score+=180 end
				if part.Anchored then score+=25 else score-=70 end
	...
	if best and bestScore>120 then return best end
	return nil
end
