-- PetAlignHub v41_EXTRACTED_METHODS
...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v41 rock scan")
...
local function runOnTreadmill(part,targetTotal,rarity)
	...
	if cam then
		pcall(function()
			...
			cam.CFrame=CFrame.lookAt(startPos-dir*14+Vector3.new(0,7,0),startPos+dir*10)
		end)
	end
	...
	if cam and oldCamType then
		pcall(function()
			...
			if oldCamCF then cam.CFrame=oldCamCF end
		end)
	end
...
title.Text="Pet Align Comfort v41"
...
if okMath then
	statusText("v41: SpeedHub-метод W key + punch remote.")
else
