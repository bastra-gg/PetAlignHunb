...
local function runOnTreadmill(part,seconds,pet,rarity,targetTotal)
	...
	local runOffset=math.clamp(length*0.42,3,math.max(3,length/2-1.5))

	-- v24: посадка считается от верхней поверхности конкретной дорожки.
	-- Можно вручную подкрутить: _G.PetAlignHeightOffset = 3.1
	local up=part.CFrame.UpVector
	local heightAbove=_G.PetAlignHeightOffset or math.clamp((hum.HipHeight or 2)+1.25,2.85,4.25)
...
local function render()
	...
	status.Text="Карточка = ровнять. После ровно жми GO BUG."
end
...
local function useTreadmill(gain,count)
	...
	status.Text="Прямой бег +"..gain..fallbackInfo.." ×"..count.." ~"..string.format("%.1f",seconds).."с"
...
local function runAlign()
	...
	status.Text="Ровняю до "..fmt(current.startTotal)..": "..planText(plan)
	_G.PetAlignStop=false
