...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v40 rock scan")
...
local function runOnTreadmill(part,targetTotal,rarity)
	...
	-- v40: один телепорт на дорожку, дальше только Humanoid:Move/MoveTo.
	-- Никакого AssemblyLinearVelocity, поэтому сопротивление дорожки не игнорируется.
	...
	while not _G.PetAlignStop do
		...
		-- Резервный пинок через обычный Humanoid MoveTo, не через физическую скорость.
		if os.clock()>=nextMoveTo then
			nextMoveTo=os.clock()+0.45
			pcall(function()
...
title.Text="Pet Align Comfort v40"
...
if okMath then
	statusText("v40: безопасный запуск + бег без форс-скорости.")
else
