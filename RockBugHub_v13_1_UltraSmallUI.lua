...
for _,name in ipairs(FALLBACK_SHOP_PETS) do
	SHOP_KNOWN_PET_NAME_SET[string.lower(name)]=true
end
...
local function shopContainers()
	...
	local function remember(item)
		if item and not seen[item] and not item:IsA("RemoteEvent") and not item:IsA("RemoteFunction") then
			seen[item]=true
	...
	for _,child in ipairs(ReplicatedStorage:GetChildren()) do
		...
		if lower:find("pet",1,true) and (
			lower:find("shop",1,true) or lower:find("catalog",1,true)
			or lower:find("definition",1,true) or lower:find("data",1,true)
		) then
			remember(child)
		elseif lower=="shops" or lower=="shop" or lower=="shared" or lower=="assets"
			or lower=="data" or lower=="catalog" then
			for _,nested in ipairs(child:GetChildren()) do
				local nestedName=string.lower(tostring(nested.Name))
				if nestedName:find("pet",1,true) or nestedName:find("aura",1,true)
					or nestedName:find("shop",1,true) then
					remember(nested)
				end
			end
		end
...
local function isShopCategory(item)
	...
	if name=="pets" or name=="pet" or name=="auras" or name=="aura"
		...
		or name=="mythic" or name=="secret" or name=="limited" or name=="new" then
		return true
	end
	if not item:IsA("Folder") and not item:IsA("Configuration") then return false end
	if SHOP_KNOWN_PET_NAME_SET[name] or SHOP_AURA_NAME_SET[name] then return false end
	for _,field in ipairs({"priceValue","Price","price","Cost","cost","GemPrice","gemPrice","gemsCost"}) do
...
local function shopCatalog(kind)
	...
	local function inspect(folder,depth,inheritedKind)
		if not folder or depth>4 then return end
			...
			if not item:IsA("RemoteFunction") and not item:IsA("RemoteEvent")
				...
				if isShopCategory(item) then
					...
					local nextKind=(lower:find("aura",1,true) or lower:find("trail",1,true)) and "aura"
						or (lower:find("pet",1,true) and "pet" or inheritedKind)
					inspect(item,depth+1,nextKind)
				else
					local name=tostring(item.Name)
					local key=string.lower(name)
					local aura=inheritedKind=="aura" or (inheritedKind~="pet" and isAuraShopItem(item))
					if name~="" and ((kind=="aura" and aura) or (kind=="pet" and not aura))
						...
						table.insert(result,{
							...
							item=item,
							isNew=kind=="pet" and not SHOP_KNOWN_PET_NAME_SET[key],
						})
					end
				end
			end
...
local function findShopItem(name,kind)
	...
	for _,entry in ipairs(shopCatalog(kind or "pet")) do
		if string.lower(entry.name)==wanted and entry.item and entry.item.Parent then
...
local function shopRemoteCandidates()
	...
	local events=ReplicatedStorage:FindFirstChild("rEvents")
	for _,container in ipairs({ReplicatedStorage,events}) do
		if container then
			for _,name in ipairs({
			...
			for _,remote in ipairs(container:GetChildren()) do
				local lower=string.lower(tostring(remote.Name))
				if (lower:find("pet",1,true) or lower:find("aura",1,true)
					or lower:find("shop",1,true)) and (lower:find("buy",1,true)
					or lower:find("purchase",1,true) or lower:find("shop",1,true)) then
					add(remote)
				end
			end
		end
		...
		if node:IsA("TextButton") or node:IsA("ImageButton") then
			...
			if buttonName:find("buy",1,true) or buttonName:find("purchase",1,true)
				...
				for _=1,5 do
					...
					if string.lower(tostring(current.Name or ""))==wanted then matched=true end
					for _,sibling in ipairs(current:GetChildren()) do
						if sibling:IsA("TextLabel") or sibling:IsA("TextButton") then
							local read,labelText=safe(function() return string.lower(tostring(sibling.Text or "")) end)
							if read and labelText==wanted then matched=true break end
						end
					end
					current=current.Parent
				end
...
local function shopRequestArguments(strategy,item,kind)
	local name=tostring(item.Name)
	local action=kind=="aura" and "Aura" or "Pet"
	if strategy=="object" then return {item} end
	if strategy=="named" then return {name} end
	if strategy=="objectQuantity" then return {item,1} end
	if strategy=="buyObject" then return {"buy"..action,item} end
	if strategy=="purchaseObject" then return {"purchase"..action,item} end
	if strategy=="buyItem" then return {"buyItem",item} end
	if strategy=="purchaseItem" then return {"purchaseItem",item} end
	if strategy=="buyNamed" then return {"buy"..action,name} end
	if strategy=="purchaseNamed" then return {"purchase"..action,name} end
	if strategy=="genericBuy" then return {"buy",item} end
	if strategy=="genericPurchase" then return {"purchase",item} end
	return nil
end

local function tryShopRoute(remote,strategy,item,kind,beforeCount,beforeGems)
	local args=shopRequestArguments(strategy,item,kind)
...
local function buyShopItemOnce(name,kind)
	...
	if remembered and remembered.remote and remembered.remote.Parent then
		local ok,message,terminal=tryShopRoute(remembered.remote,remembered.strategy,item,kind,beforeCount,beforeGems)
		if ok then return true,nil,item.Name end
		if terminal then return false,message,item.Name end
		lastError=message
		Runtime.shopPurchaseRoute[kind]=nil
	end

	local remotes=shopRemoteCandidates()
	if #remotes==0 then return false,"remote покупки не найден",item.Name end
	local strategies={
		"object","buyObject","purchaseObject","buyItem","purchaseItem",
		"objectQuantity","named","buyNamed","purchaseNamed","genericBuy","genericPurchase",
	}
	...
	for index,remote in ipairs(remotes) do
		for _,strategy in ipairs(strategies) do
			if not remembered or remembered.remote~=remote or remembered.strategy~=strategy then
				local ok,message,terminal=tryShopRoute(remote,strategy,item,kind,beforeCount,beforeGems)
				if ok then
					Runtime.shopPurchaseRoute[kind]={remote=remote,strategy=strategy}
					return true,nil,item.Name
				end
				lastError=message or lastError
				if terminal then return false,message,item.Name end
			end
			if index==1 and strategy=="object" then
				local clicked,clickError=triggerActualShopButton(item.Name,kind,beforeCount,beforeGems)
				if clicked then return true,nil,item.Name end
				if clickError then lastError=clickError end
			end
		end
	end
	return false,lastError or "сервер отклонил все способы покупки",item.Name
end
...
local function startCrystalAutomation(mode)
	...
	if mode~="crystal" then
		if not findShopItem(targetName,mode) then
