	...
	for _,folderName in ipairs(folders) do
		...
		if folder then
			for _,entry in ipairs(folder:GetDescendants()) do
				if string.lower(tostring(entry.Name))==target then
					total=total+1
				end
			end
		end
...
local function shopGemBalance()
	for _,container in ipairs({lp,lp:FindFirstChild("leaderstats"),lp:FindFirstChild("stats")}) do
		if container then
...
local function shopPurchaseEvidence(kind,name,beforeCount,beforeGems,response)
	...
	for attempt=1,3 do
		...
		if beforeGems and afterGems and afterGems<beforeGems then return true end
		if attempt<3 then task.wait(0.08) end
	end
	return false,"сервер не подтвердил покупку"
end
...
local function tryShopRoute(remote,strategy,item,kind,beforeCount,beforeGems,selectedName)
	...
	local ok,response=safe(function()
		if remote:IsA("RemoteFunction") then
			return remote:InvokeServer((table.unpack or unpack)(args))
		end
		remote:FireServer((table.unpack or unpack)(args))
		return nil
	end)
...
local function showTab(name)
	...
	rescanBtn.Visible=bug or crystal
	rescanBtn.Text=crystal and "↻ ПЕТЫ" or "↻ КАМНИ"
end
...
addConn(rescanBtn.Activated:Connect(function()
	if activeTab=="crystal" then
		local pets=shopCatalog("pet")
		local fresh=0
		for _,entry in ipairs(pets) do
			if entry.isNew then fresh=fresh+1 end
		end
		normalizeShopSelection("pet")
		normalizeShopSelection("aura")
		refreshExtraUI()
		setStatus("МАГАЗИН: "..#pets.." петов • новых: "..fresh)
		return
	end
