...
local Runtime={
	...
	shopCrystalRoute={},
	shopObservedRoutes={},
	shopHookInstalled=false,
	shopReplaying=false,
	shopPendingReplay=nil,
...
-- The direct shop no longer works reliably. These are the actual crystal pools,
-- so opening the mapped crystal remains a legitimate way to obtain the pet.
local SHOP_ITEM_CRYSTALS={}
for crystal,names in pairs({
	["Blue Crystal"]={"Blue Birdie","Orange Hedgehog","Red Kitty","Dark Vampy","Blue Bunny","Blue Aura","Green Aura","Purple Aura","Red Aura","Yellow Aura"},
	...
	["Inferno Crystal"]={"Red Firecaster","Infernal Dragon","White Pegasus","Inferno","Dark Storm"},
	...
	["Galaxy Oracle Crystal"]={"Eternal Strike Leviathan","Lightning Strike Phantom","Lighting Strike Phantom","Darkstar Hunter","Muscle King","Azure Tundra","Ultra Inferno"},
	...
	["Industrial Crystal"]={"Core Pup","Volt Talon","Reactor Beast","Plasma Ravager","Titan Reactor","Apex Overlord"},
}) do
	for _,name in ipairs(names) do
		SHOP_ITEM_CRYSTALS[string.lower(name)]=crystal
	end
end
...
local function shopInventoryCount(kind,name)
	local target=shopNameKey(name)
	if target=="" then return 0 end
	local total=0
	local folders=kind=="aura" and {"trailsFolder","aurasFolder","Auras","Trails"}
		or {"petsFolder","Pets","pets","petInventory"}
	for _,folderName in ipairs(folders) do
		local folder=lp:FindFirstChild(folderName)
		if folder then
			for _,entry in ipairs(folder:GetDescendants()) do
				if shopNameKey(entry.Name)==target or shopNameKey(shopDisplayName(entry))==target then
					local quantity=1
					for _,field in ipairs({"Amount","amount","Count","count","Quantity","quantity"}) do
						local value=entry:FindFirstChild(field)
						if value then
							local ok,number=safe(function() return tonumber(value.Value) end)
							if ok and number and number>0 then quantity=number break end
						end
						local ok,number=safe(function() return tonumber(entry:GetAttribute(field)) end)
						if ok and number and number>0 then quantity=number break end
					end
...
local function shopInventoryAvailable(kind)
	local folders=kind=="aura" and {"trailsFolder","aurasFolder","Auras","Trails"}
		or {"petsFolder","Pets","pets","petInventory"}
	for _,name in ipairs(folders) do
		if lp:FindFirstChild(name) then return true end
	end
	return false
end

local function shopGemBalance()
	local containers={lp}
	for _,name in ipairs({"leaderstats","stats","Stats","Currencies","currencyFolder","PlayerData","Data"}) do
		local container=lp:FindFirstChild(name)
		if container then table.insert(containers,container) end
	end
	for _,container in ipairs(containers) do
		if container then
			for _,name in ipairs({"Gems","gems","Gem","gem","Diamonds","diamonds"}) do
				local value=container:FindFirstChild(name)
				if value then
					local ok,amount=safe(function() return tonumber(value.Value) end)
...
local function shopResponseResult(response)
	if type(response)=="boolean" then
		return response,response and nil or "сервер отклонил покупку"
	end
	if type(response)=="table" then
		for _,key in ipairs({"success","Success","purchased","Purchased","ok","Ok"}) do
			if type(response[key])=="boolean" then
				return response[key],response[key] and nil
					or tostring(response.message or response.Message or response.error or response.Error or "сервер отклонил покупку")
			end
		end
		if response.error or response.Error then
			return false,tostring(response.error or response.Error)
		end
	end
	if type(response)=="string" then
		local lower=string.lower(response)
		if lower:find("success",1,true) or lower:find("purchased",1,true)
			or lower:find("bought",1,true) then return true end
		if lower:find("error",1,true) or lower:find("fail",1,true)
			or lower:find("not enough",1,true) or lower:find("invalid",1,true)
			or lower:find("denied",1,true) then return false,response end
	end
	return nil
end

local function terminalShopError(message)
	local lower=string.lower(tostring(message or ""))
	for _,fragment in ipairs({
		"not enough","insufficient","недостат","inventory full","capacity",
		"no space","нет места","cooldown","подожд","locked","rebirth",
		"гемы списаны",
	}) do
...
local function shopRemoteCandidates()
	local result={}
	local seen={}
	local searched={}
	local function add(remote)
		if remote then
			local name=string.lower(tostring(remote.Name or ""))
			if name:find("rebirth",1,true) or name:find("ascend",1,true)
				or name:find("prestige",1,true) or name:find("reset",1,true)
				or name:find("delete",1,true) or name:find("sell",1,true) then return end
		end
		if remote and not seen[remote]
			and (remote:IsA("RemoteFunction") or remote:IsA("RemoteEvent")) then
			seen[remote]=true
			table.insert(result,remote)
		end
	end
	local function search(container,depth,inShop)
		if not container or searched[container] or depth>4 then return end
		searched[container]=true
		for _,name in ipairs({
			"cPetShopRemote","petShopRemote","PetShopRemote","buyPetRemote",
			"BuyPetRemote","purchasePetRemote","PurchasePetRemote","buyShopItemRemote",
			"purchaseShopItemRemote","shopPurchaseRemote","ShopPurchaseRemote","shopRemote",
			"BuyPet","PurchasePet","BuyAura","PurchaseAura","ApexShopRemote",
		}) do
			add(container:FindFirstChild(name))
		end
		for _,child in ipairs(container:GetChildren()) do
			local lower=string.lower(tostring(child.Name))
			local product=lower:find("pet",1,true) or lower:find("aura",1,true)
				or lower:find("shop",1,true) or lower:find("apex",1,true)
			local purchase=lower:find("buy",1,true) or lower:find("purchase",1,true)
				or lower:find("shop",1,true)
			if (product or inShop) and purchase then add(child) end
			if not child:IsA("RemoteEvent") and not child:IsA("RemoteFunction")
				and not child:IsA("LocalScript") and not child:IsA("ModuleScript") then
				local network=lower:find("event",1,true) or lower:find("remote",1,true)
					or lower:find("network",1,true) or lower:find("shared",1,true)
				if network or product or inShop then
					search(child,depth+1,inShop or product)
				end
			end
		end
	end
	search(ReplicatedStorage,1,false)
	return result
end

local function shopPurchaseEvidence(kind,name,beforeCount,beforeGems,response)
	local verdict,message=shopResponseResult(response)
	if verdict==false then return false,message end
	local inventoryAvailable=shopInventoryAvailable(kind)
	local gemsSpent=false
	-- A success-shaped reply does not prove anything: several current remotes
	-- return true without creating the selected pet. Require replicated proof.
	for attempt=1,12 do
		if shopInventoryCount(kind,name)>beforeCount then return true end
		local afterGems=shopGemBalance()
		if beforeGems and afterGems and afterGems<beforeGems then
			if not inventoryAvailable then return true end
			gemsSpent=true
		end
		if attempt<12 then task.wait(0.1) end
	end
	if gemsSpent then return false,"гемы списаны, но выбранный питомец не появился" end
	return false,verdict==true and "питомец не появился, гемы не списаны"
		or "покупка не подтверждена игрой"
end

local function shopArgumentMatches(value,wanted,depth,visited)
	if depth>4 or value==nil then return false end
	local valueType=type(value)
	if valueType=="string" then return shopNameKey(value)==wanted end
	if valueType=="userdata" or (valueType=="table" and type(value.IsA)=="function") then
		local ok,name=safe(function() return shopDisplayName(value) end)
		return ok and (shopNameKey(name)==wanted or shopNameKey(value.Name)==wanted)
	end
	if valueType~="table" or visited[value] then return false end
	visited[value]=true
	local checked=0
	for _,nested in pairs(value) do
		checked=checked+1
		if checked>30 then break end
		if shopArgumentMatches(nested,wanted,depth+1,visited) then return true end
	end
	return false
end

local function observedPurchaseTarget(remote,arguments)
	for _,kind in ipairs({"pet","aura"}) do
		local selected=kind=="pet" and Runtime.selectedPet or Runtime.selectedAura
		local wanted=shopNameKey(selected)
		if wanted~="" then
			for index=1,arguments.n do
				if shopArgumentMatches(arguments[index],wanted,0,{}) then
	...
	local remoteName=string.lower(tostring(remote and remote.Name or ""))
	local relevant=remoteName:find("shop",1,true) or remoteName:find("buy",1,true)
		or remoteName:find("purchase",1,true) or remoteName:find("pet",1,true)
		or remoteName:find("aura",1,true)
	local pending=Runtime.shopPendingReplay
	if relevant and pending and pending.name then return pending.kind,pending.name end
	if relevant and (Runtime.crystalMode=="pet" or Runtime.crystalMode=="aura") then
		return Runtime.crystalMode,
			Runtime.crystalMode=="pet" and Runtime.selectedPet or Runtime.selectedAura
	end
	return nil,nil
end

local function installShopPurchaseObserver()
	if type(hookmetamethod)~="function" or type(getnamecallmethod)~="function"
		or not game then return false end
	local previous
	local wrap=type(newcclosure)=="function" and newcclosure or function(fn) return fn end
	local installed=safe(function()
		previous=hookmetamethod(game,"__namecall",wrap(function(remote,...)
			local method=getnamecallmethod()
			if not Runtime.alive or Runtime.shopReplaying
				or (method~="InvokeServer" and method~="FireServer") then
				return previous(remote,...)
			end

			local arguments={n=select("#",...),...}
			local kind,name=observedPurchaseTarget(remote,arguments)
			if not kind or not name then return previous(remote,...) end
			local beforeCount=shopInventoryCount(kind,name)
			local beforeGems=shopGemBalance()
			local returned={n=0}
			local function pack(...)
				returned={n=select("#",...),...}
			end
			pack(previous(remote,...))

			local function remember()
				if not Runtime.alive then return end
				local confirmed=shopPurchaseEvidence(kind,name,beforeCount,beforeGems,returned[1])
				if not confirmed then return end
				Runtime.shopObservedRoutes[kind]={
					remote=remote,
					method=method,
					arguments=arguments,
					name=name,
					item=findShopItem(name,kind),
				}
				Runtime.shopPurchaseRoute[kind]=nil
				local pending=Runtime.shopPendingReplay
				if pending and pending.kind==kind and shopNameKey(pending.name)==shopNameKey(name) then
					Runtime.shopPendingReplay=nil
					setStatus("ПОКУПКА ЗАПОМНЕНА: "..tostring(name))
					local lever=Runtime.leverRefs.crystal and Runtime.leverRefs.crystal[kind]
					if lever then lever.Set(true,false) end
				end
			end
			if type(task.defer)=="function" then task.defer(remember)
			else task.spawn(remember) end
			return (table.unpack or unpack)(returned,1,returned.n)
		end))
	end)
	Runtime.shopHookInstalled=installed and type(previous)=="function"
	return Runtime.shopHookInstalled
end

local function adaptObservedArgument(value,recorded,currentName,currentItem,depth,visited)
	if depth>5 or value==nil then return value end
	local valueType=type(value)
	if valueType=="string" then
		local key=shopNameKey(value)
		if key==shopNameKey(recorded.name) then return currentName end
		if recorded.item and key==shopNameKey(recorded.item.Name) then
			return currentItem and tostring(currentItem.Name) or currentName
		end
		return value
	end
	if valueType=="userdata" or (valueType=="table" and type(value.IsA)=="function") then
		local ok,name=safe(function() return shopDisplayName(value) end)
		if value==recorded.item or (ok and shopNameKey(name)==shopNameKey(recorded.name)) then
			return currentItem or (shopNameKey(recorded.name)==shopNameKey(currentName) and value or currentName)
		end
		return value
	end
	...
	for key,nested in pairs(value) do
		copy[key]=adaptObservedArgument(nested,recorded,currentName,currentItem,depth+1,visited)
	end
	return copy
end

local function invokeActualShopRemote(remote,arguments,method)
	local wasReplaying=Runtime.shopReplaying
	Runtime.shopReplaying=true
	local ok,response=safe(function()
		if method=="InvokeServer" or (not method and remote:IsA("RemoteFunction")) then
			return remote:InvokeServer((table.unpack or unpack)(arguments,1,arguments.n or #arguments))
		end
		remote:FireServer((table.unpack or unpack)(arguments,1,arguments.n or #arguments))
		return nil
	end)
	Runtime.shopReplaying=wasReplaying
	return ok,response
end

local function tryObservedPurchase(recorded,item,kind,name,beforeCount,beforeGems)
	if not recorded or not recorded.remote or not recorded.remote.Parent then
		return false,"запомненный вызов больше недоступен"
	end
	local arguments={n=recorded.arguments.n or #recorded.arguments}
	for index=1,arguments.n do
		arguments[index]=adaptObservedArgument(recorded.arguments[index],recorded,name,item,0,{})
	end
	local ok,response=invokeActualShopRemote(recorded.remote,arguments,recorded.method)
	if not ok then return false,tostring(response),terminalShopError(response) end
	local confirmed,message=shopPurchaseEvidence(kind,name,beforeCount,beforeGems,response)
	return confirmed,message,terminalShopError(message)
end

local function nativeShopButton(node)
	return node and (node:IsA("TextButton") or node:IsA("ImageButton"))
end

local function visibleNativeShopNode(node)
	local current=node
	for _=1,12 do
		if not current then break end
		if current==Runtime.uiRoot then return false end
		local visibleOk,visible=safe(function() return current.Visible end)
		if visibleOk and visible==false then return false end
		local enabledOk,enabled=safe(function() return current.Enabled end)
		if enabledOk and enabled==false then return false end
		current=current.Parent
	end
	return true
end

local function nativeShopText(node)
	local read,text=safe(function() return tostring(node.Text or "") end)
	return tostring(node.Name or "").." "..(read and text or "")
end

local function nativePurchaseScore(node)
	local text=string.lower(nativeShopText(node))
	if text:find("rebirth",1,true) or text:find("ascend",1,true)
		or text:find("reset",1,true) or text:find("prestige",1,true)
		or text:find("sell",1,true) or text:find("delete",1,true) then return 0 end
	if text:find("buy",1,true) or text:find("purchase",1,true)
		or text:find("куп",1,true) or text:find("getpet",1,true) then return 100 end
	return 0
end

local function nativeNodeMatches(node,wanted)
	if shopNameKey(node.Name)==wanted then return true end
	if node:IsA("TextLabel") or nativeShopButton(node) then
		local read,text=safe(function() return tostring(node.Text or "") end)
		if read then
			for line in text:gmatch("[^\r\n]+") do
				if shopNameKey(line)==wanted then return true end
			end
		end
	end
	return false
...
local function nativeShopCandidates(wanted)
	local candidates={}
	local inspected=0
	for _,screen in ipairs(playerGui:GetChildren()) do
		if screen~=Runtime.uiRoot and screen:IsA("ScreenGui") and visibleNativeShopNode(screen) then
			for _,button in ipairs(screen:GetDescendants()) do
				inspected=inspected+1
				if inspected>2400 then return candidates end
				if nativeShopButton(button) and nativePurchaseScore(button)>=100
					and visibleNativeShopNode(button) then
					local current=button
					local matched=false
					local unsafe=false
					for _=1,4 do
						if not current or current:IsA("ScreenGui") then break end
						local context=string.lower(tostring(current.Name or ""))
						if context:find("rebirth",1,true) or context:find("ascend",1,true)
							or context:find("prestige",1,true) then unsafe=true break end
						if nativeNodeMatches(current,wanted) then matched=true end
						for _,label in ipairs(current:GetChildren()) do
							if (label:IsA("TextLabel") or nativeShopButton(label))
								and nativeNodeMatches(label,wanted) then matched=true break end
						end
						current=current.Parent
					end
					if matched and not unsafe then
						table.insert(candidates,{button=button,score=100})
...
local function dispatchNativeShopButton(button)
	if not nativeShopButton(button) or not visibleNativeShopNode(button)
		or nativePurchaseScore(button)<100 then return false end
	for _,eventName in ipairs({"Activated","MouseButton1Click"}) do
		local found,event=safe(function() return button[eventName] end)
		if found and event then
			local listeners=nil
			if type(getconnections)=="function" then
				local read,result=safe(function() return getconnections(event) end)
				if read and type(result)=="table" then listeners=result end
			end
			if not listeners or #listeners>0 then
				if type(firesignal)=="function" then
					local fired=safe(function() firesignal(event) end)
					if fired then return true end
				elseif listeners then
					local fired=safe(function()
...
local function triggerActualShopButton(name,kind,beforeCount,beforeGems)
	if type(firesignal)~="function" and type(getconnections)~="function" then return false end
	local wanted=shopNameKey(name)
	if wanted=="" then return false end
	for _,entry in ipairs(nativeShopCandidates(wanted)) do
		if dispatchNativeShopButton(entry.button) then
			local purchased,message=shopPurchaseEvidence(kind,name,beforeCount,beforeGems,nil)
			if purchased then return true,nil,"native-signal" end
			-- Screen-coordinate clicks can hit rebirth or other destructive controls.
			-- Stop after this exact product button instead of guessing alternatives.
...
local function shopRequestArguments(strategy,item,kind,selectedName)
	local name=tostring(selectedName or (item and shopDisplayName(item)) or "")
	if name=="" then return nil end
	local action=kind=="aura" and "Aura" or "Pet"
	if strategy=="object" then return item and {item} or nil end
	if strategy=="named" then return {name} end
	if strategy=="namedQuantity" then return {name,1} end
	if strategy=="objectQuantity" then return item and {item,1} or nil end
	if strategy=="buyObject" then return item and {"buy"..action,item} or nil end
	if strategy=="purchaseObject" then return item and {"purchase"..action,item} or nil end
	if strategy=="buyItem" then return item and {"buyItem",item} or nil end
	if strategy=="purchaseItem" then return item and {"purchaseItem",item} or nil end
	if strategy=="buyNamed" then return {"buy"..action,name} end
	if strategy=="purchaseNamed" then return {"purchase"..action,name} end
	if strategy=="buyNamedItem" then return {"buyItem",name} end
	if strategy=="purchaseNamedItem" then return {"purchaseItem",name} end
	if strategy=="genericNamedBuy" then return {"buy",name} end
	if strategy=="genericNamedPurchase" then return {"purchase",name} end
	if strategy=="namedId" then return item and {tostring(item.Name)} or nil end
	if strategy=="genericBuy" then return item and {"buy",item} or nil end
	if strategy=="genericPurchase" then return item and {"purchase",item} or nil end
	return nil
end

local function tryShopRoute(remote,strategy,item,kind,beforeCount,beforeGems,selectedName)
	local args=shopRequestArguments(strategy,item,kind,selectedName)
	if not args or not remote or not remote.Parent then return false,"remote недоступен" end
	local ok,response=invokeActualShopRemote(remote,args,nil)
	if not ok then return false,tostring(response),terminalShopError(response) end
	local confirmed,message=shopPurchaseEvidence(kind,selectedName,beforeCount,beforeGems,response)
	local responseVerdict=shopResponseResult(response)
	return confirmed,message,terminalShopError(message),not confirmed and responseVerdict~=false
end

local function shopInventoryFootprint()
	local total=0
	local counted={}
	for _,name in ipairs({
		"petsFolder","Pets","pets","petInventory",
		"trailsFolder","aurasFolder","Auras","Trails",
	}) do
		local folder=lp:FindFirstChild(name)
		if folder and not counted[folder] then
			counted[folder]=true
			total=total+#folder:GetDescendants()
		end
	end
	return total
end

local function tryMappedCrystal(name,kind)
	local crystal=SHOP_ITEM_CRYSTALS[string.lower(tostring(name or ""))]
	local events=ReplicatedStorage:FindFirstChild("rEvents")
	if not crystal or not events or not events:FindFirstChild("openCrystalRemote") then
		return nil
	end
	local beforeTarget=shopInventoryCount(kind,name)
	local beforeItems=shopInventoryFootprint()
	local beforeGems=shopGemBalance()
	local opened,message,resolved,response=openCrystalOnce(crystal)
	if not opened then return false,message,name end
	local accepted,responseError=shopResponseResult(response)
	if accepted==false then return false,responseError,name end
	for attempt=1,10 do
		if shopInventoryCount(kind,name)>beforeTarget then
			return true,nil,name,"target:"..resolved
		end
		local afterGems=shopGemBalance()
		if (beforeGems and afterGems and afterGems<beforeGems)
			or shopInventoryFootprint()>beforeItems then
			-- A different pet is a real crystal result, not a direct purchase of
			-- the selected pet. The status explicitly labels this as a crystal.
			return true,nil,name,"crystal:"..resolved
		end
		if attempt<10 then task.wait(0.1) end
	end
	return false,"кристалл не открылся и питомец не появился",name
end

local function buyShopItemOnce(name,kind)
	kind=kind=="aura" and "aura" or "pet"
	local requested=tostring(name or ""):gsub("^%s+",""):gsub("%s+$","")
	if requested=="" then return false,"питомец не выбран" end
	local item,entry=findShopItem(requested,kind)
	local selectedName=entry and entry.name or requested
	local beforeCount=shopInventoryCount(kind,selectedName)
	local beforeGems=shopGemBalance()
	local remembered=Runtime.shopPurchaseRoute[kind]
	local lastError=nil
	local observed=Runtime.shopObservedRoutes[kind]
	local function crystalFallback(message)
		local opened,crystalError,resolved,route=tryMappedCrystal(selectedName,kind)
		if opened~=nil then
			if opened and Runtime.shopCrystalRoute then
				Runtime.shopCrystalRoute[kind]=selectedName
			end
			return opened,crystalError,resolved,route
		end
		return false,message or "сервер отклонил безопасную покупку",selectedName
	end
	local crystalRoute=Runtime.shopCrystalRoute and Runtime.shopCrystalRoute[kind]
	if crystalRoute and shopNameKey(crystalRoute)==shopNameKey(selectedName) then
		local opened,crystalError,resolved,route=tryMappedCrystal(selectedName,kind)
		if opened~=nil then return opened,crystalError,resolved,route end
		Runtime.shopCrystalRoute[kind]=nil
	end

	if observed then
		local ok,message,terminal=tryObservedPurchase(
			observed,item,kind,selectedName,beforeCount,beforeGems
		)
		if ok then return true,nil,selectedName end
		if terminal then return false,message,selectedName end
		lastError=message
		Runtime.shopObservedRoutes[kind]=nil
	end

	if remembered and remembered.remote and remembered.remote.Parent then
		local ok,message,terminal,uncertain=tryShopRoute(
			remembered.remote,remembered.strategy,item,kind,beforeCount,beforeGems,selectedName
		)
		if ok then return true,nil,selectedName end
		if terminal then return false,message,selectedName end
		if uncertain then return crystalFallback(message) end
		lastError=message
		Runtime.shopPurchaseRoute[kind]=nil
	end

	-- Manual purchases still work after the old direct-shop remote was patched.
	-- Drive the real game button first so its current LocalScript supplies the
	-- genuine server arguments, including any private per-session receipt.
	local clicked,clickError=triggerActualShopButton(selectedName,kind,beforeCount,beforeGems)
	if clicked then return true,nil,selectedName end
	if clickError then
		lastError=clickError
		if terminalShopError(clickError) then return false,clickError,selectedName end
	end

	local remotes=shopRemoteCandidates()
	if #remotes==0 then
		return crystalFallback(lastError or "кнопка покупки не найдена в меню игры")
	end
	local strategies=item and {
		"object","buyObject","purchaseObject","buyNamed","purchaseNamed",
		"named","buyItem","purchaseItem","objectQuantity","namedId",
		"buyNamedItem","purchaseNamedItem","namedQuantity","genericBuy","genericPurchase",
		"genericNamedBuy","genericNamedPurchase",
	} or {
		"named","buyNamed","purchaseNamed","buyNamedItem","purchaseNamedItem",
		"namedQuantity","genericNamedBuy","genericNamedPurchase",
	}

	local attempts=0
	for _,remote in ipairs(remotes) do
		for _,strategy in ipairs(strategies) do
			if not remembered or remembered.remote~=remote or remembered.strategy~=strategy then
				if attempts>=8 then
					return crystalFallback(lastError or "безопасный лимит попыток покупки")
				end
				attempts=attempts+1
				local ok,message,terminal,uncertain=tryShopRoute(
					remote,strategy,item,kind,beforeCount,beforeGems,selectedName
				)
				if ok then
					Runtime.shopPurchaseRoute[kind]={remote=remote,strategy=strategy}
					return true,nil,selectedName
				end
	...
	return crystalFallback(lastError or "сервер отклонил все способы покупки")
end

installShopPurchaseObserver()
...
local function startCrystalAutomation(mode)
	...
	task.spawn(function()
		while Runtime.alive and Runtime.crystalMode==mode and Runtime.crystalToken==token do
			if not Runtime.networkPaused then
				local ok,err,resolved,route
				if mode=="crystal" then
					ok,err,resolved=openCrystalOnce(targetName)
				else
					ok,err,resolved,route=buyShopItemOnce(targetName,mode)
				end
				if ok then
					Runtime.purchaseAttempts=Runtime.purchaseAttempts+1
					if type(route)=="string" and route:sub(1,8)=="crystal:" then
						setStatus("КРИСТАЛЛ #"..Runtime.purchaseAttempts..": "
							..route:sub(9).." → "..tostring(targetName))
					elseif type(route)=="string" and route:sub(1,7)=="target:" then
						setStatus("ПИТОМЕЦ ПОЛУЧЕН: "..tostring(resolved))
					else
					...
					if mode~="crystal" then
						if Runtime.shopHookInstalled and not terminalShopError(err) then
							Runtime.shopPendingReplay={kind=mode,name=targetName}
							stopCrystalAutomation("КУПИ 1 РАЗ ВРУЧНУЮ: "..targetName)
						else
