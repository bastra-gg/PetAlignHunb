...
local SHOP_UPDATE_PETS={"Apex Overlord"}
...
local function triggerActualShopButton(name,kind,beforeCount,beforeGems)
	...
	for _,node in ipairs(playerGui:GetDescendants()) do
		if node:IsA("TextButton") or node:IsA("ImageButton") then
			local buttonName=string.lower(tostring(node.Name or ""))
			local ok,text=safe(function() return string.lower(tostring(node.Text or "")) end)
			text=ok and text or ""
			if buttonName:find("buy",1,true) or buttonName:find("purchase",1,true)
				or text:find("buy",1,true) or text:find("purchase",1,true)
				or text:find("куп",1,true) or buttonName:find("getpet",1,true) then
				local current=node
				local matched=false
				local ownWindow=false
				for _=1,8 do
					if not current then break end
					if current==Runtime.uiRoot then ownWindow=true break end
					if shopNameKey(current.Name)==wanted
						or shopNameKey(shopDisplayName(current))==wanted then matched=true end
					for _,sibling in ipairs(current:GetChildren()) do
						if sibling:IsA("TextLabel") or sibling:IsA("TextButton") then
							local read,labelText=safe(function() return tostring(sibling.Text or "") end)
							if read and shopNameKey(labelText):find(wanted,1,true) then matched=true break end
						end
					end
					current=current.Parent
				end
				if matched and not ownWindow then
					for _,eventName in ipairs({"Activated","MouseButton1Click"}) do
						local eventOk,event=safe(function() return node[eventName] end)
						if eventOk and event then
							local hasConnections=true
							if type(getconnections)=="function" then
								local checked,connections=safe(function() return getconnections(event) end)
								hasConnections=checked and type(connections)=="table" and #connections>0
							end
							if hasConnections then
								local fired=false
								if type(firesignal)=="function" then
									fired=safe(function() firesignal(event) end)
								...
								if fired then
									local confirmed,message=shopPurchaseEvidence(kind,name,beforeCount,beforeGems,nil)
									if confirmed then return true,nil,"manual" end
...
local function buyShopItemOnce(name,kind)
	...
	if not item then
		...
		if clickError then lastError=clickError end
	end
	...
	if #remotes==0 then
		...
		return false,lastError or "remote покупки не найден — открой магазин игры",selectedName
	end
	...
	for index,remote in ipairs(remotes) do
		for _,strategy in ipairs(strategies) do
			...
			if item and index==1 and strategy=="object" then
				local clicked,clickError=triggerActualShopButton(selectedName,kind,beforeCount,beforeGems)
				if clicked then return true,nil,selectedName end
				if clickError then lastError=clickError end
			end
