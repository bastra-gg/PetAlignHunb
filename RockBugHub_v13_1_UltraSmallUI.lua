...
local THEME={
	Bg=Color3.fromRGB(9,15,24),
	Panel=Color3.fromRGB(15,24,37),
	Surface=Color3.fromRGB(20,32,47),
	SurfaceAlt=Color3.fromRGB(28,44,62),
	Accent=Color3.fromRGB(164,118,255),
	Accent2=Color3.fromRGB(208,159,255),
	Neon=Color3.fromRGB(109,194,255),
	Success=Color3.fromRGB(83,207,153),
	Danger=Color3.fromRGB(255,102,119),
	Text=Color3.fromRGB(237,243,250),
	Muted=Color3.fromRGB(155,174,194),
	Border=Color3.fromRGB(58,88,117),
...
local function windowMetrics(viewport)
	...
	local minHeight=math.min(370,availableHeight)
	local widthRatio=mobile and 0.94 or 0.50
	local heightRatio=mobile and 0.96 or 0.76
	local defaultWidth=math.min(520,math.max(minWidth,math.floor(viewport.X*widthRatio)))
	local defaultHeight=math.min(620,math.max(minHeight,math.floor(viewport.Y*heightRatio)))
...
local author=label(topBar,"ВЫБЕРИ НУЖНЫЙ РАЗДЕЛ",8,Enum.Font.GothamBold,THEME.Muted)
...
rail.Parent=main
rail.Size=UDim2.new(1,-16,1,-62)
rail.Position=UDim2.fromOffset(8,54)
rail.BackgroundColor3=THEME.Panel
rail.BackgroundTransparency=0.68
rail.BorderSizePixel=0
rail.ClipsDescendants=true
corner(rail,12)

Runtime.layoutUI.homeTitle=label(rail,"ЧТО НУЖНО СДЕЛАТЬ?",15,Enum.Font.GothamBold,THEME.Text)
Runtime.layoutUI.homeTitle.Size=UDim2.new(1,-24,0,22)
Runtime.layoutUI.homeTitle.Position=UDim2.fromOffset(12,8)

Runtime.layoutUI.homeHint=label(rail,"Нажми на нужную функцию",10,Enum.Font.Gotham,THEME.Muted)
Runtime.layoutUI.homeHint.Size=UDim2.new(1,-24,0,18)
Runtime.layoutUI.homeHint.Position=UDim2.fromOffset(12,31)

local railScroll=Instance.new("ScrollingFrame")
railScroll.Parent=rail
railScroll.Size=UDim2.new(1,-16,1,-61)
railScroll.Position=UDim2.fromOffset(8,54)
...
railScroll.CanvasSize=UDim2.fromOffset(0,428)
...
Runtime.layoutUI.navigationGrid.CellPadding=UDim2.fromOffset(0,8)
Runtime.layoutUI.navigationGrid.CellSize=UDim2.new(1,-3,0,62)

Runtime.layoutUI.homeDescriptions={
	[1]={title="БАГ ПИТОМЦЕВ",hint="Камни, автоудар и ускорение"},
	[2]={title="ТРЕНИРОВКА",hint="Гантели, отжимания, бег и другое"},
	[3]={title="РЕБИРТЫ И KING",hint="Перерождения, лимит и размер"},
	[4]={title="МАГАЗИН",hint="Кристаллы, питомцы и ауры"},
	[5]={title="АВТОКИЛ",hint="Игроки, цели и исключения"},
	[6]={title="НАСТРОЙКИ",hint="Графика, интернет и защита"},
}

local railLine=Instance.new("Frame")
railLine.Parent=rail
railLine.Size=UDim2.new(1,-14,0,1)
railLine.Position=UDim2.new(0,7,1,-1)
...
local function styleTab(tab,y)
	tab.Size=UDim2.new(1,-3,0,62)
	tab.LayoutOrder=math.floor((y-6)/56)+1
	local symbol=tostring(tab.Text or ""):match("^([^%s]+)") or "›"
	local description=Runtime.layoutUI.homeDescriptions[tab.LayoutOrder]
	tab.Text=""
	tab.TextSize=11
	...
	local icon=label(tab,symbol,20,Enum.Font.GothamBold,THEME.Accent)
	icon.Name="HomeIcon"
	icon.Size=UDim2.fromOffset(35,35)
	icon.Position=UDim2.fromOffset(9,13)
	icon.TextXAlignment=Enum.TextXAlignment.Center

	local name=label(tab,description and description.title or "РАЗДЕЛ",12,Enum.Font.GothamBold,THEME.Text)
	name.Name="HomeTitle"
	name.Size=UDim2.new(1,-94,0,21)
	name.Position=UDim2.fromOffset(52,9)
	name.TextWrapped=false
	name.TextTruncate=Enum.TextTruncate.AtEnd

	local hint=label(tab,description and description.hint or "",9,Enum.Font.Gotham,THEME.Muted)
	hint.Name="HomeDescription"
	hint.Size=UDim2.new(1,-94,0,19)
	hint.Position=UDim2.fromOffset(52,32)
	hint.TextWrapped=false
	hint.TextTruncate=Enum.TextTruncate.AtEnd

	local arrow=label(tab,"›",22,Enum.Font.GothamBold,THEME.Muted)
	arrow.Name="HomeArrow"
	arrow.Size=UDim2.fromOffset(26,45)
	arrow.Position=UDim2.new(1,-34,0,8)
	arrow.TextXAlignment=Enum.TextXAlignment.Center

	local mark=Instance.new("Frame")
	mark.Name="ActiveMark"
	mark.Parent=tab
	mark.Size=UDim2.new(0,3,1,-18)
	mark.Position=UDim2.fromOffset(1,9)
...
local bugTab=button(railScroll,"◇  КАМНИ",THEME.Accent)
styleTab(bugTab,6)

local trainTab=button(railScroll,"↗  ТРЕНИРОВКА",THEME.Surface)
styleTab(trainTab,62)

local rebTab=button(railScroll,"↻  РЕБИРТЫ",THEME.Surface)
styleTab(rebTab,118)

local crystalTab=button(railScroll,"▣  МАГАЗИН",THEME.Surface)
styleTab(crystalTab,174)

local killTab=button(railScroll,"✦  ИГРОКИ",THEME.Surface)
styleTab(killTab,230)

Runtime.layoutUI.systemTab=button(railScroll,"◌  НАСТРОЙКИ",THEME.Surface)
...
content.Position=UDim2.fromOffset(6,52)
...
quickBar.Size=UDim2.new(1,-12,0,96)
...
rescanBtn.Size=UDim2.fromOffset(86,23)
rescanBtn.Position=UDim2.new(1,-92,0,5)
rescanBtn.TextSize=8

local quickTitle=label(quickBar,"ФАРМ КАМНЕЙ",12,Enum.Font.GothamBold,THEME.Text)
quickTitle.Size=UDim2.new(1,-104,0,18)
...
Runtime.layoutUI.quickHint.Size=UDim2.new(1,-16,0,20)
Runtime.layoutUI.quickHint.Position=UDim2.fromOffset(8,25)
...
quickBody.Size=UDim2.new(1,-12,0,35)
quickBody.Position=UDim2.fromOffset(6,52)
...
local function makePage(color)
	...
	page.Parent=content
	page.Size=UDim2.new(1,-12,1,-158)
	page.Position=UDim2.fromOffset(6,107)
...
local function makeFeaturePanel(parent,titleText,height,columns)
	...
	grid.CellSize=UDim2.new(1,0,0,52)
	...
	local function refreshFeatureGrid()
		...
		local gap=6
		grid.CellSize=UDim2.new(1,0,0,52)
		panel.Size=UDim2.new(1,0,0,42+count*52+(count-1)*gap)
		updateCanvas()
	end
...
local function makeFeatureToggle(parent,iconText,name,desc,initial,callback)
	...
	local glyph=label(tile,iconText,17,Enum.Font.GothamBold,THEME.Text)
	glyph.Size=UDim2.fromOffset(30,32)
	glyph.Position=UDim2.fromOffset(7,10)
	glyph.TextXAlignment=Enum.TextXAlignment.Center

	local stateLabel=label(tile,"ВЫКЛ",9,Enum.Font.GothamBold,THEME.Muted)
	stateLabel.Size=UDim2.fromOffset(49,25)
	stateLabel.Position=UDim2.new(1,-58,0,14)
	stateLabel.BackgroundColor3=THEME.Bg
	stateLabel.BackgroundTransparency=0.24
	...
	n.Size=UDim2.new(1,-112,0,19)
	n.Position=UDim2.fromOffset(43,7)
	n.TextXAlignment=Enum.TextXAlignment.Left
	n.TextWrapped=false
	n.TextTruncate=Enum.TextTruncate.AtEnd

	local d=label(tile,desc,9,Enum.Font.Gotham,THEME.Muted)
	d.Size=UDim2.new(1,-112,0,17)
	d.Position=UDim2.fromOffset(43,27)
	...
	local function paint()
		...
		stateLabel.TextColor3=state and THEME.Success or THEME.Muted
		stateLabel.BackgroundColor3=state and THEME.SurfaceAlt or THEME.Bg
		stateLabel.BackgroundTransparency=state and 0.06 or 0.24
	end
...
local bugFeaturePanel,bugFeatureBody=makeFeaturePanel(bugPage,"2 · ЗАПУСК ФАРМА",106,3)
bugFeaturePanel.LayoutOrder=2
local bugSettingsPanel,bugSettingsBody=makeSettingsPanel(bugPage,"1 · ВЫБОР КАМНЯ",170)
...
local trainFeaturePanel,trainFeatureBody=makeFeaturePanel(trainPage,"1 · ВИД ТРЕНИРОВКИ",170,3)
trainFeaturePanel.LayoutOrder=1
local trainSettingsPanel,trainSettingsBody=makeFeaturePanel(trainPage,"2 · ПОЛОЖЕНИЕ ИГРОКА",106,2)
trainSettingsPanel.LayoutOrder=2

Runtime.layoutUI.systemVisualPanel,Runtime.layoutUI.systemVisualBody=makeFeaturePanel(Runtime.layoutUI.systemPage,"1 · ПРОИЗВОДИТЕЛЬНОСТЬ",106,2)
Runtime.layoutUI.systemVisualPanel.LayoutOrder=1
Runtime.layoutUI.systemNetworkPanel,Runtime.layoutUI.systemNetworkBody=makeFeaturePanel(Runtime.layoutUI.systemPage,"2 · УПРАВЛЕНИЕ СЕТЬЮ",106,2)
Runtime.layoutUI.systemNetworkPanel.LayoutOrder=2
Runtime.layoutUI.makeGuideCard(Runtime.layoutUI.systemPage,"БЫСТРАЯ ЗАЩИТА", "Anti-AFK и защита сети доступны сверху в каждом разделе.",3)
...
local rebFeaturePanel,rebFeatureBody=makeFeaturePanel(rebPage,"2 · РЕБИРТ БЕЗ ЛИМИТА",106,2)
rebFeaturePanel.LayoutOrder=2
local rebSettingsPanel,rebSettingsBody=makeSettingsPanel(rebPage,"3 · ЗНАЧЕНИЕ РАЗМЕРА",80)
rebSettingsPanel.LayoutOrder=3
Runtime.layoutUI.rebSupportPanel,Runtime.layoutUI.rebSupportBody=makeFeaturePanel(rebPage,"4 · KING И РАЗМЕР",106,2)
Runtime.layoutUI.rebSupportPanel.LayoutOrder=4

local rebInfo=card(rebPage,52)
rebInfo.LayoutOrder=1
local rebInfoTitle=label(rebInfo,"1 · ЦЕЛЬ РЕБИРТОВ",10,Enum.Font.GothamBlack,THEME.Accent2)
...
local killFeaturePanel,killFeatureBody=makeFeaturePanel(killPage,"2 · РЕЖИМ АВТОКИЛА",106,3)
killFeaturePanel.LayoutOrder=2
local killSettingsPanel,killSettingsBody=makeSettingsPanel(killPage,"1 · КОГО АТАКОВАТЬ",141)
...
local crystalFeaturePanel,crystalFeatureBody=makeFeaturePanel(crystalPage,"2 · ВКЛЮЧИТЬ ПОКУПКУ",106,3)
crystalFeaturePanel.LayoutOrder=2
local crystalSettingsPanel,crystalSettingsBody=makeSettingsPanel(crystalPage,"1 · ВЫБОР ПОКУПКИ",141)
...
Runtime.layoutUI.sectionInfo={
	bug={title="ФАРМ КАМНЕЙ",hint="Выбери камень, затем включи автоудар."},
	train={title="ТРЕНИРОВКА",hint="Выбери одно упражнение и закрепись при необходимости."},
	reb={title="РЕБИРТЫ И KING",hint="Задай цель или включи ребирты без ограничения."},
	crystal={title="МАГАЗИН И ГЕМЫ",hint="Сначала выбери товар, потом включи покупку."},
	kill={title="ИГРОКИ И PVP",hint="Отметь игроков, затем выбери режим атаки."},
	system={title="НАСТРОЙКИ И СЕТЬ",hint="Графика, сетевые режимы и защита клиента."},
}
...
local function applyResponsiveLayout()
	...
	content.Size=UDim2.new(1,-12,1,-54)
	content.Position=UDim2.fromOffset(6,52)
	Runtime.layoutUI.navigationGrid.CellSize=UDim2.new(1,-3,0,short and 55 or 62)
	railScroll.CanvasSize=UDim2.fromOffset(0,6*(short and 55 or 62)+5*8+8)
	...
	Runtime.layoutUI.homeTitle.TextSize=compact and 13 or 15
	Runtime.layoutUI.homeHint.TextSize=compact and 9 or 10
	if short then
		quickBar.Size=UDim2.new(1,-12,0,77)
		Runtime.layoutUI.quickHint.Position=UDim2.fromOffset(8,23)
		Runtime.layoutUI.quickHint.Size=UDim2.new(1,-16,0,15)
		quickBody.Position=UDim2.fromOffset(6,39)
		quickBody.Size=UDim2.new(1,-12,0,30)
	else
		quickBar.Size=UDim2.new(1,-12,0,96)
		Runtime.layoutUI.quickHint.Position=UDim2.fromOffset(8,25)
		Runtime.layoutUI.quickHint.Size=UDim2.new(1,-16,0,20)
	...
	for _,page in ipairs({bugPage,trainPage,rebPage,killPage,crystalPage,Runtime.layoutUI.systemPage}) do
		page.Position=UDim2.fromOffset(6,short and 88 or 107)
		page.Size=UDim2.new(1,-12,1,short and -139 or -158)
	end
	updateCanvas()
end
...
local function paintTab(tab,active)
	...
	local icon=tab:FindFirstChild("HomeIcon")
	if icon then icon.TextColor3=active and THEME.Accent2 or THEME.Accent end
	local heading=tab:FindFirstChild("HomeTitle")
	if heading then heading.TextColor3=active and THEME.Text or THEME.Text end
end
...
local function showTab(name)
	...
	if info then
		...
		author.Text="НАЗАД В ГЛАВНОЕ МЕНЮ"
		quickTitle.Text="КАК ПОЛЬЗОВАТЬСЯ"
...
function Runtime.layoutUI.showHome()
	...
	author.Text="ВЫБЕРИ НУЖНЫЙ РАЗДЕЛ"
end
