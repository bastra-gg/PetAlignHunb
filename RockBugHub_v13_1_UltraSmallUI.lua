...
local THEME={
	...
	Accent=Color3.fromRGB(81,183,255),
	Accent2=Color3.fromRGB(147,139,255),
	Neon=Color3.fromRGB(82,226,198),
...
local function windowMetrics(viewport)
	...
	local defaultWidth=math.min(650,math.max(minWidth,math.floor(viewport.X*widthRatio)))
...
local title=label(topBar,"ROCKBUG CONTROL",14,Enum.Font.GothamBold,THEME.Text)
title.Size=UDim2.new(1,-160,0,20)
title.Position=UDim2.fromOffset(49,5)

local author=label(topBar,"МОБИЛЬНАЯ ПАНЕЛЬ УПРАВЛЕНИЯ",8,Enum.Font.GothamBold,THEME.Muted)
...
railScroll.Size=UDim2.new(1,-8,1,-8)
railScroll.Position=UDim2.fromOffset(4,4)
railScroll.BackgroundTransparency=1
railScroll.BorderSizePixel=0
railScroll.Active=false
...
Runtime.layoutUI.navigationGrid.CellPadding=UDim2.fromOffset(6,6)
Runtime.layoutUI.navigationGrid.CellSize=UDim2.new(1/3,-4,0,39)
...
local function styleTab(tab,y)
	...
	mark.Parent=tab
	mark.Size=UDim2.new(1,-14,0,3)
	mark.Position=UDim2.new(0,7,1,-4)
...
content.Parent=main
content.Size=UDim2.new(1,-16,1,-150)
content.Position=UDim2.fromOffset(8,150)
...
local function makeFeaturePanel(parent,titleText,height,columns)
	...
	grid.CellSize=UDim2.new(0.5,-4,0,70)
	...
	local function refreshFeatureGrid()
		...
		local maxColumns=math.max(1,tonumber(columns) or 2)
		local wanted=width>=430 and 3 or (width>=230 and 2 or 1)
		local actual=math.max(1,math.min(maxColumns,wanted,count))
		local rows=math.max(1,math.ceil(count/actual))
		local gap=7
		grid.CellSize=UDim2.new(1/actual,-gap*(actual-1)/actual,0,70)
		panel.Size=UDim2.new(1,0,0,42+rows*70+(rows-1)*gap)
		updateCanvas()
	end
...
local function makeFeatureToggle(parent,iconText,name,desc,initial,callback)
	...
	local glyph=label(tile,iconText,15,Enum.Font.GothamBold,THEME.Text)
	glyph.Size=UDim2.fromOffset(22,19)
	glyph.Position=UDim2.fromOffset(7,5)
	glyph.TextXAlignment=Enum.TextXAlignment.Center

	local stateLabel=label(tile,"ВЫКЛ",8,Enum.Font.GothamBold,THEME.Muted)
	stateLabel.Size=UDim2.fromOffset(38,17)
	stateLabel.Position=UDim2.new(1,-44,0,6)
	...
	n.Size=UDim2.new(1,-14,0,16)
	n.Position=UDim2.fromOffset(7,29)
	n.TextXAlignment=Enum.TextXAlignment.Left
	n.TextWrapped=false
	n.TextTruncate=Enum.TextTruncate.AtEnd

	local d=label(tile,desc,8,Enum.Font.Gotham,THEME.Muted)
	d.Size=UDim2.new(1,-14,0,14)
	d.Position=UDim2.fromOffset(7,49)
...
local function applyResponsiveLayout()
	...
	local navigationHeight=short and 78 or 92
	local contentTop=58+navigationHeight
	rail.Size=UDim2.new(1,-16,0,navigationHeight)
	rail.Position=UDim2.fromOffset(8,52)
	content.Size=UDim2.new(1,-16,1,-contentTop)
	content.Position=UDim2.fromOffset(8,contentTop)
	Runtime.layoutUI.navigationGrid.CellSize=UDim2.new(1/3,-4,0,short and 32 or 39)
	railScroll.ScrollBarThickness=0
	...
	for _,tab in ipairs({bugTab,trainTab,rebTab,killTab,crystalTab,Runtime.layoutUI.systemTab}) do
		tab.TextSize=compact and 8 or 9
	end
	updateCanvas()
end
