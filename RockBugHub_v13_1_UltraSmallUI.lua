...
local function nativePurchaseScore(node)
	...
	if text:find("buy",1,true) or text:find("purchase",1,true)
		or text:find("куп",1,true) or text:find("getpet",1,true)
		or text:find("confirm",1,true) then return 100 end
	if text:find("select",1,true) or text:find("choose",1,true)
		or text:find("card",1,true) then return 35 end
	return 20
end

local function nativeNodeMatches(node,wanted)
	if shopNameKey(node.Name)==wanted or shopNameKey(shopDisplayName(node))==wanted then
		return true
	end
	if node:IsA("TextLabel") or nativeShopButton(node) then
		local read,text=safe(function() return tostring(node.Text or "") end)
		return read and shopNameKey(text):find(wanted,1,true)~=nil
	end
	return false
end
...
local function nativeShopCandidates(wanted)
	local candidates={}
	local indexes={}
	local function remember(node,score,scope)
		if not nativeShopButton(node) or not visibleNativeShopNode(node) then return end
		if indexes[node] then
			if score>indexes[node].score then indexes[node].score=score end
			return
		end
		local entry={button=node,score=score,scope=scope}
		indexes[node]=entry
		table.insert(candidates,entry)
	end

	for _,node in ipairs(playerGui:GetDescendants()) do
		if visibleNativeShopNode(node) and nativeNodeMatches(node,wanted) then
			local current=node
			for depth=0,5 do
				if not current or current==playerGui or current==Runtime.uiRoot then break end
				if nativeShopButton(current) then
					remember(current,nativePurchaseScore(current)+65-depth,current.Parent)
				end
				if current:IsA("ScreenGui") then break end
				if depth<=3 then
					local nearby={}
					for _,child in ipairs(current:GetDescendants()) do
						if nativeShopButton(child) and visibleNativeShopNode(child) then
							table.insert(nearby,child)
							if #nearby>8 then break end
						end
					end
					if #nearby<=8 then
						for _,button in ipairs(nearby) do
							remember(button,nativePurchaseScore(button)+45-depth,current)
						end
					end
				end
				current=current.Parent
...
local function dispatchNativeShopButton(button,method)
	if not nativeShopButton(button) or not visibleNativeShopNode(button) then return false end
	...
	local positionOk,position=safe(function() return button.AbsolutePosition end)
	local sizeOk,size=safe(function() return button.AbsoluteSize end)
	if not positionOk or not sizeOk or not position or not size then return false end
	...
	local found,input=safe(function() return game:GetService("VirtualInputManager") end)
	if not found or not input then return false end
	local overlayWasEnabled=nil
	if Runtime.uiRoot then
		local read,enabled=safe(function() return Runtime.uiRoot.Enabled end)
		if read and enabled then
			overlayWasEnabled=enabled
			safe(function() Runtime.uiRoot.Enabled=false end)
		end
	end
	local clicked=safe(function()
		if method=="mouse" then
			input:SendMouseButtonEvent(x,y,0,true,game,0)
			task.wait(0.04)
			input:SendMouseButtonEvent(x,y,0,false,game,0)
		else
			input:SendTouchEvent(1,0,x,y)
			task.wait(0.04)
			input:SendTouchEvent(1,2,x,y)
		end
	end)
	if overlayWasEnabled~=nil then
		safe(function() Runtime.uiRoot.Enabled=overlayWasEnabled end)
	end
	return clicked
end

local function nativeConfirmationButtons(scope,wanted,kind)
	local result={}
	local root=scope
	while root and root.Parent and not root:IsA("ScreenGui") do root=root.Parent end
	if not root or root==Runtime.uiRoot then return result end
	local otherNames={}
	for _,entry in ipairs(shopCatalog(kind)) do
		local key=shopNameKey(entry.name)
		if key~=wanted then otherNames[key]=true end
	end
	for _,button in ipairs(root:GetDescendants()) do
		if nativeShopButton(button) and visibleNativeShopNode(button)
			and nativePurchaseScore(button)>=100 then
			local wrongItem=false
			...
			for depth=1,2 do
				...
				for _,label in ipairs(current:GetChildren()) do
					if label:IsA("TextLabel") then
						local read,text=safe(function() return tostring(label.Text or "") end)
						if read and otherNames[shopNameKey(text)] then wrongItem=true end
					end
				end
			end
			if not wrongItem then table.insert(result,button) end
		end
	end
	return result
end

local function tryNativeShopCandidate(entry,name,kind,beforeCount,beforeGems)
	for _,method in ipairs({"signal","mouse","touch"}) do
		if dispatchNativeShopButton(entry.button,method) then
			local purchased,message=shopPurchaseEvidence(kind,name,beforeCount,beforeGems,nil)
			if purchased then return true,nil,"native-"..method end
			if terminalShopError(message) then return false,message end
			for _,confirm in ipairs(nativeConfirmationButtons(entry.scope,name and shopNameKey(name),kind)) do
				if confirm~=entry.button then
					for _,confirmMethod in ipairs({method,"signal","mouse","touch"}) do
						if dispatchNativeShopButton(confirm,confirmMethod) then
							purchased,message=shopPurchaseEvidence(kind,name,beforeCount,beforeGems,nil)
							if purchased then return true,nil,"native-"..confirmMethod end
							if terminalShopError(message) then return false,message end
						end
					end
				end
...
local function openNativePetShop(wanted)
	local openButtons={}
	for _,node in ipairs(playerGui:GetDescendants()) do
		if nativeShopButton(node) and visibleNativeShopNode(node) then
			local text=string.lower(nativeShopText(node))
			local isShop=text:find("shop",1,true) or text:find("магаз",1,true)
			local isPet=text:find("pet",1,true) or text:find("пет",1,true)
			if isShop and nativePurchaseScore(node)<100 then
				table.insert(openButtons,{button=node,score=isPet and 2 or 1})
			end
		end
	end
	table.sort(openButtons,function(a,b) return a.score>b.score end)
	for _,entry in ipairs(openButtons) do
		for _,method in ipairs({"signal","mouse","touch"}) do
			if dispatchNativeShopButton(entry.button,method) then
				task.wait(0.15)
				local candidates=nativeShopCandidates(wanted)
...
local function triggerActualShopButton(name,kind,beforeCount,beforeGems)
	...
	local candidates=nativeShopCandidates(wanted)
	if #candidates==0 then candidates=openNativePetShop(wanted) end
	local lastError=nil
	for _,entry in ipairs(candidates) do
		local purchased,message,route=tryNativeShopCandidate(entry,name,kind,beforeCount,beforeGems)
		if purchased then return true,nil,route end
		if message then
			lastError=message
			if terminalShopError(message) then return false,message end
		end
...
local function buyShopItemOnce(name,kind)
	...
	if remembered and remembered.remote and remembered.remote.Parent then
		local ok,message,terminal=tryShopRoute(
	...
	if #remotes==0 then
		return false,lastError or "кнопка покупки не найдена в меню игры",selectedName
	end
	local strategies=item and {
		"object","buyObject","purchaseObject","buyItem","purchaseItem",
		"objectQuantity","named","buyNamed","purchaseNamed","namedId",
	...
	return false,lastError or "сервер отклонил все способы покупки",selectedName
end
