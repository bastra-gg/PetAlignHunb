...
local function cleanupSelectedPets(limit)
	...
	for _,name in ipairs(crystalPetNames(Runtime.selectedCrystal)) do
		allowed[shopNameKey(name)]=true
	end
	if next(allowed)==nil then return nil,"питомцы выбранного кристалла не найдены" end
	for name,enabled in pairs(Runtime.petCleanupTargets or {}) do
		local key=shopNameKey(name)
		if enabled and allowed[key] and not protectedCleanupName(name) then
			selected[key]=true
		end
	end
