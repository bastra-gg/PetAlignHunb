...
local function makeReport(rows)
	local lines={}
	table.insert(lines,"PetRockScanner v3 close teleport")
...
local function tpTo(row)
	...
	-- Направление от камня к текущему персонажу, чтобы не телепать внутрь.
	...
	-- ВАЖНО: раньше было +5.5, слишком далеко.
	-- Теперь становимся почти вплотную к телу камня.
	local radius=math.max(body.Size.X,body.Size.Z)/2
	local dist=math.max(radius+1.45,2.2)
	local y=body.Position.Y + math.min(body.Size.Y/2+2.6,4.2)

	local pos=Vector3.new(body.Position.X,y,body.Position.Z)+dir*dist
...
status.Text="SCAN, потом NEXT. Теперь телепорт ближе."
