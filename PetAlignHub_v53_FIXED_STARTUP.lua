-- PetAlignHub v52_EQUIPPED_XP_REMOTE_RUN
...
local function getPetTotalAny(rarity)
	if rarity=="Auto" or not rarity then
		local detected,total,src,lvl,xp=detectGuiRarity()
		return total,src,lvl,xp,detected
	end

	local total,src,lvl,xp=parsePetGuiTotal(rarity)
	if total then return total,src,lvl,xp end

	total,src,lvl,xp=readPetTotalFromData(rarity)
	if total then return total,src,lvl,xp end

	return nil,nil,nil,nil
end

local function readPetStable(rarity,tries,delay)
	tries=tries or 5
	delay=delay or .08
	local vals={}
	for _=1,tries do
...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v52 rock scan")
...
local function runOnTreadmill(part,targetTotal,rarity,expectedGain)
	...
	local speed=_G.PetAlignRunSpeed or 16
	...
	local maxRun=_G.PetAlignOneStepMaxSeconds or 32
	local minRun=_G.PetAlignMinTreadmillRunSeconds or 1.2
	local stable=0
	local lastXp=readPetStable(rarity,3,.04) or 0
	local noGainTimer=os.clock()
	local flipped=false
	...
	while not _G.PetAlignStop do
		...
		if elapsed>=minRun then
			...
			if now then
				...
				statusText("ALIGN: "..fmt(now).."/"..fmt(targetTotal).." | ok "..stable.."/4")
				if stable>=4 then break end

				if now>lastXp then
					lastXp=now
					noGainTimer=os.clock()
				end

				if not flipped and os.clock()-noGainTimer>7 then
					flipped=true
					dir=-dir
					side=Vector3.new(-dir.Z,0,dir.X)
					statusText("ALIGN: меняю сторону бега")
					place()
					task.wait(.12)
				end
			end
		...
		-- Видимый бег: Humanoid Move + небольшой velocity только если включено/по умолчанию.
		-- Он ниже скорости дорожки и не должен полностью игнорировать сопротивление.
		hum:Move(dir,false)
		pcall(function()hum:MoveTo(r.Position+dir*12)end)
		pcall(function()hum:ChangeState(Enum.HumanoidStateType.Running)end)

		if _G.PetAlignUseSoftVelocity~=false then
...
local function chooseTreadmillForDiff(diff)
	diff=round(diff)
...
local function doAlign()
	...
	if err then statusText(err) running=false pcall(uiFront) return end

	now=readPetStable(data.rarity,6,.07)
	if not now then
	...
	local diffStart=round(data.startTotal-now)
	if diffStart<=0 then
		statusText("ALIGN: выбранный вариант уже ниже/ровно. Выбери карточку выше.")
		running=false
		pcall(uiFront)
		return
	end

	statusText("ALIGN старт: цель "..fmt(data.startTotal).." XP")

	local startTime=os.clock()
	local stable=0

	while not _G.PetAlignStop do
		now=readPetStable(data.rarity,5,.05)
		...
		if diff<=0 then
			stable+=1
			statusText("ALIGN: цель "..fmt(now).."/"..fmt(data.startTotal).." | "..stable.."/5")
			...
			if not part then
				statusText("ALIGN: не нашёл дорожку. idx="..tostring(idx).."/"..tostring(total))
				break
			end

			local segmentTarget=math.min(data.startTotal,now+gain)
			statusText("ALIGN: осталось "..fmt(diff).." XP → дорожка +"..gain)
	...
	local final=readPetStable(data.rarity,7,.06)
...
mkLabel(top,"Pet Align v52",12,0,190,42,16,Color3.fromRGB(255,255,255),true)
...
statusText("v52: простой дизайн, без обрезания и центра.")
render()
