-- PetAlignHub v27 SMART_PRE_ROCK
...
local function runOnTreadmill(part,seconds,pet,rarity,targetTotal)
	...
	-- v26: НАТУРАЛЬНЫЙ бег.
	-- Больше НЕ ставим AssemblyLinearVelocity, чтобы дорожка могла нормально откидывать назад.
	...
	-- Только один мягкий заход на дорожку. Дальше никаких CFrame/Velocity.
	...
	while not _G.PetAlignStop and os.clock()-started<seconds do
		...
		-- Обычный input-бег вперёд. Пускай сама дорожка держит/толкает персонажа.
		hum:Move(dir,false)
...
local function useTreadmill(gain,count)
	...
	local seconds=math.max(_G.PetAlignTreadmillGainSeconds or 5.0,count*(_G.PetAlignTreadmillGainSeconds or 5.0))
	status.Text="Натуральный бег +"..gain..fallbackInfo.." ×"..count.." ~"..string.format("%.1f",seconds).."с"
...
local function findRockPart(rockId)
	...
	return keys and findPartSmart(keys,"rock") or nil
end
...
local function doPreRockHits(route,pet,rarity,targetTotal)
	...
	if not part then
		local n=copyScanReport("не нашёл камень", "rock", rockId)
		status.Text="Не нашёл камень для ускорения. Скан скопирован ("..n..")."
		return false
	end
	...
	for i=1,route.rockHits do
		...
		status.Text="Быстро через камень: "..route.rock.label.." "..i.."/"..route.rockHits
		touchAndActivate(part,tool)
		done=i
...
local function runBugHit(resume)
	...
	if not part then
		local n=copyScanReport("не нашёл камень", "rock", rockId)
		status.Text="Камень не найден. Скан скопирован ("..n..")."
		bugRunning=false
		running=false
		return
	end
...
local function runAlign()
	...
	status.Text="Маршрут: "..smartPlanText(route)
	_G.PetAlignStop=false
	...
	status.Text="Добиваю дорожками: "..planText(treadPlan)
