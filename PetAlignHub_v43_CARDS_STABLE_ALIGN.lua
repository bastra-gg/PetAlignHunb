-- PetAlignHub v43_CARDS_STABLE_ALIGN
...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v43 rock scan")
...
local function render()
	...
	if choiceCards then
		for i,card in ipairs(choiceCards)do
...
frame.Position=UDim2.new(.5,-230,.5,-210)
...
title.Text="Pet Align Comfort v43"
...
local function choiceCard(i,x)
	local b=Instance.new("TextButton",frame)
	b.Size=UDim2.new(0,140,0,62)
	b.Position=UDim2.new(0,x,0,94)
	b.Text="#"
	b.TextColor3=Color3.fromRGB(255,238,170)
	b.BackgroundColor3=Color3.fromRGB(28,29,55)
...
local _,targetBody=card(frame,14,164,214,96,"ЦЕЛЬ",Color3.fromRGB(22,45,38))
targetText=targetBody

local _,currentBody=card(frame,234,164,212,96,"ТЕКУЩИЙ ПЕТ",Color3.fromRGB(28,30,58))
currentText=currentBody

local _,planBody=card(frame,14,268,214,76,"ВЫРАВНИВАНИЕ",Color3.fromRGB(25,42,63))
planTextUi=planBody

local _,bugBody=card(frame,234,268,212,76,"БАГ",Color3.fromRGB(54,30,42))
bugText=bugBody

status=Instance.new("TextLabel",frame)
status.Size=UDim2.new(1,-28,0,18)
status.Position=UDim2.new(0,14,0,350)
...
local alignBtn=btn("ALIGN",14,76,Color3.fromRGB(45,100,180))
local bugBtn=btn("BUG",98,62,Color3.fromRGB(45,130,70))
local scanBtn=btn("SCAN",168,62,Color3.fromRGB(85,70,160))
local copyBtn=btn("COPY",238,62,Color3.fromRGB(115,80,45))
local stopBtn=btn("STOP",308,78,Color3.fromRGB(125,35,48))
...
if okMath then
	statusText("v43: карточки выбора + стабильный автостоп.")
else
