-- PetAlignHub v47_REBUILD_MINI_UI
...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v47 rock scan")
...
local function makeStroke(obj,col,thick)
	local s=Instance.new("UIStroke",obj)
	s.Color=col or Color3.fromRGB(120,74,255)
...
frame.Name="Main"
frame.Size=UDim2.new(0,382,0,500)
...
makeCorner(frame,16)
makeStroke(frame,Color3.fromRGB(120,74,255),1.6)

local mini=Instance.new("TextButton",gui)
mini.Name="OpenMini"
mini.Size=UDim2.new(0,64,0,36)
mini.Position=UDim2.new(0,10,0,92)
mini.BackgroundColor3=Color3.fromRGB(45,100,180)
mini.Text="PET"
...
minBtn.BackgroundColor3=Color3.fromRGB(55,48,95)
minBtn.Font=Enum.Font.GothamBlack
minBtn.TextSize=18
makeCorner(minBtn,9)

local close=Instance.new("TextButton",dragBar)
...
rebBox.Size=UDim2.new(0,220,0,34)
rebBox.Position=UDim2.new(0,12,0,48)
rebBox.Text=""
...
rebBox.TextSize=13
makeCorner(rebBox,10)

rarityBtn=Instance.new("TextButton",frame)
rarityBtn.Size=UDim2.new(0,126,0,34)
rarityBtn.Position=UDim2.new(1,-138,0,48)
rarityBtn.Text="Auto"
...
resultsScroll.Size=UDim2.new(1,-24,0,138)
resultsScroll.Position=UDim2.new(0,12,0,106)
...
makeCorner(resultsScroll,12)
makeStroke(resultsScroll,Color3.fromRGB(72,64,130),1)
...
local function infoCard(x,y,w,h,titleText,color)
	...
	body.Size=UDim2.new(1,-14,1,-28)
	body.Position=UDim2.new(0,7,0,26)
...
targetText=infoCard(12,252,176,92,"ЦЕЛЬ",Color3.fromRGB(22,45,38))
currentText=infoCard(196,252,174,92,"ПЕТ",Color3.fromRGB(28,30,58))
planTextUi=infoCard(12,352,176,72,"ALIGN",Color3.fromRGB(25,42,63))
bugText=infoCard(196,352,174,72,"BUG",Color3.fromRGB(54,30,42))

status=Instance.new("TextLabel",frame)
status.Size=UDim2.new(1,-24,0,28)
status.Position=UDim2.new(0,12,0,430)
status.BackgroundTransparency=1
status.Text="Выбери карточку. ALIGN ровняет пета, BUG бьёт камень."
...
local alignBtn=btn("ALIGN",12,66,Color3.fromRGB(45,100,180))
local bugBtn=btn("BUG",84,54,Color3.fromRGB(45,130,70))
local scanBtn=btn("SCAN",144,58,Color3.fromRGB(85,70,160))
local copyBtn=btn("COPY",208,58,Color3.fromRGB(115,80,45))
local stopBtn=btn("STOP",272,78,Color3.fromRGB(125,35,48))

-- Адаптация без прилипания к центру: просто уменьшаем окно, если экран маленький.
local function fitToScreen()
	local cam=workspace.CurrentCamera
	local vp=cam and cam.ViewportSize or Vector2.new(900,600)
	local scale=math.min(1,(vp.X-20)/382,(vp.Y-80)/500)
	if scale<.68 then scale=.68 end
	frame.Size=UDim2.new(0,382*scale,0,500*scale)
	frame.Position=UDim2.new(0,10,0,math.max(48,math.floor(vp.Y*.12)))
...
-- Драг только за верхнюю полоску, а не за кнопки.
...
statusText("v47: мини-окно сбоку. Верхнюю полоску можно перетаскивать.")
render()
