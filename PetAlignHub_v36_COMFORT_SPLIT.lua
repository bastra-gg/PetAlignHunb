...
local function bugHit(data)
	...
	while not _G.PetAlignStop and os.clock()-started<seconds do
		...
		statusText("BUG: удар "..hits.." → "..data.rockLabel)
		task.wait(delay)
	end
...
local function doBug()
	...
	if now and now<data.startTotal then
		statusText("BUG: сначала ALIGN. Сейчас "..fmt(now).."/"..fmt(data.startTotal))
		running=false
		return
	end

	statusText("BUG старт: "..data.rockLabel)
...
title.Text="Pet Align Comfort v36"
...
if okMath then
	statusText("Математика редкостей OK. ALIGN → BUG.")
else
