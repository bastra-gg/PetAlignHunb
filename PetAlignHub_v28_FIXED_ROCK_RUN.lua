-- PetAlignHub v28_FIXED_ROCK_RUN
...
local function runOnTreadmill(part,seconds,pet,rarity,targetTotal)
	...
	local minRun=_G.PetAlignMinTreadmillRunSeconds or 1.25
	local maxRun=seconds or (_G.PetAlignTreadmillMaxRunSeconds or 60)

	while not _G.PetAlignStop do
		local elapsed=os.clock()-started

		if targetTotal and elapsed>=minRun then
		...
		-- Только обычный бег. Никакого Velocity/CFrame в цикле,
		-- чтобы дорожка сама откидывала назад как при ручном беге.
		hum:Move(dir,false)
...
local ROCK_EXACT={
	AncientJungle={"ancient jungle","ancientjungle","jungle rock","ancient rock"},
	MuscleKing={"muscle king","muscleking","king rock"},
	Legends={"legends rock","legend rock","legends","legend"},
	Inferno={"inferno rock","inferno"},
	Mystic={"mystic rock","mystic"},
	Frozen={"frozen rock","ice rock","frost rock","frozen"},
	Golden={"golden rock","gold rock","golden"},
	Large={"large rock","big rock"},
	Punching={"punching rock","punch rock"},
	Tiny={"tiny rock","small rock"},
}

local ROCK_BAD_WORDS={
	"tread","treadmill","дорож","бег",
	"throw","throwing","брос","launch","catapult",
	"training","trainer","трен","machine","машин",
	"weight","barbell","dumbbell","bench","lift",
	"agility","speed","jump","chest","egg",
}

local function badRockText(txt)
	txt=tostring(txt or ""):lower()
	for _,w in ipairs(ROCK_BAD_WORDS)do
		if txt:find(w,1,true)then return true,w end
	end
	return false,nil
end

local function findRockPart(rockId)
	local exact=ROCK_EXACT[rockId] or {}
	local keys=nil
	...
	for _,obj in ipairs(workspace:GetDescendants())do
		...
		if part and part:IsA("BasePart")then
			local m=obj:IsA("Model") and obj or obj:FindFirstAncestorWhichIsA("Model") or obj
			local txt=(objectText(m).." "..objectText(obj).." "..objectPath(part)):lower()

			local badWord=nil
			local isBad,bw=badRockText(txt)
			if isBad then badWord=bw end

			local score=0

			for _,w in ipairs(exact)do
				if txt:find(w,1,true)then score+=220 end
			end
			for _,w in ipairs(keys)do
				if txt:find(tostring(w):lower(),1,true)then score+=65 end
			end

			if txt:find("rock",1,true)then score+=55 end
			if txt:find("rocks",1,true)then score+=70 end
			if txt:find("stone",1,true)or txt:find("скал",1,true)or txt:find("кам",1,true)then score+=45 end

			-- Сильно предпочитаем настоящие скалы/камни, а не тренажёры/бросалки.
			if txt:find("workspace/rocks",1,true)or txt:find("/rocks/",1,true)or txt:find("/rock/",1,true)then score+=140 end
			if part.Anchored then score+=18 else score-=55 end
			if part:FindFirstChildWhichIsA("TouchTransmitter",true)or m:FindFirstChildWhichIsA("TouchTransmitter",true)then score+=20 end
			if hrp then score-=math.clamp((part.Position-hrp.Position).Magnitude/650,0,35)end

			if badWord then score-=280 end
...
local function doPreRockHits(route,pet,rarity,targetTotal)
	...
	if not part then
		local n=copyScanReport("не нашёл камень", "rock", rockId)
		status.Text="Не нашёл скалу для ускорения. Скан скопирован ("..n..")."
		return false
	end
	...
	if r then r.Anchored=oldAnchored end
	if hum then
...
local function runBugHit(resume)
	...
	local rockId=data.rock
	local part=findRockPart(rockId)
	if not part then
		local n=copyScanReport("не нашёл камень", "rock", rockId)
		status.Text="Скала не найдена. Скан скопирован ("..n..")."
		bugRunning=false
		running=false
		return
	end
	...
	status.Text=(resume and "RESUME BUG: " or "GO BUG: ")..tostring(rockId)

	while not _G.PetAlignStop and not _G.PetAlignBugStop and os.clock()-started<seconds do
		-- Позиция уже зафиксирована Anchored=true, CFrame по КД не дёргаем.
		touchAndActivate(part,tool)
		task.wait(_G.PetAlignBugHitDelay or .12)
	end
