...
local function protectedCleanupPet(item)
	...
	for _,field in ipairs({
		...
		"PetEquipped","petEquipped","EquippedPet","equippedPet",
		"Locked","locked","IsLocked","isLocked","Favorite","favorite",
		"Favourite","favourite","Favorited","favorited","Protected","protected",
		"Bugged","bugged",
	...
	local chosenName=item:FindFirstChild("chosenName")
	if chosenName then
		local ok,value=safe(function() return tostring(chosenName.Value or "") end)
		if ok and value~="" and value~=tostring(item.Name) then return true end
	end
...
local function inventoryPetSnapshot()
	local folder=lp:FindFirstChild("petsFolder")
	if not folder then return nil,"инвентарь питомцев не найден" end
	local snapshot={items={},groups={},total=0}
	local categoryNames={
		basic=true,advanced=true,rare=true,epic=true,unique=true,legendary=true,
		mythical=true,mythic=true,secret=true,limited=true,apex=true,elite=true,
		omega=true,pets=true,pet=true,inventory=true,equipped=true,
		favorites=true,favourites=true,locked=true,protected=true,
	}
	local inspected=0
	local function inspect(item,depth)
		if not item or depth>5 or inspected>=1200 then return end
		inspected=inspected+1
		local container=item:IsA("Folder") or item:IsA("Configuration") or item:IsA("Model")
		if not container and not item:IsA("ObjectValue") then return end
		local name=tostring(item.Name or "")
		local key=shopNameKey(name)
		if key=="" then return end
		local category=categoryNames[key]==true or key:find("folder",1,true)~=nil
		if not category and depth==1 and container then
			for _,child in ipairs(item:GetChildren()) do
		...
		if category then
			for _,child in ipairs(item:GetChildren()) do inspect(child,depth+1) end
			return
		end
		local protected=protectedCleanupPet(item)
		local group=snapshot.groups[key]
		if not group then
			group={name=name,count=0,available=0,protected=0}
			snapshot.groups[key]=group
		end
		group.count=group.count+1
		if protected then group.protected=group.protected+1
		else group.available=group.available+1 end
		snapshot.total=snapshot.total+1
		table.insert(snapshot.items,{item=item,name=name,key=key,protected=protected})
	end
	for _,item in ipairs(folder:GetChildren()) do inspect(item,1) end
	return snapshot
end

local function previewSelectedPetCleanup(selected)
	local snapshot,errorMessage=inventoryPetSnapshot()
	if not snapshot then return nil,errorMessage end
	local selectedKeys={}
	for name,enabled in pairs(selected or {}) do
		if enabled and not protectedCleanupName(name) then selectedKeys[shopNameKey(name)]=true end
	end
	local deletable=0
	for _,entry in ipairs(snapshot.items) do
		if selectedKeys[entry.key] and not entry.protected then deletable=deletable+1 end
	end
	return {
		total=snapshot.total,
		deletable=math.min(deletable,math.max(0,snapshot.total-1)),
		types=selectedCount(selected),
	}
end

local function cleanupSelectedPets(limit,confirmed)
	if not Runtime.petCleanupEnabled and not confirmed then return 0 end
	...
	local sold=0
	local remaining=snapshot.total
	for _,entry in ipairs(snapshot.items) do
		if Runtime.petCleanupPending[entry.item] then remaining=remaining-1 end
	end
	for _,entry in ipairs(snapshot.items) do
		if sold>=maxCount or remaining<=1 then break end
		local item=entry.item
		if selected[entry.key] and not entry.protected
			and not Runtime.petCleanupPending[item] and not protectedCleanupPet(item) then
			local ok,result=safe(function()
				if remote:IsA("RemoteFunction") then
					return remote:InvokeServer("sellPet",item)
				end
				remote:FireServer("sellPet",item)
			end)
			...
			if ok and not rejected then
				Runtime.petCleanupPending[item]=now
...
do
local deleteConfirmShade=Instance.new("TextButton")
deleteConfirmShade.Name="DeleteConfirmation"
deleteConfirmShade.Parent=main
deleteConfirmShade.Size=UDim2.new(1,0,1,0)
deleteConfirmShade.BackgroundColor3=Color3.fromRGB(3,5,12)
deleteConfirmShade.BackgroundTransparency=0.10
deleteConfirmShade.BorderSizePixel=0
deleteConfirmShade.Text=""
deleteConfirmShade.AutoButtonColor=false
deleteConfirmShade.Visible=false
deleteConfirmShade.ZIndex=95

local deleteConfirmPanel=Instance.new("Frame")
deleteConfirmPanel.Parent=deleteConfirmShade
deleteConfirmPanel.Size=UDim2.new(1,-40,0,196)
deleteConfirmPanel.Position=UDim2.new(0.5,0,0.5,0)
deleteConfirmPanel.AnchorPoint=Vector2.new(0.5,0.5)
deleteConfirmPanel.BackgroundColor3=THEME.Bg
deleteConfirmPanel.BackgroundTransparency=0.02
deleteConfirmPanel.BorderSizePixel=0
deleteConfirmPanel.ZIndex=96
corner(deleteConfirmPanel,13)
neonStroke(deleteConfirmPanel,2,0.06)
gradient(deleteConfirmPanel,THEME.Panel,THEME.Bg,120)

local deleteConfirmTitle=label(deleteConfirmPanel,"ПОДТВЕРДИ УДАЛЕНИЕ",12,
	Enum.Font.GothamBlack,THEME.Danger)
deleteConfirmTitle.Size=UDim2.new(1,-22,0,29)
deleteConfirmTitle.Position=UDim2.fromOffset(11,9)
deleteConfirmTitle.ZIndex=97

local deleteConfirmText=label(deleteConfirmPanel,"",10,Enum.Font.Gotham,THEME.Text)
deleteConfirmText.Size=UDim2.new(1,-22,0,103)
deleteConfirmText.Position=UDim2.fromOffset(11,42)
deleteConfirmText.TextWrapped=true
deleteConfirmText.TextYAlignment=Enum.TextYAlignment.Top
deleteConfirmText.ZIndex=97

local deleteCancel=button(deleteConfirmPanel,"ОТМЕНА",THEME.SurfaceAlt)
deleteCancel.Size=UDim2.new(0.48,-13,0,33)
deleteCancel.Position=UDim2.new(0,10,1,-43)
deleteCancel.TextSize=9
deleteCancel.ZIndex=98

local deleteAccept=button(deleteConfirmPanel,"ПОДТВЕРДИТЬ",THEME.Danger)
deleteAccept.Size=UDim2.new(0.52,-13,0,33)
deleteAccept.Position=UDim2.new(0.48,3,1,-43)
deleteAccept.TextSize=8
deleteAccept.ZIndex=98

local pendingPetDeletion=nil
local function closeDeleteConfirmation()
	pendingPetDeletion=nil
	deleteConfirmShade.Visible=false
end

showDeleteConfirmation=function(selected,preview)
	pendingPetDeletion={selected=selected,preview=preview}
	deleteConfirmText.Text="Все выбранные питомцы будут удалены из инвентаря.\n\n"
		.."К удалению: "..tostring(preview.deletable).." • видов: "..tostring(preview.types).."\n"
		.."Apex, защищённые и последний питомец останутся."
	deleteConfirmShade.Visible=true
end

Runtime.closeDeleteConfirmation=closeDeleteConfirmation
addConn(deleteCancel.Activated:Connect(closeDeleteConfirmation))
addConn(deleteAccept.Activated:Connect(function()
	local pending=pendingPetDeletion
	if not pending or Runtime.petCleanupBusy then return end
	closeDeleteConfirmation()
	Runtime.petCleanupTargets=pending.selected
	refreshExtraUI()
	Runtime.petCleanupToken=(Runtime.petCleanupToken or 0)+1
	local token=Runtime.petCleanupToken
	Runtime.petCleanupBusy=true
	setStatus("УДАЛЕНИЕ ИЗ ИНВЕНТАРЯ: "..tostring(pending.preview.deletable))
	task.spawn(function()
		local removed=0
		local failed=nil
		for _=1,60 do
			if not Runtime.alive or Runtime.petCleanupToken~=token then break end
			local count,errorMessage=cleanupSelectedPets(10,true)
			if count==nil then failed=errorMessage break end
			if count==0 then break end
			removed=removed+count
			if removed>=pending.preview.deletable then break end
			task.wait(0.12)
		end
		if Runtime.petCleanupToken~=token then return end
		Runtime.petCleanupBusy=false
		if failed then setStatus("УДАЛЕНИЕ: "..tostring(failed))
		elseif removed==0 then setStatus("УДАЛЕНИЕ: нет доступных питомцев")
		else setStatus("УДАЛЕНО ИЗ ИНВЕНТАРЯ: "..tostring(removed)) end
		refreshExtraUI()
	end)
end))
end
...
petCleanupLever=makeFeatureToggle(crystalFeatureBody,"D","АВТОУДАЛЕНИЕ","только отмеченные питомцы",false,function(on,api)
	if on then
		if selectedCount(Runtime.petCleanupTargets)==0 then
			api.Set(false,true)
			setStatus("ВЫБЕРИ ПИТОМЦЕВ ДЛЯ УДАЛЕНИЯ")
			return
		end
		if not findPetSellRemote() then
			api.Set(false,true)
			setStatus("АВТОУДАЛЕНИЕ: sellPetEvent не найден")
			return
		end
...
cleanupSelection=makeSelectionRow(crystalSettingsBody,"УДАЛЯТЬ",cleanupSelectionLabel(),function()
	...
	if not snapshot then
		setStatus("ИНВЕНТАРЬ: "..tostring(errorMessage))
		return
	end
	if snapshot.total==0 then
		setStatus("ИНВЕНТАРЬ ПИТОМЦЕВ ПУСТ")
		return
	end
	local options={}
	local allowed={}
	for key,group in pairs(snapshot.groups) do
		allowed[key]=group
		local protected=protectedCleanupName(group.name) or group.available==0
		table.insert(options,{
			id=group.name,
			label=group.name,
			sub=protected and ("В инвентаре: "..tostring(group.count).." • защищены")
				or ("В инвентаре: "..tostring(group.count).." • удалить: "..tostring(group.available)),
			disabled=protected,
		})
	end
	table.sort(options,function(a,b) return string.lower(a.label)<string.lower(b.label) end)
	local current={}
	for name,enabled in pairs(Runtime.petCleanupTargets or {}) do
		local group=allowed[shopNameKey(name)]
		if enabled and group and group.available>0 and not protectedCleanupName(group.name) then
			current[group.name]=true
		end
	end
	openPicker("ПИТОМЦЫ ИЗ ИНВЕНТАРЯ",options,{
		multiple=true,
		selected=current,
		onDone=function(selected)
			local chosen={}
			for name,enabled in pairs(selected or {}) do
				local group=allowed[shopNameKey(name)]
				if enabled and group and group.available>0 and not protectedCleanupName(group.name) then
					chosen[group.name]=true
				end
			end
			if selectedCount(chosen)==0 then
				Runtime.petCleanupTargets={}
				if Runtime.petCleanupEnabled then
					Runtime.petCleanupEnabled=false
					if Runtime.leverRefs.petCleanup then Runtime.leverRefs.petCleanup.Set(false,true) end
				end
				refreshExtraUI()
				setStatus("УДАЛЕНИЕ: питомцы не выбраны")
				return
			end
			local preview,previewError=previewSelectedPetCleanup(chosen)
			if not preview then
				setStatus("УДАЛЕНИЕ: "..tostring(previewError))
				return
			end
			if preview.deletable<=0 then
				setStatus("УДАЛЕНИЕ: последний или защищённый питомец")
				return
			end
			showDeleteConfirmation(chosen,preview)
		end,
	})
end)
