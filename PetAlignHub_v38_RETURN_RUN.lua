...
local function runOnTreadmill(part,targetTotal,rarity)
	...
	-- v38: возвращён старый рабочий режим.
	-- Один телепорт на дорожку, дальше постоянный прямой бег + горизонтальная скорость.
	...
	while not _G.PetAlignStop do
		...
		-- Это то, что раньше заставляло перса реально бежать, а не просто стоять после телепорта.
		r=root()
		if r then
...
title.Text="Pet Align Comfort v38"
...
if okMath then
	statusText("v38: вернул рабочий бег дорожек + XP фикс.")
else
