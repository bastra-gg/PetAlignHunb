...
local function runOnTreadmill(part,seconds,pet,rarity,targetTotal)
	...
	local speed=_G.PetAlignRunSpeed or 30
	...
	-- ВАЖНО v14:
	-- телепорт только один раз в начало, дальше НЕ возвращаем персонажа назад,
	-- чтобы бег не сбивался.
	hum.WalkSpeed=speed
	...
	while not _G.PetAlignStop and os.clock()-started < seconds do
		...
		-- Только держим направление взгляда, позицию НЕ трогаем.
		pcall(function()
		...
		-- Мягкая скорость, без рывков назад.
		local vel=r.AssemblyLinearVelocity
		r.AssemblyLinearVelocity=Vector3.new(dir.X*speed,vel.Y,dir.Z*speed)
...
local function useTreadmill(gain,count)
	...
	local seconds=math.max(1.2,count*(_G.PetAlignRunSecondsPerUse or 0.32))
	status.Text="Ровный бег +"..gain..fallbackInfo.." ~"..string.format("%.1f",seconds).."с"
