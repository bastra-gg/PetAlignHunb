-- PetAlignHub v35_SPLIT_ALIGN_BUG
...
title.Text="Pet Align SPLIT v35"
...
outText.Text="PLAN покажет маршрут. ALIGN и BUG запускаются отдельно."

status=Instance.new("TextLabel",frame)
status.Size=UDim2.new(1,-24,0,18)
status.Position=UDim2.new(0,12,0,282)
status.BackgroundTransparency=1
status.Text="ALIGN = ровнять. BUG = ударить нужный камень."
...
local bugBtn=btn("BUG",76,48,Color3.fromRGB(45,130,70))
local planBtn=btn("PLAN",130,48,Color3.fromRGB(85,70,160))
local scanBtn=btn("SCAN",184,50,Color3.fromRGB(95,75,155))
local copyBtn=btn("COPY",240,50,Color3.fromRGB(115,80,45))
local stopBtn=btn("STOP",296,70,Color3.fromRGB(125,35,48))
...
local function buildReport()
	...
	local lines={}
	table.insert(lines,"SPLIT AUTO RESULT")
	...
	if now then
		...
		if diff<0 then
			...
			table.insert(lines,"Потом жми BUG по "..best.rockLabel)
		end
	else
...
local function chooseDataForCurrent()
	render()
...
local function runAlignOnlyButton()
	if running then return end
	running=true
	_G.PetAlignStop=false
	stopRockLock()

	local data,now=chooseDataForCurrent()
	if not data then running=false return end
	...
	if now>=data.startTotal then
		statusText("Уже ровно/выше цели. Можно жать BUG.")
		running=false
		return
	end

	statusText("ALIGN: ровняю дорожками до "..fmt(data.startTotal))
	local ok=alignOnly(data,now)

	if ok then
		statusText("ALIGN готов. Теперь жми BUG → "..data.rockLabel)
	else
		statusText("ALIGN не дошёл до цели / остановлен.")
	end

	running=false
end

local function runBugOnlyButton()
	if running then return end
	running=true
	_G.PetAlignStop=false
	stopRockLock()
	...
	statusText("BUG: "..data.rockLabel)
	local ok=bugHit(data)

	if ok then
		statusText("BUG готов: "..data.rockLabel)
	else
...
alignBtn.MouseButton1Click:Connect(function()
	task.spawn(function()
		local ok,err=pcall(runAlignOnlyButton)
...
bugBtn.MouseButton1Click:Connect(function()
	task.spawn(function()
		local ok,err=pcall(runBugOnlyButton)
...
_G.PetAlignFullAuto={
	...
	Align=function()task.spawn(runAlignOnlyButton)end,
	Bug=function()task.spawn(runBugOnlyButton)end,
