...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v50 rock scan")
...
local function runOnTreadmill(part,targetTotal,rarity,expectedGain)
	...
	local heightAbove=_G.PetAlignHeightOffset or math.clamp((hum.HipHeight or 2)+1.15,2.7,4.0)
	...
	local function place()
		local rr=root()
		if rr then
			rr.CFrame=CFrame.lookAt(startPos,startPos+dir)
			rr.AssemblyAngularVelocity=Vector3.zero
		end
	end
	...
	local maxRun=_G.PetAlignOneStepMaxSeconds or 24
	...
	local maxLong=math.max(2,length/2-1.25)
	local maxSide=math.max(.8,width/2-.75)
	...
	while not _G.PetAlignStop do
		...
		if elapsed>=minRun then
			...
			if now then
				...
				statusText("ALIGN: "..fmt(now).."/"..fmt(targetTotal).." | stable "..stable.."/4")
				...
				-- Если дорожка не даёт XP, пробуем обратное направление.
				if not flipped and os.clock()-noGainTimer>7 then
					flipped=true
					dir=-dir
					side=Vector3.new(-dir.Z,0,dir.X)
					statusText("ALIGN: меняю направление дорожки")
					place()
					task.wait(.15)
				end
			end
		...
		hum:Move(dir,false)
		pcall(function()hum:MoveTo(r.Position+dir*10)end)
		pcall(function()hum:ChangeState(Enum.HumanoidStateType.Running)end)
		pressW(true)

		-- Держим внутри полотна, но НЕ форсим скорость.
...
mkLabel(top,"Pet Align v50",12,0,190,42,16,Color3.fromRGB(255,255,255),true)
...
statusText("v50: простой дизайн, без обрезания и центра.")
render()
