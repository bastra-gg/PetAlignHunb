-- RockBugFuse portable loader
local VERSION="1.0.0"
local URL="https://raw.githubusercontent.com/bastra-gg/PetAlignHunb/main/RockBugFuse_core.lua"
local CONTRACT="-- RockBugFuse core contract: 1"
local env=_G
...
local function get(url)
 ...
 error("RockBugFuse: core download failed",0)
end
local compiler=loadstring or env.loadstring
if type(compiler)~="function"then error("RockBugFuse: executor has no loadstring",0)end
local src=get(URL.."?v="..VERSION.."&t="..tostring(os.time()))
local chunk,compileError=compiler(src)
if type(chunk)~="function"then error("RockBugFuse compile: "..tostring(compileError),0)end
local runtime=chunk()
if type(runtime)~="table"or runtime.startupReady~=true then error("RockBugFuse: core did not start",0)end
return runtime
