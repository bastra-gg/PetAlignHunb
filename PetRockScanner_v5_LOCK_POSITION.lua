-- PetRockScanner_v5_LOCK_POSITION
...
local function makeReport(rows)
	local lines={}
	table.insert(lines,"PetRockScanner v5 in-rock teleport + lock")
...
local function stopLock()
	...
	local rr=root()
	if rr and oldAnchored~=nil then
		rr.Anchored=oldAnchored
	end

	local c=lp.Character
	local hum=c and c:FindFirstChildWhichIsA("Humanoid")
	if hum then
		if oldAuto~=nil then hum.AutoRotate=oldAuto end
		if oldSpeed~=nil then hum.WalkSpeed=oldSpeed end
	end
...
local function startLock(cf)
	...
	lockedCF=cf
	oldAnchored=rr.Anchored
	if hum then
		oldAuto=hum.AutoRotate
	...
	-- Не Anchored по умолчанию, чтобы игра не ломалась, но позиция жёстко держится.
	if _G.PetRockUseAnchor then
		rr.Anchored=true
	end
	...
	lockConn=game:GetService("RunService").Heartbeat:Connect(function()
		...
		if not r or not lockedCF then return end
...
local function tpTo(row)
	...
	local cf=CFrame.lookAt(pos,Vector3.new(body.Position.X,y,body.Position.Z))
	rr.CFrame=cf
	rr.AssemblyLinearVelocity=Vector3.zero
	rr.AssemblyAngularVelocity=Vector3.zero

	-- v5: после телепорта сразу фиксируем позицию, чтобы камень не выкидывал.
	startLock(cf)
end


local gui=Instance.new("ScreenGui")
gui.Name="PetRockScannerV5"
gui.ResetOnSpawn=false
gui.Parent=lp:WaitForChild("PlayerGui")

local frame=Instance.new("Frame",gui)
frame.Size=UDim2.new(0,318,0,116)
frame.Position=UDim2.new(.5,-159,.5,-58)
...
status.Text="NEXT = в камень + FIX. UNLOCK = отпустить."
...
local unlockBtn=mkBtn("UNLOCK",226,76,Color3.fromRGB(130,45,55))
...
nextBtn.MouseButton1Click:Connect(function()
	...
	tpTo(row)
	status.Text=("#%02d FIXED IN ROCK req=%s %s"):format(cursor,tostring(row.req),row.name)
end)

unlockBtn.MouseButton1Click:Connect(function()
	stopLock()
	status.Text="Фиксация снята."
end)
