-- PetAlignHub v23 CAL_ALL_DIRECT
...
local function fallbackTreadmillPart(gain)
	...
	-- Важно: берём НЕ все 51 дорожку карты, а ближайшие 6 к персонажу.
...
local function runOnTreadmill(part,seconds,pet,rarity,targetTotal)
	...
	-- v23: корректировка высоты под размер дорожки.
	-- Не фиксированное Y, а высота от верхней поверхности детали.
	local up=part.CFrame.UpVector
	local heightAbove=math.clamp((hum.HipHeight or 2)+1.75,2.8,5.2)
	local startPos=part.Position - dir*(length*0.42) + up*(part.Size.Y/2 + heightAbove)
...
local function calibrateTreadmills()
	...
	local partsMap={}
	if hasPositive then
		for gain=1,math.min(6,#sorted)do
			map[gain]=sorted[gain].index
			partsMap[gain]=sorted[gain].partObj
		end
	else
		-- Если GUI опять читает XP криво, не строим фейковую карту.
	...
	local lines={}
	table.insert(lines,"PetAlignHub treadmill calibration v23 ALL DIRECT")
	table.insert(lines,"place: "..tostring(game.PlaceId))
	...
	for rank,r in ipairs(sorted)do
		table.insert(lines,("#%02d possibleGain=+%s testedIndex=%s directDelta=%s perSec=%.2f"):format(rank,rank,r.index,fmt(r.delta),r.perSec))
	end
