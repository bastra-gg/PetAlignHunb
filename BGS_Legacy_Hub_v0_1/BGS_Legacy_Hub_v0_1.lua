-- BGS Legacy Hub loader -> v0.4 core + v0.4.5 real-island/current-island patch
...
local PATCH_URL="https://raw.githubusercontent.com/bastra-gg/PetAlignHunb/main/BGS_Legacy_Hub_v0_1/BGS_RealTP_CurrentIslandCollect_v0_4_5.lua"
...
if not okPatch then
    warn("BGS v0.4.5 patch HttpGet failed: "..tostring(patchSource))
    return S
end
local patchChunk,patchCompileError=loadstring(patchSource)
if not patchChunk then
    warn("BGS v0.4.5 patch compile failed: "..tostring(patchCompileError))
    return S
end
...
if type(patch)=="function" then
    ...
    elseif not okApply then
        warn("BGS v0.4.5 patch apply failed: "..tostring(result))
    end
end
return S
