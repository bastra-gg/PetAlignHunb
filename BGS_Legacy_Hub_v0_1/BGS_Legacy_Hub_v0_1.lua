-- BGS Legacy Hub loader -> v0.4 core + v0.4.4 real-island TP patch
...
local PATCH_URL="https://raw.githubusercontent.com/bastra-gg/PetAlignHunb/main/BGS_Legacy_Hub_v0_1/BGS_Canonical_RealIslandTP_Patch_v0_4_4.lua"
...
if not okPatch then
    warn("BGS real-island TP patch HttpGet failed: "..tostring(patchSource))
    return S
end
local patchChunk,patchCompileError=loadstring(patchSource)
if not patchChunk then
    warn("BGS real-island TP patch compile failed: "..tostring(patchCompileError))
    return S
end
...
if type(patch)=="function" then
    ...
    elseif not okApply then
        warn("BGS real-island TP patch apply failed: "..tostring(result))
    end
end
return S
