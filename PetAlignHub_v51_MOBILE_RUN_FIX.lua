-- PetAlignHub v51_MOBILE_RUN_FIX
...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v51 rock scan")
...
local function runOnTreadmill(part,targetTotal,rarity,expectedGain)
	...
	-- v51: НЕ трогаем JumpPower/JumpHeight вообще.
	-- Иначе на мобиле пропадает кнопка прыжка.
	...
	local maxRun=_G.PetAlignOneStepMaxSeconds or 28
	...
	-- На телефоне keypress/W часто не работает или конфликтует с UI.
	-- Поэтому по умолчанию только Humanoid:Move + MoveTo.
	if _G.PetAlignUseWKey then pressW(true)end
	...
	while not _G.PetAlignStop do
		...
		-- Основной мобильный бег.
		hum:Move(dir,false)
		pcall(function()hum:ChangeState(Enum.HumanoidStateType.Running)end)

		if os.clock()>=nextMoveTo then
			nextMoveTo=os.clock()+0.18
			pcall(function()
				hum:MoveTo(r.Position+dir*16)
			end)
		end

		if _G.PetAlignUseWKey and os.clock()>=nextW then
			nextW=os.clock()+0.25
			pressW(true)
		end

		-- Удержание на полотне без форса скорости.
	...
	pcall(function()
		local rr=root()
		if rr then hum:MoveTo(rr.Position)end
	end)

	hum.WalkSpeed=oldSpeed
	hum.AutoRotate=oldAuto
	-- Прыжок не восстанавливаем, потому что мы его больше не меняем.
...
local function doAlign()
	...
	if now>=data.startTotal then
		statusText("ALIGN: этот вариант уже ниже/ровно. Выбери карточку выше.")
		running=false
		pcall(uiFront)
		return
	end
	...
	while not _G.PetAlignStop do
		...
		if diff<=0 then
			...
			statusText("ALIGN: diff "..fmt(diff).." → дорожка +"..gain.." до "..fmt(segmentTarget))
...
mkLabel(top,"Pet Align v51",12,0,190,42,16,Color3.fromRGB(255,255,255),true)
...
statusText("v51: простой дизайн, без обрезания и центра.")
render()
