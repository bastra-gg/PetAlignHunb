...
local function runOnTreadmill(part,seconds)
	local c=lp.Character or lp.CharacterAdded:Wait()
	local r=root()
	...
	local startPos=part.Position - dir*math.max(part.Size.Z,part.Size.X)*0.35 + Vector3.new(0,3.2,0)
	r.CFrame=CFrame.lookAt(startPos,startPos+dir)

	local oldSpeed=hum.WalkSpeed
	if oldSpeed < 16 then hum.WalkSpeed=16 end

	local t=os.clock()
	while os.clock()-t < seconds do
		hum:Move(dir,false)
		r.AssemblyLinearVelocity=Vector3.new(dir.X*18,r.AssemblyLinearVelocity.Y,dir.Z*18)
		task.wait(0.04)
	end
...
status.Text="Клик = бег по дорожке, потом камень"
...
local function useTreadmill(gain,count)
	...
	-- Дорожка не работает от простого touch. Нужно реально бежать.
	-- Поэтому тепаемся один раз и имитируем бег по дорожке.
	local secPerTick=_G.PetAlignTreadmillTickTime or 0.55
	for i=1,count do
		status.Text="Бег по дорожке +"..gain..fallbackInfo..": "..i.."/"..count
		runOnTreadmill(part,secPerTick)
		task.wait(0.04)
	end
	return true
end
...
local function runAlign()
	...
	status.Text="Ровняю бегом по дорожке: "..planText(plan)
