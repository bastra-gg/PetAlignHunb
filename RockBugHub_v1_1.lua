...
local function protectedCleanupName(name)
	...
	return key:find("apex",1,true)~=nil
		or key=="titanreactor" or key=="plasmaravager"
end
...
local function cleanupSelectedPets(limit)
	...
	for name,enabled in pairs(Runtime.petCleanupTargets or {}) do
		if enabled and not protectedCleanupName(name) then
			selected[shopNameKey(name)]=true
		end
	end
	if next(selected)==nil then return 0 end

	local now=os.clock()
	for pet,sentAt in pairs(Runtime.petCleanupPending or {}) do
	...
	local function inspect(item,depth)
		if not item or sold>=maxCount or scanned>=350 or depth>4 then return end
		scanned=scanned+1
		local key=shopNameKey(item.Name)
		if selected[key] then
			if not Runtime.petCleanupPending[item] and not protectedCleanupPet(item) then
		...
		if item:IsA("Folder") or item:IsA("Configuration") then
			for _,child in ipairs(item:GetChildren()) do
				if sold>=maxCount or scanned>=350 then break end
...
local function cleanupSelectionLabel()
...
cleanupSelection=makeSelectionRow(crystalSettingsBody,"УДАЛЯТЬ",cleanupSelectionLabel(),function()
	local options={}
	local seen={}
	local function add(name,isNew)
		name=tostring(name or "")
		if name=="" or protectedCleanupName(name) then return end
		local key=shopNameKey(name)
		...
		table.insert(options,{
			id=name,
			label=name,
			sub=(isNew and "НОВЫЙ • " or "").."будет удаляться при открытии",
		})
	end
	for _,entry in ipairs(shopCatalog("pet")) do add(entry.name,entry.isNew) end
	for name,enabled in pairs(Runtime.petCleanupTargets or {}) do
		if enabled then add(name,false) end
	end
	local inventory=lp:FindFirstChild("petsFolder")
	if inventory then
		local visited=0
		for _,group in ipairs(inventory:GetChildren()) do
			local entries=(group:IsA("Folder") or group:IsA("Configuration"))
				and group:GetChildren() or {group}
			for _,pet in ipairs(entries) do
				visited=visited+1
				if visited>300 then break end
				add(pet.Name,false)
			end
			if visited>300 then break end
		end
	...
	table.sort(options,function(a,b) return string.lower(a.label)<string.lower(b.label) end)
	openPicker("УДАЛЯТЬ ПРИ ОТКРЫТИИ",options,{
		multiple=true,
		selected=Runtime.petCleanupTargets,
		onDone=function(selected)
			local chosen={}
			for name,enabled in pairs(selected or {}) do
				if enabled and not protectedCleanupName(name) then chosen[name]=true end
			end
