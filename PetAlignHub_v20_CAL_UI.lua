...
local function parsePetGuiTotal(rarity)
	...
	for _,v in ipairs(pg:GetDescendants())do
		if v:IsA("TextLabel")or v:IsA("TextButton")or v:IsA("TextBox")then
			...
			if a and b and (low:find("оп") or low:find("xp") or low:find("exp") or low:find("э")) then
				xp=parseNum(a)
			end
		end
	end
...
local function runOnTreadmill(part,seconds,pet,rarity,targetTotal)
	...
	local startPos=part.Position - dir*(length*0.42) + Vector3.new(0,3.15,0)

	local oldSpeed=hum.WalkSpeed
	local oldAutoRotate=hum.AutoRotate

	-- v18: один телепорт в начало и дальше только ПРЯМОЙ непрерывный бег.
	-- Без пинг-понга, без возвратов, без коррекций позиции.
	hum.WalkSpeed=speed
	hum.AutoRotate=true
	...
	while not _G.PetAlignStop and os.clock()-started<seconds do
		...
		-- Только держим горизонтальную скорость вперёд, без телепорта/разворотов.
...
status.Size=UDim2.new(1,-176,0,30)
status.Position=UDim2.new(0,14,1,-36)
status.BackgroundTransparency=1
status.Text="Клик = прямой бег по дорожке, потом камень"
...
stopBtn.Size=UDim2.new(0,58,0,26)
stopBtn.Position=UDim2.new(1,-72,1,-36)
stopBtn.Text="STOP"
...
calBtn.Position=UDim2.new(1,-136,1,-36)
calBtn.Text="CAL"
...
local function calibrateTreadmills()
	...
	local seconds=_G.PetAlignCalibrateSeconds or 3.0
	local results={}
	local oldReverse=_G.PetAlignReverseRun

	status.Text="CAL: старт, источник XP: "..tostring(source).." • дорожек "..#list

	for i,it in ipairs(list)do
		if _G.PetAlignStop then break end

		local before=getPetTotalAny(pet,rarity)
		status.Text="CAL дорожка #"..i.."/"..#list.." старт XP="..fmt(before or 0)

		runOnTreadmill(it.part,seconds,pet,rarity,nil)
		task.wait(0.25)

		local after=getPetTotalAny(pet,rarity)
		local delta=(before and after) and (after-before) or 0
		...
		status.Text="CAL #"..i.." delta +"..fmt(delta)
		task.wait(0.35)
	end
	...
	local map={}
	for gain=1,math.min(6,#results)do
		map[gain]=results[gain].index
	end
	_G.PetAlignTreadmillIndex=map

	local lines={}
	table.insert(lines,"PetAlignHub treadmill calibration")
	table.insert(lines,"place: "..tostring(game.PlaceId))
	...
	table.insert(lines,"raw order is sorted by gained XP ascending")
	table.insert(lines,"")
	...
	for rank,r in ipairs(results)do
		table.insert(lines,("#%02d gainGuess=+%s originalIndex=%s delta=%s perSec=%.2f pos=(%.1f, %.1f, %.1f)"):format(rank,rank,r.index,fmt(r.delta),r.perSec,r.x,r.y,r.z))
	...
	status.Text="CAL готов. Отчёт скопирован. Карта применена."
	running=false
end
...
local function useTreadmill(gain,count)
	...
	status.Text="Прямой бег +"..gain..fallbackInfo.." ~"..string.format("%.1f",seconds).."с"
...
local function runAlign()
	...
	status.Text="Ровняю прямым бегом: "..planText(plan)
	_G.PetAlignStop=false
