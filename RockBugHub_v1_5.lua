local a=game:GetService("Players")local b=game:GetService("ReplicatedStorage")local c=game:GetService("RunService")local d=game:GetService("Stats")local e=game:GetService("VirtualUser")local f=game:GetService("UserInputService")local g=game:GetService("StarterGui")local h=nil;
pcall(function()h=game:GetService("LocalizationService")end)local i=nil;
pcall(function()i=game:GetService("NetworkClient")end)if not game:IsLoaded()then game.Loaded:Wait()end;
...
local l="RockBugHub_TEST_v4_31_HOLO_T38"local m="4.31HOLO-T38"local n=type(getgenv)=="function"and getgenv()or _G;
do
...
local J=b:FindFirstChild("cPetShopRemote")if J and J:IsA("RemoteFunction")then return J end;
return nil end;
local function K(L)if not L then return nil end;
for o,M in ipairs({"priceValue","Price","price","Cost","cost","GemPrice","gemPrice","gemsCost"})do local N=L:FindFirstChild(M)if N then local D,O=B(function()return tonumber(N.Value)end)if D and O then return O end end;
local D,O=B(function()return tonumber(L:GetAttribute(M))end)if D and O then return O end end;
local N=L:FindFirstChild("priceValue",true)if not N then return nil end;
local D,O=B(function()return tonumber(N.Value)end)return D and O or nil end;
local function P(u)return string.lower(tostring(u or"")):gsub("<[^>]+>",""):gsub("[^%w\128-\255]","")end;
local function Q(L)if not L then return nil end;
for o,M in ipairs({"DisplayName","displayName","PetName","petName","ItemName","itemName","Title","title"})do local D,N=B(function()return L:GetAttribute(M)end)if D and type(N)=="string"and N:match("%S")then return N end;
local R=L:FindFirstChild(M)if R then D,N=B(function()return R.Value end)if D and type(N)=="string"and N:match("%S")then return N end end end;
local u=tostring(L.Name or"")if L:IsA("StringValue")and(u:lower()=="name"or u:lower()=="petname"or u:lower()=="displayname")then local D,N=B(function()return L.Value end)if D and type(N)=="string"and N:match("%S")then return N end end;
return u end;
function q.shopItemNameAliases(L,S)local T={}local U={}local function V(N)if type(N)~="string"then return end;
N=N:gsub("<[^>]+>",""):gsub("^%s+",""):gsub("%s+$","")local y=P(N)if N~=""and y~=""and not U[y]then U[y]=true;
table.insert(T,N)end end;
if type(S)=="table"then for o,N in ipairs(S)do V(N)end else V(S)end;
if L then V(tostring(L.Name or""))V(Q(L))for o,M in ipairs({"EnglishName","englishName","NameEN","nameEN","NameEn","nameEn","OriginalName","originalName","SourceName","sourceName","PetName","petName","ItemName","itemName","DisplayName","displayName"})do local D,N=B(function()return L:GetAttribute(M)end)if D then V(N)end;
local R=L:FindFirstChild(M)if R then D,N=B(function()return R.Value end)if D then V(N)end end end end;
return T,U end;
function q.shopLocalizedAliasMap()local W={}for X,Y in pairs(q.gameLocalizedNames or{})do local Z=P(X)local _=P(Y)if Z~=""and _~=""then W[Z]=_;
W[_]=Z end end;
return W end;
function q.shopNameMatches(a0,L,S,a1)local a2=P(a0)if a2==""then return false end;
local o,a3=q.shopItemNameAliases(L,S)if a3[a2]then return true end;
...
for o,a5 in ipairs({"Type","ItemType","Category"})do local D,N=B(function()return L:GetAttribute(a5)end)if D and type(N)=="string"then local a6=string.lower(N)if string.find(a6,"aura",1,true)or string.find(a6,"trail",1,true)then return true end end end;
return false end;
local function a7()local W={}local U={}local a8={}local function a9(L)if L and not U[L]and not L:IsA("RemoteEvent")and not L:IsA("RemoteFunction")and not L:IsA("LocalScript")and not L:IsA("ModuleScript")then U[L]=true;
table.insert(W,L)end end;
a9(q.resolvePetShopFolder())for o,u in ipairs({"cPetShopFolder","petShopFolder","PetShopFolder","petShop","PetShop","shopPets","ShopPets","petShopItems","PetShopItems","petShopCatalog","PetShopCatalog","petDefinitions","PetDefinitions","petData","PetData","petsFolder","PetsFolder","petFolder","PetFolder","Pets","pets","petModels","PetModels","petAssets","PetAssets","petChances","PetChances","crystalChances","CrystalChances","eggChances","EggChances","apexPets","ApexPets","apexShop","ApexShop","apexPetShop","ApexPetShop"})do a9(b:FindFirstChild(u))end;
local function aa(ab,ac)if not ab or a8[ab]or ac>4 then return end;
a8[ab]=true;
for o,R in ipairs(ab:GetChildren())do if not R:IsA("RemoteEvent")and not R:IsA("RemoteFunction")and not R:IsA("LocalScript")and not R:IsA("ModuleScript")then local a6=string.lower(tostring(R.Name))local ad=a6:find("pet",1,true)or a6:find("aura",1,true)or a6:find("trail",1,true)or a6:find("apex",1,true)or a6:find("chance",1,true)local ae=ad or a6:find("shop",1,true)or a6:find("shared",1,true)or a6:find("runtime",1,true)or a6:find("asset",1,true)or a6:find("data",1,true)or a6:find("catalog",1,true)or a6:find("definition",1,true)or a6:find("item",1,true)or a6:find("storage",1,true)or a6:find("module",1,true)or a6:find("game",1,true)if(ad or a6:find("shop",1,true))and(R:IsA("Folder")or R:IsA("Configuration"))then a9(R)end;
if ae then aa(R,ac+1)end end end end;
aa(b,1)return W end;
...
return false end;
local function ah(ai)local aj=q.resolvePetShopFolder()local ak=aj and aj.Parent and#aj:GetChildren()>0;
...
if#at>0 then local aw={}local a1=q.shopLocalizedAliasMap()for o,ax in ipairs(at)do local o,a3=q.shopItemNameAliases(ax)for y in pairs(a3)do if not aw[y]then aw[y]=ax end;
local Y=a1[y]if Y and not aw[Y]then aw[Y]=ax end end end;
for ay=#W,1,-1 do local av=W[ay]local L=aw[P(av.id)]or aw[P(av.name)]if not L or ai=="aura"~=a4(L)then table.remove(W,ay)else av.item=L;
av.id=tostring(L.Name)av.price=K(L)or av.price end end end end;
table.sort(W,function(az,aA)if az.isNew~=aA.isNew then return az.isNew==true end;
local aB=az.price or math.huge;
local aC=aA.price or math.huge;
if aB==aC then return string.lower(az.name)<string.lower(aA.name)end;
return aB<aC end)q.shopCatalogCache=q.shopCatalogCache or{}q.shopCatalogCache[ai]={items=W,folder=aj,at=os.clock()}return W end;
local function aD(u,ai)if P(u)==""then return nil end;
local a1=q.shopLocalizedAliasMap()for o,av in ipairs(ah(ai or"pet"))do if q.shopNameMatches(u,av.item,{av.name,av.id},a1)and av.item and av.item.Parent then return av.item,av end end;
if not ai then for o,av in ipairs(ah("aura"))do if q.shopNameMatches(u,av.item,{av.name,av.id},a1)and av.item and av.item.Parent then return av.item,av end end end;
local aj=b:FindFirstChild(tostring(u or""),true)if aj and aj.Parent and(not ai or ai=="aura"==a4(aj))then return aj,{name=Q(aj),id=tostring(aj.Name),item=aj}end;
return nil end;
local function aE(O)if not O then return q.language=="en"and"in-game price"or"цена из игры"end;
local aF=tostring(math.floor(O+0.5))local aG=aF:reverse():gsub("(%d%d%d)","%1 "):reverse():gsub("^ ","")return aG..(q.language=="en"and" gems"or" гемов")end;
local function aH(ai)local y=ai=="aura"and"selectedAura"or"selectedPet"local H=q[y]local aI=ah(ai)local a1=q.shopLocalizedAliasMap()for o,av in ipairs(aI)do if q.shopNameMatches(H,av.item,{av.name,av.id},a1)then q[y]=av.name;
return av.name end end;
q[y]=aI[1]and aI[1].name or nil;
return q[y]end;
local function aJ(aK)if aK then table.insert(q.connections,aK)end;
return aK end;
local function aL()for o,aK in ipairs(q.connections)do B(function()aK:Disconnect()end)end;
q.connections={}end;
local function aM()return j.Character end;
local function aN()local aK=aM()return aK and aK:FindFirstChildWhichIsA("Humanoid")end;
local function aO()local aK=aM()return aK and aK:FindFirstChild("HumanoidRootPart")end;
local function aP(aF)q.status=tostring(aF or"")if q.ui and q.ui.status and q.ui.status.Parent then q.ui.status.Text=q.status end end;
local function aQ(N)local aR=math.max(0,math.floor(tonumber(N)or 0))local aS=math.floor(aR/86400)aR=aR%86400;
local aT=math.floor(aR/3600)local aU=math.floor(aR%3600/60)local aV=aR%60;
if aS>0 then return("%dд %02d:%02d:%02d"):format(aS,aT,aU,aV)end;
return("%02d:%02d:%02d"):format(aT,aU,aV)end;
local function aW()local D,N=B(function()return workspace.DistributedGameTime end)if D and type(N)=="number"and N==N and N>=0 then return N end;
return nil end;
local function aX(N)local aY=tonumber(N)if aY==nil or aY~=aY then return"?"end;
local aZ=aY<0 and"-"or""aY=math.abs(aY)local a_={"","K","M","B","T","Qa","Qi","Sx","Sp","Oc","No","Dc"}local b0=1;
while aY>=1000 and b0<#a_ do aY=aY/1000;
b0=b0+1 end;
if aY>=1000 then return aZ..("%.1e"):format(aY*1000^(b0-1))end;
local aG;
if b0==1 or aY>=100 then aG=("%.0f"):format(aY)elseif aY>=10 then aG=("%.1f"):format(aY):gsub("%.0$","")else aG=("%.2f"):format(aY):gsub("0+$",""):gsub("%.$","")end;
return aZ..aG..a_[b0]end;
local function b1(N)local aR=math.max(0,math.floor(tonumber(N)or 0))local aS=math.floor(aR/86400)local aT=math.floor(aR%86400/3600)local aU=math.floor(aR%3600/60)local aV=aR%60;
if aS>0 then return("%dд %02d:%02d"):format(aS,aT,aU)end;
if aT>0 then return("%02d:%02d:%02d"):format(aT,aU,aV)end;
return("%02d:%02d"):format(aU,aV)end;
local function b2()if q.ui and q.ui.uptime and q.ui.uptime.Parent then local b3=aW()local b4=tostring(game.PrivateServerId or"")~=""local b5;
if q.language=="en"then b5=b4 and"VIP SERVER ~ "or"IN SERVER "else b5=b4 and"VIP-СЕРВЕР ~ "or"В СЕРВЕРЕ "end;
q.ui.uptime.Text=b5 ..(b3 and aQ(b3)or"--:--:--")end;
if q.ui and q.ui.net and q.ui.net.Parent then local b6=os.clock()if b6>=(q.sessionStatsNextPageAt or 0)then q.sessionStatsPage=((tonumber(q.sessionStatsPage)or 0)+1)%4;
q.sessionStatsNextPageAt=b6+3 end;
local b7=q.pingAvailable and tostring(math.floor((q.pingMs or 0)+0.5)).."ms"or"?"if q.networkPaused then local b8=math.max(0,os.clock()-(q.networkHoldSince or os.clock()))q.ui.net.Text=(q.language=="en"and"NET: PAUSED %.1fs | %s"or"СЕТЬ: ПАУЗА %.1fs | %s"):format(b8,b7)elseif q.sessionStatsPage==1 then q.ui.net.Text=(q.language=="en"and"STR %s | +%s"or"СИЛА %s | +%s"):format(aX(q.sessionStrengthCurrent),aX(q.sessionStrengthGained or 0))elseif q.sessionStatsPage==2 then local b9=math.max(1,b6-(q.sessionStatsStartedAt or b6))local ba=(tonumber(q.sessionStrengthGained)or 0)*60/b9;
q.ui.net.Text=(q.language=="en"and"RATE %s/m | %s"or"ТЕМП %s/м | %s"):format(aX(ba),b1(b9))elseif q.sessionStatsPage==3 then q.ui.net.Text=(q.language=="en"and"REB %s | +%s"or"РЕБ %s | +%s"):format(aX(q.sessionRebirthCurrent),aX(q.sessionRebirthGained or 0))elseif q.networkProbeUnsupported then q.ui.net.Text=(q.language=="en"and"NET: LIMITED | HIT %.1f/s"or"СЕТЬ: ОГРАНИЧЕНО | УДАР %.1f/s"):format(q.remotePps or 0)else q.ui.net.Text=(q.language=="en"and"PING %s | HIT %.1f/s"or"PING %s | УДАР %.1f/s"):format(b7,q.remotePps or 0)end end end;
local bb={{id="AncientJungle",label="Древний лес",req=10000000,mult=16.25},{id="MuscleKing",label="Король мышц",req=5000000,mult=12.5},{id="Legends",label="Легенды",req=1000000,mult=2.5},{id="Inferno",label="Инферно",req=750000,mult=1.125},{id="Mystic",label="Мистический",req=400000,mult=0.75},{id="Frozen",label="Ледяной",req=150000,mult=0.375},{id="Golden",label="Золотой",req=5000,mult=0.2},{id="Large",label="Большой",req=100,mult=0.075},{id="Punching",label="Пробивной",req=10,mult=0.05},{id="Tiny",label="Маленький",req=0,mult=0.025}}local bc={}local function bd(be)if not be then return nil end;
local D,E=pcall(function()return be.Value end)if not D then return nil end;
return tonumber(E)end;
local function bf(bg)if not bg then return false end;
return bg:FindFirstChild("LeftHand",true)~=nil and bg:FindFirstChild("RightHand",true)~=nil end;
local function bh(bi)local bj=bi;
for o=1,8 do if not bj or bj==workspace then break end;
if bj:IsA("Model")and bf(bj)then return bj end;
bj=bj.Parent end;
bj=bi;
for o=1,8 do if not bj or bj==workspace then break end;
if bj:IsA("Model")then return bj end;
bj=bj.Parent end;
return bi.Parent end;
local function bk(bg)if not bg then return nil end;
if bg:IsA("BasePart")then return bg end;
local bl=nil;
local bm=-1;
for o,bn in ipairs(bg:GetDescendants())do if bn:IsA("BasePart")then local bo=bn.Size.X*bn.Size.Y*bn.Size.Z;
if bo>bm then bm=bo;
bl=bn end end end;
return bl end;
local function bp(bq)local br={}local bs=0;
local bt=0;
for o,bu in ipairs(bb)do br[bu.req]=bu;
if not bu.discovered and bu.req>bs then bs=bu.req;
bt=bu.mult end end;
for bv,bw in pairs(bq or{})do if bv>bs and bw.model then local bx=bw.model;
local by=nil;
for o,u in ipairs({"petXpMultiplier","PetXpMultiplier","xpMultiplier","XPMultiplier","experienceMultiplier","rockMultiplier","RockMultiplier","Multiplier","multiplier"})do local N=bx:FindFirstChild(u,true)by=N and bd(N)or nil;
if not by then local D,bz=pcall(function()return bx:GetAttribute(u)end)if D then by=tonumber(bz)end end;
if by and by>0 then break end;
by=nil end;
local bA=bx;
local bB=false;
for o=1,7 do if not bA or bA==workspace then break end;
local u=tostring(bA.Name or""):lower()if u:find("industrial",1,true)or u:find("factory",1,true)or u:find("reactor",1,true)or u:find("plasma",1,true)then bB=true;
break end;
bA=bA.Parent end;
local bu=br[bv]if not bu then bu={id="DynamicRock_"..tostring(bv),req=bv,discovered=true}table.insert(bb,bu)br[bv]=bu end;
bu.label=bB and"Индустриальный"or tostring(bx.Name or"Новый камень")bu.mult=by or bt;
bu.multVerified=by~=nil end end;
table.sort(bb,function(bC,bD)return bC.req>bD.req end)end;
local function bE()local bq={}local bF=workspace:GetDescendants()for bG,be in ipairs(bF)do if tostring(be.Name)=="neededDurability"then local bH=bd(be)if bH~=nil then local bx=bh(be)local bI=bk(bx)local bJ=bx and bx:FindFirstChild("LeftHand",true)local bK=bx and bx:FindFirstChild("RightHand",true)local bL=bK or bJ or bI;
if bI or bL then local H=bq[bH]local bM=not H or bf(bx)and not bf(H.model)if bM then bq[bH]={req=bH,model=bx,body=bI,hit=bL,left=bJ,right=bK,name=bx and bx.Name or"?"}end end end end;
if bG%800==0 then task.wait()end end;
bp(bq)bc=bq;
return bq end;
local function bN(bu)if not bu then return nil end;
return bc[bu.req]end;
local function bO(aF)local bP=tostring(aF or""):lower()bP=bP:gsub("%s+"," ")local bQ,bR=bP:match("([%d][%d,%.]*)%s*([kmbt]?)")if not bQ then return nil end;
if not bQ:find(".",1,true)and bQ:match("^%d+,%d%d?$")then bQ=bQ:gsub(",",".")else bQ=bQ:gsub(",","")end;
local bS=tonumber(bQ)if not bS then return nil end;
local bT={k=1e3,m=1e6,b=1e9,t=1e12}if bR~=""then bS=bS*(bT[bR]or 1)end;
return math.floor(bS+0.5)end;
local function bU(bP)bP=tostring(bP or""):lower()return bP:find("rebirth",1,true)or bP:find("rebs",1,true)or bP:find("реб",1,true)or bP:find("перерожд",1,true)end;
local function bV(bW)if not bW or not bW.Parent then return nil end;
local D,N=pcall(function()return bW.Value end)if not D then return nil end;
return tonumber(N)or bO(N)end;
local function bX(u)local aF=tostring(u or""):lower():gsub("[%s_%-]","")if aF:find("cost",1,true)or aF:find("price",1,true)or aF:find("need",1,true)or aF:find("require",1,true)then return 0 end;
if aF=="rebirths"or aF=="rebirth"or aF=="rebs"or aF=="ребирты"or aF=="ребы"or aF=="перерождения"then return 3 end;
return bU(aF)and 1 or 0 end;
local function bY(bW,bZ)if not bW or not bW.Parent or(tonumber(bZ)or 0)<3 then return false end;
if not(bW:IsA("IntValue")or bW:IsA("NumberValue")or bW:IsA("StringValue"))then return false end;
local D,bQ=pcall(function()return bW.Value end)if not D then return false end;
local aY=tonumber(bQ)return aY~=nil and aY==aY and aY~=math.huge and aY~=-math.huge and aY>=0 and math.abs(aY-math.floor(aY+0.5))<0.000001 end;
local function b_(bW,bA,bZ)if bW and bW.Parent and bW:IsA("ValueBase")then q.rebirthCounter=bW;
q.rebirthCounterSource=bA;
local c0=tonumber(bZ)or bX(bW.Name)q.rebirthCounterExact=bY(bW,c0)end end;
local function c1()local al=q.rebirthCounter;
if al and al.Parent and q.rebirthCounterExact and q.rebirthCounterSource=="leaderstats"then local aY=bV(al)if aY~=nil then return aY,q.rebirthCounterSource or"cached",al end elseif not al or not al.Parent then q.rebirthCounter=nil;
q.rebirthCounterSource=nil;
q.rebirthCounterExact=false end;
local c2=j:FindFirstChild("leaderstats")if c2 then local bl,c3,c4=nil,nil,0;
for o,bn in ipairs(c2:GetChildren())do local bZ=bn:IsA("ValueBase")and bX(bn.Name)or 0;
if bZ>c4 then local aY=bV(bn)if aY~=nil then bl,c3,c4=bn,aY,bZ end end end;
if bl then b_(bl,"leaderstats",c4)return c3,"leaderstats",bl end end;
if al and al.Parent then local aY=bV(al)if aY~=nil then return aY,q.rebirthCounterSource or"cached",al end end;
local c5=0;
local bl,c3,c4=nil,nil,0;
for o,bn in ipairs(j:GetDescendants())do c5=c5+1;
if c5>1800 then break end;
local bZ=bn:IsA("ValueBase")and bX(bn.Name)or 0;
if bZ>c4 then local aY=bV(bn)if aY~=nil then bl,c3,c4=bn,aY,bZ;
if bZ>=3 then break end end end end;
if bl then b_(bl,"value",c4)return c3,"value",bl end;
local c6=j:FindFirstChild("PlayerGui")if c6 then local c7=0;
for o,bn in ipairs(c6:GetDescendants())do if not(q.uiRoot and bn:IsDescendantOf(q.uiRoot))then c7=c7+1;
if c7>2600 then break end;
if bn:IsA("TextLabel")or bn:IsA("TextButton")or bn:IsA("TextBox")then local aF=tostring(bn.Text or"")if bU(aF)or bU(bn.Name)then local bS=bO(aF)if bS then return bS,"gui",nil end end end end end end;
return nil,"not found",nil end;
local function c8(u)local c9=tostring(u or""):lower():gsub("[%s_%-]","")if c9=="strength"or c9=="сила"then return 4 end;
if c9=="muscles"or c9=="musclepower"then return 2 end;
return 0 end;
local function ca()local al=q.sessionStrengthCounter;
if al and al.Parent and al:IsA("ValueBase")then local D,bQ=pcall(function()return al.Value end)local aY=D and(tonumber(bQ)or bO(bQ))or nil;
if aY~=nil and aY==aY and aY~=math.huge and aY~=-math.huge then return math.max(0,aY)end else q.sessionStrengthCounter=nil end;
local bl,cb,c4=nil,nil,0;
local cc={j:FindFirstChild("leaderstats"),j}for o,cd in ipairs(cc)do if cd then for o,bW in ipairs(cd:GetChildren())do local bZ=bW:IsA("ValueBase")and c8(bW.Name)or 0;
if bZ>c4 then local D,bQ=pcall(function()return bW.Value end)local aY=D and(tonumber(bQ)or bO(bQ))or nil;
if aY~=nil and aY==aY and aY~=math.huge and aY~=-math.huge then bl,cb,c4=bW,math.max(0,aY),bZ end end end end end;
q.sessionStrengthCounter=bl;
return cb end;
function q.updateSessionStats(b6)b6=tonumber(b6)or os.clock()local ce=ca()if ce~=nil then local cf=q.sessionStrengthLast;
if cf~=nil and ce>cf then q.sessionStrengthGained=(tonumber(q.sessionStrengthGained)or 0)+ce-cf end;
q.sessionStrengthCurrent=ce;
q.sessionStrengthLast=ce end;
if b6>=(q.sessionStatsNextRebirthSample or 0)then local cg=c1()q.sessionStatsNextRebirthSample=b6+(cg~=nil and 2 or 5)if cg~=nil then cg=math.max(0,math.floor(cg+0.5))local cf=q.sessionRebirthLast;
if q.sessionRebirthStart==nil then q.sessionRebirthStart=cg end;
if cf~=nil and cg>cf then q.sessionRebirthGained=(tonumber(q.sessionRebirthGained)or 0)+cg-cf end;
q.sessionRebirthCurrent=cg;
q.sessionRebirthLast=cg end end end;
local function ch(N)local aY=math.max(0,math.floor(tonumber(N)or 0))local aF=("%.0f"):format(aY)return aF:reverse():gsub("(%d%d%d)","%1 "):reverse():gsub("^ ","")end;
local function ci(H,cj)if H~=nil then q.rebirthGoalCurrent=math.max(0,math.floor(tonumber(H)or 0))end;
local ck=q.ui and q.ui.rebirthGoalProgress;
...
local function cq()local cr,bA=c1()local cs=cn()if cr then cr=math.max(0,math.floor(cr+0.5))local ct=cs.base*19*20/2*40;
for cu,bu in ipairs(bb)do if bc[bu.req]and bu.multVerified~=false then local cv=math.floor(bu.mult*40+0.5)local cw=(cr+20)*cv;
if cw>0 and cw<=ct then local cx=nil;
for cy=1,19 do local cz=cs.base*cy*(cy+1)/2*40;
if cz>=cw then local cA=math.max(1,math.floor(cz/cw))local cB=cz-cA*cw;
local cC=1-cB/cz;
local ax={row=bu,rockIndex=cu,level=cy,hits=cA,hitXp40=cw,remainder40=cB,accuracy=cC}if not cx or ax.accuracy>cx.accuracy or ax.accuracy==cx.accuracy and ax.hits<cx.hits or ax.accuracy==cx.accuracy and ax.hits==cx.hits and ax.level<cx.level then cx=ax end end end;
if cx then local cD=cx.remainder40==0;
local cE=math.floor(cx.accuracy*1000+0.5)/10;
local cF=("%d реб • %s XP • L%d / %d уд. • %.1f%%"):format(cr,co(cx.hitXp40),cx.level,cx.hits,cE)local cG={rebirths=cr,xp40=cx.hitXp40,level=cx.level,hits=cx.hits,accuracy=cE,grade=cs.name,exact=cD,source=bA}return bu,cF,cr,cD,cG end end end end;
for bG=#bb,1,-1 do local bu=bb[bG]if bc[bu.req]then local cw=(cr+20)*math.floor(bu.mult*40+0.5)local cG={rebirths=cr,xp40=cw,level=19,hits=1,accuracy=0,grade=cs.name,exact=false,overLimit=true,source=bA}return bu,("%d реб • XP выше лимита питомца"):format(cr),cr,false,cG end end end;
for bG=#bb,1,-1 do local bu=bb[bG]if bc[bu.req]then return bu,"ребы не найдены ("..tostring(bA)..")",nil,false,{grade=cs.name,exact=false}end end;
return bb[#bb],"камни не найдены",nil,false,{grade=cs.name,exact=false}end;
local function cH(cI)if not q.autoRockSelection and not cI then return false end;
local bu,cF,cr,cD,cG=cq()if not cI and cr~=nil and q.lastAutoRockRebs==cr then return false end;
local cf=q.selectedRock;
q.selectedRock=bu;
q.lastAutoRockRebs=cr;
q.autoRockReason=cF;
q.autoRockExact=cD;
q.autoRockCalc=cG;
if q.ui then local cJ=q.language;
local cK=q.layoutUI and q.layoutUI.rockName and q.layoutUI.rockName(bu)or bu.label;
if q.ui.autoRockTitle and q.ui.autoRockTitle.Parent then q.ui.autoRockTitle.Text=cJ=="en"and"AUTO ROCK BY REBIRTHS"or"АВТОПОДБОР ПО РЕБЁРТАМ"end;
if q.ui.autoRockName and q.ui.autoRockName.Parent then q.ui.autoRockName.Text=cK..(cJ=="en"and"  •  pet "or"  •  питомец ")..tostring(cG and cG.grade or"-")end;
if q.ui.autoRockStats and q.ui.autoRockStats.Parent then local cL=cG and cG.rebirths~=nil and tostring(cG.rebirths)or(cJ=="en"and"not found"or"не найдены")local cM=cG and cG.xp40 and co(cG.xp40)or"?"if cG and cG.overLimit then q.ui.autoRockStats.Text=(cJ=="en"and"Rebirths: %s  •  XP/hit: %s  •  above pet limit"or"Ребёрты: %s  •  XP/удар: %s  •  превышен лимит питомца"):format(cL,cM)elseif cG and cG.level and cG.hits then q.ui.autoRockStats.Text=(cJ=="en"and"Rebirths: %s  •  XP/hit: %s  •  target L%d  •  %d hits  •  %.1f%%"or"Ребёрты: %s  •  XP/удар: %s  •  цель L%d  •  %d уд.  •  %.1f%%"):format(cL,cM,cG.level,cG.hits,tonumber(cG.accuracy)or(cG.exact and 100 or 0))else q.ui.autoRockStats.Text=(cJ=="en"and"Rebirths: %s  •  XP/hit: %s  •  character data not found"or"Ребёрты: %s  •  XP/удар: %s  •  данные персонажа не найдены"):format(cL,cM)end end end;
if type(q.refreshRockList)=="function"then q.refreshRockList()end;
if cf and cf.id~=bu.id and q.lockRock and type(q.teleportInsideSelected)=="function"then q.teleportInsideSelected()end;
return true end;
...
local function d7(cX)return cY(function(cO)return cW(cO,cX)end)end;
local function d8(bg)local bP=tostring(bg.Name)if bg:IsA("TextButton")or bg:IsA("TextLabel")or bg:IsA("TextBox")then bP=bP.." "..tostring(bg.Text or"")end;
return bP:lower()end;
local function d9(aF,cX)local c9=" "..tostring(aF or""):lower():gsub("[^%w]+"," ").." "for o,da in ipairs(cX.words)do local cR=tostring(da):lower()if#cR<=3 and cR:match("^[%w]+$")then if c9:find(" "..cR.." ",1,true)then return true end elseif tostring(aF or""):lower():find(cR,1,true)then return true end end;
return false end;
local function db(bg,k)local H=bg;
while H and H~=k do if H:IsA("GuiObject")and not H.Visible then return false end;
if H:IsA("LayerCollector")and not H.Enabled then return false end;
H=H.Parent end;
return H==k end;
local function dc(cX)local c6=j:FindFirstChild("PlayerGui")if not c6 then return nil end;
local c7=0;
for o,bn in ipairs(c6:GetDescendants())do c7=c7+1;
if c7>2600 then break end;
if(bn:IsA("TextButton")or bn:IsA("ImageButton"))and bn.Active and db(bn,c6)then local dd=string.lower(bn:GetFullName())local de=cP(dd,{"trade","purchase","shop","store","buy","магаз","купить"})if not de then local df=d8(bn)if d9(df,cX)then return bn end end end end;
return nil end;
local function dg(cX)local dh=dc(cX)if not dh then return false end;
local di=B(function()dh:Activate()end)if not di and type(firesignal)=="function"then di=B(function()firesignal(dh.Activated)end)end;
if di then task.wait(0.18)end;
return di end;
...
local function dk(cX,dl)local cO,d2=d7(cX)if not cO and dl~=false then dg(cX)cO,d2=d7(cX)end;
...
local D,dx=B(function()for o,R in ipairs(i:GetChildren())do if R:IsA("ClientReplicator")then return true end end;
return false end)if not D then return nil end;
return dx end;
local function dy(N)return type(N)=="number"and N==N and N>0 and N<math.huge end;
local function dz()local dA,aj=B(function()return j:GetNetworkPing()*1000 end)if not dA or not dy(aj)then aj=nil end;
local D,E=B(function()local dB=d:FindFirstChild("Network")local dC=dB and dB:FindFirstChild("ServerStatsItem")local dD=dC and dC:FindFirstChild("Data Ping")if dD then local aF=tostring(dD:GetValueString())return tonumber(aF:match("([%d%.]+)"))end end)if not D or not dy(E)then E=nil end;
if aj and E then return math.max(aj,E)end;
if aj then return aj end;
if E then return E end;
return nil end;
local function dE()local dF=q.networkHoldRoot;
if dF and dF.Parent and q.networkHoldSavedAnchored~=nil then B(function()if q.kingLock and q.kingRoot==dF then dF.Anchored=false;
q.nextKingTick=0 else dF.Anchored=q.networkHoldSavedAnchored end;
dF.AssemblyLinearVelocity=Vector3.new(0,0,0)dF.AssemblyAngularVelocity=Vector3.new(0,0,0)end)end;
q.networkHoldRoot=nil;
q.networkHoldCF=nil;
q.networkHoldSavedAnchored=nil;
q.nextNetworkHoldTick=0 end;
local function dG()local dH=aO()if not dH then return false end;
if q.networkHoldRoot~=dH then dE()q.networkHoldRoot=dH;
q.networkHoldCF=dH.CFrame;
q.networkHoldSavedAnchored=dH.Anchored end;
local dI=q.networkHoldCF or dH.CFrame;
B(function()dH.CFrame=dI;
dH.AssemblyLinearVelocity=Vector3.new(0,0,0)dH.AssemblyAngularVelocity=Vector3.new(0,0,0)local dJ=q.kingLock and q.kingRoot==dH;
if dJ then if q.kingHoldPosition and q.kingHoldPosition.Parent==dH then q.kingHoldPosition.Position=dI.Position end;
if q.kingHoldGyro and q.kingHoldGyro.Parent==dH then q.kingHoldGyro.CFrame=dI end end;
dH.Anchored=not dJ end)return true end;
local function dK(cF,b6)if not q.netGuardEnabled then return end;
b6=b6 or os.clock()if not q.networkPaused then q.networkPaused=true;
q.networkHoldSince=b6;
q.networkState="SUSPECT"q.transientFailures={}q.remoteTokens=0;
q.nextAction=b6+0.25;
q.nextRebirth=b6+0.5;
q.nextSize=b6+0.5 end;
q.networkReason=tostring(cF or"connection unstable")dG()aP("СЕТЬ: ПАУЗА • "..q.networkReason)end;
local function dL(b6,cF)b6=b6 or os.clock()local dM=q.networkPaused;
q.networkPaused=false;
q.networkState="HEALTHY"q.networkBadSamples=0;
q.networkGoodSamples=0;
q.networkHoldSince=0;
q.networkReason=nil;
q.networkLastGoodAt=b6;
q.networkRecoveryProofRequired=false;
q.transientFailures={}q.remoteTokens=0;
q.remoteLastRefill=b6;
q.nextAction=b6+0.25;
if q.rebirthGoalEnabled and q.rebirthGoalAwaitingFrom~=nil then q.rebirthGoalAwaitingUntil=math.max(q.rebirthGoalAwaitingUntil or 0,b6+2.5)q.nextRebirth=q.rebirthGoalAwaitingUntil else q.nextRebirth=b6+0.5 end;
q.nextSize=b6+0.5;
dE()if dM then q.networkRecoveries=q.networkRecoveries+1;
aP("СЕТЬ ВОССТАНОВЛЕНА • режим продолжен")end end;
...
if dQ and dQ>0.01 then q.networkTrafficLastSeen=b6 end;
if dR==true then q.networkReplicatorSeen=true;
q.networkReplicatorMissingSince=nil elseif dR==false and q.networkReplicatorSeen then q.networkReplicatorMissingSince=q.networkReplicatorMissingSince or b6 end;
if not q.netGuardEnabled then q.networkBadSamples=0;
q.networkGoodSamples=0;
return dP end;
if q.manualNetworkHold then dK("manual WiFi hold",b6)q.networkState="MANUAL HOLD"return dP end;
if dP==nil and not q.pingAvailable and not q.networkReplicatorSeen and b6>=q.networkProbeDeadline then if q.networkPaused then dL(b6,"LIMITED / NO PING PROBE")end;
q.networkProbeUnsupported=true;
q.networkState="LIMITED"q.networkBadSamples=0;
q.networkGoodSamples=0;
return nil end;
local dS=dP==nil;
...
local b9=b6-e4;
return b9>=e3,b9 end;
local function e5()if q.networkPaused then return 1,0 end;
local e6=q.bugActive and 1 or 0;
for o,e7 in ipairs(q.trainOrder or{})do if q.activeTrains[e7]and not(q.bugActive and e7=="Punch")then e6=e6+1 end end;
if q.machineActive then e6=e6+1 end;
e6=math.max(1,e6)local function e8(e9,ea,eb,ec)local ed=e9+math.max(0,e6-1)*2;
return math.min(eb,ed),math.min(ec,ea)end;
if q.netGuardEnabled and not q.pingAvailable then return e8(6,2,14,2)end;
local dD=q.pingMs or 0;
if dD>=700 then return e8(3,0,8,0)elseif dD>=450 then return e8(5,1,10,1)elseif dD>=300 then return e8(7,2,14,2)elseif dD>=200 then return e8(9,3,18,3)end;
return e8(11,5,22,5)end;
local function ee()if q.networkPaused then q.remoteTokens=0;
q.remoteLastRefill=os.clock()return end;
local b6=os.clock()local o,ef=e5()local b9=b6-q.remoteLastRefill;
q.remoteLastRefill=b6;
q.remoteTokens=math.min(ef,q.remoteTokens+b9*ef)end;
...
local function ej()q.remoteSentWindow=q.remoteSentWindow+1;
ei()end;
local function ek()if not q.directRemoteEnabled then return false end;
...
local eq;
local er;
do local es={{id="Tiny Island",order=1,words={"tiny island","tiny"},position={-34,7,1903}},{id="Starter Island",order=2,words={"starter island","starter","spawn"},position={-17,3,-2}},{id="Legend Beach",order=3,words={"legend beach","beach"},position={470,3,-321}},{id="Frost Gym",order=4,words={"frost","frozen","ice gym"},position={-3000,35,-400}},{id="Mythical Gym",order=5,words={"mythical","mystic"},position={2400,20,1050}},{id="Eternal Gym",order=6,words={"eternal","inferno"},position={-7176,45,-1106}},{id="Legend Gym",order=7,words={"legend gym","legends gym","legends"},position={4200,1000,-4000}},{id="Muscle King Gym",order=8,words={"muscle king","king gym","king"},position={-8700,35,-5850}},{id="Jungle Gym",order=9,words={"ancient jungle","jungle"},position={-8500,35,2400}},{id="Industrial Gym",order=10,words={"industrial","factory","reactor","plasma"}}}local et={{id="bench",label="Bench Press",order=1,words={"bench","press"}},{id="squat",label="Squat",order=2,words={"squat"}},{id="bar",label="Bar Lift",order=3,words={"bar lift","barlift","barbell","bar lift"}},{id="deadlift",label="Deadlift",order=4,words={"deadlift","dead lift"}},{id="pullup",label="Pull Up",order=5,words={"pull up","pullup","pull-up"}},{id="boulder",label="Boulder",order=6,words={"boulder"}},{id="treadmill",label="Treadmill",order=7,words={"treadmill","tread mill","tread"}},{id="lift",label="Lift",order=8,words={" lift","lifting","weight"}}}local function eu(u)local aF=tostring(u or""):lower()for o,ai in ipairs(et)do for o,da in ipairs(ai.words)do if aF:find(da,1,true)then return ai end end end;
return nil end;
local function ev(ew)local H=ew and ew.Parent;
local bC=nil;
local cf=nil;
for o=1,8 do if not H or H==workspace then break end;
if H:IsA("Model")then bC=bC or H;
if eu(H.Name)then if cf and cf~=H then local ex=tostring(cf.Name or""):lower()local ey=ex:find("tiny",1,true)or ex:find("small",1,true)or ex:find("mini",1,true)or ex:find("big",1,true)or ex:find("large",1,true)or ex:find("giant",1,true)or ex:find("huge",1,true)if ey then return cf end;
local ez,bF=B(function()return H:GetDescendants()end)if ez and#bF<=500 then local eA=0;
for o,L in ipairs(bF)do if L:IsA("BasePart")and tostring(L.Name):lower()=="interactseat"then eA=eA+1;
if eA>1 then return cf end end end end end;
return H end;
cf=H end;
H=H.Parent end;
return bC or ew and ew.Parent end;
local function eB(bx)local H=bx;
for o=1,8 do if not H or H==workspace then break end;
local ai=eu(H.Name)if H:IsA("Model")and ai then return tostring(H.Name),ai,H end;
H=H.Parent end;
return tostring(bx and bx.Name or"Тренажёр"),nil,bx end;
local function eC(bx,ew)local ez,eD=B(function()return bx:GetExtentsSize()end)if not ez or not eD then ez,eD=B(function()local eE=bx.PrimaryPart or bk(bx)or ew;
return eE and eE.Size end)end;
if ez and eD then local eF=math.max(tonumber(eD.X)or 0,0)local eG=math.max(tonumber(eD.Y)or 0,0)local eH=math.max(tonumber(eD.Z)or 0,0)return math.max(eF*eG*eH,eF+eG+eH)end;
return 0 end;
local function eI(bx,eJ)local H=bx;
for o=1,5 do if not H or H==workspace then break end;
local u=tostring(H.Name or""):lower()if u:find("tiny",1,true)or u:find("small",1,true)or u:find("mini",1,true)then return"Tiny"end;
if u:find("big",1,true)or u:find("large",1,true)or u:find("giant",1,true)or u:find("huge",1,true)then return"Big"end;
if H==eJ then break end;
H=H.Parent end;
return nil end;
local function eK(bx,ew)local eL={}local H=bx;
for o=1,8 do if not H or H==workspace then break end;
table.insert(eL,tostring(H.Name or""):lower())H=H.Parent end;
local u=table.concat(eL," ")for o,ay in ipairs({10,8,9,5,6,4,3,1,2,7})do local eM=es[ay]for o,da in ipairs(eM.words)do if u:find(da,1,true)then return eM end end end;
local D,eN=B(function()return ew.Position end)if D and eN then local eO=nil;
local eP=math.huge;
for o,eM in ipairs(es)do if eM.position then local eQ=eN.X-eM.position[1]local eR=eN.Y-eM.position[2]local eS=eN.Z-eM.position[3]local eT=eQ*eQ+eR*eR+eS*eS;
if eT<eP then eP=eT;
eO=eM end end end;
...
if q.machineScanned and not cI then return q.machineCatalog end;
q.machineScanInFlight=true;
local D,aI=B(function()local eV={}local eW=workspace:FindFirstChild("machinesFolder")if eW then table.insert(eV,eW)end;
for o,R in ipairs(workspace:GetChildren())do if R~=eW and tostring(R.Name):lower():find("machine",1,true)then table.insert(eV,R)end end;
local eX=#eV==0;
if eX then eV[1]=workspace end;
local bq={}local eY={}local c7=0;
local ef=eX and 16000 or 32000;
local function eZ(bx,ew)if not bx or not ew or eY[ew]then return end;
if not ew:IsA("BasePart")then return end;
local u,ai,eJ=eB(bx)if not ai and tostring(ew.Name):lower()~="interactseat"then return end;
eY[ew]=true;
local eM=eK(bx,ew)local e7=tostring(#bq+1)..":"..eM.id..":"..u;
table.insert(bq,{id=e7,name=u,label=u,model=bx,identity=eJ,seat=ew,zone=eM.id,zoneOrder=eM.order,kind=ai and ai.label or"Тренажёр",typeOrder=ai and ai.order or 99,requirement=eU(bx)or(eJ~=bx and eU(eJ)or nil),sizeScore=eC(bx,ew),explicitVariant=eI(bx,eJ)})end;
for o,cd in ipairs(eV)do local e_={cd}local f0=1;
while f0<=#e_ and c7<ef do local L=e_[f0]f0=f0+1;
c7=c7+1;
if L:IsA("BasePart")and tostring(L.Name):lower()=="interactseat"then eZ(ev(L),L)elseif L:IsA("Model")and eu(L.Name)then local ew=L:FindFirstChild("interactSeat",true)or L:FindFirstChildWhichIsA("Seat",true)or L:FindFirstChildWhichIsA("VehicleSeat",true)if not ew and not eX then ew=bk(L)end;
if ew then eZ(ev(ew)or L,ew)end end;
for o,R in ipairs(L:GetChildren())do table.insert(e_,R)end;
if c7%350==0 then task.wait()end end end;
local f1={}for o,f2 in ipairs(bq)do local f3=f2.name:lower():gsub("%f[%a]tiny%f[%A]",""):gsub("%f[%a]small%f[%A]",""):gsub("%f[%a]mini%f[%A]",""):gsub("%f[%a]big%f[%A]",""):gsub("%f[%a]large%f[%A]",""):gsub("%s+"," ")local y=f2.zone..":"..f2.kind..":"..f3;
f1[y]=f1[y]or{}table.insert(f1[y],f2)end;
for o,f4 in pairs(f1)do if#f4>1 then table.sort(f4,function(az,aA)if az.sizeScore~=aA.sizeScore then return az.sizeScore<aA.sizeScore end;
if(az.requirement or 0)~=(aA.requirement or 0)then return(az.requirement or 0)<(aA.requirement or 0)end;
return az.id<aA.id end)for ay,f2 in ipairs(f4)do f2.variant=f2.explicitVariant or(ay==1 and"Tiny"or(ay==#f4 and"Big"or"Medium "..tostring(ay)))f2.variantOrder=ay end end end;
table.sort(bq,function(az,aA)if az.zoneOrder~=aA.zoneOrder then return az.zoneOrder<aA.zoneOrder end;
if az.zone~=aA.zone then return az.zone<aA.zone end;
if az.typeOrder~=aA.typeOrder then return az.typeOrder<aA.typeOrder end;
if(az.requirement or 0)~=(aA.requirement or 0)then return(az.requirement or 0)<(aA.requirement or 0)end;
if az.name~=aA.name then return az.name<aA.name end;
if(az.variantOrder or 0)~=(aA.variantOrder or 0)then return(az.variantOrder or 0)<(aA.variantOrder or 0)end;
return az.id<aA.id end)return bq end)q.machineScanInFlight=false;
if not D then aP("ТРЕНАЖЁРЫ: "..tostring(aI):sub(1,85))return q.machineCatalog end;
q.machineCatalog=aI;
q.machineScanned=true;
q.machineZones={}local f5={}for o,f2 in ipairs(aI)do local av=f5[f2.zone]if not av then av={id=f2.zone,label=f2.zone,order=f2.zoneOrder,count=0}f5[f2.zone]=av;
table.insert(q.machineZones,av)end;
av.count=av.count+1 end;
if q.machineZone and not f5[q.machineZone]then q.machineZone=nil end;
if not q.machineZone and q.machineZones[1]then q.machineZone=q.machineZones[1].id end;
if q.selectedMachine then local f6=nil;
for o,f2 in ipairs(aI)do if f2.seat==q.selectedMachine.seat then f6=f2;
break end end;
if not f6 then for o,f2 in ipairs(aI)do if f2.model==q.selectedMachine.model and f2.variant==q.selectedMachine.variant then f6=f2;
break end end end;
q.selectedMachine=f6 end;
return aI end;
local function f7()local I=b:FindFirstChild("rEvents")local cc={}if I then table.insert(cc,I)end;
...
local function fo(eE)if not eE then return false end;
if eE==f2.seat then return true end;
for o,fp in ipairs({f2.model,f2.identity})do if fp then local D,W=B(function()return eE==fp or eE:IsDescendantOf(fp)end)if D and W then return true end end end;
return false end;
if(fm or fn)and fi then for o,L in ipairs(fi:GetDescendants())do local u=tostring(L.Name or""):lower()local fq=not fe or not fe[L]local fr=u:find("boulder",1,true)or u=="rock"local fs=u:find("lift",1,true)or u:find("weight",1,true)local ft=false;
if fn and(L:IsA("Weld")or L:IsA("WeldConstraint")or L:IsA("Motor6D"))then local fu,fv;
B(function()fu=L.Part0;
fv=L.Part1 end)ft=fo(fu)or fo(fv)end;
if fq and(fm and fr or fn and(fs or ft))then if L:IsA("Tool")then table.insert(fk,L)elseif not L:IsA("Accessory")and not L:IsA("Humanoid")then table.insert(fj,L)end end end end;
local function fw()for o,cO in ipairs(fk)do if cO.Parent==fi then local fx=j:FindFirstChildOfClass("Backpack")if fx then B(function()cO.Parent=fx end)end end end;
for o,L in ipairs(fj)do if L.Parent and fi and L:IsDescendantOf(fi)then B(function()L:Destroy()end)end end end;
local fc=aN()local fuseSeatOwned=fc and f2.seat and fc.SeatPart==f2.seat;
local function detachMachineWelds()if not fi then return end;
for o,fp in ipairs({f2.seat,f2.model,f2.identity})do if fp and fp.Parent then local fD,fE=B(function()return fp:GetDescendants()end)if fD and type(fE)=="table"then for o,fF in ipairs(fE)do if fF:IsA("Weld")or fF:IsA("WeldConstraint")or fF:IsA("Motor6D")then local fG,fH;
B(function()fG=fF.Part0;
fH=fF.Part1 end)local fI=false;
B(function()fI=(fG and fG:IsDescendantOf(fi))or(fH and fH:IsDescendantOf(fi))end)if fI or(fuseSeatOwned and fF.Parent==f2.seat and string.lower(tostring(fF.Name or""))=="seatweld")then B(function()fF:Destroy()end)end end end end end end end;
detachMachineWelds()if fc then B(function()fc:UnequipTools()end)B(function()fc.Sit=false end)B(function()fc.Jump=true end)B(function()fc:ChangeState(Enum.HumanoidStateType.GettingUp)end)B(function()local fy=fc:FindFirstChildOfClass("Animator")local fz=fy and fy:GetPlayingAnimationTracks()or fc:GetPlayingAnimationTracks()for o,fA in ipairs(fz)do if not ff or not ff[fA]then fA:Stop(0)end end end)end;
task.wait()detachMachineWelds()fw()if f2.homePivot and f2.model and f2.model.Parent then B(function()local fJ=f2.model:GetPivot()if(fJ.Position-f2.homePivot.Position).Magnitude>4 then f2.model:PivotTo(f2.homePivot)end end)elseif f2.homeSeatCF and f2.seat and f2.seat.Parent then B(function()if(f2.seat.Position-f2.homeSeatCF.Position).Magnitude>4 then f2.seat.CFrame=f2.homeSeatCF end end)end;
task.spawn(function()task.wait(0.2)if q.machineActive then return end;
fw()end)end;
er=function()local f2=q.selectedMachine;
if not q.machineActive or not f2 then return end;
local fl=(tostring(f2.kind or"").." "..tostring(f2.name or"")):lower()if f2.kind~="Lift"and not fl:find("lift",1,true)then return end;
...
eq(f2,fe,ff,true)end end;
local function fC(fD)local fE=0;
for o,fF in pairs(fD or{})do if fF then fE=fE+1 end end;
return fE end;
local function fG()if type(q.refreshExtraUI)=="function"then B(q.refreshExtraUI)end end;
local function fH(L,fI,fJ)if not L then return nil end;
for o,M in ipairs(fI)do local D,N=B(function()return L:GetAttribute(M)end)if D and(type(N)=="string"and N:match("%S")or fJ and type(N)=="number")then return N end;
local R=L:FindFirstChild(M)if R then D,N=B(function()return R.Value end)if D and(type(N)=="string"and N:match("%S")or fJ and type(N)=="number")then return N end end end;
return nil end;
local function fK(fL)local b6=os.clock()if not fL and q.crystalCatalogAt and b6-q.crystalCatalogAt<3 and#q.crystalNames>0 then return q.crystalNames end;
local eL={}local aI={}local eV={}local fM={}local fN={}local fO=0;
local fP={"mapCrystalsFolder","crystalsFolder","CrystalsFolder","crystalFolder","CrystalFolder","Crystals","crystals","PetCrystals","petCrystals","CrystalDisplays","crystalDisplays","CrystalShop","crystalShop","CrystalStations","crystalStations","Hatchables","hatchables","Eggs","eggs","IndustrialGym","industrialGym","Gyms","gyms","Map","map","World","Assets","assets","Shared","shared","GameData","gameData","Definitions","definitions"}local fQ={crystal=true,crystals=true,egg=true,eggs=true,hatchables=true,hatching=true,categories=true,category=true,catalog=true,catalogue=true,definitions=true,data=true,config=true,configuration=true,displays=true,stations=true,models=true,assets=true,items=true,shop=true,gyms=true,map=true,world=true,mapcrystalsfolder=true,crystalsfolder=true,crystalfolder=true,petcrystals=true,crystaldisplays=true,crystalshop=true,crystalstations=true,crystaldata=true,crystaldefinitions=true,crystalcatalog=true,crystalchances=true}local function fR(L)if L and not fM[L]then fM[L]=true;
table.insert(eV,L)end end;
for o,ab in ipairs({workspace,b})do for o,u in ipairs(fP)do fR(ab:FindFirstChild(u))end;
local c5=0;
for o,R in ipairs(ab:GetChildren())do c5=c5+1;
if c5>100 then break end;
local a6=string.lower(tostring(R.Name or""))if a6:find("crystal",1,true)or a6:find("hatch",1,true)or a6:find("industrial",1,true)or a6:find("gym",1,true)then fR(R)end end end;
local function fS(L,fT)if not L or L:IsA("RemoteEvent")or L:IsA("RemoteFunction")or L:IsA("LocalScript")or L:IsA("ModuleScript")then return false end;
local fU=tostring(L.Name or"")local fV=P(fU)if fV==""or fQ[fV]or fV:find("crystal",1,true)and(fV:match("folders?$")or fV:match("systems?$")or fV:match("remotes?$")or fV:match("events?$")or fV:match("definitions?$")or fV:match("catalogs?$")or fV:match("configs?$")or fV:match("chances?$")or fV:match("displays?$")or fV:match("stations?$")or fV:match("models?$")or fV:match("assets?$"))then return false end;
local fW=fH(L,{"CrystalName","crystalName","CrystalDisplayName","crystalDisplayName","HatchName"})local fX=fW or fH(L,{"DisplayName","displayName","Title","title"})or fU;
local fY=string.lower(fU)local fZ=string.lower(tostring(fX))local f_=fY:find("crystal",1,true)or fZ:find("crystal",1,true)local g0=L:IsA("Model")or L:IsA("Folder")or L:IsA("Configuration")or L:IsA("ObjectValue")if not fW and not f_ and not fT then return false end;
if not g0 and not fW then return false end;
if not fW and not f_ and not fH(L,{"CrystalId","crystalId","CrystalID","OpenId","HatchId"},true)then return false end;
local u=tostring(fX):gsub("^%s+",""):gsub("%s+$","")local y=P(u)if y==""or fQ[y]then return false end;
if not aI[y]then local g1=fH(L,{"CrystalId","crystalId","CrystalID","OpenId","HatchId","Id","ID"},true)or fU;
aI[y]={name=u,id=g1,action=fH(L,{"OpenAction","openAction","RemoteAction"}),source=L}table.insert(eL,u)end;
return true end;
local function am(L,ac,fT)if not L or fN[L]or ac>5 or fO>=320 then return end;
fN[L]=true;
fO=fO+1;
if L:IsA("RemoteEvent")or L:IsA("RemoteFunction")or L:IsA("LocalScript")or L:IsA("ModuleScript")then return end;
local a6=string.lower(tostring(L.Name or""))local g2=fT or a6:find("crystal",1,true)~=nil or a6:find("hatch",1,true)~=nil;
if fS(L,fT)then return end;
for o,R in ipairs(L:GetChildren())do if fO>=320 then break end;
local ex=string.lower(tostring(R.Name or""))local ae=g2 or ex:find("crystal",1,true)or ex:find("hatch",1,true)or ex:find("egg",1,true)or ex:find("industrial",1,true)or ex:find("gym",1,true)or ex:find("station",1,true)or ex:find("display",1,true)or ex:find("interact",1,true)or ex:find("catalog",1,true)or ex:find("definition",1,true)or ex:find("data",1,true)or ex:find("asset",1,true)or ex:find("pet",1,true)if ae then am(R,ac+1,g2)end end end;
...
return W end;
local g8={}for o,av in ipairs(ah("pet"))do g8[P(av.name)]=av.name end;
local W={}local U={}local function V(u)local y=P(u)local cD=g8[y]if cD and not U[y]then U[y]=true;
table.insert(W,cD)end end;
local fO=0;
local function g9(L,ac)if not L or ac>5 or fO>=180 then return end;
fO=fO+1;
V(Q(L)or L.Name)if L:IsA("StringValue")then local D,N=B(function()return L.Value end)if D then V(N)end elseif L:IsA("ObjectValue")then local D,N=B(function()return L.Value end)if D and N then V(Q(N)or N.Name)end end;
for o,M in ipairs({"PetName","petName","ItemName","itemName","DisplayName","displayName"})do local N=fH(L,{M})if N then V(N)end end;
for o,R in ipairs(L:GetChildren())do g9(R,ac+1)end end;
fK()local av=q.crystalCatalog[g6]if av and av.source then g9(av.source,1)end;
local a8={}local function ga(L,ac)if not L or a8[L]or ac>6 or fO>=360 then return end;
a8[L]=true;
local gb=P(L.Name)local gc=fH(L,{"CrystalName","crystalName","CrystalDisplayName","crystalDisplayName","HatchName"})if gb==g6 or gc and P(gc)==g6 then g9(L,1)return end;
for o,R in ipairs(L:GetChildren())do ga(R,ac+1)end end;
if#W==0 then for o,aO in ipairs(a7())do ga(aO,1)if fO>=360 then break end end end;
table.sort(W,function(az,aA)return string.lower(az)<string.lower(aA)end)q.crystalPetCache=q.crystalPetCache or{}q.crystalPetCache[g6]={names=W,at=os.clock()}local gd={}for o,u in ipairs(W)do table.insert(gd,u)end;
return gd end;
local function ge(a2)fK()local gf=string.lower(tostring(a2 or""))local av=q.crystalCatalog[P(gf)]if not av then local gg=gf=="frost crystal"and"frozen crystal"or gf=="frozen crystal"and"frost crystal"av=gg and q.crystalCatalog[P(gg)]or nil end;
return av and av.id or a2,av end;
local function gh(fL)local eL=fK(fL)local y=P(q.selectedCrystal)if q.crystalCatalog[y]then q.selectedCrystal=q.crystalCatalog[y].name elseif#eL>0 then q.selectedCrystal=eL[1]else q.selectedCrystal=nil end;
return q.selectedCrystal end;
local function gi(gj,gk)local an=j:FindFirstChild(gj)if not an then return false end;
local cl=string.lower(tostring(gk or""))for o,L in ipairs(an:GetDescendants())do if string.lower(tostring(L.Name))==cl then return true end;
if L:IsA("StringValue")then local D,N=B(function()return string.lower(tostring(L.Value))end)if D and N==cl then return true end end end;
return false end;
local function gl(u)return gi("petsFolder",u)end;
local function gm(u)return gi("trailsFolder",u)or gi("aurasFolder",u)end;
local function gn(go,gp)if not go or go==j then return false end;
if gp=="all"then return true end;
if gp=="whitelist"then return q.killWhitelist[go.UserId]~=true end;
if gp=="blacklist"then return q.killBlacklist[go.UserId]==true end;
return false end;
local function gq(go,cO)local gr=aM()local gs=go and go.Character;
local gt=gs and gs:FindFirstChildWhichIsA("Humanoid")local gu=gs and gs:FindFirstChild("HumanoidRootPart")if not gr or not gu or not gt or gt.Health<=0 then return end;
if cO and cO.Parent then d4(cO)B(function()cO:Activate()end)end;
ek()if type(firetouchinterest)=="function"then for o,gv in ipairs({"RightHand","LeftHand","Right Arm","Left Arm"})do local el=gr:FindFirstChild(gv,true)if el and el:IsA("BasePart")then B(function()firetouchinterest(el,gu,0)firetouchinterest(el,gu,1)end)end end end end;
local function gw(gx)q.killToken=q.killToken+1;
q.killMode="off"for o,gy in pairs(q.leverRefs.kill or{})do if gy then gy.Set(false,true)end end;
fG()if gx then aP(gx)end end;
local function gz(gp)local cO,gA=dj()if not cO then gw(nil)aP("АВТОКИЛ: "..tostring(gA or"Punch не найден"))return false end;
q.killToken=q.killToken+1;
q.killMode=gp;
local fd=q.killToken;
fG()aP("АВТОКИЛ: "..string.upper(gp))task.spawn(function()while q.alive and q.killMode==gp and q.killToken==fd do if not q.networkPaused then local gB=dj()for o,go in ipairs(a:GetPlayers())do if q.killToken~=fd then break end;
if gn(go,gp)then gq(go,gB)end end end;
task.wait(0.10)end end)return true end;
local function gC()local gD=b:FindFirstChild("rEvents")local J=gD and gD:FindFirstChild("openCrystalRemote")if J and(J:IsA("RemoteFunction")or J:IsA("RemoteEvent"))then return J,true end;
local cc={b}local U={[b]=true}local function gE(L)if L and not U[L]then U[L]=true;
table.insert(cc,L)end end;
for o,u in ipairs({"rEvents","Remotes","remotes","Events","events","Network","network","CrystalRemotes","crystalRemotes","PetCrystals","petCrystals","Crystals","crystals"})do gE(b:FindFirstChild(u))end;
local gF=#cc;
for ay=1,gF do local ab=cc[ay]for o,u in ipairs({"Crystals","crystals","Crystal","crystal","PetCrystals","petCrystals","CrystalRemotes","crystalRemotes","Hatching","hatching"})do gE(ab:FindFirstChild(u))end end;
local gG={"openCrystalRemote","OpenCrystalRemote","openCrystalEvent","OpenCrystalEvent","openCrystal","OpenCrystal","hatchCrystalRemote","HatchCrystalRemote","hatchCrystal","HatchCrystal","crystalOpenRemote","CrystalOpenRemote","crystalHatchRemote","CrystalHatchRemote"}for o,cd in ipairs(cc)do for o,u in ipairs(gG)do local eh=cd:FindFirstChild(u)if eh and(eh:IsA("RemoteFunction")or eh:IsA("RemoteEvent"))then return eh,P(u)=="opencrystalremote"end end end;
local fO=0;
for o,cd in ipairs(cc)do for o,L in ipairs(cd:GetChildren())do fO=fO+1;
if fO>120 then return nil,false end;
if L:IsA("RemoteFunction")or L:IsA("RemoteEvent")then local a6=string.lower(tostring(L.Name or""))if a6:find("crystal",1,true)and(a6:find("open",1,true)or a6:find("hatch",1,true))and not a6:find("rebirth",1,true)and not a6:find("shop",1,true)and not a6:find("buy",1,true)and not a6:find("purchase",1,true)then return L,P(L.Name)=="opencrystalremote"end end end end;
return nil,false end;
local function gH(u)local eh,J=gC()if not eh then return false,"remote открытия кристаллов не найден"end;
local gI=tonumber(q.crystalAmount)or 1;
if gI~=1 and gI~=3 and gI~=10 then return false,"доступно только открытие 1, 3 или 10 кристаллов"end;
local gJ,av=ge(u)if gJ==nil or tostring(gJ)==""then return false,"кристалл не найден в игре"end;
local gK=av and av.action or nil;
if J then gK=gI==1 and(gK or"openCrystal")or"openCrystalBulk"end;
local D,gL=B(function()if eh:IsA("RemoteFunction")then if gK then if gI>1 then return eh:InvokeServer(gK,gJ,gI)end;
return eh:InvokeServer(gK,gJ)end;
if gI>1 then return eh:InvokeServer(gJ,gI)end;
return eh:InvokeServer(gJ)end;
if gK then if gI>1 then eh:FireServer(gK,gJ,gI)else eh:FireServer(gK,gJ)end elseif gI>1 then eh:FireServer(gJ,gI)else eh:FireServer(gJ)end end)if not D then return false,gL,gJ end;
if gL==false then return false,"сервер отклонил открытие кристалла",gJ end;
if type(gL)=="table"then for o,M in ipairs({"success","Success","ok","Ok"})do if gL[M]==false then return false,tostring(gL.message or gL.Message or gL.error or gL.Error or"сервер отклонил открытие кристалла"),gJ end end end;
return true,nil,gJ,gL end;
local function gM()local I=b:FindFirstChild("rEvents")local eh=I and I:FindFirstChild("sellPetEvent")if eh and(eh:IsA("RemoteEvent")or eh:IsA("RemoteFunction"))then return eh end;
return nil end;
function q.findPetEquipRemote()local I=b:FindFirstChild("rEvents")local eh=I and I:FindFirstChild("equipPetEvent")if eh and(eh:IsA("RemoteEvent")or eh:IsA("RemoteFunction"))then return eh end;
return nil end;
function q.equippedPetState()local an=j:FindFirstChild("equippedPets")if not an then return nil,0,0,{}end;
local d2={}local gN={}local gO=0;
local gP={}for o,gQ in ipairs(an:GetChildren())do local gR;
if gQ:IsA("ObjectValue")and gQ.Name=="petReference"then gR=gQ else gR=gQ:FindFirstChild("petReference")end;
if gR and gR:IsA("ObjectValue")then table.insert(gP,gR)end end;
if#gP==0 then for o,L in ipairs(an:GetDescendants())do if L:IsA("ObjectValue")and L.Name=="petReference"then table.insert(gP,L)if#gP>=32 then break end end end end;
for o,gR in ipairs(gP)do gO=gO+1;
local D,gS=B(function()return gR.Value end)if D and gS then d2[gS]=true;
table.insert(gN,gS)end end;
return d2,gO,#gN,gN end;
function q.isPetCurrentlyEquipped(gS)local d2=q.equippedPetState()return d2 and d2[gS]==true end;
q.petRarityScore={Basic=1,Rare=2,Epic=3,Unique=4,Advanced=5}function q.readPetNumber(gS,eL)local cc={gS}local gT=gS and gS:FindFirstChild("perksFolder")if gT then table.insert(cc,1,gT)end;
for o,cd in ipairs(cc)do for o,u in ipairs(eL)do local gU=cd:FindFirstChild(u)if gU then local D,N=B(function()return gU.Value end)if D then local gV=tonumber(N)or bO(N)if gV then return gV end end end;
local D,N=B(function()return cd:GetAttribute(u)end)if D then local gV=tonumber(N)or bO(N)if gV then return gV end end end end;
return 0 end;
q.petGlobalFunctions=nil;
q.petGlobalFunctionsCheckedAt=-1e9;
function q.getPetGlobalFunctions()if type(q.petGlobalFunctions)=="table"then return q.petGlobalFunctions end;
if os.clock()-(q.petGlobalFunctionsCheckedAt or 0)<5 then return nil end;
q.petGlobalFunctionsCheckedAt=os.clock()local F=b:FindFirstChild("shared")local gW=F and F:FindFirstChild("modules")local bA=gW and gW:FindFirstChild("GlobalFunctions")if not bA or not bA:IsA("ModuleScript")then return nil end;
local D,W=B(function()return require(bA)end)if D and type(W)=="table"then q.petGlobalFunctions=W;
return W end;
return nil end;
function q.petMeetsEquipRequirement(gS)local gX=q.getPetGlobalFunctions()local gY=gX and gX.checkIfPlayerHasEnoughStatsForPet;
if type(gY)~="function"then return true end;
local D,W=B(function()return gY(j,gS)end)return not(D and W==false)end;
function q.petRankRow(gS,gZ)local g_=gS.Parent and gS.Parent.Name or""local h0=gS:FindFirstChild("evolved")and 1 or 0;
if h0==0 then local D,N=B(function()return gS:GetAttribute("evolved")end)if D and(N==true or N==1 or tostring(N):lower()=="true")then h0=1 end end;
q.petStableOrder=q.petStableOrder or setmetatable({},{__mode="k"})local h1=q.petStableOrder[gS]if h1==nil then q.petStableNext=(tonumber(q.petStableNext)or 0)+1;
h1=q.petStableNext;
q.petStableOrder[gS]=h1 end;
return{pet=gS,strength=q.readPetNumber(gS,{"strength","Strength","strengthMultiplier","StrengthMultiplier"}),level=q.readPetNumber(gS,{"level","Level","petLevel","PetLevel"}),evolved=h0,rarity=q.petRarityScore[g_]or 0,name=string.lower(tostring(gS.Name or"")),equipped=gZ and gZ[gS]==true or false,stableOrder=h1}end;
function q.rankedEquipPets()local an=j:FindFirstChild("petsFolder")if not an then return nil,"инвентарь питомцев не найден"end;
local gZ,o,o,h2=q.equippedPetState()gZ=gZ or{}local h3={}local U={}local function h4(gS)if not gS or U[gS]or not gS:IsA("StringValue")then return end;
U[gS]=true;
if gZ[gS]or q.petMeetsEquipRequirement(gS)then table.insert(h3,q.petRankRow(gS,gZ))end end;
for o,h5 in ipairs(an:GetChildren())do local h6=(h5:IsA("Folder")or h5:IsA("Configuration"))and h5:GetChildren()or{h5}for o,gS in ipairs(h6)do h4(gS)end end;
for o,gS in ipairs(h2 or{})do h4(gS)end;
table.sort(h3,function(az,aA)if az.strength~=aA.strength then return az.strength>aA.strength end;
if az.level~=aA.level then return az.level>aA.level end;
if az.evolved~=aA.evolved then return az.evolved>aA.evolved end;
if az.equipped~=aA.equipped then return az.equipped==true end;
if az.rarity~=aA.rarity then return az.rarity>aA.rarity end;
if az.name~=aA.name then return az.name<aA.name end;
return az.stableOrder<aA.stableOrder end)return h3,nil end;
function q.buildBestFullPetTarget()local o,gO=q.equippedPetState()if gO<=0 then return nil,nil,0,"слоты экипировки не найдены"end;
local h3,h7=q.rankedEquipPets()if not h3 then return nil,nil,gO,h7 end;
if#h3<gO then return nil,nil,gO,("для полного комплекта нужно %d петов, доступно %d"):format(gO,#h3)end;
local cl={}local h8={}for ay=1,gO do local gS=h3[ay].pet;
table.insert(cl,gS)h8[gS]=true end;
return cl,h8,gO,nil end;
function q.petEquipRequest(gK,gS)local eh=q.findPetEquipRemote()if not eh then return false,"equipPetEvent не найден"end;
if not gS then return false,"питомец не найден"end;
local D,gL=B(function()if eh:IsA("RemoteFunction")then return eh:InvokeServer(gK,gS)end;
eh:FireServer(gK,gS)return true end)if not D then return false,tostring(gL)end;
if gL==false or type(gL)=="table"and(gL.success==false or gL.Success==false or gL.ok==false or gL.Ok==false)then return false,tostring(type(gL)=="table"and(gL.message or gL.Message or gL.error or gL.Error)or"сервер отклонил экипировку")end;
return true,nil end;
function q.waitPetEquipState(gS,a2,fd)local d1=os.clock()+2;
repeat local d2=q.equippedPetState()if d2 and d2[gS]==true==a2 then return true end;
if fd and q.petEquipToken~=fd then return false end;
task.wait(0.05)until os.clock()>=d1;
local d2=q.equippedPetState()return d2 and d2[gS]==true==a2 or false end;
function q.restoreOriginalPetSet(h9,ha)local H=q.equippedPetState()if not H then return end;
for gS in pairs(H)do if not ha[gS]then q.petEquipRequest("unequipPet",gS)task.wait(0.06)end end;
H=q.equippedPetState()for o,gS in ipairs(h9)do if gS.Parent and not H[gS]then q.petEquipRequest("equipPet",gS)task.wait(0.06)H=q.equippedPetState()end end end;
function q.applyBestFullPetSet(fd)local cl,h8,gO,h7=q.buildBestFullPetTarget()if not cl then return false,h7,0 end;
if not q.findPetEquipRemote()then return false,"equipPetEvent не найден",0 end;
for o,gS in ipairs(cl)do if not gS.Parent then return false,"инвентарь изменился, повторяю позже",0 end end;
local H,o,hb,h9=q.equippedPetState()local ha={}for o,gS in ipairs(h9)do ha[gS]=true end;
local hc=hb==gO;
for o,gS in ipairs(cl)do if not H[gS]then hc=false;
break end end;
if hc then return true,nil,0 end;
local hd=0;
local function he(gx)q.restoreOriginalPetSet(h9,ha)return false,gx,hd end;
while hb<gO do local hf;
for o,gS in ipairs(cl)do if not H[gS]then hf=gS;
break end end;
if not hf then return he("не удалось собрать полный комплект")end;
if q.petEquipToken~=fd or not q.autoEquipBestPets then return he("подбор остановлен")end;
local D,hg=q.petEquipRequest("equipPet",hf)if not D or not q.waitPetEquipState(hf,true,fd)then return he(hg or"питомец не экипировался")end;
hd=hd+1;
H,o,hb=q.equippedPetState()end;
while true do local hf;
for o,gS in ipairs(cl)do if not H[gS]then hf=gS;
break end end;
if not hf then break end;
local hh;
for gS in pairs(H)do if not h8[gS]then hh=gS;
break end end;
if not hh then return he("не удалось определить замену")end;
if q.petEquipToken~=fd or not q.autoEquipBestPets then return he("подбор остановлен")end;
local D,hg=q.petEquipRequest("unequipPet",hh)if not D or not q.waitPetEquipState(hh,false,fd)then return he(hg or"старый питомец не снялся")end;
D,hg=q.petEquipRequest("equipPet",hf)if not D or not q.waitPetEquipState(hf,true,fd)then q.petEquipRequest("equipPet",hh)q.waitPetEquipState(hh,true,nil)return he(hg or"новый питомец не экипировался")end;
hd=hd+1;
H,o,hb=q.equippedPetState()end;
H,o,hb=q.equippedPetState()if hb~=gO then return he("комплект заполнен не полностью")end;
for o,gS in ipairs(cl)do if not H[gS]then return he("проверка полного комплекта не пройдена")end end;
return true,nil,hd end;
function q.stopAutoEquipBestPets(gx)q.autoEquipBestPets=false;
q.petEquipToken=(q.petEquipToken or 0)+1;
q.petEquipLastMessage=nil;
if q.leverRefs.petFullEquip then q.leverRefs.petFullEquip.Set(false,true)end;
if gx then aP(gx)end end;
function q.startAutoEquipBestPets()local cl,o,gO,h7=q.buildBestFullPetTarget()if not cl then return false,h7 end;
if not q.findPetEquipRemote()then return false,"equipPetEvent не найден"end;
q.petEquipToken=(q.petEquipToken or 0)+1;
q.autoEquipBestPets=true;
q.petEquipLastMessage=nil;
local fd=q.petEquipToken;
aP((q.language=="en"and"BEST PETS: full set "or"ЛУЧШИЕ ПЕТЫ: полный комплект ")..tostring(gO).."/"..tostring(gO))task.spawn(function()while q.alive and q.autoEquipBestPets and q.petEquipToken==fd do if not q.networkPaused then local D,hg,hd=q.applyBestFullPetSet(fd)if q.petEquipToken~=fd then return end;
local gx;
if D and hd>0 then local o,hi,fE=q.equippedPetState()gx=(q.language=="en"and"BEST PETS: full set "or"ЛУЧШИЕ ПЕТЫ: полный комплект ")..tostring(fE).."/"..tostring(hi)elseif not D and hg~="подбор остановлен"then gx=(q.language=="en"and"BEST PETS: waiting for full set • "or"ЛУЧШИЕ ПЕТЫ: жду полный комплект • ")..tostring(hg)end;
if gx and gx~=q.petEquipLastMessage then q.petEquipLastMessage=gx;
aP(gx)end end;
for o=1,120 do if not q.alive or not q.autoEquipBestPets or q.petEquipToken~=fd then return end;
task.wait(0.1)end end end)return true,nil end;
function q.findQuestRemote()local I=b:FindFirstChild("rEvents")local eh=I and I:FindFirstChild("questsEvent")if eh and(eh:IsA("RemoteEvent")or eh:IsA("RemoteFunction"))then return eh end;
return nil end;
function q.scanQuestNpcs()local an=workspace:FindFirstChild("questsNpcs")if not an then return nil,"questsNpcs не найден"end;
local hj={}local hk=false;
for o,hl in ipairs(an:GetChildren())do local hm=hl:FindFirstChild("questLink")local D,hn=B(function()return hm and hm:IsA("ObjectValue")and hm.Value or nil end)if D and hn then local ho=hn:FindFirstChild("Rebirths")local cg=0;
if ho then local hp,N=B(function()return tonumber(ho.Value)end)if hp and N then cg=N end end;
local bu={id=tostring(hl.Name),name=tostring(hl.Name),npc=hl,questLine=hn,questLineName=tostring(hn.Name),rebirths=cg}table.insert(hj,bu)if q.selectedQuestNpc==bu.id then hk=true end end end;
table.sort(hj,function(bJ,bK)if bJ.rebirths~=bK.rebirths then return bJ.rebirths<bK.rebirths end;
return string.lower(bJ.name)<string.lower(bK.name)end)if not hk then local H=c1()local as=hj[1]for o,bu in ipairs(hj)do if not H or bu.rebirths<=H then as=bu;
break end end;
q.selectedQuestNpc=as and as.id or nil end;
return hj,nil end;
function q.selectedQuestNpcEntry()local hj,h7=q.scanQuestNpcs()if not hj then return nil,h7 end;
for o,bu in ipairs(hj)do if bu.id==q.selectedQuestNpc then return bu,nil end end;
return nil,"NPC не выбран"end;
function q.questRequest(gK,hq)local eh=q.findQuestRemote()if not eh then return false,"questsEvent не найден"end;
if not hq or not hq.Parent then return false,"квест больше недоступен"end;
local D,gL=B(function()if eh:IsA("RemoteFunction")then return eh:InvokeServer(gK,hq)end;
eh:FireServer(gK,hq)return true end)if not D then return false,tostring(gL)end;
if gL==false or type(gL)=="table"and(gL.success==false or gL.Success==false or gL.ok==false or gL.Ok==false)then return false,tostring(type(gL)=="table"and(gL.message or gL.Message or gL.error or gL.Error)or"сервер отклонил квест")end;
return true,nil end;
function q.collectSelectedNpcQuests(av,fd)local gX=q.getPetGlobalFunctions()local gY=gX and gX.checkForCompleteQuest;
if type(gY)~="function"then return nil,"проверка квестов не найдена"end;
local aO=j:FindFirstChild("Quests")if not aO then return nil,"папка квестов не найдена"end;
local hr={}local hs=aO:FindFirstChild("Story Quests")local ht=hs and hs:FindFirstChild(av.questLineName)if ht then table.insert(hr,ht)end;
if av.questLineName=="Industrial Gains"then local hu=aO:FindFirstChild("Industrial Daily")if hu then table.insert(hr,hu)end end;
local hv={}local U={}for o,hw in ipairs(hr)do if hw:IsA("Folder")and hw:FindFirstChild("requirements")then U[hw]=true;
table.insert(hv,hw)end;
for o,hq in ipairs(hw:GetDescendants())do if hq:IsA("Folder")and hq:FindFirstChild("requirements")and not U[hq]then U[hq]=true;
table.insert(hv,hq)end end end;
local hx=0;
for o,hq in ipairs(hv)do if fd and(q.questToken~=fd or not q.autoQuest)then break end;
local c5,hy=B(function()return gY(hq)end)if c5 and hy==true then local hz,h7=q.questRequest("collectQuest",hq)if not hz then return nil,h7 end;
hx=hx+1;
task.wait(0.4)end end;
return hx,nil end;
function q.nextSelectedNpcQuest(av)local aO=j:FindFirstChild("Quests")local hs=aO and aO:FindFirstChild("Story Quests")local hA=aO and aO:FindFirstChild("completedQuests")local aI=av.questLine and av.questLine:FindFirstChild("Quests")if not aO or not hs or not hA or not aI then return nil,nil,"данные цепочки квестов не найдены"end;
if hs:FindFirstChild(av.questLineName)then return nil,nil,nil end;
local hB=hA:FindFirstChild(av.questLineName)local hC={}for o,hq in ipairs(aI:GetChildren())do local aY=tonumber(hq.Name)if aY then table.insert(hC,aY)end end;
table.sort(hC)if not hB then local bC=aI:FindFirstChild("1")return bC,bC and"createNewStoryQuest"or nil,bC and nil or"первый квест цепочки не найден"end;
for o,aY in ipairs(hC)do local u=tostring(aY)if not hB:FindFirstChild(u)then local hq=aI:FindFirstChild(u)return hq,hq and"createNewStoryQuest"or nil,hq and nil or"следующий квест не найден"end end;
if av.questLineName=="Industrial Gains"then local bB=aO:FindFirstChild("Industrial Daily")if bB and#bB:GetChildren()>0 then return nil,nil,nil end;
local F=b:FindFirstChild("shared")local hD=F and F:FindFirstChild("catalogs")local hE=hD and hD:FindFirstChild("Quests")local hu=hE and hE:FindFirstChild("Industrial Daily")local hF=hu and hu:FindFirstChild(os.date("!%A"))local hG=hF and hF:GetChildren()or{}table.sort(hG,function(bJ,bK)return bJ.Name<bK.Name end)local hq=hG[1]return hq,hq and"createNewIndustrialDaily"or nil,hq and nil or"сегодняшний квест не найден"end;
return nil,nil,"цепочка NPC завершена"end;
function q.acceptSelectedNpcQuest(av)local H=c1()if av.rebirths>0 and not H then return false,"Rebirths не найдены"end;
if H and av.rebirths>H then return false,("нужно %d Rebirths"):format(av.rebirths)end;
local hq,gK,h7=q.nextSelectedNpcQuest(av)if not hq then return false,h7,nil end;
q.questAcceptCooldown=q.questAcceptCooldown or{}local hH=q.questAcceptCooldown[hq]or 0;
local hI=gK=="createNewIndustrialDaily"and 60 or 8;
if os.clock()-hH<hI then return false,nil,nil end;
local hz,hJ=q.questRequest(gK,hq)if not hz then return false,hJ,nil end;
q.questAcceptCooldown[hq]=os.clock()return true,nil,tostring(hq.Name)end;
function q.runSelectedQuestCycle(fd)local av,h7=q.selectedQuestNpcEntry()if not av then return false,h7 end;
local hx,hK=q.collectSelectedNpcQuests(av,fd)if hx==nil then return false,hK end;
if q.questToken~=fd or not q.autoQuest then return false,"автоквест остановлен"end;
if hx>0 then task.wait(0.6)end;
local hL,hM,hN=q.acceptSelectedNpcQuest(av)local gx;
if hL then gx=(q.language=="en"and"AUTO QUEST: accepted "or"АВТОКВЕСТ: взят ")..av.name.." #"..tostring(hN)elseif hx>0 then gx=(q.language=="en"and"AUTO QUEST: reward collected • "or"АВТОКВЕСТ: награда получена • ")..av.name elseif hM then gx=(q.language=="en"and"AUTO QUEST: "or"АВТОКВЕСТ: ")..tostring(hM)else gx=(q.language=="en"and"AUTO QUEST: active • "or"АВТОКВЕСТ: выполняется • ")..av.name end;
if gx~=q.questLastMessage then q.questLastMessage=gx;
aP(gx)end;
if type(q.refreshQuestUI)=="function"then q.refreshQuestUI()end;
return true,nil end;
function q.stopAutoQuest(gx)q.autoQuest=false;
q.questToken=(q.questToken or 0)+1;
q.questLastMessage=nil;
if q.leverRefs.autoQuest then q.leverRefs.autoQuest.Set(false,true)end;
if gx then aP(gx)end;
if type(q.refreshQuestUI)=="function"then q.refreshQuestUI()end end;
function q.startAutoQuest()local av,h7=q.selectedQuestNpcEntry()if not av then return false,h7 end;
if not q.findQuestRemote()then return false,"questsEvent не найден"end;
local H=c1()if av.rebirths>0 and H and av.rebirths>H then return false,("нужно %d Rebirths"):format(av.rebirths)end;
q.questToken=(q.questToken or 0)+1;
q.autoQuest=true;
q.questLastMessage=nil;
local fd=q.questToken;
aP((q.language=="en"and"AUTO QUEST: started • "or"АВТОКВЕСТ: запущен • ")..av.name)task.spawn(function()while q.alive and q.autoQuest and q.questToken==fd do if not q.networkPaused then local D,hg=q.runSelectedQuestCycle(fd)if q.questToken~=fd then return end;
if not D and hg~="автоквест остановлен"then local gx=(q.language=="en"and"AUTO QUEST: waiting • "or"АВТОКВЕСТ: ожидание • ")..tostring(hg)if gx~=q.questLastMessage then q.questLastMessage=gx;
aP(gx)end end end;
...
for o,M in ipairs({"Equipped","equipped","IsEquipped","isEquipped","equippedValue","Locked","locked","IsLocked","isLocked","Favorite","favorite","Favourite","favourite","Protected","protected","Bugged","bugged","IsBugged","isBugged","Modded","modded","IsModded","isModded","Evolved","evolved","IsEvolved","isEvolved"})do local D,N=B(function()return L:GetAttribute(M)end)if D and(N==true or N==1 or tostring(N):lower()=="true"or tostring(N)=="1")then return true end;
local gU=L:FindFirstChild(M)if gU then D,N=B(function()return gU.Value end)if D and(N==true or N==1 or tostring(N):lower()=="true"or tostring(N)=="1")then return true end end end;
for o,M in ipairs({"Level","level","PetLevel","petLevel"})do local D,N=B(function()return tonumber(L:GetAttribute(M))end)if D and N and N>1 then return true end;
local gU=L:FindFirstChild(M)if gU then D,N=B(function()return tonumber(gU.Value)end)if D and N and N>1 then return true end end end;
for o,M in ipairs({"XP","xp","Xp","PetXP","petXP","Experience","experience"})do local D,N=B(function()return tonumber(L:GetAttribute(M))end)if D and N and N>0 then return true end;
local gU=L:FindFirstChild(M)if gU then D,N=B(function()return tonumber(gU.Value)end)if D and N and N>0 then return true end end end;
local ab=L.Parent;
for o=1,4 do if not ab or ab==j then break end;
local a6=string.lower(tostring(ab.Name or""))if a6:find("equip",1,true)or a6:find("locked",1,true)or a6:find("favorite",1,true)then return true end;
ab=ab.Parent end;
return false end;
local function hQ(ef)if not q.petCleanupEnabled then return 0 end;
local eh=gM()if not eh then return nil,"sellPetEvent не найден"end;
local an=j:FindFirstChild("petsFolder")if not an then return nil,"инвентарь питомцев не найден"end;
local hR={}for u,fF in pairs(q.petCleanupTargets or{})do local y=P(u)if fF and not hO(u)then hR[y]=true end end;
if next(hR)==nil then return 0 end;
local b6=os.clock()q.petCleanupPending=q.petCleanupPending or{}for gS,hS in pairs(q.petCleanupPending)do if not gS.Parent or b6-hS>2 then q.petCleanupPending[gS]=nil end end;
local hT={}for o,h5 in ipairs(an:GetChildren())do if h5:IsA("Folder")or h5:IsA("Configuration")then for o,gS in ipairs(h5:GetChildren())do if#hT>=1000 then break end;
table.insert(hT,gS)end else table.insert(hT,h5)end;
if#hT>=1000 then break end end;
local hU=math.min(10,math.max(1,tonumber(ef)or 1))local hV=0;
local hW=#hT;
for o,gS in ipairs(hT)do if q.petCleanupPending[gS]then hW=hW-1 end end;
for o,gS in ipairs(hT)do if hV>=hU or hW<=1 then break end;
if hR[P(gS.Name)]and not q.petCleanupPending[gS]and not hP(gS)then local D,W=B(function()if eh:IsA("RemoteFunction")then return eh:InvokeServer("sellPet",gS)end;
eh:FireServer("sellPet",gS)end)local hX=W==false or type(W)=="table"and(W.success==false or W.Success==false or W.ok==false)if D and not hX then q.petCleanupPending[gS]=b6;
q.petCleanupSold=(q.petCleanupSold or 0)+1;
hV=hV+1;
hW=hW-1 end end end;
return hV end;
local function hY()if fC(q.petCleanupTargets)==0 then return false,"ВЫБЕРИ ПИТОМЦЕВ ДЛЯ УДАЛЕНИЯ"end;
if not gM()then return false,"sellPetEvent не найден"end;
if not j:FindFirstChild("petsFolder")then return false,"инвентарь питомцев не найден"end;
q.petCleanupToken=(q.petCleanupToken or 0)+1;
q.petCleanupEnabled=true;
local fd=q.petCleanupToken;
task.spawn(function()while q.alive and q.petCleanupEnabled and q.petCleanupToken==fd do if not q.networkPaused then local hZ,h7=hQ(10)if hZ==nil then q.petCleanupEnabled=false;
q.petCleanupToken=q.petCleanupToken+1;
if q.leverRefs.petCleanup then q.leverRefs.petCleanup.Set(false,true)end;
aP("АВТОУДАЛЕНИЕ: "..tostring(h7))return end;
if hZ>0 then aP("УДАЛЕНО ИЗ ИНВЕНТАРЯ: "..tostring(q.petCleanupSold))end end;
task.wait(0.25)end end)return true end;
function q.findPetEvolveRemote()local I=b:FindFirstChild("rEvents")local eh=I and I:FindFirstChild("petEvolveEvent")if eh and(eh:IsA("RemoteEvent")or eh:IsA("RemoteFunction"))then return eh end;
local J=b:FindFirstChild("petEvolveEvent")if J and(J:IsA("RemoteEvent")or J:IsA("RemoteFunction"))then return J end;
return nil end;
function q.countUnevolvedPetCopies(u)local an=j:FindFirstChild("petsFolder")if not an then return 0 end;
local cl=P(u)local fE=0;
for o,h5 in ipairs(an:GetChildren())do local h6=(h5:IsA("Folder")or h5:IsA("Configuration"))and h5:GetChildren()or{h5}for o,gS in ipairs(h6)do if gS:IsA("StringValue")and P(gS.Name)==cl and gS:FindFirstChild("evolved")==nil then fE=fE+1 end end end;
return fE end;
function q.autoEvolvePurchasedPet(u)if not q.autoEvolvePurchasedPets then return false,nil end;
local y=P(u)if y==""then return false,nil end;
local fE=q.countUnevolvedPetCopies(u)local b6=os.clock()q.petEvolvePending=q.petEvolvePending or{}local h_=q.petEvolvePending[y]if h_ then local i0=q.petPurchaseCounts and q.petPurchaseCounts[y]or 0;
local i1=i0-(tonumber(h_.purchases)or i0)if fE<h_.count or i1>=5 then q.petEvolvePending[y]=nil elseif b6-h_.sentAt<1.5 then return false,nil else q.petEvolvePending[y]=nil end end;
if fE<5 then return false,nil end;
local eh=q.findPetEvolveRemote()if not eh then return false,"petEvolveEvent не найден"end;
local D,gL=B(function()if eh:IsA("RemoteFunction")then return eh:InvokeServer("evolvePet",tostring(u))end;
eh:FireServer("evolvePet",tostring(u))return true end)if not D then return false,tostring(gL)end;
if gL==false or type(gL)=="table"and(gL.success==false or gL.Success==false or gL.ok==false or gL.Ok==false)then return false,tostring(type(gL)=="table"and(gL.message or gL.Message or gL.error or gL.Error)or"сервер отклонил эволюцию")end;
q.petEvolvePending[y]={count=fE,sentAt=b6,purchases=q.petPurchaseCounts and q.petPurchaseCounts[y]or 0}q.petEvolveSent=(q.petEvolveSent or 0)+1;
return true,nil end;
local function i2(u,ai)local an=q.resolvePetShopFolder()local eh=q.resolvePetShopRemote()if not an then return false,"cPetShopFolder не найден"end;
if not eh then return false,"cPetShopRemote не найден"end;
local a0=tostring(u or""):gsub("^%s+",""):gsub("%s+$","")if a0==""then return false,"товар не выбран"end;
local L=an:FindFirstChild(a0)if not L then local a1=q.shopLocalizedAliasMap()local hv=an:GetChildren()local D,bF=B(function()return an:GetDescendants()end)if D then for ay,ax in ipairs(bF)do if ay>320 then break end;
table.insert(hv,ax)end end;
for o,ax in ipairs(hv)do if q.shopNameMatches(a0,ax,nil,a1)then L=ax;
break end end end;
if not L then return false,"товар не найден: "..a0 end;
if ai and ai=="aura"~=a4(L)then return false,"неверный тип товара: "..a0 end;
local D,gL=B(function()return eh:InvokeServer(L)end)if not D then return false,tostring(gL),a0 end;
if gL~=true then if type(gL)=="table"then local hL=gL.success==true or gL.Success==true or gL.ok==true or gL.Ok==true;
if hL then return true,nil,a0 end;
return false,tostring(gL.message or gL.Message or gL.error or gL.Error or"сервер отклонил покупку"),a0 end;
return false,"сервер отклонил покупку",a0 end;
return true,nil,a0 end;
local function i3(gp)if gp=="crystal"then return q.selectedCrystal end;
if gp=="pet"then return q.selectedPet end;
if gp=="aura"then return q.selectedAura end;
return nil end;
local function i4(gx)if q.crystalMode=="pet"and q.autoEvolvePurchasedPets and q.selectedPet then B(function()q.autoEvolvePurchasedPet(q.selectedPet)end)end;
q.petEvolveLastError=nil;
q.crystalToken=q.crystalToken+1;
q.crystalMode="off"for o,gy in pairs(q.leverRefs.crystal or{})do if gy then gy.Set(false,true)end end;
fG()if gx then aP(gx)end end;
local function i5(gp)if gp=="crystal"then gh(true)end;
if gp=="pet"then aH("pet")q.petEvolveLastError=nil end;
if gp=="aura"then aH("aura")end;
local gk=i3(gp)if not gk then aP("СНАЧАЛА ВЫБЕРИ ЦЕЛЬ")return false end;
q.crystalToken=q.crystalToken+1;
q.crystalMode=gp;
q.purchaseAttempts=0;
local fd=q.crystalToken;
local i6=0;
fG()aP((gp=="crystal"and"ОТКРЫТИЕ ×"..tostring(q.crystalAmount or 1)..": "or"ПРЯМАЯ ПОКУПКА: ")..gk)task.spawn(function()while q.alive and q.crystalMode==gp and q.crystalToken==fd do local i7=math.clamp(tonumber(q.purchaseDelay)or 0.5,0.05,5)local i8=false;
if not q.networkPaused then local D,hg,gJ;
if gp=="crystal"then if q.petCleanupEnabled then local hV,i9=hQ(q.crystalAmount)if hV==nil then i4("АВТОУДАЛЕНИЕ: "..tostring(i9))return end;
if hV>0 then task.wait(0.12)end end;
D,hg,gJ=gH(gk)else if gp=="pet"and q.autoEvolvePurchasedPets and q.purchaseAttempts==0 then local h0,ia=q.autoEvolvePurchasedPet(gk)i8=h0 and true or false;
q.petEvolveLastError=ia end;
D,hg,gJ=i2(gk,gp)end;
if D then i6=0;
q.purchaseAttempts=q.purchaseAttempts+1;
if gp=="pet"then local ib=P(gk)q.petPurchaseCounts=q.petPurchaseCounts or{}q.petPurchaseCounts[ib]=(q.petPurchaseCounts[ib]or 0)+1;
if q.autoEvolvePurchasedPets and q.petPurchaseCounts[ib]%5==0 then local h0,ia=q.autoEvolvePurchasedPet(gk)i8=i8 or(h0 and true or false)if ia then q.petEvolveLastError=ia end end end;
local gK=gp=="crystal"and"ОТКРЫТИЕ ×"..tostring(q.crystalAmount or 1)or"ПОКУПКА"local ic=i8 and(q.language=="en"and" • evolved"or" • эволюция")or""if q.petEvolveLastError then ic=(q.language=="en"and" • evolve: "or" • эволюция: ")..tostring(q.petEvolveLastError):sub(1,45)q.petEvolveLastError=nil end;
aP(gK.." #"..q.purchaseAttempts..": "..tostring(gJ)..ic)else if gp~="crystal"then i4("НЕ КУПЛЕНО: "..tostring(hg))return end;
i6=i6+1;
...
local function ii()q.rebirthGoalAwaitingFrom=nil;
q.rebirthGoalAwaitingSince=0;
q.rebirthGoalAwaitingUntil=0;
q.rebirthGoalReadFailures=0 end;
local function ij(cF,hA,H)q.rebirthToken=q.rebirthToken+1;
q.autoRebirth=false;
q.rebirthGoalEnabled=false;
q.rebirthGoalCompleted=hA==true;
q.nextRebirth=0;
ii()if H~=nil then q.rebirthGoalCurrent=math.max(0,math.floor(tonumber(H)or 0))end;
if q.leverRefs.autoRebirth then q.leverRefs.autoRebirth.Set(false,true)end;
if q.leverRefs.rebirthGoal then q.leverRefs.rebirthGoal.Set(false,true)end;
ci(q.rebirthGoalCurrent)if cF then aP(cF)end end;
local function ik(H)local cl=math.max(1,math.floor(tonumber(q.rebirthGoal)or 1))local il=math.max(0,math.floor(tonumber(H)or cl))ij(("ЦЕЛЬ ДОСТИГНУТА: %s / %s • АВТОРЕБИРТ СТОП"):format(ch(il),ch(cl)),true,il)end;
local function im(io,N)if not q.alive then return end;
if io and q.rebirthCounterWatched and io~=q.rebirthCounterWatched then return end;
local H=tonumber(N)if H==nil then return end;
H=math.max(0,math.floor(H+0.5))q.rebirthGoalCurrent=H;
local ip=q.rebirthGoalAwaitingFrom;
if ip~=nil and H>ip then ii()end;
ci(H)if q.rebirthGoalEnabled then local cl=math.max(1,math.floor(tonumber(q.rebirthGoal)or 1))if H>=cl then ik(H)else q.nextRebirth=0 end end end;
local function iq()local H,bA,io=c1()local ir=io and(io:IsA("IntValue")or io:IsA("NumberValue"))if not ir or not q.rebirthCounterExact or bA~="leaderstats"or not io:IsDescendantOf(j)then if q.rebirthCounterConn then B(function()q.rebirthCounterConn:Disconnect()end)end;
q.rebirthCounterConn=nil;
q.rebirthCounterWatched=nil;
return H,bA,nil end;
if q.rebirthCounterWatched~=io or not q.rebirthCounterConn then if q.rebirthCounterConn then B(function()q.rebirthCounterConn:Disconnect()end)end;
q.rebirthCounterWatched=io;
local D,is=B(function()return io:GetPropertyChangedSignal("Value"):Connect(function()if not q.alive or q.rebirthCounterWatched~=io then return end;
local N=bV(io)if N~=nil then im(io,N)end end)end)q.rebirthCounterConn=D and aJ(is)or nil;
if not q.rebirthCounterConn then q.rebirthCounterWatched=nil;
return H,bA,nil end end;
if H~=nil then im(io,H)end;
return H,bA,io end;
local function it(H,cl,iu)local iv=q.pingAvailable and math.max(0,tonumber(q.pingMs)or 0)/1000 or 0.1;
...
local io=nil;
local cl=nil;
if q.rebirthGoalEnabled then local N,o,iz=iq()if not iy()then return end;
if N==nil or not iz then local b6=os.clock()q.rebirthGoalCurrent=nil;
q.rebirthGoalReadFailures=q.rebirthGoalReadFailures+1;
q.nextRebirth=b6+1;
ci(nil,"Точный счётчик Rebirths не найден • запрос на паузе")if q.rebirthGoalReadFailures==1 or b6-q.rebirthGoalStatusAt>=3 then q.rebirthGoalStatusAt=b6;
aP("ЦЕЛЬ: ТОЧНЫЙ СЧЁТЧИК REBIRTHS НЕ НАЙДЕН • ПАУЗА")end;
return end;
q.rebirthGoalReadFailures=0;
io=iz;
fe=math.max(0,math.floor(N+0.5))cl=math.max(1,math.floor(tonumber(q.rebirthGoal)or 1))ci(fe)if fe>=cl then ik(fe)return end;
local ip=q.rebirthGoalAwaitingFrom;
if ip~=nil then if fe>ip then ii()else local b6=os.clock()if b6<q.rebirthGoalAwaitingUntil then q.nextRebirth=q.rebirthGoalAwaitingUntil;
return end;
ii()end end end;
if not iy()then return end;
local eh=ig()if not eh then q.nextRebirth=os.clock()+1;
aP("AUTO REB: rebirthRemote не найден")return end;
if cl and not eh:IsA("RemoteFunction")then ij("ЦЕЛЬ РЕБИРТОВ: НУЖЕН ТОЧНЫЙ REMOTEFUNCTION",false,fe)return end;
local iu=eh:IsA("RemoteFunction")local iA=cl and it(fe,cl,iu)or 0.1;
if q.rebirthGoalEnabled then local hS=os.clock()q.rebirthGoalAwaitingFrom=fe;
q.rebirthGoalAwaitingSince=hS;
q.rebirthGoalAwaitingUntil=hS+iA end;
local D,hg=ih(eh)if not iy()then return end;
if not D then if q.rebirthGoalEnabled then if q.rebirthGoalAwaitingFrom==fe then q.nextRebirth=q.rebirthGoalAwaitingUntil;
aP("AUTO REB: ЖДУ СЧЁТЧИК ПОСЛЕ ОШИБКИ СЕТИ")else q.nextRebirth=0 end else q.nextRebirth=os.clock()+0.25;
aP("AUTO REB: "..tostring(hg))end;
return end;
if q.rebirthGoalEnabled and io then local iB=bV(io)if iB~=nil then im(io,iB)end;
if not iy()then return end;
if q.rebirthGoalAwaitingFrom==fe then q.rebirthGoalAwaitingUntil=math.max(q.rebirthGoalAwaitingUntil,os.clock()+iA)q.nextRebirth=q.rebirthGoalAwaitingUntil else q.nextRebirth=0 end elseif not q.rebirthGoalEnabled then q.nextRebirth=os.clock()+0.05 end end;
local function iC()local I=b:FindFirstChild("rEvents")local eh=I and I:FindFirstChild("changeSpeedSizeRemote")if eh and(eh:IsA("RemoteFunction")or eh:IsA("RemoteEvent"))then return eh end;
return nil end;
local function iD(N)if q.networkPaused then return false,"network hold"end;
local eh=iC()if not eh then return false,"changeSpeedSizeRemote не найден"end;
local eD=tonumber(N)if not eD then return false,"неверный размер"end;
...
local U={}for o,ax in ipairs(hv)do if ax and not U[ax]then U[ax]=true;
for o,R in ipairs(ax:GetChildren())do if R.Name=="RockBugKingPhysicalHold"or R.Name=="RockBugKingPhysicalGyro"then B(function()R:Destroy()end)end end end end end;
local function iI(iJ)local iK=q.kingTouchTrigger;
if iK and iK.Parent and type(firetouchinterest)=="function"then for o,eE in ipairs(q.kingTouchContacts or{})do if eE and eE.Parent and eE:IsA("BasePart")then B(function()firetouchinterest(eE,iK,1)end)end end end;
q.kingTouchContacts={}q.kingTouchTrigger=nil;
if iJ then q.kingTriggerPart=nil end end;
local function iL(eE,iM)local eT=(eE.Position-iM).Magnitude;
if eT>320 then return nil end;
local dd=tostring(eE:GetFullName()):lower()local iN=eE:FindFirstChildOfClass("TouchTransmitter")~=nil;
local iO=dd:find("muscle king",1,true)or dd:find("muscleking",1,true)or dd:find("king",1,true)local iP=cP(dd,{"trigger","zone","area","hill","capture","touch","boost"})local iQ=cP(dd,{"rock","mountain","machine","crystal","shop","neededdurability"})if iQ then return nil end;
if not iN and not iP then return nil end;
if not iO and not iP and eT>35 then return nil end;
local bZ=-eT*0.08;
if iN then bZ=bZ+100 end;
if dd:find("muscle king",1,true)or dd:find("muscleking",1,true)then bZ=bZ+170 end;
if dd:find("king",1,true)then bZ=bZ+80 end;
if iP then bZ=bZ+55 end;
if eE.CanTouch then bZ=bZ+8 end;
if not eE.CanCollide then bZ=bZ+6 end;
if eE.Size.X>=8 and eE.Size.Z>=8 then bZ=bZ+8 end;
return bZ end;
local function iR()if q.kingTriggerPart and q.kingTriggerPart.Parent then return q.kingTriggerPart end;
local iM=iE().Position;
local bl=nil;
local c4=-math.huge;
local function iS(eE)if not eE or not eE:IsA("BasePart")then return end;
local bZ=iL(eE,iM)if bZ and bZ>c4 then bl=eE;
c4=bZ end end;
local iT,iU=B(function()return workspace:GetPartBoundsInRadius(iM,320)end)if iT and type(iU)=="table"then for o,eE in ipairs(iU)do iS(eE)end end;
if not bl then for o,eE in ipairs(workspace:GetDescendants())do iS(eE)end end;
q.kingTriggerPart=bl;
return bl end;
local function iV(iK)local iW=q.kingCF or iE()if not iK or not iK.Parent then return iW end;
if(iK.Position-iW.Position).Magnitude>160 then return iW end;
local iX=iK.CFrame:PointToObjectSpace(iW.Position)local iY=iK.Size*0.5;
local iZ=math.abs(iX.X)<=iY.X+3 and math.abs(iX.Y)<=iY.Y+4 and math.abs(iX.Z)<=iY.Z+3;
if iZ then return iW end;
local eG=iK.Position.Y;
if iK.CanCollide or iK.Size.Y<4 then eG=eG+iY.Y+2.8 end;
return CFrame.new(iK.Position.X,eG,iK.Position.Z)*iW.Rotation end;
local function i_(dH,dI)iG()dH.Anchored=false;
dH.CFrame=dI;
dH.AssemblyLinearVelocity=Vector3.new(0,0,0)dH.AssemblyAngularVelocity=Vector3.new(0,0,0)local D,hg=xpcall(function()local eN=Instance.new("BodyPosition")eN.Name="RockBugKingPhysicalHold"eN.MaxForce=Vector3.new(1e9,1e9,1e9)eN.P=50000;
eN.D=1800;
eN.Position=dI.Position;
eN.Parent=dH;
q.kingHoldPosition=eN;
local j0=Instance.new("BodyGyro")j0.Name="RockBugKingPhysicalGyro"j0.MaxTorque=Vector3.new(1e9,1e9,1e9)j0.P=40000;
j0.D=1200;
j0.CFrame=dI;
j0.Parent=dH;
q.kingHoldGyro=j0 end,function(j1)return tostring(j1)end)if not D then iG()end;
return D,hg end;
local function j2(iK)q.nextKingTouchPulse=os.clock()+2;
if not iK or not iK.Parent or type(firetouchinterest)~="function"then return end;
local aK=aM()local j3={q.kingRoot,aK and(aK:FindFirstChild("LeftFoot")or aK:FindFirstChild("Left Leg")),aK and(aK:FindFirstChild("RightFoot")or aK:FindFirstChild("Right Leg")),aK and(aK:FindFirstChild("LowerTorso")or aK:FindFirstChild("Torso"))}local j4=q.kingTouchContacts or{}local j5=q.kingTouchTrigger==iK and#j4>0;
if j5 then for o,eE in ipairs(j4)do if not eE or not eE.Parent then j5=false;
break end end end;
if j5 then return end;
if#j4>0 or q.kingTouchTrigger then iI(false)end;
q.kingTouchContacts={}q.kingTouchTrigger=iK;
for o,eE in ipairs(j3)do if eE and eE.Parent and eE:IsA("BasePart")then table.insert(q.kingTouchContacts,eE)B(function()firetouchinterest(eE,iK,0)end)end end end;
local function j6(dH)if q.kingPresenceInFlight or not q.kingLock or not dH or not dH.Parent then return false end;
q.kingPresenceInFlight=true;
local fd=q.kingPresenceToken;
local D,hg=xpcall(function()iG()iI(false)local iK=iR()local dI=iV(iK)q.kingCF=dI;
local j7=dI*CFrame.new(0,8,0)dH.Anchored=false;
dH.CFrame=j7;
dH.AssemblyAngularVelocity=Vector3.new(0,0,0)for j8=1,12 do if not q.alive or q.networkPaused or not q.kingLock or q.kingPresenceToken~=fd or q.kingRoot~=dH or not dH.Parent then return end;
local j9=j8/12;
dH.CFrame=j7:Lerp(dI,j9)dH.AssemblyLinearVelocity=Vector3.new(0,-4,0)dH.AssemblyAngularVelocity=Vector3.new(0,0,0)c.Heartbeat:Wait()end;
task.wait(0.12)if not q.alive or q.networkPaused or not q.kingLock or q.kingPresenceToken~=fd or q.kingRoot~=dH or not dH.Parent then return end;
j2(iK)local ja,jb=i_(dH,dI)if not ja then error(jb or"physical hold failed")end end,function(j1)return tostring(j1)end)if q.kingPresenceToken==fd then q.kingPresenceInFlight=false end;
if not D and q.alive and q.kingLock and q.kingPresenceToken==fd then aP("KING PHYSICS: "..tostring(hg))end;
return D end;
local function jc()local jd=q.kingRoot;
local je=q.kingTouchTrigger or q.kingTriggerPart;
local jf={}for o,eE in ipairs(q.kingTouchContacts or{})do table.insert(jf,eE)end;
q.kingPresenceToken=q.kingPresenceToken+1;
local jg=q.kingPresenceToken;
q.kingLock=false;
iG()iI(true)if jd and jd.Parent then B(function()jd.Anchored=q.kingSavedAnchored==true;
jd.AssemblyLinearVelocity=Vector3.new(0,0,0)jd.AssemblyAngularVelocity=Vector3.new(0,0,0)end)end;
q.kingCF=nil;
q.kingRoot=nil;
q.kingSavedAnchored=nil;
q.kingPresenceInFlight=false;
q.nextKingTouchPulse=0;
q.nextKingTick=0;
task.spawn(function()c.Heartbeat:Wait()c.Heartbeat:Wait()if not q.alive or q.kingLock or q.kingPresenceToken~=jg then return end;
iG()if je and je.Parent and type(firetouchinterest)=="function"then for o,eE in ipairs(jf)do if eE and eE.Parent and eE:IsA("BasePart")then B(function()firetouchinterest(eE,je,1)end)end end end;
local jh=aO()if jh and jh.Parent and not q.networkPaused then B(function()jh.Anchored=false;
jh.AssemblyLinearVelocity=Vector3.new(0,0,0)jh.AssemblyAngularVelocity=Vector3.new(0,0,0)end)end end)end;
local function ji()local dH=aO()if not dH then return false,"нет root"end;
jc()q.kingCF=iE()q.kingLock=true;
q.kingRoot=dH;
if q.networkHoldRoot==dH and q.networkHoldSavedAnchored~=nil then q.kingSavedAnchored=q.networkHoldSavedAnchored else q.kingSavedAnchored=dH.Anchored end;
q.nextKingTick=0;
if q.networkPaused then return true end;
local D=j6(dH)if not D then jc()return false,"не удалось создать физическое присутствие"end;
return true end;
local function jj()local jk=q.characterLockSaved;
if not jk then q.characterCollisionSaved={}return end;
local dH=jk.root;
local d0=jk.humanoid;
if dH and dH.Parent then B(function()dH.Anchored=jk.rootAnchored;
dH.AssemblyLinearVelocity=Vector3.new(0,0,0)dH.AssemblyAngularVelocity=Vector3.new(0,0,0)end)end;
if d0 and d0.Parent then B(function()d0.AutoRotate=jk.autoRotate;
if jk.walkSpeed~=nil then d0.WalkSpeed=jk.walkSpeed end end)end;
for eE,jl in pairs(q.characterCollisionSaved)do if eE and eE.Parent then B(function()eE.CanCollide=jl.CanCollide;
eE.CanTouch=jl.CanTouch end)end end;
q.characterCollisionSaved={}q.characterLockSaved=nil end;
local function jm(dI)local aK=aM()local dH=aO()local d0=aN()if not aK or not dH then return false,"нет персонажа"end;
jj()q.characterLockSaved={character=aK,root=dH,humanoid=d0,rootAnchored=dH.Anchored,autoRotate=d0 and d0.AutoRotate or true,walkSpeed=d0 and d0.WalkSpeed or 16}q.characterCollisionSaved={}for o,eE in ipairs(aK:GetDescendants())do if eE:IsA("BasePart")then q.characterCollisionSaved[eE]={CanCollide=eE.CanCollide,CanTouch=eE.CanTouch}eE.CanCollide=false end end;
if d0 then d0.AutoRotate=false;
d0.WalkSpeed=0 end;
dH.CFrame=dI;
dH.AssemblyLinearVelocity=Vector3.new(0,0,0)dH.AssemblyAngularVelocity=Vector3.new(0,0,0)dH.Anchored=false;
q.lockCF=dI;
return true end;
local function jn()local bw=bN(q.selectedRock)return bw and(bw.hit or bw.body)or nil end;
local function jo(jp)local function jq()if type(jp)=="function"then B(jp)end end;
if type(firetouchinterest)~="function"then jq()return false end;
local cl=jn()local aK=aM()if not cl or not aK or not cl:IsA("BasePart")then jq()return false end;
local jr={}local js=aK:FindFirstChild("RightHand")or aK:FindFirstChild("Right Arm")local jt=aK:FindFirstChild("LeftHand")or aK:FindFirstChild("Left Arm")if js then table.insert(jr,js)end;
if jt then table.insert(jr,jt)end;
if#jr==0 then local ju=aK:FindFirstChild("HumanoidRootPart")if ju then table.insert(jr,ju)end end;
local j3={}for o,el in ipairs(jr)do if el and el:IsA("BasePart")then local jv=B(function()firetouchinterest(el,cl,0)end)if jv then table.insert(j3,el)end end end;
jq()for o,el in ipairs(j3)do B(function()firetouchinterest(el,cl,1)end)end;
return#j3>0 end;
local function jw(bu)local bw=bN(bu)if not bw then return nil,"камень не найден"end;
local bI=bw.body;
local bJ=bw.left;
local bK=bw.right;
local bL=bw.hit;
local dI=nil;
if bL and bL:IsA("BasePart")then local jx=math.clamp(bL.Size.Y*0.03,0,0.75)dI=bL.CFrame*CFrame.new(0,jx,0)elseif bJ and bJ:IsA("BasePart")and bK and bK:IsA("BasePart")then local jy=(bJ.Position+bK.Position)/2;
local jz=bI and bI:IsA("BasePart")and bI.CFrame-bI.Position or CFrame.new()dI=CFrame.new(jy)*jz elseif bI and bI:IsA("BasePart")then local jx=math.clamp(bI.Size.Y*0.08,0,2)dI=bI.CFrame*CFrame.new(0,jx,0)end;
if not dI then return nil,"нет точки внутри камня"end;
local iF=n.RockBugInsideOffset;
if typeof(iF)=="Vector3"then dI=dI*CFrame.new(iF)end;
return dI end;
local function jA()local dI,hg=jw(q.selectedRock)if not dI then return false,hg end;
local D,jB=jm(dI)if not D then return false,jB end;
q.nextLockTick=0;
return true end;
q.teleportInsideSelected=jA;
local function jC()local dH=aO()local cl=jn()if not dH or not cl then return false,"нет цели"end;
local eT=(dH.Position-cl.Position).Magnitude;
local jD=math.max(cl.Size.X,cl.Size.Y,cl.Size.Z)local ef=math.max(70,jD+38)if eT>ef then return false,"вышел из камня"end;
return true end;
...
for o,gy in pairs(q.leverRefs.train or{})do if gy then gy.Set(false,true)end end end;
local function jK()local jL=nil;
for o,e7 in ipairs(q.trainOrder)do local ax=q.activeTrains[e7]if ax then jL=ax;
break end end;
q.trainActive=jL~=nil;
...
q.bugActive=false;
q.trainActive=false;
q.activeTrains={}q.trainOrder={}q.trainRetryAt={}q.trainCursor=0;
q.selectedTrain=nil;
q.activeTool=nil;
q.nextAction=0;
q.nextEquip=0;
q.nextCooldownSweep=0;
q.punchCycle=0;
q.hybridRole=nil;
q.hybridSwitchAt=0;
q.nextBackgroundPunch=0;
q.nextPunchEquip=0;
q.nextTrainGuiRetry=0;
q.toolTransition=false;
q.lockRock=false;
q.lockCF=nil;
q.transientFailures={}jj()jI()if jN then jJ()end;
if cF then aP(cF)end end;
local function jO(cF)jM(cF or"STOP",true)end;
local function jP(cF)q.modeToken=q.modeToken+1;
q.bugActive=false;
q.activeTool=nil;
q.nextAction=0;
q.nextEquip=0;
...
q.nextPunchEquip=0;
q.trainCursor=0;
q.transientFailures={}if q.leverRefs.bug then q.leverRefs.bug.Set(false,true)end;
jK()if not q.trainActive and not q.machineActive then jI()end;
if cF then aP(cF)end end;
local function jQ(cF)q.lockRock=false;
q.lockCF=nil;
q.transientFailures.rockRoot=nil;
q.transientFailures.rockNear=nil;
jj()if q.leverRefs.lockRock then q.leverRefs.lockRock.Set(false,true)end;
if cF then aP(cF)end end;
local function jR()if not q.selectedRock then aP("камень не выбран")return false end;
if q.kingLock then jc()if q.leverRefs.kingLock then q.leverRefs.kingLock.Set(false,true)end end;
if q.machineActive and type(q.stopMachineFarm)=="function"then q.stopMachineFarm(nil)end;
local D,hg=jA()if not D then aP("ФИКСАЦИЯ: "..tostring(hg))return false end;
q.lockRock=true;
q.nextLockTick=0;
q.nextNearCheck=0;
if q.lockPosition then q.positionCF=q.lockCF end;
...
for ay=#q.trainOrder,1,-1 do if q.trainOrder[ay]==e7 then table.remove(q.trainOrder,ay)end end;
if q.selectedTrain and q.selectedTrain.id==e7 then q.selectedTrain=nil end;
q.activeTool=nil;
q.nextAction=0;
q.nextEquip=0;
q.nextCooldownSweep=0;
...
if cF then aP(cF)end end;
local function jT()if q.bossCycle and not q.bossCycle.internal then q.bossCycle:Cancel(false)end;if q.boss then q.boss:Stop("Остановлено",true)end;jM("ВСЁ ОСТАНОВЛЕНО",true)if type(q.stopMachineFarm)=="function"then q.stopMachineFarm(nil)end;
...
q.toolTransition=true;
local cO,jV=dj()q.toolTransition=false;
if not cO then aP(tostring(jV))return false end;
q.activeTool=cO;
q.modeToken=q.modeToken+1;
q.bugActive=true;
q.nextAction=0;
...
q.hybridSwitchAt=0;
q.nextBackgroundPunch=0;
q.nextPunchEquip=os.clock()+0.28;
q.trainCursor=0;
jK()local o,ec=e5()q.remoteTokens=ec;
if q.leverRefs.bug then q.leverRefs.bug.Set(true,true)end;
d4(cO)jo(function()B(function()cO:Activate()end)ek()end)aP(q.trainActive and"БАГ + КАЧ: "..tostring(q.selectedTrain and q.selectedTrain.label or"ON").." • "..tostring(q.selectedRock.label)or"АВТОУДАР: включён из любой точки • "..tostring(q.selectedRock.label))return true end;
local function jW(cX)gw(nil)if not cX or not cX.id then return false end;
if q.machineActive and type(er)=="function"then er()end;
q.toolTransition=true;
local ez,cO,jV=pcall(dk,cX)q.toolTransition=false;
if not ez then jV=cO;
cO=nil end;
if not q.activeTrains[cX.id]then table.insert(q.trainOrder,cX.id)end;
q.activeTrains[cX.id]=cX;
q.trainRetryAt[cX.id]=cO and 0 or os.clock()+1.25;
if cO then q.activeTool=cO end;
q.selectedTrain=cX;
q.modeToken=q.modeToken+1;
q.nextAction=0;
q.nextEquip=0;
...
q.hybridSwitchAt=0;
q.nextBackgroundPunch=0;
q.nextPunchEquip=q.bugActive and os.clock()+0.28 or 0;
q.trainCursor=0;
jK()local o,ec=e5()q.remoteTokens=ec;
...
local ff=q.machineAnimationsBefore;
q.machineToken=q.machineToken+1;
q.machineActive=false;
q.machineAttached=false;
q.machineAttachInFlight=false;
q.machineNextAttach=0;
q.machineNextRep=0;
...
if not f2.homePivot and f2.model and f2.model.Parent then B(function()f2.homePivot=f2.model:GetPivot()end)end;
if q.machineActive then jX(nil)task.wait(0.12)end;
gw(nil)if q.lockRock then jQ(nil)end;
if q.kingLock then jc()if q.leverRefs.kingLock then q.leverRefs.kingLock.Set(false,true)end end;
if q.lockPosition then q.lockPosition=false;
q.positionCF=nil;
...
for o=1,6 do if not H or H==workspace then break end;
local D,u=pcall(function()return H.Name end)if D and u then table.insert(eL,tostring(u))end;
local ez,ab=pcall(function()return H.Parent end)if not ez then break end;
H=ab end;
return k0.key(table.concat(eL," "))end;
function k0.zoneMatch(aF,k2)local y=k0.key(aF)if y==""then return false end;
local T=k2.aliases;
if not T then T={k0.key(k2.id)}if k2.id=="Industrial Gym"then T={"industrialgym","industrialzone","factorygym","industrial","factory"}end end;
for o,gg in ipairs(T)do if gg~=""and y:find(gg,1,true)then return true end end;
return false end;
function k0.marker(u)local y=k0.key(u)return y:find("destination",1,true)or y:find("arrival",1,true)or y:find("spawn",1,true)or y:find("landing",1,true)or y:find("exit",1,true)or y:find("endpoint",1,true)or y:find("target",1,true)or y:find("teleportto",1,true)end;
function k0.related(u)local y=k0.key(u)return k0.marker(y)or y:find("portal",1,true)or y:find("teleport",1,true)or y:find("warpgate",1,true)end;
function k0.targetFrame(L,iK)if not L then return nil end;
if k0.isA(L,"CFrameValue")then local D,N=pcall(function()return L.Value end)return D and N or nil end;
if k0.isA(L,"Vector3Value")then local D,N=pcall(function()local eN=L.Value;
return CFrame.new(eN.X,eN.Y,eN.Z)end)return D and N or nil end;
if k0.isA(L,"ObjectValue")then local D,N=pcall(function()return L.Value end)return D and k0.targetFrame(N,false)or nil end;
local eE=k0.isA(L,"BasePart")and L or nil;
if not eE then local D,N=pcall(function()return L.PrimaryPart end)if D and N then eE=N end end;
if not eE then local D,N=pcall(function()return L:FindFirstChildWhichIsA("BasePart",true)end)if D and N then eE=N end end;
if eE then local D,N=pcall(function()return eE.CFrame*CFrame.new(0,3,iK and-9 or 0)end)return D and N or nil end;
local D,N=pcall(function()return L:GetPivot()*CFrame.new(0,3,iK and-9 or 0)end)return D and N or nil end;
function k0.position(L)if not L then return nil end;
if k0.isA(L,"ObjectValue")then local D,N=pcall(function()return L.Value end)return D and k0.position(N)or nil end;
if k0.isA(L,"CFrameValue")or k0.isA(L,"Vector3Value")then local D,N=pcall(function()local k3=L.Value;
return k3.Position or k3 end)return D and N or nil end;
local D,N=pcall(function()return L.Position end)if D and N then return N end;
D,N=pcall(function()local k4=L.CFrame;
return k4.Position or k4 end)return D and N or nil end;
function k0.areaAnchor(k2)if k2.position then return k2.position end;
for o,f2 in ipairs(q.machineCatalog or{})do if f2.zone==k2.id then local k3=k0.position(f2.seat or f2.model)if k3 then return{k3.X,k3.Y,k3.Z}end end end;
if k2.id=="Industrial Gym"then local k5=q.crystalCatalog and q.crystalCatalog[P("Industrial Crystal")]local k3=k5 and k0.position(k5.source)or nil;
if k3 then return{k3.X,k3.Y,k3.Z}end end;
return nil end;
function k0.nearDestination(L,k6)if not k6 then return true end;
local k3=k0.position(L)if not k3 then return true end;
local D,eT=pcall(function()local eQ=k3.X-k6[1]local eS=k3.Z-k6[3]return math.sqrt(eQ*eQ+eS*eS)end)return not D or eT<=1450 end;
function k0.excluded(k7)for o,da in ipairs({"machinesfolder","crystal","punchingrock","gymrock","interactseat","treadmill","benchpress","boulder","squat","deadlift","pullup","robloxforwardportals","adportal"})do if k7:find(da,1,true)then return true end end;
return false end;
function k0.resolve(k2)local al=q.teleportPortalCache[k2.id]if al and al.node and al.node.Parent then if not al.target or al.target.Parent then local k4=k0.targetFrame(al.target or al.node,al.trigger)if k4 then return k4 end end end;
q.teleportPortalCache[k2.id]=nil;
local D,k8=pcall(function()return workspace:GetDescendants()end)if not D or type(k8)~="table"then return nil end;
local k6=k0.areaAnchor(k2)local bl=nil;
local c4=-1;
local function iS(k9,cl,bZ,iK,ka)if bZ<=c4 then return end;
if not k0.nearDestination(cl or k9,k6)then return end;
local k4=k0.targetFrame(cl or k9,iK)if not k4 then return end;
bl={node=k9,target=cl,trigger=iK,frame=k4}c4=bZ end;
for o,k9 in ipairs(k8)do local kb=k0.isA(k9,"BasePart")local kc=k0.isA(k9,"ObjectValue")or k0.isA(k9,"CFrameValue")or k0.isA(k9,"Vector3Value")if kb or kc then local k7=k0.ancestry(k9)if not k0.excluded(k7)then local u=k0.key(k9.Name)local eM=k0.zoneMatch(k7,k2)local gc=false;
local H=k9;
for o=1,3 do if not H or H==workspace then break end;
if k0.related(k7)then for o,M in ipairs(k0.metadataFields)do local ez,N=pcall(function()return H:GetAttribute(M)end)if ez and type(N)=="string"and k0.zoneMatch(N,k2)then eM=true;
gc=true end end end;
local ez,ab=pcall(function()return H.Parent end)H=ez and ab or nil end;
if k2.id=="Starter Island"and k0.isA(k9,"SpawnLocation")and k0.nearDestination(k9,k6)then eM=true end;
if eM and k0.related(k7)then local iW=gc and 180 or 100;
if kc and k0.marker(u)then if k0.isA(k9,"ObjectValue")then local ez,cl=pcall(function()return k9.Value end)if ez and cl then iS(k9,cl,iW+520,false,true)end else iS(k9,nil,iW+510,false,true)end elseif kb then local gU=k0.marker(u)local kd=k0.isA(k9,"SpawnLocation")local bZ=iW+(kd and 380 or gU and 330 or 120)iS(k9,nil,bZ,not gU and not kd,false)end end end end end;
if not bl then return nil end;
q.teleportPortalCache[k2.id]=bl;
return bl.frame end;
function q.teleportToIsland(ke)local a2=tostring(ke or"")local k2=nil;
for o,av in ipairs(q.teleportDestinations or{})do if av.id==a2 then k2=av;
break end end;
if not k2 then for o,eM in ipairs(q.machineZones or{})do if eM.id==a2 then k2={id=a2}break end end end;
if not k2 then return false,"локация не найдена"end;
if q.networkPaused then return false,"сеть на паузе"end;
local fB=aO()if not fB then return false,"персонаж не загрузился"end;
local cl=k0.resolve(k2)if not cl and k2.position then local eN=k2.position;
cl=CFrame.new(eN[1],eN[2],eN[3])end;
if not cl then return false,"точка выхода портала ещё не загрузилась"end;
if q.machineActive then jX(nil)end;
if q.lockRock then jQ(nil)end;
if q.kingLock then jc()if q.leverRefs.kingLock then q.leverRefs.kingLock.Set(false,true)end end;
if q.lockPosition then q.lockPosition=false;
q.positionCF=nil;
if q.leverRefs.lockPosition then q.leverRefs.lockPosition.Set(false,true)end end;
local kf,hg=B(function()fB.Anchored=false;
fB.CFrame=cl;
fB.AssemblyLinearVelocity=Vector3.new(0,0,0)fB.AssemblyAngularVelocity=Vector3.new(0,0,0)end)if not kf then return false,tostring(hg)end;
...
if fE==nil then local D,N=pcall(function()return cO:GetAttribute(u)end)if D then fE=tonumber(N)end end;
if fE~=nil then return math.max(0,math.floor(fE))end end;
return 1 end;
function q.proteinEggInventoryCount()local ki=0;
local fi=aM()local fx=j:FindFirstChildOfClass("Backpack")local cc={}if fi then table.insert(cc,fi)end;
if fx then table.insert(cc,fx)end;
for o,cd in ipairs(cc)do for o,L in ipairs(cd:GetChildren())do if L.Name=="Protein Egg"and L:IsA("Tool")then ki=ki+kh(L)end end end;
return ki end;
local function kj()local an=j:FindFirstChild("boostTimersFolder")local kk=an and an:FindFirstChild("Protein Egg")if not kk then return nil end;
local D,N=pcall(function()return kk.Value end)return D and tostring(N)or"present"end;
local function kl(cf)local H=kj()if cf==nil then return H~=nil end;
local fe=tonumber(cf)local iB=tonumber(H)return fe~=nil and iB~=nil and iB>fe end;
local function km(kn)local H=kn;
while H and H~=k do if H:IsA("GuiObject")and H.Visible==false then return false end;
if H:IsA("ScreenGui")then if H.Enabled==false then return false end;
local u=string.lower(tostring(H.Name or""))if u=="proteineggtest"or u:find("rockbughub",1,true)then return false end end;
H=H.Parent end;
return H==k end;
local function ko(L)local W={tostring(L.Name or"")}if L:IsA("TextLabel")or L:IsA("TextButton")or L:IsA("TextBox")then local D,aF=pcall(function()return L.Text end)if D and type(aF)=="string"then table.insert(W,aF)end;
D,aF=pcall(function()return L.ContentText end)if D and type(aF)=="string"then table.insert(W,aF)end end;
return table.concat(W," ")end;
local function kp(aF,fD)for o,N in ipairs(fD)do if aF:find(N,1,true)then return true end end;
return false end;
local function kq(kr)local aO=kr.Parent;
for o=1,7 do if not aO or aO==k or aO:IsA("ScreenGui")then break end;
if not km(aO)then return nil end;
local ks={ko(aO)}local D,bF=pcall(function()return aO:GetDescendants()end)if D and#bF<=140 then for o,L in ipairs(bF)do if km(L)then table.insert(ks,ko(L))end end;
local aF=table.concat(ks," ")local X=string.lower(aF)local kt=X:find("protein",1,true)or kp(aF,{"протеин","Протеин","ПРОТЕИН"})local ku=X:find("egg",1,true)or kp(aF,{"яйц","Яйц","ЯЙЦ"})local kv=kp(X,{"use","consume","activate"})or kp(aF,{"использ","Использ","ИСПОЛЬЗ"})local kw=kp(X,{"robux","purchase","buy","delete","sell"})or kp(aF,{"купить","Купить","КУПИТЬ","покуп","Покуп","ПОКУП","удал","Удал","УДАЛ","продать","Продать","ПРОДАТЬ"})if kt and ku and kv and not kw then return aO end end;
aO=aO.Parent end;
return nil end;
local function kx(kr)if not(kr:IsA("TextButton")or kr:IsA("ImageButton"))then return false end;
local fD={ko(kr)}for o,R in ipairs(kr:GetChildren())do table.insert(fD,ko(R))end;
for o,N in ipairs(fD)do for da in tostring(N):gmatch("%S+")do local ky=string.lower(da:gsub("<[^>]*>",""):gsub("[%s%p_]",""))if ky=="yes"or ky=="yesbutton"or ky=="confirm"or ky=="confirmbutton"or ky=="accept"or ky=="acceptbutton"or ky=="use"or ky=="usebutton"or ky=="ok"or ky=="okay"or da=="ДА"or da=="Да"or da=="да"then return true end end end;
return false end;
local function kz()local D,bF=pcall(function()return k:GetDescendants()end)if not D then return nil end;
for o,kr in ipairs(bF)do if kx(kr)and km(kr)and kq(kr)then return kr end end;
return nil end;
local function kA(kr,fd,kB,kC)if not kr or not q.alive or not q.eggEnabled or q.eggToken~=fd or q.networkPaused then return false end;
local function kD()return q.proteinEggInventoryCount()<kB or kl(kC)or not kr.Parent or not km(kr)end;
local function kE(kF)if not q.alive or not q.eggEnabled or q.eggToken~=fd or q.networkPaused then return false end;
local D=pcall(kF)if not D then return false end;
task.wait(0.11)return kD()end;
if kE(function()kr:Activate()end)then return true end;
if type(firesignal)=="function"then if kE(function()firesignal(kr.Activated)end)then return true end;
if kE(function()firesignal(kr.MouseButton1Click)end)then return true end end;
if type(getconnections)=="function"then for o,kG in ipairs({kr.Activated,kr.MouseButton1Click})do local D,kH=pcall(getconnections,kG)if D then for o,is in ipairs(kH or{})do if is.Enabled~=false and type(is.Function)=="function"and kE(function()is.Function()end)then return true end end end end end;
return kD()end;
local function kI(fd,kB,kC,kJ)local kK=false;
local kL={}for ay=1,kJ do if not q.alive or not q.eggEnabled or q.eggToken~=fd or q.networkPaused then break end;
if q.proteinEggInventoryCount()<kB or kl(kC)then return true,kK end;
local kr=kz()if kr then kK=true;
if(kL[kr]or 0)<2 then kL[kr]=(kL[kr]or 0)+1;
kA(kr,fd,kB,kC)end;
if q.proteinEggInventoryCount()<kB or kl(kC)then return true,true end end;
if ay<kJ then task.wait(0.12)end end;
return q.proteinEggInventoryCount()<kB or kl(kC),kK end;
function q.runProteinEggBatch(gI,fd)local a0=math.clamp(math.floor(tonumber(gI)or 1),1,10)if a0~=1 and a0~=3 and a0~=10 then a0=1 end;
if not q.eggEnabled or q.eggToken~=fd then return 0 end;
if not aM()then return 0 end;
q.eggConfirmationError=nil;
local kM=0;
local D,hg=xpcall(function()for o=1,a0 do if not q.alive or not q.eggEnabled or q.eggToken~=fd or q.networkPaused then break end;
local H=aM()local fx=j:FindFirstChildOfClass("Backpack")local kt=kg(fx)or kg(H)local eh=eg()if not kt or not eh then break end;
local kB=q.proteinEggInventoryCount()local kN=kj()if kB<1 then break end;
local hz=B(function()eh:FireServer("proteinEgg",kt)end)if not hz then break end;
ej()task.wait(0.10)if not q.eggEnabled or q.eggToken~=fd or q.networkPaused then break end;
local kO=q.proteinEggInventoryCount()<kB or kl(kN)local kK=false;
if not kO then kO,kK=kI(fd,kB,kN,7)end;
if kK and not kO then q.eggConfirmationError="подтверждение не сработало"end;
if not kO then break end;
kM=kM+1;
if type(q.refreshEggUI)=="function"then q.refreshEggUI()end;
if kM<a0 then task.wait(0.08)end end end,function(gx)return tostring(gx)end)q.eggLastUsed=kM;
if type(q.refreshEggUI)=="function"then q.refreshEggUI()end;
if not D then aP("PROTEIN EGG: "..tostring(hg):sub(1,75))end;
return kM end;
function q.startProteinEggAutomation()q.eggToken=(q.eggToken or 0)+1;
q.eggEnabled=true;
q.eggNextUseAt=os.time()if q.leverRefs.proteinEgg then q.leverRefs.proteinEgg.Set(true,true)end;
if type(q.refreshEggUI)=="function"then q.refreshEggUI()end;
aP("PROTEIN EGG: автоиспользование включено")return true end;
function q.stopProteinEggAutomation(cF)q.eggToken=(q.eggToken or 0)+1;
q.eggEnabled=false;
q.eggNextUseAt=0;
...
return tostring(j1)..kQ end)q.rebirthInFlight=false;
if not D and q.alive and q.rebirthToken==ix then q.nextRebirth=os.clock()+1;
aP("AUTO REB ERROR: "..tostring(hg):sub(1,90))end end)end;
if not q.networkPaused and q.autoSize and not q.sizeInFlight and b6>=q.nextSize then q.nextSize=b6+0.25;
q.sizeInFlight=true;
local kR=q.sizeTarget;
task.spawn(function()local D,hg=iD(kR)q.sizeInFlight=false;
if not D and q.alive and q.autoSize then aP("AUTO SIZE: "..tostring(hg))end end)end;
if q.eggEnabled and not q.eggInFlight and not q.networkPaused and not q.toolTransition and os.time()>=(q.eggNextUseAt or 0)then q.eggInFlight=true;
local kS=q.eggToken;
task.spawn(function()local D,kM=pcall(q.runProteinEggBatch,q.eggAmount,kS)q.eggInFlight=false;
if q.alive and q.eggEnabled and q.eggToken==kS then if D and(tonumber(kM)or 0)>0 then local by=math.clamp(math.floor(tonumber(q.eggIntervalMultiplier)or 1),1,10)q.eggNextUseAt=os.time()+by*1800;
aP("PROTEIN EGG: использовано ×"..tostring(kM))else q.eggNextUseAt=os.time()+30;
aP(D and"PROTEIN EGG: "..tostring(q.eggConfirmationError or"нет в инвентаре или не активировалось")or"PROTEIN EGG: "..tostring(kM):sub(1,75))end;
if type(q.refreshEggUI)=="function"then q.refreshEggUI()end end end)end;
if not q.networkPaused and q.kingLock and q.kingCF and b6>=q.nextKingTick then q.nextKingTick=b6+0.25;
local dH=aO()if dH then if q.kingRoot~=dH then iG()iI(false)q.kingPresenceToken=q.kingPresenceToken+1;
q.kingPresenceInFlight=false;
q.kingRoot=dH;
q.kingSavedAnchored=dH.Anchored;
if not q.kingPresenceInFlight then task.spawn(function()j6(dH)end)end else local eN=q.kingHoldPosition;
local j0=q.kingHoldGyro;
local kT=eN and eN.Parent==dH and j0 and j0.Parent==dH;
local kU=(dH.Position-q.kingCF.Position).Magnitude;
if dH.Anchored or not kT or kU>3 then if not q.kingPresenceInFlight then task.spawn(function()j6(dH)end)end else dH.Anchored=false;
eN.Position=q.kingCF.Position;
j0.CFrame=q.kingCF;
if b6>=q.nextKingTouchPulse then j2(iR())end end end end end;
if not q.networkPaused and not q.lockRock and q.lockPosition and q.positionCF and b6>=q.nextPosTick then q.nextPosTick=b6+0.05;
local dH=aO()if dH then dH.Anchored=false;
dH.CFrame=q.positionCF;
dH.AssemblyLinearVelocity=Vector3.new(0,0,0)dH.AssemblyAngularVelocity=Vector3.new(0,0,0)end end;
if q.lockRock and q.lockCF then local dH=aO()if not dH then if not q.networkPaused then local kV=e2("rockRoot",b6,2.5)if kV then jQ("ФИКСАЦИЯ: нет персонажа")end end elseif(dH.Position-q.lockCF.Position).Magnitude>1.25 then e1("rockRoot")local kU=(dH.Position-q.lockCF.Position).Magnitude;
if jE(dH)then q.lockCF=dH.CFrame elseif kU>8 then dH.CFrame=q.lockCF;
dH.AssemblyLinearVelocity=Vector3.new(0,0,0)dH.AssemblyAngularVelocity=Vector3.new(0,0,0)end else e1("rockRoot")end;
...
jo(function()ek()end)end end;
if not q.networkPaused and not q.toolTransition and(kX or kY)and b6>=q.nextAction then q.nextAction=b6+1/e9;
local k_={}if kX and(not kZ or not q.machineActive and b6>=q.nextPunchEquip)then table.insert(k_,"bug")end;
for o,e7 in ipairs(q.trainOrder)do if q.activeTrains[e7]and not(kX and e7=="Punch")and b6>=(q.trainRetryAt[e7]or 0)then table.insert(k_,"train:"..e7)end end;
local l0=nil;
if#k_==1 then l0=k_[1]q.hybridRole=l0;
q.trainCursor=1 elseif#k_>1 then local l1=false;
for ay,ax in ipairs(k_)do if ax==q.hybridRole then q.trainCursor=ay;
l1=true;
break end end;
if not l1 or b6>=q.hybridSwitchAt then q.trainCursor=q.trainCursor%#k_+1;
q.hybridRole=k_[q.trainCursor]local l2=math.max(0.012,math.min(0.045,1/e9))q.hybridSwitchAt=b6+l2;
q.activeTool=nil end;
l0=q.hybridRole end;
if l0=="bug"and q.bugActive then q.punchCycle=q.punchCycle+1;
if kZ then q.nextPunchEquip=b6+0.28 end;
local cO,jV=dj()q.activeTool=cO;
if cO then jo(function()B(function()cO:Activate()end)ek()end)elseif jV then aP(jV)end elseif l0 and l0:sub(1,6)=="train:"and q.trainActive then local l3=l0:sub(7)local hR=q.activeTrains[l3]local dl=b6>=q.nextTrainGuiRetry;
if dl then q.nextTrainGuiRetry=b6+2.5 end;
local cO,jV=nil,nil;
if hR then cO,jV=dk(hR,dl)end;
q.activeTool=cO;
if cO then q.selectedTrain=hR;
q.trainRetryAt[l3]=0;
...
return function(api)
    ...
    local savedFields={"bugActive","lockRock","lockPosition","positionCF","kingLock","machineActive","selectedMachine",
        "autoRebirth","rebirthGoalEnabled","rebirthGoal","rebirthGoalCompleted","rebirthGoalAwaitingFrom",
        "rebirthGoalAwaitingSince","rebirthGoalAwaitingUntil","rebirthGoalReadFailures","nextRebirth",
        "autoSize","crystalMode","killMode","petCleanupEnabled","autoEquipBestPets","autoQuest","autoWheel",
        "eggEnabled","eggNextUseAt"}
    ...
    local function restoreBody(snapshot,current)
        ...
        if not current() then return false end
        -- Scheduling flags, not immediate Activate/Remote calls. No extra rebirth or egg on resume.
        q.activeTrains=copy(snapshot.trains) q.trainOrder=copy(snapshot.order) q.trainRetryAt={}
        q.bugActive=f.bugActive q.modeToken+=1 q.activeTool=nil q.nextEquip=0 q.nextAction=0
        jK() paint("bug",q.bugActive)
        ...
        if f.autoEquipBestPets then q.startAutoEquipBestPets() paint("petFullEquip",q.autoEquipBestPets) end
        if not current() then return false end
        if f.autoWheel then q.startAutoWheel() paint("autoWheel",q.autoWheel) end
    ...
    q.bossCycle=createCycle({
        ...
        idle=function()return not q.boss.enabled and not q.rebirthInFlight and not q.eggInFlight and not q.toolTransition
        ...
        start=function(current)
            ...
            if not current()then q.boss:Stop(nil,false)return false end
...
pcall(function()l7.AutoLocalize=false end)l7.Parent=k;
q.uiRoot=l7;
...
function q.layoutUI.rememberOfficialName(bA,Y,l8)local l9=tostring(bA or""):gsub("<[^>]*>",""):gsub("^%s+",""):gsub("%s+$","")local N=tostring(Y or""):gsub("<[^>]*>",""):gsub("^%s+",""):gsub("%s+$","")if l9==""or not l9:find("[A-Za-z]")then return false end;
local y=q.layoutUI.localizationKey(l9)if l8 and not q.gameLocalizationContexts[y]then q.gameLocalizationContexts[y]=l8 end;
if N==""or N==l9 or not N:find("[\208\209][\128-\191]")then return false end;
q.gameLocalizedNames[y]=N;
return true end;
function q.layoutUI.captureOfficialGameLabel(L)if not L then return false end;
local ez,la=pcall(function()return L:IsA("TextLabel")or L:IsA("TextButton")or L:IsA("TextBox")end)if not ez or not la then return false end;
local H=L;
for o=1,14 do if not H or H==k then break end;
if H==q.uiRoot then return false end;
local D,lb=pcall(function()return H:IsA("ScreenGui")and string.lower(tostring(H.Name or"")):find("rockbughub",1,true)end)if D and lb then return false end;
H=H.Parent end;
local D,bA=pcall(function()return L.Text end)if not D or type(bA)~="string"or bA==""then return false end;
local y=q.layoutUI.localizationKey(bA)if bA:find("[A-Za-z]")then q.gameLocalizationContexts[y]=L end;
for o,lc in ipairs({"LocalizedText","ContentText"})do local bq,N=pcall(function()return L[lc]end)if bq and type(N)=="string"and q.layoutUI.rememberOfficialName(bA,N,L)then return true end end;
return false end;
function q.layoutUI.scanOfficialGameNames()local ld=false;
local le={k}local f0=1;
while f0<=#le and f0<=2400 and q.alive do local L=le[f0]f0=f0+1;
if L~=k and q.layoutUI.captureOfficialGameLabel(L)then ld=true end;
local D,lf=pcall(function()return L:GetChildren()end)if D then for o,R in ipairs(lf)do if#le>=2400 then break end;
table.insert(le,R)end end;
if f0%90==0 then task.wait()end end;
local lg={}if h then table.insert(lg,h)end;
if b then local bq,lf=pcall(function()return b:GetChildren()end)if bq then for o,R in ipairs(lf)do local u=string.lower(tostring(R.Name or""))local aj=false;
pcall(function()aj=R:IsA("LocalizationTable")end)if aj or u:find("localiz",1,true)or u:find("translat",1,true)or u:find("language",1,true)then table.insert(lg,R)end end end end;
local c5={}local fE=0;
for o,cd in ipairs(lg)do local lh={cd}local bq,bF=pcall(function()return cd:GetDescendants()end)if bq then for ay,L in ipairs(bF)do if ay>320 then break end;
table.insert(lh,L)end else local li,lf=pcall(function()return cd:GetChildren()end)if li then for o,L in ipairs(lf)do table.insert(lh,L)end end end;
for o,L in ipairs(lh)do local ez,lj=pcall(function()return L:IsA("LocalizationTable")end)if ez and lj and not c5[L]and fE<12 then c5[L]=true;
fE=fE+1;
local lk,ll=pcall(function()return L:GetEntries()end)if lk and type(ll)=="table"then for ay,av in ipairs(ll)do if ay>2400 then break end;
local l9=av.Source or av.source;
local fD=av.Values or av.values;
if l9 and type(fD)=="table"then local lm=fD.ru or fD["ru-RU"]or fD["ru-ru"]if not lm then for ln,N in pairs(fD)do local lo=string.lower(tostring(ln))if lo=="ru"or lo:sub(1,3)=="ru-"then lm=N;
break end end end;
if type(lm)=="string"and q.layoutUI.rememberOfficialName(l9,lm,nil)then ld=true end;
if av.Key and av.Key~=""then q.gameLocalizationKeys[q.layoutUI.localizationKey(l9)]=av.Key end end;
if ay%320==0 then task.wait()end end end;
local Y,lp=pcall(function()return L:GetTranslator("ru")end)if Y and lp then q.gameLocalizationTables[L]=lp end end end end;
return ld end;
function q.layoutUI.gameObjectContext(bA,eW)local y=q.layoutUI.localizationKey(bA)if eW then q.gameLocalizationObjects[y]=eW;
return eW end;
if q.gameLocalizationObjects[y]then return q.gameLocalizationObjects[y]end;
if q.gameLocalizationContexts[y]then return q.gameLocalizationContexts[y]end;
local k5=q.crystalCatalog and q.crystalCatalog[P(bA)]if k5 and k5.source then q.gameLocalizationObjects[y]=k5.source;
return k5.source end;
local bq,an=pcall(function()return j:FindFirstChild("petsFolder")end)if bq and an then local lq,L=pcall(function()return an:FindFirstChild(tostring(bA),true)end)if lq and L then q.gameLocalizationObjects[y]=L;
return L end end;
return nil end;
function q.layoutUI.officialName(bA,l8)local l9=tostring(bA or"")if l9==""or q.language=="en"then return l9 end;
local y=q.layoutUI.localizationKey(l9)local al=q.gameLocalizedNames[y]if al then return al end;
local aq=q.layoutUI.gameObjectContext(l9,l8)if aq then q.layoutUI.captureOfficialGameLabel(aq)local D,lf=pcall(function()return aq:GetChildren()end)if D then for ay,R in ipairs(lf)do if ay>28 then break end;
q.layoutUI.captureOfficialGameLabel(R)end end;
al=q.gameLocalizedNames[y]if al then return al end end;
local lr={}local ls=q.gameLocalizationContexts[y]if ls then table.insert(lr,ls)end;
if aq and aq~=ls then table.insert(lr,aq)end;
if#lr==0 then table.insert(lr,game)end;
local lt={}local lp=q.gameTranslators and q.gameTranslators.ru;
if lp then table.insert(lt,lp)end;
for o,lu in pairs(q.gameLocalizationTables or{})do if lu~=lp then table.insert(lt,lu)end end;
for o,H in ipairs(lt)do for o,cl in ipairs(lr)do local D,Y=pcall(function()return H:Translate(cl,l9)end)if D and q.layoutUI.rememberOfficialName(l9,Y,ls)then return q.gameLocalizedNames[y]end end;
local lv=q.gameLocalizationKeys[y]if lv then local D,Y=pcall(function()return H:FormatByKey(lv,{})end)if D and q.layoutUI.rememberOfficialName(l9,Y,ls)then return q.gameLocalizedNames[y]end end end;
return l9 end;
function q.layoutUI.machineDisplayName(f2)if not f2 then return q.layoutUI.staticText("ВЫБРАТЬ")end;
local N=q.layoutUI.officialName(f2.name,f2.identity or f2.model)if f2.variant then N=N.." • "..tostring(f2.variant)end;
return N end;
pcall(function()aJ(k.DescendantAdded:Connect(function(L)if q.alive and q.language=="ru"then q.layoutUI.captureOfficialGameLabel(L)end end))end)local lw={Bg=Color3.fromRGB(17,18,22),Panel=Color3.fromRGB(23,24,29),Surface=Color3.fromRGB(29,30,36),SurfaceAlt=Color3.fromRGB(38,39,47),Accent=Color3.fromRGB(142,118,255),Accent2=Color3.fromRGB(191,174,255),Neon=Color3.fromRGB(113,161,250),Success=Color3.fromRGB(96,199,147),Danger=Color3.fromRGB(240,108,119),Text=Color3.fromRGB(238,239,244),Muted=Color3.fromRGB(159,161,174),Border=Color3.fromRGB(65,66,76),Warm=Color3.fromRGB(237,179,101)}q.layoutUI.themeIcons={}q.layoutUI.themeAccentFills={}q.layoutUI.themeIconFills={}q.layoutUI.themeStrokes={}q.layoutUI.themeGradients={}q.layoutUI.themeCorners={}q.layoutUI.themeBackgrounds={}q.layoutUI.themeBackgroundSeen={}q.layoutUI.themeLabels={}q.layoutUI.colorChoices={{id="purple",ru="Фиолетовый",en="Purple",color=Color3.fromRGB(142,118,255)},{id="violet",ru="Виолетовый",en="Violet",color=Color3.fromRGB(177,97,255)},{id="lavender",ru="Лавандовый",en="Lavender",color=Color3.fromRGB(196,166,255)},{id="indigo",ru="Индиго",en="Indigo",color=Color3.fromRGB(108,124,247)},{id="blue",ru="Синий",en="Blue",color=Color3.fromRGB(93,150,255)},{id="ice",ru="Ледяной",en="Ice",color=Color3.fromRGB(174,216,255)},{id="cyan",ru="Голубой",en="Cyan",color=Color3.fromRGB(82,224,237)},{id="turquoise",ru="Бирюзовый",en="Turquoise",color=Color3.fromRGB(69,209,192)},{id="mint",ru="Мятный",en="Mint",color=Color3.fromRGB(133,240,181)},{id="green",ru="Зелёный",en="Green",color=Color3.fromRGB(96,215,150)},{id="lime",ru="Лаймовый",en="Lime",color=Color3.fromRGB(169,228,94)},{id="gold",ru="Золотой",en="Gold",color=Color3.fromRGB(245,194,91)},{id="amber",ru="Янтарный",en="Amber",color=Color3.fromRGB(255,168,74)},{id="orange",ru="Оранжевый",en="Orange",color=Color3.fromRGB(255,132,87)},{id="red",ru="Красный",en="Red",color=Color3.fromRGB(242,105,117)},{id="crimson",ru="Малиновый",en="Crimson",color=Color3.fromRGB(233,83,137)},{id="pink",ru="Розовый",en="Pink",color=Color3.fromRGB(241,131,206)},{id="rose",ru="Розовое золото",en="Rose gold",color=Color3.fromRGB(246,168,173)},{id="white",ru="Белый",en="White",color=Color3.fromRGB(235,237,246)}}q.layoutUI.backgroundChoices={{id="dark",ru="Тёмный",en="Dark",bg={17,18,22},panel={23,24,29},surface={29,30,36},surfaceAlt={38,39,47}},{id="black",ru="Чёрный",en="Black",bg={8,9,11},panel={14,15,18},surface={20,21,25},surfaceAlt={30,31,36}},{id="navy",ru="Тёмно-синий",en="Navy",bg={13,18,29},panel={19,26,39},surface={26,35,51},surfaceAlt={37,48,66}},{id="purple",ru="Тёмно-фиолетовый",en="Dark purple",bg={21,15,30},panel={29,21,41},surface={37,28,51},surfaceAlt={49,37,66}},{id="forest",ru="Тёмно-зелёный",en="Dark green",bg={13,22,19},panel={18,30,25},surface={25,39,32},surfaceAlt={35,51,42}},{id="graphite",ru="Графит",en="Graphite",bg={28,29,32},panel={37,38,42},surface={46,47,52},surfaceAlt={58,59,65}},{id="midnight",ru="Полночь",en="Midnight",bg={8,12,23},panel={14,20,34},surface={21,29,45},surfaceAlt={32,42,61}},{id="ocean",ru="Глубокий океан",en="Deep ocean",bg={9,20,25},panel={14,28,35},surface={20,38,45},surfaceAlt={30,49,57}},{id="cherry",ru="Тёмная вишня",en="Dark cherry",bg={25,13,20},panel={36,19,28},surface={46,26,36},surfaceAlt={59,36,46}},{id="coffee",ru="Тёмный кофе",en="Dark coffee",bg={24,19,16},panel={35,28,23},surface={45,36,29},surfaceAlt={57,46,38}},{id="emerald",ru="Изумрудный",en="Emerald",bg={10,21,18},panel={16,31,26},surface={22,41,34},surfaceAlt={32,54,44}},{id="slate",ru="Сланцевый",en="Slate",bg={20,23,29},panel={28,33,41},surface={37,43,53},surfaceAlt={49,56,67}}}do local jk=type(n.RockBugAppearance)=="table"and n.RockBugAppearance or{}q.layoutUI.appearance={icon=tostring(jk.icon or"purple"),background=tostring(jk.background or"dark"),neon=tostring(jk.neon or"purple"),text=tostring(jk.text or"white"),transparency=math.clamp(tonumber(jk.transparency)or 0,0,80),neonStrength=math.clamp(tonumber(jk.neonStrength)or 60,20,100),rounding=math.clamp(tonumber(jk.rounding)or 100,60,140),panelContrast=math.clamp(tonumber(jk.panelContrast)or 50,0,100),fontSize=math.clamp(tonumber(jk.fontSize)or 100,80,125),borderWeight=math.clamp(tonumber(jk.borderWeight)or 100,50,150),animation=math.clamp(tonumber(jk.animation)or 70,0,100),motionBlur=math.clamp(tonumber(jk.motionBlur)or 20,0,100),gradientStrength=math.clamp(tonumber(jk.gradientStrength)or 100,0,100),windowShadow=math.clamp(tonumber(jk.windowShadow)or 45,0,100),pageMotion=math.clamp(tonumber(jk.pageMotion)or 40,0,100),windowMotion=math.clamp(tonumber(jk.windowMotion)or 70,0,100),dragTabs=jk.dragTabs~=false,rememberTab=jk.rememberTab~=false}end;
function q.layoutUI.animate(k9,lx,aR,hB)if not k9 then return false end;
local cy=math.clamp(tonumber(q.layoutUI.appearance.animation)or 70,0,100)if cy>0 and not q.visualLow then local bq,ly=pcall(function()return game:GetService("TweenService")end)if bq and ly and TweenInfo and type(TweenInfo.new)=="function"then local lz=math.max(0.06,(tonumber(aR)or 0.16)*(1.38-cy/115))local lA,lB=pcall(function()return ly:Create(k9,TweenInfo.new(lz,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),lx)end)if lA and lB then if hB then pcall(function()lB.Completed:Connect(hB)end)end;
local lC=pcall(function()lB:Play()end)if lC then return true end end end end;
for lc,N in pairs(lx)do pcall(function()k9[lc]=N end)end;
if hB then pcall(hB)end;
return false end;
function q.layoutUI.pulseMotionBlur()local ce=math.clamp(tonumber(q.layoutUI.appearance.motionBlur)or 0,0,100)if ce<=0 or q.visualLow or(tonumber(q.layoutUI.appearance.animation)or 0)<=0 then return end;
local D,lD=pcall(function()return game:GetService("Lighting")end)if not D or not lD then return end;
local lE=q.layoutUI.motionBlurEffect;
if not lE or not lE.Parent then local lA,L=pcall(function()local lF=Instance.new("BlurEffect")lF.Name="RockBugHubMotionBlur"lF.Size=0;
lF.Parent=lD;
return lF end)if not lA then return end;
lE=L;
q.layoutUI.motionBlurEffect=lE end;
lE.Size=math.max(1,math.floor(ce*0.12+0.5))q.layoutUI.animate(lE,{Size=0},0.19,function()if lE and lE.Parent then lE.Size=0 end end)end;
function q.layoutUI.copyProfile(bA)local W={}for y,N in pairs(type(bA)=="table"and bA or{})do if type(N)=="table"then local lG={}for u,L in pairs(N)do if type(L)=="string"or type(L)=="number"or type(L)=="boolean"then lG[u]=L end end;
W[y]=lG elseif type(N)=="string"or type(N)=="number"or type(N)=="boolean"then W[y]=N end end;
return W end;
function q.layoutUI.memorySecret()return table.concat({"RB",string.char(95,118,50),".","47Q",string.char(33,122),"mK8","/","PetAlign"})end;
function q.layoutUI.memoryChecksum(aF,lH)local bC,bD=1,0;
local N=q.layoutUI.memorySecret()..tostring(lH or"")..tostring(aF or"")for ay=1,#N do bC=(bC+string.byte(N,ay))%65521;
bD=(bD+bC)%65521 end;
return string.format("%08x",bD*65536+bC)end;
function q.layoutUI.memoryBase64(N,lI)local lJ="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_"local lK={}if lI then local lL,lM=0,0;
for ay=1,#N do local bq=string.find(lJ,string.sub(N,ay,ay),1,true)if not bq then return nil end;
lL=lL*64+bq-1;
lM=lM+6;
while lM>=8 do lM=lM-8;
table.insert(lK,string.char(math.floor(lL/2^lM)%256))lL=lL%2^lM end end;
return table.concat(lK)end;
local lL,lM=0,0;
for ay=1,#N do lL=lL*256+string.byte(N,ay)lM=lM+8;
while lM>=6 do lM=lM-6;
local eN=math.floor(lL/2^lM)%64+1;
table.insert(lK,string.sub(lJ,eN,eN))lL=lL%2^lM end end;
if lM>0 then local eN=lL*2^(6-lM)%64+1;
table.insert(lK,string.sub(lJ,eN,eN))end;
return table.concat(lK)end;
function q.layoutUI.memoryCipher(N,lH,lI)local lN=q.layoutUI.memorySecret().."|"..tostring(lH or"")local jl={}for ay=0,255 do jl[ay]=ay end;
local f0=0;
for ay=0,255 do f0=(f0+jl[ay]+string.byte(lN,ay%#lN+1))%256;
jl[ay],jl[f0]=jl[f0],jl[ay]end;
local bC,bD=0,0;
local W={}for ay=1,#N do bC=(bC+1)%256;
bD=(bD+jl[bC])%256;
jl[bC],jl[bD]=jl[bD],jl[bC]local y=jl[(jl[bC]+jl[bD])%256]local lO=string.byte(N,ay)table.insert(W,string.char((lO+(lI and 256-y or y))%256))end;
return table.concat(W)end;
function q.layoutUI.sealMemory(aF,b5)if type(aF)~="string"or#aF>131072 then return nil end;
local lH=nil;
local lP,lQ=pcall(function()return game:GetService("HttpService"):GenerateGUID(false)end)if lP and type(lQ)=="string"then lH=string.gsub(lQ,"[^%da-fA-F]","")end;
if not lH or#lH<12 then lH=string.format("%08x%06x",os.time()%4294967296,math.random(0,16777215))end;
lH=string.lower(string.sub(lH,1,16))local lR=q.layoutUI.memoryCipher(aF,lH,false)return tostring(b5 or"RBM2").."."..lH.."."..q.layoutUI.memoryChecksum(aF,lH).."."..q.layoutUI.memoryBase64(lR,false)end;
function q.layoutUI.openMemory(N,lS)if type(N)~="string"or#N>200000 then return nil end;
N=string.gsub(N,"%s","")local b5,lH,lT,lU=string.match(N,"^([A-Z0-9]+)%.([%da-f]+)%.([%da-f]+)%.([%w_-]+)$")if b5~=lS or not lH or#lH<12 or#lH>32 or not lT or#lT~=8 then return nil end;
local lV=q.layoutUI.memoryBase64(lU,true)if not lV then return nil end;
local aF=q.layoutUI.memoryCipher(lV,lH,true)if q.layoutUI.memoryChecksum(aF,lH)~=lT then return nil end;
return aF end;
function q.layoutUI.readConfigs()local W=type(n.RockBugConfigs)=="table"and n.RockBugConfigs or{}q.layoutUI.lastSavedSession=type(n.RockBugLastSession)=="table"and q.layoutUI.copyProfile(n.RockBugLastSession)or nil;
local lW=nil;
...
if type(m0.lastTab)=="string"then q.layoutUI.storedLastTab=m0.lastTab end;
if type(m0.lastSession)=="table"then q.layoutUI.lastSavedSession=q.layoutUI.copyProfile(m0.lastSession)end;
q.layoutUI.selectedConfigSlot=math.clamp(math.floor(tonumber(m0.selectedSlot)or 1),1,5)end end;
local m1={}for gQ=1,5 do local m2=W[tostring(gQ)]or W[gQ]if type(m2)=="table"then m1[tostring(gQ)]=q.layoutUI.copyProfile(m2)end end;
q.layoutUI.configs=m1;
n.RockBugConfigs=m1;
n.RockBugLastSession=q.layoutUI.lastSavedSession;
return m1 end;
function q.layoutUI.writeConfigs()local m3=q.layoutUI.configs or{}n.RockBugConfigs=m3;
if type(writefile)~="function"then return false end;
local m4={version=3,profiles=m3,appearance=q.layoutUI.copyProfile(q.layoutUI.appearance),tabOrder=type(q.layoutUI.captureTabOrder)=="function"and q.layoutUI.captureTabOrder()or q.layoutUI.pendingTabOrder or{},lastTab=q.layoutUI.appearance.rememberTab and(q.layoutUI.lastActiveTab or q.layoutUI.storedLastTab)or nil,lastSession=q.layoutUI.copyProfile(q.layoutUI.lastSavedSession or{}),selectedSlot=math.clamp(tonumber(q.layoutUI.selectedConfigSlot)or 1,1,5)}local m5,aF=pcall(function()return game:GetService("HttpService"):JSONEncode(m4)end)if not m5 then return false end;
local m6=q.layoutUI.sealMemory(aF,"RBM2")if not m6 then return false end;
local m7=pcall(writefile,"RockBugHub_memory_v2.dat",m6)if m7 and q.layoutUI.legacyConfigMigration and type(readfile)=="function"then local m8,m0=pcall(readfile,"RockBugHub_memory_v2.dat")if m8 and q.layoutUI.openMemory(m0,"RBM2")==aF then if type(delfile)=="function"then pcall(delfile,"RockBugHub_configs_v1.json")end;
q.layoutUI.legacyConfigMigration=false end end;
return m7 end;
function q.layoutUI.queueMemoryWrite()q.layoutUI.memoryWriteToken=(q.layoutUI.memoryWriteToken or 0)+1;
local fd=q.layoutUI.memoryWriteToken;
task.delay(0.45,function()if q.alive and q.layoutUI and q.layoutUI.memoryWriteToken==fd then q.layoutUI.writeConfigs()end end)end;
...
if q.lockRock then jQ(nil)end;
if q.mode then jO(nil)end;
jc()q.petGradeIndex=math.clamp(math.floor(tonumber(mc.petGradeIndex)or q.petGradeIndex or 5),1,5)q.eggAmount=({[1]=true,[3]=true,[10]=true})[tonumber(mc.eggAmount)]and tonumber(mc.eggAmount)or 1;
q.eggIntervalMultiplier=math.clamp(math.floor(tonumber(mc.eggIntervalMultiplier)or 1),1,10)q.rebirthGoal=math.max(1,math.floor(tonumber(mc.rebirthGoal)or q.rebirthGoal or 100))q.sizeTarget=math.clamp(tonumber(mc.sizeTarget)or 1,0.1,1000)q.crystalAmount=({[1]=true,[3]=true,[10]=true})[tonumber(mc.crystalAmount)]and tonumber(mc.crystalAmount)or 1;
q.purchaseDelay=math.clamp(tonumber(mc.purchaseDelay)or 0.5,0.05,5)if type(mc.selectedCrystal)=="string"then q.selectedCrystal=mc.selectedCrystal end;
if type(mc.selectedPet)=="string"then q.selectedPet=mc.selectedPet end;
if type(mc.selectedAura)=="string"then q.selectedAura=mc.selectedAura end;
if type(mc.selectedQuestNpc)=="string"then q.selectedQuestNpc=mc.selectedQuestNpc end;
if type(mc.selectedTeleport)=="string"then q.selectedTeleport=mc.selectedTeleport end;
q.petCleanupTargets={}for u,fF in pairs(type(mc.petCleanupTargets)=="table"and mc.petCleanupTargets or{})do if type(u)=="string"and fF then q.petCleanupTargets[u]=true end end;
q.autoEvolvePurchasedPets=mc.autoEvolvePurchasedPets~=false;
...
q.machineZone=f2.zone;
break end end end;
if q.layoutUI.purchaseDelayInput then q.layoutUI.purchaseDelayInput.Set(q.purchaseDelay,true)end;
if q.layoutUI.sizeInput then q.layoutUI.sizeInput.Set(q.sizeTarget,true)end;
if q.ui and q.ui.rebirthGoalInput then q.ui.rebirthGoalInput.Text=("%.0f"):format(q.rebirthGoal)end;
...
if mc.autoSize and q.leverRefs.autoSize then q.leverRefs.autoSize.Set(true,false)end;
...
function q.layoutUI.exportConfigKey(gQ)local y=tostring(math.clamp(math.floor(tonumber(gQ)or 1),1,5))local m2=q.layoutUI.configs and q.layoutUI.configs[y]if type(m2)~="table"then m2=q.layoutUI.captureConfig()end;
local m5,aF=pcall(function()return game:GetService("HttpService"):JSONEncode({version=2,profile=m2})end)if not m5 then return nil end;
local m6=q.layoutUI.sealMemory(aF,"RBC2")if not m6 then return nil end;
if q.layoutUI.configKeyInput then q.layoutUI.configKeyInput.Text=m6 end;
local md=type(setclipboard)=="function"and setclipboard or(type(toclipboard)=="function"and toclipboard or nil)local me=md and pcall(md,m6)or false;
aP(q.language=="en"and(me and"CONFIG KEY: COPIED"or"CONFIG KEY: READY")or(me and"КЛЮЧ КОНФИГА: скопирован"or"КЛЮЧ КОНФИГА: готов"))return m6 end;
function q.layoutUI.importConfigKey(N,gQ)local aF=q.layoutUI.openMemory(N,"RBC2")if not aF then aP(q.language=="en"and"CONFIG KEY: INVALID"or"КЛЮЧ КОНФИГА: неверный")return false end;
local lV,m0=pcall(function()return game:GetService("HttpService"):JSONDecode(aF)end)if not lV or type(m0)~="table"or type(m0.profile)~="table"or type(m0.profile.appearance)~="table"then aP(q.language=="en"and"CONFIG KEY: DAMAGED"or"КЛЮЧ КОНФИГА: повреждён")return false end;
local y=tostring(math.clamp(math.floor(tonumber(gQ)or 1),1,5))q.layoutUI.configs=q.layoutUI.configs or{}q.layoutUI.configs[y]=q.layoutUI.copyProfile(m0.profile)q.layoutUI.writeConfigs()q.layoutUI.loadConfig(y)aP(q.language=="en"and"CONFIG KEY: PROFILE "..y.." IMPORTED"or"КЛЮЧ КОНФИГА: профиль "..y.." импортирован")return true end;
q.layoutUI.selectedConfigSlot=1;
q.layoutUI.readConfigs()if q.layoutUI.legacyConfigMigration then q.layoutUI.writeConfigs()end;
function q.layoutUI.themeColorRole(N)for o,y in ipairs({"Bg","Panel","Surface","SurfaceAlt","Accent","Accent2","Neon","Success","Danger","Text","Muted","Border","Warm"})do if N==lw[y]then return y end end;
return nil end;
function q.layoutUI.appearanceChoice(ai,e7)local hG=ai=="background"and q.layoutUI.backgroundChoices or q.layoutUI.colorChoices;
for o,av in ipairs(hG)do if av.id==e7 then return av end end;
return hG[1]end;
function q.layoutUI.appearanceChoiceName(ai,e7)local mf=q.layoutUI.appearanceChoice(ai,e7)return q.language=="en"and mf.en or mf.ru end;
local function mg(mh,dH)local aK=Instance.new("UICorner")aK.CornerRadius=UDim.new(0,dH)aK.Parent=mh;
table.insert(q.layoutUI.themeCorners,{node=aK,radius=dH})end;
local function mi(mh,mj,mk,ml)local bP=Instance.new("UIStroke")bP.Color=mj;
bP.Thickness=mk;
bP.Transparency=ml;
bP.ApplyStrokeMode=Enum.ApplyStrokeMode.Border;
bP.LineJoinMode=Enum.LineJoinMode.Round;
bP.Parent=mh;
return bP end;
local function mm(mh,mn,mo,mp)local p=Instance.new("UIGradient")p.Color=ColorSequence.new(mn,mo)p.Rotation=mp or 0;
p.Parent=mh;
table.insert(q.layoutUI.themeGradients,{node=p,from=q.layoutUI.themeColorRole(mn),to=q.layoutUI.themeColorRole(mo),originalFrom=mn,originalTo=mo})return p end;
local function mq(mh,mk,ml)local j9=ml or 0.42;
local mr=mk or 1.2;
local ms=mi(mh,lw.Accent,mr,j9)table.insert(q.layoutUI.themeStrokes,{node=ms,transparency=j9,thickness=mr})return ms end;
local function mt(ab,aF,eD,mu,mj)local mv=Instance.new("TextLabel")mv.Parent=ab;
mv.BackgroundTransparency=1;
q.layoutUI.registerText(mv,aF)mv.TextColor3=mj or lw.Text;
mv.Font=mu or Enum.Font.GothamBold;
mv.TextSize=eD or 12;
mv.TextXAlignment=Enum.TextXAlignment.Left;
mv.TextYAlignment=Enum.TextYAlignment.Center;
mv.TextWrapped=true;
return mv end;
local function kr(ab,aF,mj)local aA=Instance.new("TextButton")aA.Parent=ab;
q.layoutUI.registerText(aA,aF)aA.TextColor3=lw.Text;
aA.BackgroundColor3=mj;
aA.BackgroundTransparency=0.10;
aA.BorderSizePixel=0;
aA.AutoButtonColor=true;
aA.Font=Enum.Font.GothamBlack;
aA.TextSize=13;
mg(aA,8)local ms=mq(aA,1,0.68)ms.Name="NeonEdge"return aA end;
function q.layoutUI.captureAppearanceNodes()if q.layoutUI.appearanceCaptured then return end;
local D,k8=pcall(function()return l7:GetDescendants()end)if not D then return end;
for o,k9 in ipairs(k8)do if not q.layoutUI.themeBackgroundSeen[k9]then local ez,mw=pcall(function()return k9:IsA("GuiObject")end)if ez and mw then local mx,mj,j9=pcall(function()return k9.BackgroundColor3,k9.BackgroundTransparency end)local l0=mx and q.layoutUI.themeColorRole(mj)or nil;
if l0=="Bg"or l0=="Panel"or l0=="Surface"or l0=="SurfaceAlt"then q.layoutUI.themeBackgroundSeen[k9]=true;
table.insert(q.layoutUI.themeBackgrounds,{node=k9,role=l0,transparency=tonumber(j9)or 0})end;
local my,mz,mA=pcall(function()return k9.TextColor3,k9.TextSize end)local mB=my and q.layoutUI.themeColorRole(mz)or nil;
if mB=="Text"or mB=="Muted"then table.insert(q.layoutUI.themeLabels,{node=k9,role=mB,size=tonumber(mA)or 9})end end end end;
q.layoutUI.appearanceCaptured=true end;
function q.layoutUI.applyAppearance(mC)q.layoutUI.captureAppearanceNodes()local mD=q.layoutUI.appearance;
local mE=q.layoutUI.appearanceChoice("icon",mD.icon)local mF=q.layoutUI.appearanceChoice("neon",mD.neon)local mz=q.layoutUI.appearanceChoice("text",mD.text)local mG=q.layoutUI.appearanceChoice("background",mD.background)mD.icon=mE.id;
mD.neon=mF.id;
mD.text=mz.id;
mD.background=mG.id;
local mH=math.clamp(tonumber(mD.panelContrast)or 50,0,100)-50;
local function mI(mJ,mr)return Color3.fromRGB(math.clamp(math.floor(mJ[1]+mH*mr+0.5),0,255),math.clamp(math.floor(mJ[2]+mH*mr+0.5),0,255),math.clamp(math.floor(mJ[3]+mH*mr+0.5),0,255))end;
lw.Bg=Color3.fromRGB(table.unpack(mG.bg))lw.Panel=mI(mG.panel,0.08)lw.Surface=mI(mG.surface,0.14)lw.SurfaceAlt=mI(mG.surfaceAlt,0.20)lw.Accent=mF.color;
lw.Neon=mF.color;
lw.Accent2=mE.color;
lw.Text=mz.color;
lw.Muted=mz.color:Lerp(Color3.fromRGB(90,94,108),0.57)q.layoutUI.iconColor=mE.color;
local mK=math.clamp((tonumber(mD.transparency)or 0)/100,0,0.80)for o,av in ipairs(q.layoutUI.themeBackgrounds)do if av.node and av.node.Parent then av.node.BackgroundColor3=lw[av.role]av.node.BackgroundTransparency=math.clamp(av.transparency+(1-av.transparency)*mK,0,0.94)end end;
local mL=math.clamp((tonumber(mD.neonStrength)or 60)/60,0.34,1.67)for o,av in ipairs(q.layoutUI.themeStrokes)do if av.node and av.node.Parent then av.node.Color=mF.color;
av.node.Transparency=math.clamp(1-(1-av.transparency)*mL,0.04,0.95)av.node.Thickness=math.max(0.5,(av.thickness or 1)*math.clamp((tonumber(mD.borderWeight)or 100)/100,0.5,1.5))end end;
local mM=math.clamp((tonumber(mD.gradientStrength)or 100)/100,0,1)for o,av in ipairs(q.layoutUI.themeGradients)do if av.node and av.node.Parent then local mN=av.from and lw[av.from]or av.originalFrom;
local cl=av.to and lw[av.to]or av.originalTo;
av.node.Color=ColorSequence.new(mN,mN:Lerp(cl,mM))end end;
local mO=math.clamp((tonumber(mD.fontSize)or 100)/100,0.8,1.25)for o,av in ipairs(q.layoutUI.themeLabels)do if av.node and av.node.Parent then av.node.TextColor3=lw[av.role]av.node.TextSize=math.max(6,math.floor(av.size*mO+0.5))end end;
local mP=math.clamp((tonumber(mD.rounding)or 100)/100,0.60,1.40)for o,av in ipairs(q.layoutUI.themeCorners)do if av.node and av.node.Parent then av.node.CornerRadius=UDim.new(0,math.max(2,math.floor(av.radius*mP+0.5)))end end;
for o,k9 in ipairs(q.layoutUI.themeIcons)do if k9 and k9.Parent then k9.TextColor3=mE.color end end;
for o,k9 in ipairs(q.layoutUI.themeAccentFills)do if k9 and k9.Parent then k9.BackgroundColor3=mF.color end end;
for o,k9 in ipairs(q.layoutUI.themeIconFills)do if k9 and k9.Parent then k9.BackgroundColor3=mE.color end end;
if q.layoutUI.windowShadow then q.layoutUI.windowShadow.BackgroundTransparency=1-math.clamp((tonumber(mD.windowShadow)or 0)/100,0,1)*0.48 end;
n.RockBugAppearance=q.layoutUI.copyProfile(mD)if type(q.refreshAppearanceUI)=="function"then q.refreshAppearanceUI()end;
if type(q.refreshTeleportUI)=="function"then q.refreshTeleportUI()end;
if not mC then q.layoutUI.queueMemoryWrite()aP(q.language=="en"and"INTERFACE: UPDATED"or"ИНТЕРФЕЙС: обновлён")end end;
local function mQ()local mR=workspace.CurrentCamera;
return mR and mR.ViewportSize or Vector2.new(800,600)end;
local function mS(mT)local mU=math.max(220,math.floor(mT.X)-8)local mV=math.max(240,math.floor(mT.Y)-8)local mW=mT.X<=700 or mT.Y<=500;
local mX=math.min(320,mU)local mY=math.min(350,mV)local mZ=mW and 0.92 or 0.50;
local m_=mW and 0.78 or 0.76;
local n0=math.min(500,math.max(mX,math.floor(mT.X*mZ)))local n1=math.min(540,math.max(mY,math.floor(mT.Y*m_)))return mX,mY,mU,mV,n0,n1 end;
local n2=mQ()local n3,n4,n5,n6,n0,n1=mS(n2)local n7=Instance.new("Frame")n7.Parent=l7;
q.layoutUI.mainWindow=n7;
n7.Size=UDim2.fromOffset(n0,n1)n7.Position=UDim2.fromOffset(math.max(6,math.floor((n2.X-n0)/2)),math.max(18,math.floor((n2.Y-n1)/2)))n7.BackgroundColor3=lw.Bg;
n7.BackgroundTransparency=0.02;
n7.BorderSizePixel=0;
n7.Active=true;
n7.ClipsDescendants=false;
q.layoutUI.windowScale=Instance.new("UIScale")q.layoutUI.windowScale.Name="WindowMotionScale"q.layoutUI.windowScale.Scale=1;
q.layoutUI.windowScale.Parent=n7;
q.layoutUI.windowShadow=Instance.new("Frame")q.layoutUI.windowShadow.Name="WindowShadow"q.layoutUI.windowShadow.Parent=n7;
q.layoutUI.windowShadow.Size=UDim2.new(1,14,1,14)q.layoutUI.windowShadow.Position=UDim2.fromOffset(-7,-4)q.layoutUI.windowShadow.BackgroundColor3=Color3.fromRGB(0,0,0)q.layoutUI.windowShadow.BackgroundTransparency=0.78;
q.layoutUI.windowShadow.BorderSizePixel=0;
q.layoutUI.windowShadow.ZIndex=0;
mg(q.layoutUI.windowShadow,17)mg(n7,13)local n8=mq(n7,1.1,0.56)mm(n7,lw.Panel,lw.Bg,125)local n9=Instance.new("Frame")n9.Parent=n7;
n9.Size=UDim2.new(1,0,0,48)n9.BackgroundColor3=lw.Panel;
n9.BackgroundTransparency=0.05;
n9.BorderSizePixel=0;
n9.Active=true;
mm(n9,lw.Panel,lw.Surface,0)local na=Instance.new("Frame")na.Parent=n9;
na.Size=UDim2.new(1,0,0,1)na.Position=UDim2.new(0,0,1,-1)na.BackgroundColor3=lw.Border;
na.BackgroundTransparency=0.58;
na.BorderSizePixel=0;
local nb=kr(n9,"RB",lw.SurfaceAlt)nb.Size=UDim2.fromOffset(30,30)nb.Position=UDim2.fromOffset(9,9)nb.TextColor3=lw.Accent;
nb.TextSize=12;
table.insert(q.layoutUI.themeIcons,nb)local nc=mt(n9,"ROCK BUG HUB",14,Enum.Font.GothamBold,lw.Text)nc.Size=UDim2.new(1,-188,0,20)nc.Position=UDim2.fromOffset(49,5)local nd=mt(n9,"v"..tostring(m or"1.0"),8,Enum.Font.GothamBold,lw.Muted)nd.Name="VersionLabel"nd.Size=UDim2.fromOffset(32,16)nd.Position=UDim2.new(1,-130,0,7)nd.TextXAlignment=Enum.TextXAlignment.Right;
nd.TextWrapped=false;
local ne=mt(n9,"The Great Bastra",7,Enum.Font.GothamBold,lw.Muted)ne.Name="AuthorCredit"ne.Size=UDim2.new(1,-155,0,14)ne.Position=UDim2.fromOffset(50,26)ne.TextWrapped=false;
ne.TextTruncate=Enum.TextTruncate.AtEnd;
local nf=kr(n9,"×",lw.SurfaceAlt)nf.Size=UDim2.fromOffset(26,26)nf.Position=UDim2.new(1,-34,0,11)nf.TextColor3=lw.Danger;
nf.TextSize=19;
local ng=kr(n9,"−",lw.SurfaceAlt)ng.Size=UDim2.fromOffset(26,26)ng.Position=UDim2.new(1,-64,0,11)ng.TextColor3=lw.Muted;
ng.TextSize=18;
local nh=Instance.new("Frame")nh.Parent=n7;
nh.Size=UDim2.new(1,-16,0,38)nh.Position=UDim2.fromOffset(8,52)nh.BackgroundColor3=lw.Panel;
nh.BackgroundTransparency=0.08;
nh.BorderSizePixel=0;
nh.ClipsDescendants=true;
mg(nh,9)local ni=Instance.new("ScrollingFrame")ni.Parent=nh;
...
ni.ScrollBarImageTransparency=0.18;
ni.ElasticBehavior=Enum.ElasticBehavior.WhenScrollable;
q.layoutUI.navigationGrid=Instance.new("UIGridLayout")q.layoutUI.navigationGrid.Parent=ni;
q.layoutUI.navigationGrid.SortOrder=Enum.SortOrder.LayoutOrder;
q.layoutUI.navigationGrid.CellPadding=UDim2.fromOffset(3,0)q.layoutUI.navigationGrid.CellSize=UDim2.new(1/10,-3,1,0)local nj=Instance.new("Frame")nj.Parent=nh;
nj.Size=UDim2.new(1,-10,0,1)nj.Position=UDim2.new(0,5,1,-1)nj.BackgroundColor3=lw.Border;
nj.BackgroundTransparency=0.78;
nj.BorderSizePixel=0;
local function nk(nl,eG)nl.Size=UDim2.new(1/9,-3,1,0)nl.LayoutOrder=math.floor((eG-6)/56)+1;
nl.TextSize=9;
nl.TextWrapped=false;
nl.TextTruncate=Enum.TextTruncate.AtEnd;
nl.BackgroundTransparency=1;
local nm=nl:FindFirstChild("NeonEdge")nm.Name="TabStroke"nm.Transparency=1;
local nn=Instance.new("Frame")nn.Name="ActiveMark"nn.Parent=nl;
nn.Size=UDim2.new(1,-12,0,2)nn.Position=UDim2.new(0,6,1,-3)nn.BackgroundColor3=lw.Accent;
...
for o,av in ipairs(ll)do if not nw[av.id]then nx=nx+1;
av.node.LayoutOrder=nx end end;
q.layoutUI.pendingTabOrder=q.layoutUI.captureTabOrder()if not mC then q.layoutUI.queueMemoryWrite()end;
return q.layoutUI.pendingTabOrder end;
function q.layoutUI.consumeDraggedTab(e7)if q.layoutUI.tabDrag and q.layoutUI.tabDrag.dragging then return true end;
if q.layoutUI.suppressTabActivation=="*"or q.layoutUI.suppressTabActivation==e7 then if q.layoutUI.suppressTabActivation~="*"then q.layoutUI.suppressTabActivation=nil end;
return true end;
return false end;
q.layoutUI.applyTabOrder(q.layoutUI.pendingTabOrder,true)q.layoutUI.tabDragGhost=mt(n7,"",8,Enum.Font.GothamBlack,lw.Bg)q.layoutUI.tabDragGhost.Name="NavigationDragGhost"q.layoutUI.tabDragGhost.Size=UDim2.fromOffset(42,29)q.layoutUI.tabDragGhost.BackgroundColor3=lw.Accent;
q.layoutUI.tabDragGhost.BackgroundTransparency=0.08;
q.layoutUI.tabDragGhost.BorderSizePixel=0;
q.layoutUI.tabDragGhost.TextXAlignment=Enum.TextXAlignment.Center;
q.layoutUI.tabDragGhost.TextWrapped=false;
q.layoutUI.tabDragGhost.ZIndex=70;
q.layoutUI.tabDragGhost.Active=false;
q.layoutUI.tabDragGhost.Visible=false;
mg(q.layoutUI.tabDragGhost,7)mq(q.layoutUI.tabDragGhost,1.2,0.14)table.insert(q.layoutUI.themeAccentFills,q.layoutUI.tabDragGhost)q.layoutUI.tabDropMarker=Instance.new("Frame")q.layoutUI.tabDropMarker.Name="NavigationDropMarker"q.layoutUI.tabDropMarker.Parent=nh;
q.layoutUI.tabDropMarker.Size=UDim2.new(0,3,1,-8)q.layoutUI.tabDropMarker.BackgroundColor3=lw.Accent;
q.layoutUI.tabDropMarker.BackgroundTransparency=0.05;
q.layoutUI.tabDropMarker.BorderSizePixel=0;
q.layoutUI.tabDropMarker.ZIndex=71;
q.layoutUI.tabDropMarker.Visible=false;
mg(q.layoutUI.tabDropMarker,3)table.insert(q.layoutUI.themeAccentFills,q.layoutUI.tabDropMarker)function q.layoutUI.updateTabDragVisual(eN)local ny=q.layoutUI.tabDrag;
if not ny or not ny.dragging or not eN then return end;
local nz=q.layoutUI.tabDragGhost;
local nA=math.max(28,ny.entry.node.AbsoluteSize.X)local nB=math.max(25,ny.entry.node.AbsoluteSize.Y)nz.Size=UDim2.fromOffset(nA,nB)nz.Position=UDim2.fromOffset(math.clamp(math.floor(eN.X-n7.AbsolutePosition.X-nA*0.5+0.5),0,math.max(0,n7.AbsoluteSize.X-nA)),math.clamp(math.floor(eN.Y-n7.AbsolutePosition.Y-nB*0.5+0.5),0,math.max(0,n7.AbsoluteSize.Y-nB)))local fE=#ny.order;
if fE==0 then return end;
local nC=eN.X-ni.AbsolutePosition.X;
local nD=math.max(1,ni.AbsoluteSize.X/fE)ny.targetIndex=math.clamp(math.floor(nC/nD)+1,1,fE)local nE=ny.order[ny.targetIndex]for o,av in ipairs(q.layoutUI.navigationTabs)do if av.id==nE then local ms=av.node.AbsolutePosition.X-nh.AbsolutePosition.X;
if ny.targetIndex>ny.sourceIndex then ms=ms+av.node.AbsoluteSize.X end;
q.layoutUI.tabDropMarker.Position=UDim2.fromOffset(math.clamp(math.floor(ms+0.5),1,math.max(1,nh.AbsoluteSize.X-4)),4)break end end end;
do for o,av in ipairs(q.layoutUI.navigationTabs)do aJ(av.node.InputBegan:Connect(function(f9)if not q.layoutUI.appearance.dragTabs then return end;
if f9.UserInputType~=Enum.UserInputType.MouseButton1 and f9.UserInputType~=Enum.UserInputType.Touch then return end;
if q.layoutUI.tabDrag then return end;
local nu=q.layoutUI.captureTabOrder()local nF=1;
for ay,e7 in ipairs(nu)do if e7==av.id then nF=ay;
break end end;
local ax={entry=av,input=f9,start=f9.Position,last=f9.Position,touch=f9.UserInputType==Enum.UserInputType.Touch,dragging=false,order=nu,sourceIndex=nF,targetIndex=nF}q.layoutUI.tabDrag=ax;
task.delay(0.22,function()if q.alive and q.layoutUI.tabDrag==ax and q.layoutUI.appearance.dragTabs then ax.dragging=true;
q.layoutUI.suppressTabActivation="*"av.node.TextTransparency=0.60;
av.node.BackgroundTransparency=0.74;
q.layoutUI.tabDragGhost.Text=av.node.Text;
q.layoutUI.tabDragGhost.TextColor3=lw.Bg;
q.layoutUI.tabDragGhost.Visible=true;
q.layoutUI.tabDropMarker.Visible=true;
q.layoutUI.updateTabDragVisual(ax.last)end end)end))end;
aJ(f.InputChanged:Connect(function(f9)local ny=q.layoutUI.tabDrag;
if not ny then return end;
if ny.touch then if f9~=ny.input or f9.UserInputType~=Enum.UserInputType.Touch then return end elseif f9.UserInputType~=Enum.UserInputType.MouseMovement then return end;
ny.last=f9.Position;
if ny.dragging then q.layoutUI.updateTabDragVisual(f9.Position)end end))aJ(f.InputEnded:Connect(function(f9)local ny=q.layoutUI.tabDrag;
if not ny then return end;
if ny.touch then if f9~=ny.input or f9.UserInputType~=Enum.UserInputType.Touch then return end elseif f9.UserInputType~=Enum.UserInputType.MouseButton1 then return end;
q.layoutUI.tabDrag=nil;
if not ny or not ny.dragging then return end;
local k9=ny.entry.node;
k9.TextTransparency=0;
local nn=k9:FindFirstChild("ActiveMark")local ht=nn and nn.Visible;
k9.BackgroundColor3=ht and lw.SurfaceAlt or lw.Surface;
k9.BackgroundTransparency=ht and 0.34 or 1;
q.layoutUI.tabDragGhost.Visible=false;
q.layoutUI.tabDropMarker.Visible=false;
if ny.targetIndex~=ny.sourceIndex then local nu={}for o,e7 in ipairs(ny.order)do table.insert(nu,e7)end;
table.remove(nu,ny.sourceIndex)table.insert(nu,math.clamp(ny.targetIndex,1,#nu+1),ny.entry.id)q.layoutUI.applyTabOrder(nu,false)end;
q.layoutUI.tabSuppressToken=(q.layoutUI.tabSuppressToken or 0)+1;
local fd=q.layoutUI.tabSuppressToken;
task.delay(0.3,function()if q.layoutUI and q.layoutUI.tabSuppressToken==fd then q.layoutUI.suppressTabActivation=nil end end)end))end;
q.layoutUI.interfaceSideTab=kr(n7,"",lw.SurfaceAlt)q.layoutUI.interfaceSideTab.Name="InterfaceSideTab"q.layoutUI.interfaceSideTab.Size=UDim2.fromOffset(25,108)q.layoutUI.interfaceSideTab.Position=UDim2.fromOffset(-21,98)q.layoutUI.interfaceSideTab.ZIndex=45;
q.layoutUI.interfaceSideTab.ClipsDescendants=false;
q.layoutUI.interfaceSideLabel=mt(q.layoutUI.interfaceSideTab,"ИНТЕРФЕЙС",8,Enum.Font.GothamBlack,lw.Accent2)q.layoutUI.interfaceSideLabel.Size=UDim2.fromOffset(96,20)q.layoutUI.interfaceSideLabel.Position=UDim2.fromOffset(-35,44)q.layoutUI.interfaceSideLabel.Rotation=-90;
q.layoutUI.interfaceSideLabel.TextXAlignment=Enum.TextXAlignment.Center;
q.layoutUI.interfaceSideLabel.TextWrapped=false;
q.layoutUI.interfaceSideLabel.ZIndex=46;
table.insert(q.layoutUI.themeIcons,q.layoutUI.interfaceSideLabel)local nG=kr(ni,"↻ ОБНОВИТЬ",lw.SurfaceAlt)nG.Size=UDim2.new(1,-12,0,28)nG.Position=UDim2.fromOffset(6,342)nG.TextSize=9;
local nH=kr(n9,"■",lw.SurfaceAlt)nH.Size=UDim2.fromOffset(26,26)nH.Position=UDim2.new(1,-94,0,11)nH.TextColor3=lw.Danger;
nH.TextSize=13;
local nI=Instance.new("Frame")nI.Parent=n7;
nI.Size=UDim2.new(1,-12,1,-96)nI.Position=UDim2.fromOffset(6,94)nI.BackgroundColor3=lw.Bg;
nI.BackgroundTransparency=0.38;
nI.BorderSizePixel=0;
nI.ClipsDescendants=true;
nI.Visible=true;
local nJ=Instance.new("Frame")nJ.Parent=nI;
nJ.Size=UDim2.new(1,-12,0,76)nJ.Position=UDim2.fromOffset(6,5)nJ.BackgroundColor3=lw.Surface;
nJ.BackgroundTransparency=0.22;
nJ.BorderSizePixel=0;
mg(nJ,10)mq(nJ,1,0.76)mm(nJ,lw.Surface,lw.Panel,0)nG.Parent=nJ;
nG.Size=UDim2.fromOffset(76,21)nG.Position=UDim2.new(1,-82,0,5)nG.TextSize=8;
local nK=mt(nJ,"ФАРМ КАМНЕЙ",12,Enum.Font.GothamBold,lw.Text)nK.Size=UDim2.new(1,-92,0,17)nK.Position=UDim2.fromOffset(8,5)nK.TextWrapped=false;
nK.TextTruncate=Enum.TextTruncate.AtEnd;
q.layoutUI.quickHint=mt(nJ,"Выбери камень, затем включи автоудар.",9,Enum.Font.Gotham,lw.Muted)q.layoutUI.quickHint.Size=UDim2.new(1,-16,0,15)q.layoutUI.quickHint.Position=UDim2.fromOffset(8,23)q.layoutUI.quickHint.TextWrapped=false;
q.layoutUI.quickHint.TextTruncate=Enum.TextTruncate.AtEnd;
local nL=Instance.new("Frame")nL.Parent=nJ;
nL.Size=UDim2.new(1,-12,0,26)nL.Position=UDim2.fromOffset(6,43)nL.BackgroundTransparency=1;
local nM=Instance.new("Frame")nM.Parent=nI;
nM.Size=UDim2.new(1,-12,0,40)nM.Position=UDim2.new(0,6,1,-46)nM.BackgroundColor3=lw.Surface;
nM.BackgroundTransparency=0.30;
nM.BorderSizePixel=0;
mg(nM,10)mq(nM,1.1,0.58)local nN=mt(nM,"⌁  СТАТУС",8,Enum.Font.GothamBold,lw.Accent)nN.Size=UDim2.new(0.34,-6,0,12)nN.Position=UDim2.fromOffset(6,1)local nO=mt(nM,"VIP-СЕРВЕР ~ 00:00:00",8,Enum.Font.GothamBold,lw.Accent2)nO.Size=UDim2.new(0.66,-10,0,12)nO.Position=UDim2.new(0.34,4,0,1)nO.TextXAlignment=Enum.TextXAlignment.Right;
nO.TextWrapped=false;
nO.TextTruncate=Enum.TextTruncate.AtEnd;
local nP=mt(nM,"ГОТОВО",9,Enum.Font.GothamBold,lw.Text)nP.Size=UDim2.new(0.62,-7,0,21)nP.Position=UDim2.fromOffset(4,14)nP.BackgroundColor3=lw.SurfaceAlt;
nP.BackgroundTransparency=0.34;
nP.BorderSizePixel=0;
nP.TextXAlignment=Enum.TextXAlignment.Center;
nP.TextWrapped=false;
nP.TextTruncate=Enum.TextTruncate.AtEnd;
mg(nP,7)mq(nP,1,0.66)local nQ=mt(nM,"PING ? | УДАР 0/s",8,Enum.Font.GothamBold,lw.Accent)nQ.Size=UDim2.new(0.38,-5,0,21)nQ.Position=UDim2.new(0.62,1,0,14)nQ.BackgroundColor3=lw.SurfaceAlt;
nQ.BackgroundTransparency=0.34;
nQ.BorderSizePixel=0;
nQ.TextXAlignment=Enum.TextXAlignment.Center;
nQ.TextWrapped=false;
nQ.TextTruncate=Enum.TextTruncate.AtEnd;
mg(nQ,7)mq(nQ,1,0.66)q.ui={status=nP,net=nQ,uptime=nO}local function nR(mj)local nS=Instance.new("ScrollingFrame")nS.Parent=nI;
nS.Size=UDim2.new(1,-12,1,-138)nS.Position=UDim2.fromOffset(6,87)nS.BackgroundTransparency=1;
nS.BorderSizePixel=0;
nS.ScrollBarThickness=3;
nS.ScrollBarImageColor3=mj;
nS.ScrollBarImageTransparency=0.12;
nS.CanvasSize=UDim2.new(0,0,0,0)nS.ScrollingDirection=Enum.ScrollingDirection.Y;
nS.ScrollingEnabled=true;
nS.Active=true;
nS.ElasticBehavior=Enum.ElasticBehavior.WhenScrollable;
nS.VerticalScrollBarInset=Enum.ScrollBarInset.ScrollBar;
return nS end;
local nT=nR(lw.Accent)local nU=nR(lw.Warm)nU.Visible=false;
local nV=nR(lw.Success)nV.Visible=false;
...
q.layoutUI.systemPage=nR(lw.Warm)q.layoutUI.systemPage.Visible=false;
q.layoutUI.interfacePage=nR(lw.Accent)q.layoutUI.interfacePage.Visible=false;
local nZ=kr(n7,"◢",lw.SurfaceAlt)nZ.Size=UDim2.fromOffset(18,18)nZ.Position=UDim2.new(1,-18,1,-18)nZ.TextColor3=lw.Accent;
nZ.TextSize=13;
nZ.BackgroundTransparency=0.42;
local n_=kr(l7,"RH\n+",lw.Panel)n_.Size=UDim2.fromOffset(42,42)n_.Position=n7.Position;
n_.TextColor3=lw.Accent;
n_.TextSize=12;
n_.Visible=false;
n_.Active=true;
n_.ZIndex=30;
table.insert(q.layoutUI.themeIcons,n_)mq(n_,1.5,0.18)mm(n_,lw.Surface,lw.Bg,135)function q.layoutUI.animateWindow(o0,hB)local o1=q.layoutUI.windowScale;
if not o1 then if hB then hB()end;
return false end;
q.layoutUI.windowAnimationToken=(q.layoutUI.windowAnimationToken or 0)+1;
local fd=q.layoutUI.windowAnimationToken;
local gI=math.clamp(tonumber(q.layoutUI.appearance.windowMotion)or 70,0,100)local ht=gI>0 and(tonumber(q.layoutUI.appearance.animation)or 0)>0 and not q.visualLow;
if not ht then o1.Scale=1;
if hB then hB()end;
return false end;
local o2=1-0.16*gI/100;
if o0 then o1.Scale=o2 end;
q.layoutUI.animate(o1,{Scale=o0 and 1 or o2},o0 and 0.23 or 0.17,function()if fd~=q.layoutUI.windowAnimationToken then return end;
if not o0 then o1.Scale=1 end;
if hB then hB()end end)if o0 then q.layoutUI.pulseMotionBlur()end;
return true end;
local function o3(k4)local o4=Instance.new("UIPadding")o4.Parent=k4;
...
j8.TextTruncate=Enum.TextTruncate.AtEnd;
...
bu.BackgroundTransparency=0.18;
bu.BorderSizePixel=0;
mg(bu,7)local oA=mq(bu,1,0.74)local oB=mt(bu,u=="АНТИ-AFK"and"♢"or(u=="KING GYM"and"♛"or"◌"),15,Enum.Font.GothamBold,lw.Accent)oB.Size=UDim2.fromOffset(16,20)oB.Position=UDim2.fromOffset(4,3)oB.TextXAlignment=Enum.TextXAlignment.Center;
table.insert(q.layoutUI.themeIcons,oB)local bS=mt(bu,u,9,Enum.Font.GothamBold,lw.Text)bS.Size=UDim2.new(1,-24,1,0)bS.Position=UDim2.fromOffset(21,0)bS.TextWrapped=false;
bS.TextTruncate=Enum.TextTruncate.AtEnd;
local jl=ov and true or false;
local ox={}local function oy()bu.BackgroundColor3=jl and lw.SurfaceAlt or lw.Surface;
bu.BackgroundTransparency=jl and 0.08 or 0.30;
bS.TextColor3=jl and lw.Text or lw.Muted;
oA.Color=jl and lw.Accent or lw.Border;
oA.Thickness=jl and 1.2 or 1;
oA.Transparency=jl and 0.42 or 0.78;
oB.TextColor3=q.layoutUI.iconColor or(jl and lw.Accent2 or lw.Muted)end;
...
ox.Commit=oM;
ox.Box=oL;
function ox.Get()return N end;
function ox.Set(be,mC)local gV=tonumber(be)if not gV then return end;
N=math.clamp(gV,tonumber(oJ)or 0.1,tonumber(oK)or 1000)oL.Text=tostring(N)if kF and not mC then kF(N,ox)end end;
aJ(oL.FocusLost:Connect(oM))return ox,bu end;
...
fA.Size=UDim2.new(1,0,0,7)fA.Position=UDim2.new(0,0,0.5,-3)fA.BackgroundColor3=lw.SurfaceAlt;
fA.BackgroundTransparency=0.02;
fA.BorderSizePixel=0;
mg(fA,5)local oS=Instance.new("Frame")oS.Parent=fA;
oS.Name="SliderFill"oS.Size=UDim2.new(0,0,1,0)oS.BackgroundColor3=lw.Accent;
oS.BackgroundTransparency=0;
oS.BorderSizePixel=0;
mg(oS,5)table.insert(q.layoutUI.themeAccentFills,oS)local ow=Instance.new("Frame")ow.Parent=oR;
ow.Name="SliderKnob"ow.AnchorPoint=Vector2.new(0.5,0.5)ow.Size=UDim2.fromOffset(17,17)ow.Position=UDim2.new(0,0,0.5,0)ow.BackgroundColor3=lw.Accent2;
ow.BorderSizePixel=0;
mg(ow,10)mq(ow,1.2,0.24)table.insert(q.layoutUI.themeIconFills,ow)local N=oN;
local oT=false;
local ox={Row=bu,Track=oR,Fill=oS,Knob=ow,ValueLabel=oQ,Min=oN,Max=oO,Step=j8}local function oy()local oU=oO>oN and(N-oN)/(oO-oN)or 0;
oS.Size=UDim2.new(oU,0,1,0)ow.Position=UDim2.new(oU,0,0.5,0)oQ.Text=tostring(math.floor(N+0.5)).."%"end;
function ox.Get()return N end;
function ox.Set(oV,mC)local gV=math.clamp(tonumber(oV)or N,oN,oO)gV=math.floor((gV-oN)/j8+0.5)*j8+oN;
N=math.clamp(gV,oN,oO)oy()if kF and not mC then kF(N,ox)end end;
local function oW(f9)return f9.UserInputType==Enum.UserInputType.MouseButton1 or f9.UserInputType==Enum.UserInputType.Touch end;
local function oX(f9)local nA=math.max(1,oR.AbsoluteSize.X)local oU=math.clamp((f9.Position.X-oR.AbsolutePosition.X)/nA,0,1)ox.Set(oN+(oO-oN)*oU,false)end;
...
p0.Size=UDim2.new(1,0,1,0)p0.BackgroundColor3=Color3.fromRGB(3,5,12)p0.BackgroundTransparency=0.16;
p0.BorderSizePixel=0;
p0.Text=""p0.AutoButtonColor=false;
p0.Visible=false;
p0.ZIndex=80;
...
p3.TextSize=18;
p3.ZIndex=83;
local p4=Instance.new("TextBox")p4.Parent=p1;
p4.Size=UDim2.new(1,-20,0,32)p4.Position=UDim2.fromOffset(10,43)p4.BackgroundColor3=lw.SurfaceAlt;
p4.BackgroundTransparency=0.08;
p4.BorderSizePixel=0;
p4.ClearTextOnFocus=false;
...
p5.PaddingLeft=UDim.new(0,10)p5.PaddingRight=UDim.new(0,10)local p6=Instance.new("ScrollingFrame")p6.Parent=p1;
p6.Size=UDim2.new(1,-20,1,-132)p6.Position=UDim2.fromOffset(10,82)p6.BackgroundColor3=lw.Bg;
p6.BackgroundTransparency=0.10;
p6.BorderSizePixel=0;
p6.ScrollBarThickness=4;
p6.ScrollBarImageColor3=lw.Accent;
p6.CanvasSize=UDim2.new(0,0,0,0)p6.ScrollingDirection=Enum.ScrollingDirection.Y;
p6.ZIndex=82;
p6.Active=true;
mg(p6,10)mq(p6,1,0.68)local p7=Instance.new("UIPadding")p7.Parent=p6;
p7.PaddingTop=UDim.new(0,6)p7.PaddingBottom=UDim.new(0,6)p7.PaddingLeft=UDim.new(0,6)p7.PaddingRight=UDim.new(0,6)local p8=Instance.new("UIListLayout")p8.Parent=p6;
p8.SortOrder=Enum.SortOrder.LayoutOrder;
p8.Padding=UDim.new(0,5)local p9=kr(p1,"ГОТОВО",lw.SurfaceAlt)p9.Size=UDim2.new(1,-20,0,34)p9.Position=UDim2.new(0,10,1,-42)p9.ZIndex=83;
local pa;
do local pb=Instance.new("TextButton")pb.Name="DeleteConfirmation"pb.Parent=n7;q.layoutUI.hologramDeleteOverlay=pb;
pb.Size=UDim2.new(1,0,1,0)pb.BackgroundColor3=Color3.fromRGB(3,5,12)pb.BackgroundTransparency=0.10;
pb.BorderSizePixel=0;
pb.Text=""pb.AutoButtonColor=false;
pb.Visible=false;
pb.ZIndex=95;
local pc=Instance.new("Frame")pc.Parent=pb;
pc.Size=UDim2.new(1,-40,0,202)pc.Position=UDim2.new(0.5,0,0.5,0)pc.AnchorPoint=Vector2.new(0.5,0.5)pc.BackgroundColor3=lw.Bg;
pc.BackgroundTransparency=0.02;
...
pb.Visible=false end;
pa=function(hR,pj)local eL={}for u,fF in pairs(hR or{})do if fF then table.insert(eL,q.layoutUI.officialName(u,q.layoutUI.gameObjectContext(u)))end end;
table.sort(eL)local pk={}for ay=1,math.min(#eL,3)do pk[ay]=eL[ay]end;
local pl=table.concat(pk,", ")if#eL>3 then pl=pl..(q.language=="en"and" and "or" и ещё ")..tostring(#eL-3)end;
if q.language=="en"then pe.Text="All selected pets will be deleted from your inventory, ".."including pets you already own:\n\n"..pl.."\n\nApex, protected pets and your final pet remain safe."else pe.Text="Все выбранные питомцы будут удаляться из инвентаря, ".."включая уже имеющихся:\n\n"..pl.."\n\nApex, защищённые и последний питомец останутся."end;
ph={selected=hR,onConfirm=pj}pb.Visible=true end;
q.closeDeleteConfirmation=pi;
aJ(pf.Activated:Connect(pi))aJ(pg.Activated:Connect(function()local h_=ph;
if not h_ then return end;
pi()if h_.onConfirm then h_.onConfirm(h_.selected)end end))end;
local pm=nil;
local pn={}local po={}local function pp()for o,is in ipairs(po)do B(function()is:Disconnect()end)end;
po={}for o,L in ipairs(pn)do if L.Parent then L:Destroy()end end;
pn={}end;
local function pq()pm=nil;
p0.Visible=false;
p4.Text=""pp()end;
local function pr(ps)local pt=p6.CanvasPosition;
pp()if not pm then return end;
local pu=string.lower(tostring(p4.Text or""))local pv=0;
...
if pm.multiple then pm.selected[e7]=not pm.selected[e7]pr(false)else local pE=pm.onDone;
pq()if pE then pE(pw)end end end)table.insert(po,pD)table.insert(pn,L)end end;
if pv==0 then local pF=mt(p6,"НИЧЕГО НЕ НАЙДЕНО",10,Enum.Font.GothamBold,lw.Muted)pF.Size=UDim2.new(1,-2,0,42)pF.LayoutOrder=1;
...
local k2=p1.Position;
p1.Position=UDim2.new(k2.X.Scale,k2.X.Offset,k2.Y.Scale,k2.Y.Offset+9)q.layoutUI.animate(p1,{Position=k2},0.18)q.layoutUI.pulseMotionBlur()pr(true)end;
aJ(p3.Activated:Connect(pq))aJ(p9.Activated:Connect(function()if not pm then return end;
local jl=pm;
...
do
    ...
    launch.LayoutOrder = 1
    launch.TextSize = 12
    local card, body = oq(q.layoutUI.bossPage, "ТЕКУЩИЙ БОСС • ТЕСТ", 184)
    card.LayoutOrder = 2
    ...
    q.refreshBossUI = function()
        ...
        cycleStatus.Text=q.bossCycleStatus or "Выключено"
...
pT.Active=true;
local pU=Instance.new("Frame")pU.Parent=pT;
pU.Size=UDim2.new(1,0,0,4)pU.Position=UDim2.new(0,0,0.5,-2)pU.BackgroundColor3=lw.Border;
pU.BackgroundTransparency=0.34;
pU.BorderSizePixel=0;
mg(pU,3)local pV=Instance.new("Frame")pV.Parent=pU;
pV.Size=UDim2.new(0,0,1,0)pV.BackgroundColor3=lw.Accent;
pV.BorderSizePixel=0;
mg(pV,3)mm(pV,lw.Accent2,lw.Neon,0)for bG=1,#bb do local pW=Instance.new("Frame")pW.Parent=pU;
pW.Size=UDim2.fromOffset(1,8)pW.Position=UDim2.new((bG-1)/(#bb-1),0,0.5,-4)pW.AnchorPoint=Vector2.new(0.5,0)pW.BackgroundColor3=lw.Text;
pW.BackgroundTransparency=0.42;
pW.BorderSizePixel=0 end;
local pX=Instance.new("Frame")pX.Parent=pU;
pX.Size=UDim2.fromOffset(12,12)pX.AnchorPoint=Vector2.new(0.5,0.5)pX.Position=UDim2.new(0,0,0.5,0)pX.BackgroundColor3=lw.Text;
pX.BorderSizePixel=0;
mg(pX,6)mq(pX,2,0.04)local pY={}local pZ=false;
local function p_()for o,is in ipairs(pY)do B(function()is:Disconnect()end)end;
pY={}end;
local function q0()for bG,bu in ipairs(bb)do if q.selectedRock and q.selectedRock.id==bu.id then return bG end end;
return#bb end;
local q1;
function q.layoutUI.rockName(bu)if not bu then return""end;
local bw=bc[bu.req]local bA=bw and bw.name;
if not bA or bA==""or bA=="?"then return tostring(bu.label or"")end;
local Y=q.layoutUI.officialName(bA,bw.model)if q.language~="en"and Y==bA and not q.gameLocalizationReady and not(q.gameTranslators and q.gameTranslators.ru)then return tostring(bu.label or bA)end;
return Y end;
local function q2(ay)ay=math.clamp(math.floor(tonumber(ay)or#bb),1,#bb)local bu=bb[ay]q.autoRockSelection=false;
q.selectedRock=bu;
local q3=q.layoutUI.rockName(bu)pO.Text=q.layoutUI.staticText("РУЧНАЯ НАСТРОЙКА")pP.Text=q3 ..(q.language=="en"and"  •  multiplier "or"  •  множитель ")..(bu.multVerified==false and"?"or tostring(bu.mult))local cr=q.autoRockCalc and q.autoRockCalc.rebirths;
if cr and bu.multVerified~=false then local q4=(cr+20)*math.floor(bu.mult*40+0.5)pQ.Text=(q.language=="en"and"Rebirths: %d  •  XP/hit: %s  •  manual selection"or"Ребёрты: %d  •  XP/удар: %s  •  ручной выбор"):format(cr,co(q4))elseif cr then pQ.Text=(q.language=="en"and"Rebirths: %d  •  new rock XP is not exposed"or"Ребёрты: %d  •  XP нового камня: не раскрыто игрой"):format(cr)else pQ.Text=q.language=="en"and"Rebirths not found  •  manual selection"or"Ребёрты не найдены  •  ручной выбор"end;
q1()if q.lockRock then local q5,hg=jA()if not q5 then jQ("ФИКСАЦИЯ: "..tostring(hg))end end;
aP((q.language=="en"and"Rock: "or"Камень: ")..q3)end;
q1=function()local bu=q.selectedRock;
if not bu then pS.Text=q.layoutUI.staticText("камень не выбран")pV.Size=UDim2.new(0,0,1,0)pX.Position=UDim2.new(0,0,0.5,0)return end;
local bw=bc[bu.req]local ay=q0()local oU=(ay-1)/(#bb-1)local q6=bw and(q.language=="en"and"  —  available"or"  —  доступен")or(q.language=="en"and"  —  not found"or"  —  не найден")pS.Text=q.layoutUI.rockName(bu)..q6;
pV.Size=UDim2.new(oU,0,1,0)pX.Position=UDim2.new(oU,0,0.5,0)end;
local function q7(eF)local nA=math.max(1,pU.AbsoluteSize.X)local oU=math.clamp((eF-pU.AbsolutePosition.X)/nA,0,1)return math.clamp(math.floor(oU*(#bb-1)+1.5),1,#bb)end;
local function q8(f9)if f9 and f9.Position then q2(q7(f9.Position.X))end end;
table.insert(pY,pT.InputBegan:Connect(function(f9)if f9.UserInputType==Enum.UserInputType.MouseButton1 or f9.UserInputType==Enum.UserInputType.Touch then pZ=true;
q8(f9)end end))table.insert(pY,f.InputChanged:Connect(function(f9)if pZ and(f9.UserInputType==Enum.UserInputType.MouseMovement or f9.UserInputType==Enum.UserInputType.Touch)then q8(f9)end end))table.insert(pY,f.InputEnded:Connect(function(f9)if f9.UserInputType==Enum.UserInputType.MouseButton1 or f9.UserInputType==Enum.UserInputType.Touch then pZ=false end end))q.refreshRockList=q1;
aJ(q.layoutUI.autoRockButton.Activated:Connect(function()q.autoRockSelection=true;
q.lastAutoRockRebs=nil;
cH(true)q1()aP("АВТОПОДБОР КАМНЯ: включён")end))aJ(q.layoutUI.chooseRockButton.Activated:Connect(function()local pH={}for ay,bu in ipairs(bb)do local bw=bc[bu.req]local q9=bw and(q.language=="en"and"  •  found"or"  •  найден")or(q.language=="en"and"  •  not found"or"  •  не найден")local qa=bu.multVerified==false and(q.language=="en"and"New rock • multiplier not exposed"or"Новый камень • множитель не раскрыт")or(q.language=="en"and"Multiplier ×"or"Множитель ×")..tostring(bu.mult)table.insert(pH,{id=tostring(ay),label=q.layoutUI.rockName(bu),sub=qa..q9})end;
pG("ВЫБОР КАМНЯ",pH,{selected={[tostring(q0())]=true},onDone=function(pw)q2(tonumber(pw.id))end})end))local qb;
local qc;
...
local qg;
local qh;
local qi=mt(qf,"Поиск доступных тренажёров...",8,Enum.Font.Gotham,lw.Muted)qi.Size=UDim2.new(1,-4,0,18)qi.LayoutOrder=3;
qi.TextWrapped=false;
qi.TextTruncate=Enum.TextTruncate.AtEnd;
local function qj()local eM=q.machineZone;
local hR=q.selectedMachine;
if qg then qg.Set(eM and q.layoutUI.officialName(eM,q.layoutUI.gameObjectContext(eM))or q.layoutUI.staticText("ВЫБРАТЬ"))end;
if qh then qh.Set(hR and hR.zone==eM and q.layoutUI.machineDisplayName(hR)or q.layoutUI.staticText("ВЫБРАТЬ"))end;
local fE=0;
for o,f2 in ipairs(q.machineCatalog or{})do if f2.zone==eM then fE=fE+1 end end;
local qk=#(q.machineZones or{})if qk==0 then if q.language=="en"then qi.Text=q.machineScanInFlight and"Searching for machines..."or"Machines have not loaded yet • ↻ GYMS"else qi.Text=q.machineScanInFlight and"Поиск тренажёров..."or"Тренажёры ещё не загрузились • ↻ ЗАЛЫ"end else qi.Text=q.language=="en"and tostring(qk).." locations  •  "..tostring(fE).." machines here"or tostring(qk).." локаций  •  "..tostring(fE).." тренажёров здесь"end end;
q.refreshMachineUI=qj;
local function ql(cI)if q.machineScanInFlight then return end;
qi.Text="Поиск тренажёров..."task.spawn(function()eo(cI)if q.alive then qj()if type(q.refreshTeleportUI)=="function"then q.refreshTeleportUI()end;
aP("ТРЕНАЖЁРЫ: найдено "..tostring(#(q.machineCatalog or{})))end end)end;
q.refreshMachineCatalog=ql;
qg=oY(qf,"ЛОКАЦИЯ","ВЫБРАТЬ",function()local pH={}for o,eM in ipairs(q.machineZones or{})do table.insert(pH,{id=eM.id,label=q.layoutUI.officialName(eM.id,q.layoutUI.gameObjectContext(eM.id)),sub=tostring(eM.count)..(q.language=="en"and" available machines"or" доступных тренажёров")})end;
if#pH==0 then ql(true)aP("ТРЕНАЖЁРЫ: сначала дождись поиска")return end;
pG("ВЫБОР ЛОКАЦИИ",pH,{selected=q.machineZone and{[q.machineZone]=true}or{},onDone=function(pw)if q.machineZone~=pw.id then if q.machineActive then jX(nil)end;
q.machineZone=pw.id;
q.selectedMachine=nil end;
qj()aP("ЛОКАЦИЯ: "..tostring(pw.label))end})end)qg.Row.LayoutOrder=1;
qh=oY(qf,"ТРЕНАЖЁР","ВЫБРАТЬ",function()if not q.machineZone then aP("ТРЕНАЖЁР: сначала выбери локацию")return end;
local pH={}for o,f2 in ipairs(q.machineCatalog or{})do if f2.zone==q.machineZone then local og=q.layoutUI.officialName(f2.kind,f2.model)if f2.requirement then og=og.."  •  "..co(f2.requirement)end;
table.insert(pH,{id=f2.id,label=q.layoutUI.machineDisplayName(f2),sub=og,machine=f2})end end;
if#pH==0 then aP("ТРЕНАЖЁР: в этой локации ничего не найдено")return end;
pG((q.language=="en"and"MACHINES • "or"ТРЕНАЖЁРЫ • ")..q.layoutUI.officialName(q.machineZone,q.layoutUI.gameObjectContext(q.machineZone)),pH,{selected=q.selectedMachine and{[q.selectedMachine.id]=true}or{},onDone=function(pw)if q.machineActive then jX(nil)end;
q.selectedMachine=pw.machine;
q.machineZone=pw.machine and pw.machine.zone or q.machineZone;
qj()aP("ТРЕНАЖЁР: выбран • "..tostring(pw.machine and pw.machine.name or pw.label))end})end)qh.Row.LayoutOrder=2;
q.layoutUI.machineZoneSelection=qg;
q.layoutUI.machineSelection=qh;
local qm,qn=oj(nU,"АВТОТРЕНИРОВКА",92,1)qm.LayoutOrder=2;
local qo=oC(qn,"◈","АВТОТРЕНАЖЁР","телепорт и повторения",false,function(jH,ox)if jH then if not q.selectedMachine then aP("ТРЕНАЖЁР: сначала выбери тренажёр")ox.Set(false,true)return end;
local qC,qD=pcall(jZ,q.selectedMachine)if not qC or qD~=true then if not qC then aP("ТРЕНАЖЁР: ошибка запуска • "..tostring(qD):sub(1,70))end;
ox.Set(false,true)end else jX("ТРЕНАЖЁР: выключен")end end)q.leverRefs.machineFarm=qo;
qj()end;
...
mg(qr,7)mq(qr,1,0.66)local qs=mt(qr,"ИСПОЛЬЗ.",8,Enum.Font.GothamBlack,lw.Accent2)qs.Size=UDim2.new(0.38,-7,1,0)qs.Position=UDim2.fromOffset(7,0)local qt={}q.layoutUI.eggAmountButtons=qt;
local function qu()local hR=tonumber(q.eggAmount)or 1;
for gI,mf in pairs(qt)do local ht=gI==hR;
mf.BackgroundColor3=ht and lw.Success or lw.SurfaceAlt;
mf.BackgroundTransparency=ht and 0.04 or 0.12;
...
qu()local function qv(by)local aU=math.clamp(math.floor(tonumber(by)or 1),1,10)*30;
if aU<60 then return tostring(aU)..(q.language=="en"and" MIN"or" МИН")end;
local aT=math.floor(aU/60)if q.language=="en"then return aU%60==0 and tostring(aT).." H"or tostring(aT).." H 30 MIN"end;
return aU%60==0 and tostring(aT).." Ч"or tostring(aT).." Ч 30 МИН"end;
local qw;
qw=oY(qq,"КАЖДЫЕ",qv(q.eggIntervalMultiplier),function()local pH={}for by=1,10 do table.insert(pH,{id=tostring(by),label=qv(by),sub=q.language=="en"and"Use every "..tostring(by*30).." minutes"or"Использование каждые "..tostring(by*30).." минут"})end;
pG("ПЕРИОД • PROTEIN EGG",pH,{selected={[tostring(q.eggIntervalMultiplier or 1)]=true},onDone=function(pw)q.eggIntervalMultiplier=math.clamp(tonumber(pw.id)or 1,1,10)qw.Set(qv(q.eggIntervalMultiplier))if q.eggEnabled and(q.eggLastUsed or 0)>0 then q.eggNextUseAt=os.time()+q.eggIntervalMultiplier*1800 end;
if type(q.refreshEggUI)=="function"then q.refreshEggUI()end;
aP("PROTEIN EGG: каждые "..qv(q.eggIntervalMultiplier))end})end)qw.Row.LayoutOrder=2;
q.layoutUI.eggIntervalSelection=qw;
local qx=mt(qq,"В инвентаре: —",8,Enum.Font.Gotham,lw.Muted)qx.Size=UDim2.new(1,-4,0,18)qx.LayoutOrder=3;
qx.TextWrapped=false;
qx.TextTruncate=Enum.TextTruncate.AtEnd;
q.layoutUI.eggInventoryLabel=qx;
...
local bu=Instance.new("Frame")bu.Parent=qF;
bu.Size=UDim2.new(1,0,0,34)bu.BackgroundTransparency=1;
local qG={}q.layoutUI.languageButtons=qG;
for ay,av in ipairs({{id="ru",label="RU • РУССКИЙ"},{id="en",label="EN • ENGLISH"}})do local mf=kr(bu,av.label,lw.SurfaceAlt)mf.Size=UDim2.new(0.5,-3,0,30)mf.Position=UDim2.new((ay-1)*0.5,ay==1 and 0 or 3,0,1)mf.TextSize=9;
qG[av.id]=mf;
aJ(mf.Activated:Connect(function()if type(q.layoutUI.applyLanguage)=="function"then q.layoutUI.applyLanguage(av.id)end end))end;
function q.layoutUI.paintLanguage()for e7,mf in pairs(qG)do local ht=q.language==e7;
mf.BackgroundColor3=ht and lw.Success or lw.SurfaceAlt;
mf.BackgroundTransparency=ht and 0.04 or 0.12;
mf.TextColor3=ht and lw.Bg or lw.Text;
local ms=mf:FindFirstChild("NeonEdge")if ms then ms.Color=ht and lw.Success or lw.Border;
ms.Transparency=ht and 0.15 or 0.72 end end end;
q.layoutUI.paintLanguage()end;
do local qH,qI=oj(q.layoutUI.systemPage,"ПИТОМЦЫ",92,1)qH.LayoutOrder=2;
...
local H=c1()or 0;
local pH={}for o,bu in ipairs(hj)do local q5=bu.rebirths>H;
table.insert(pH,{id=bu.id,label=q.layoutUI.officialName(bu.name,q.layoutUI.gameObjectContext(bu.name)),sub=bu.questLineName..(bu.rebirths>0 and(q.language=="en"and(" • %d rebirths"):format(bu.rebirths)or(" • %d ребиртов"):format(bu.rebirths))or"")..(q5 and(q.language=="en"and" • locked"or" • закрыт")or"")})end;
pG("ВЫБОР NPC",pH,{selected={[tostring(q.selectedQuestNpc or"")]=true},onDone=function(pw)local qJ=q.autoQuest;
if qJ then q.stopAutoQuest(nil)end;
...
q.layoutUI.interfaceCategoryBar.BackgroundTransparency=0.15;
q.layoutUI.interfaceCategoryBar.BorderSizePixel=0;
q.layoutUI.interfaceCategoryBar.LayoutOrder=1;
mg(q.layoutUI.interfaceCategoryBar,8)do local on=Instance.new("UIGridLayout")on.Parent=q.layoutUI.interfaceCategoryBar;
on.CellSize=UDim2.new(0.2,-3,1,-6)on.CellPadding=UDim2.fromOffset(3,0)on.SortOrder=Enum.SortOrder.LayoutOrder;
local qM=Instance.new("UIPadding")qM.Parent=q.layoutUI.interfaceCategoryBar;
qM.PaddingTop=UDim.new(0,3)qM.PaddingLeft=UDim.new(0,3)end;
for ay,av in ipairs({{id="colors",title="ЦВЕТА"},{id="style",title="ВИД"},{id="effects",title="ЭФФЕКТЫ"},{id="memory",title="КОНФИГ"},{id="tabs",title="ВКЛАДКИ"}})do local L=kr(q.layoutUI.interfaceCategoryBar,av.title,lw.SurfaceAlt)L.Name="InterfaceSection_"..av.id;
L.TextSize=7;
L.LayoutOrder=ay;
q.layoutUI.interfaceCategoryButtons[av.id]=L;
aJ(L.Activated:Connect(function()q.layoutUI.showInterfaceSection(av.id)end))end;
for o,av in ipairs({{id="colors",title="ЦВЕТА ИНТЕРФЕЙСА",height=177},{id="style",title="ВНЕШНИЙ ВИД",height=340},{id="effects",title="АНИМАЦИЯ И ЭФФЕКТЫ",height=340},{id="memory",title="КОНФИГИ",height=132},{id="tabs",title="ВКЛАДКИ",height=218}})do local oh,bI=oq(q.layoutUI.interfacePage,av.title,av.height)oh.Name="InterfacePanel_"..av.id;
oh.LayoutOrder=av.id=="memory"and 3 or 2;
oh.Visible=av.id=="colors"qL[av.id]={panel=oh,body=bI}end;
q.layoutUI.configTransferPanel,q.layoutUI.configTransferBody=oq(q.layoutUI.interfacePage,"КОД КОНФИГУРАЦИИ",154)q.layoutUI.configTransferPanel.Name="ConfigurationCodePanel"q.layoutUI.configTransferPanel.LayoutOrder=2;
q.layoutUI.configTransferPanel.Visible=false;
local qN={}q.layoutUI.appearanceSelectors=qN;
local function qO(ai,ok)local pH={}local hG=ai=="background"and q.layoutUI.backgroundChoices or q.layoutUI.colorChoices;
for o,av in ipairs(hG)do table.insert(pH,{id=av.id,label=q.language=="en"and av.en or av.ru,sub=q.layoutUI.staticText(ok)})end;
pG("ВЫБОР ЦВЕТА",pH,{selected={[q.layoutUI.appearance[ai]]=true},onDone=function(pw)q.layoutUI.appearance[ai]=pw.id;
q.layoutUI.applyAppearance(false)end})end;
qN.icon=oY(qL.colors.body,"ЦВЕТ ИКОНОК",q.layoutUI.appearanceChoiceName("icon",q.layoutUI.appearance.icon),function()qO("icon","ЦВЕТ ИКОНОК")end)qN.icon.Row.LayoutOrder=1;
qN.background=oY(qL.colors.body,"ЦВЕТ ФОНА",q.layoutUI.appearanceChoiceName("background",q.layoutUI.appearance.background),function()qO("background","ЦВЕТ ФОНА")end)qN.background.Row.LayoutOrder=2;
qN.neon=oY(qL.colors.body,"ЦВЕТ НЕОНА",q.layoutUI.appearanceChoiceName("neon",q.layoutUI.appearance.neon),function()qO("neon","ЦВЕТ НЕОНА")end)qN.neon.Row.LayoutOrder=3;
qN.text=oY(qL.colors.body,"ЦВЕТ ТЕКСТА",q.layoutUI.appearanceChoiceName("text",q.layoutUI.appearance.text),function()qO("text","ЦВЕТ ТЕКСТА")end)qN.text.Row.LayoutOrder=4;
local function qP(M,ok,oN,oO,j8,qQ,nu)local qR=q.layoutUI.makePercentSlider(qL[qQ].body,ok,q.layoutUI.appearance[M],oN,oO,j8,function(N)q.layoutUI.appearance[M]=N;
q.layoutUI.applyAppearance(false)end)qR.Row.LayoutOrder=nu;
qN[M]=qR end;
qP("transparency","ПРОЗРАЧНОСТЬ",0,80,5,"style",1)qP("neonStrength","ЯРКОСТЬ НЕОНА",20,100,5,"style",2)qP("rounding","СКРУГЛЕНИЕ",60,140,5,"style",3)qP("panelContrast","КОНТРАСТ ПАНЕЛЕЙ",0,100,5,"style",4)qP("fontSize","РАЗМЕР ТЕКСТА",80,125,5,"style",5)qP("borderWeight","ТОЛЩИНА РАМОК",50,150,5,"style",6)qP("animation","СКОРОСТЬ АНИМАЦИИ",0,100,5,"effects",1)qP("windowMotion","АНИМАЦИЯ ОКНА",0,100,5,"effects",2)qP("motionBlur","МОУШЕН БЛЮР",0,100,5,"effects",3)qP("gradientStrength","ГРАДИЕНТ",0,100,5,"effects",4)qP("windowShadow","ТЕНЬ ОКНА",0,100,5,"effects",5)qP("pageMotion","СДВИГ ВКЛАДОК",0,100,5,"effects",6)local function qS(gQ)return q.layoutUI.staticText("ПРОФИЛЬ "..tostring(gQ))end;
local qT=oY(qL.memory.body,"ПРОФИЛЬ",qS(1),function()local pH={}for gQ=1,5 do local jk=q.layoutUI.configs and q.layoutUI.configs[tostring(gQ)]table.insert(pH,{id=tostring(gQ),label=qS(gQ),sub=jk and(q.language=="en"and"saved settings"or"настройки сохранены")or(q.language=="en"and"empty profile"or"пустой профиль")})end;
pG("ВЫБОР ПРОФИЛЯ",pH,{selected={[tostring(q.layoutUI.selectedConfigSlot or 1)]=true},onDone=function(pw)q.layoutUI.selectedConfigSlot=tonumber(pw.id)or 1;
q.layoutUI.refreshConfigUI()q.layoutUI.queueMemoryWrite()end})end)qT.Row.LayoutOrder=1;
q.layoutUI.configSelection=qT;
local qU=Instance.new("Frame")qU.Parent=qL.memory.body;
qU.Size=UDim2.new(1,0,0,34)qU.BackgroundTransparency=1;
qU.LayoutOrder=2;
q.layoutUI.configButtons={}for ay,av in ipairs({{id="save",title="СОХРАНИТЬ"},{id="load",title="ЗАГРУЗИТЬ"},{id="delete",title="УДАЛИТЬ"}})do local gK=kr(qU,av.title,lw.SurfaceAlt)gK.Size=UDim2.new(1/3,-4,0,29)gK.Position=UDim2.new((ay-1)/3,ay==1 and 0 or 3,0,2)gK.TextSize=8;
q.layoutUI.configButtons[av.id]=gK;
aJ(gK.Activated:Connect(function()local gQ=q.layoutUI.selectedConfigSlot or 1;
if av.id=="save"then q.layoutUI.saveConfig(gQ)elseif av.id=="load"then q.layoutUI.loadConfig(gQ)else q.layoutUI.deleteConfig(gQ)end end))end;
local qV=mt(qL.memory.body,"Выбери профиль и сохрани свои настройки.",8,Enum.Font.Gotham,lw.Muted)qV.Size=UDim2.new(1,-4,0,22)qV.LayoutOrder=3;
qV.TextWrapped=true;
q.layoutUI.configStatus=qV;
q.layoutUI.configKeyInput=Instance.new("TextBox")q.layoutUI.configKeyInput.Name="ConfigurationCodeInput"q.layoutUI.configKeyInput.Parent=q.layoutUI.configTransferBody;
q.layoutUI.configKeyInput.Size=UDim2.new(1,0,0,36)q.layoutUI.configKeyInput.LayoutOrder=1;
q.layoutUI.configKeyInput.BackgroundColor3=lw.Surface;
q.layoutUI.configKeyInput.BackgroundTransparency=0.18;
q.layoutUI.configKeyInput.BorderSizePixel=0;
q.layoutUI.configKeyInput.Text=""q.layoutUI.configKeyInput.TextColor3=lw.Text;
q.layoutUI.configKeyInput.PlaceholderColor3=lw.Muted;
q.layoutUI.configKeyInput.PlaceholderText=q.layoutUI.staticText("Вставь код с другого устройства")q.layoutUI.configKeyInput.TextSize=9;
q.layoutUI.configKeyInput.Font=Enum.Font.Gotham;
q.layoutUI.configKeyInput.TextXAlignment=Enum.TextXAlignment.Left;
q.layoutUI.configKeyInput.ClearTextOnFocus=false;
mg(q.layoutUI.configKeyInput,7)mq(q.layoutUI.configKeyInput,1,0.69)do local qW=Instance.new("UIPadding")qW.Parent=q.layoutUI.configKeyInput;
qW.PaddingLeft=UDim.new(0,8)qW.PaddingRight=UDim.new(0,8)end;
local qX=Instance.new("Frame")qX.Parent=q.layoutUI.configTransferBody;
qX.Size=UDim2.new(1,0,0,34)qX.BackgroundTransparency=1;
qX.LayoutOrder=2;
for ay,av in ipairs({{id="export",title="КОПИРОВАТЬ"},{id="paste",title="ВСТАВИТЬ"},{id="import",title="ПРИМЕНИТЬ"}})do local gK=kr(qX,av.title,lw.SurfaceAlt)gK.Size=UDim2.new(1/3,-4,0,29)gK.Position=UDim2.new((ay-1)/3,ay==1 and 0 or 3,0,2)gK.TextSize=7;
q.layoutUI.configButtons[av.id]=gK;
aJ(gK.Activated:Connect(function()if av.id=="export"then q.layoutUI.exportConfigKey(q.layoutUI.selectedConfigSlot)elseif av.id=="paste"then local qY=type(getclipboard)=="function"and getclipboard or(type(get_clipboard)=="function"and get_clipboard or nil)local me,N=false,nil;
if qY then me,N=pcall(qY)end;
if me and type(N)=="string"and N:match("%S")then q.layoutUI.configKeyInput.Text=N;
aP(q.language=="en"and"CONFIG CODE: PASTED"or"КОД КОНФИГУРАЦИИ: вставлен")else pcall(function()q.layoutUI.configKeyInput:CaptureFocus()end)aP(q.language=="en"and"PASTE THE CODE INTO THE FIELD"or"ВСТАВЬ КОД В ПОЛЕ")end else q.layoutUI.importConfigKey(q.layoutUI.configKeyInput.Text,q.layoutUI.selectedConfigSlot)end end))end;
local qZ=mt(q.layoutUI.configTransferBody,"Код переносит настройки и порядок вкладок между устройствами.",8,Enum.Font.Gotham,lw.Muted)qZ.Size=UDim2.new(1,-4,0,31)qZ.LayoutOrder=3;
qZ.TextWrapped=true;
function q.layoutUI.refreshConfigUI()local gQ=q.layoutUI.selectedConfigSlot or 1;
local jk=q.layoutUI.configs and q.layoutUI.configs[tostring(gQ)]qT.Set(qS(gQ))qV.Text=jk and(q.language=="en"and"Saved: appearance, tab order and settings."or"Сохранены: оформление, вкладки и настройки.")or q.layoutUI.staticText("Выбери профиль и сохрани свои настройки.")q.layoutUI.configKeyInput.PlaceholderText=q.layoutUI.staticText("Вставь код с другого устройства")end;
q.layoutUI.refreshConfigUI()local q_=mt(qL.tabs.body,"Зажми вкладку сверху и передвинь её в нужное место.",8,Enum.Font.Gotham,lw.Muted)q_.Size=UDim2.new(1,-4,0,34)q_.LayoutOrder=1;
q_.TextWrapped=true;
qN.dragTabs,q.layoutUI.dragTabsRow=ot(qL.tabs.body,"ПЕРЕТАСКИВАНИЕ","зажми вкладку и перемести",q.layoutUI.appearance.dragTabs,function(fF)q.layoutUI.appearance.dragTabs=fF;
q.layoutUI.applyAppearance(false)end)q.layoutUI.dragTabsRow.LayoutOrder=2;
qN.rememberTab,q.layoutUI.rememberTabRow=ot(qL.tabs.body,"ПОСЛЕДНЯЯ ВКЛАДКА","открывать прежний раздел",q.layoutUI.appearance.rememberTab,function(fF)q.layoutUI.appearance.rememberTab=fF;
if not fF then q.layoutUI.lastActiveTab=nil;
q.layoutUI.storedLastTab=nil end;
q.layoutUI.applyAppearance(false)end)q.layoutUI.rememberTabRow.LayoutOrder=3;
local r0=kr(qL.tabs.body,"СБРОСИТЬ ПОРЯДОК",lw.SurfaceAlt)r0.Name="ResetNavigationOrder"r0.Size=UDim2.new(1,0,0,32)r0.LayoutOrder=4;
r0.TextSize=9;
aJ(r0.Activated:Connect(function()q.layoutUI.applyTabOrder({},false)aP(q.language=="en"and"TABS: DEFAULT ORDER"or"ВКЛАДКИ: стандартный порядок")end))local r1=kr(q.layoutUI.interfacePage,"СБРОСИТЬ ОФОРМЛЕНИЕ",lw.SurfaceAlt)r1.Name="ResetAppearanceButton"r1.Size=UDim2.new(1,-4,0,33)r1.LayoutOrder=4;
r1.TextSize=9;
q.layoutUI.appearanceResetButton=r1;
aJ(r1.Activated:Connect(function()q.layoutUI.appearance={icon="purple",background="dark",neon="purple",text="white",transparency=0,neonStrength=60,rounding=100,panelContrast=50,fontSize=100,borderWeight=100,animation=70,motionBlur=20,gradientStrength=100,windowShadow=45,pageMotion=40,windowMotion=70,dragTabs=true,rememberTab=true}q.layoutUI.applyAppearance(false)end))function q.layoutUI.showInterfaceSection(e7,r2)if not qL[e7]then e7="colors"end;
q.layoutUI.interfaceSection=e7;
for u,av in pairs(qL)do av.panel.Visible=u==e7 end;
q.layoutUI.configTransferPanel.Visible=e7=="memory"for u,L in pairs(q.layoutUI.interfaceCategoryButtons)do local ht=u==e7;
L.BackgroundColor3=ht and lw.Accent or lw.SurfaceAlt;
L.BackgroundTransparency=ht and 0.08 or 0.34;
L.TextColor3=ht and lw.Bg or lw.Text;
local ms=L:FindFirstChild("NeonEdge")if ms then ms.Transparency=ht and 0.14 or 0.76 end end;
r1.Visible=e7~="memory"and e7~="tabs"if not r2 then q.layoutUI.interfacePage.CanvasPosition=Vector2.new(0,0)end;
oc()end;
q.layoutUI.showInterfaceSection("colors")function q.refreshAppearanceUI()local jl=q.layoutUI.appearance;
for o,M in ipairs({"icon","background","neon","text"})do qN[M].Set(q.layoutUI.appearanceChoiceName(M,jl[M]))end;
for o,M in ipairs({"transparency","neonStrength","rounding","panelContrast","fontSize","borderWeight","animation","windowMotion","motionBlur","gradientStrength","windowShadow","pageMotion","dragTabs","rememberTab"})do qN[M].Set(jl[M],true)end;
q.layoutUI.showInterfaceSection(q.layoutUI.interfaceSection or"colors",true)q.layoutUI.refreshConfigUI()end end;
...
rf.TextXAlignment=Enum.TextXAlignment.Center;
rf.TextWrapped=false;
rf.TextTruncate=Enum.TextTruncate.AtEnd;
mg(rf,7)mq(rf,1,0.68)q.layoutUI.teleportButtons={}q.layoutUI.teleportGroups={islands={},gyms={}}local rg=0;
...
if ri then table.insert(q.layoutUI.teleportGroups.islands,e7)else rg=rg+1;
table.insert(q.layoutUI.teleportGroups.gyms,e7)end;
aJ(mf.Activated:Connect(function()r4(e7)end))end;
...
ms.Transparency=ht and 0.15 or 0.68 end end;
oc()end;
q.layoutUI.teleportCurrentLabel=rf;
q.refreshTeleportUI()end;
local rj=oC(qD,"◇","ЗАКРЕПИТЬСЯ","не сдвигаться",false,function(jH,ox)if jH then local dH=aO()if not dH then aP("LOCK POSITION: нет root")ox.Set(false,true)return end;
jc()dH.Anchored=false;
q.positionCF=q.lockRock and q.lockCF or dH.CFrame;
q.lockPosition=true;
q.nextPosTick=0;
if q.leverRefs.kingLock then q.leverRefs.kingLock.Set(false,true)end;
aP("ПОЗИЦИЯ: зафиксирована")else q.lockPosition=false;
q.positionCF=nil;
...
local function rn(k9)local H=k9;
for o=1,8 do if not H or H==workspace then break end;
local k4=nil;
if H:IsA("Attachment")then local D,N=pcall(function()return H.WorldCFrame end)if D then k4=N end elseif H:IsA("BasePart")then local D,N=pcall(function()return H.CFrame end)if D then k4=N end elseif H:IsA("Model")then local D,eE=pcall(function()return H.PrimaryPart or H:FindFirstChildWhichIsA("BasePart",true)end)if D and eE then D,k4=pcall(function()return eE.CFrame end)if not D then k4=nil end end;
if not k4 then local ez,N=pcall(function()return H:GetPivot()end)if ez then k4=N end end end;
if k4 then local D,cl=pcall(function()return k4*CFrame.new(0,3,-4)end)if D then return cl end end;
H=H.Parent end;
return nil end;
local function ro()local al=q.fuseStationNode;
if al and al.Parent then local cl=rn(al)if cl then return al,cl end end;
q.fuseStationNode=nil;
local D,k8=pcall(function()return workspace:GetDescendants()end)if not D or type(k8)~="table"then return nil,nil end;
local bl,rp,c4=nil,nil,0;
for ay,k9 in ipairs(k8)do if ay>16000 or not q.alive then break end;
local fb=k9:IsA("ProximityPrompt")local ax=fb or k9:IsA("Model")or k9:IsA("BasePart")or k9:IsA("Attachment")if ax then local eL={tostring(k9.Name or"")}if fb then pcall(function()table.insert(eL,tostring(k9.ActionText or""))table.insert(eL,tostring(k9.ObjectText or""))end)end;
local ab=k9.Parent;
for o=1,5 do if not ab or ab==workspace then break end;
table.insert(eL,tostring(ab.Name or""))ab=ab.Parent end;
local nc=table.concat(eL," ")if rm(nc)then local cl=rn(k9)if cl then local a6=string.lower(nc)local bZ=fb and 200 or k9:IsA("Model")and 130 or 100;
if rl(k9.Name)then bZ=bZ+45 end;
if a6:find("machine",1,true)or a6:find("station",1,true)then bZ=bZ+30 end;
if a6:find("pet",1,true)then bZ=bZ+12 end;
if bZ>c4 then bl,rp,c4=k9,cl,bZ end end end end end;
q.fuseStationNode=bl;
return bl,rp end;
function q.layoutUI.refreshFuseQuickButton()local rq=q.layoutUI.fuseQuickButton;
if not rq or not rq.Parent then return end;
rq.Text=q.fuseSession and(q.language=="en"and"BACK"or"НАЗАД")or"FUSE"end;
local function rr(rs)local mc=q.fuseSession;
if not mc then return false end;
q.fuseSession=nil;
q.layoutUI.refreshFuseQuickButton()if mc.panel and mc.panel.Parent and mc.panel:IsA("GuiObject")then pcall(function()mc.panel.Visible=false end)end;
for o,is in ipairs(mc.connections or{})do pcall(function()is:Disconnect()end)end;
local fB=aO()if fB and fB==mc.root and mc.origin then pcall(function()fB.Anchored=false;
fB.CFrame=mc.origin;
fB.AssemblyLinearVelocity=Vector3.new(0,0,0)fB.AssemblyAngularVelocity=Vector3.new(0,0,0)end)end;
if rs~=false and q.alive and mc.paused then if mc.positionLocked then q.lockPosition=true;
...
if mc.kingLocked then jc()if q.leverRefs and q.leverRefs.kingLock then q.leverRefs.kingLock.Set(false,true)end end;
if mc.positionLocked then q.lockPosition=false;
q.positionCF=nil;
if q.leverRefs and q.leverRefs.lockPosition then q.leverRefs.lockPosition.Set(false,true)end end;
local kf=pcall(function()fB.Anchored=false;
fB.CFrame=cl;
fB.AssemblyLinearVelocity=Vector3.new(0,0,0)fB.AssemblyAngularVelocity=Vector3.new(0,0,0)end)if not kf then rr(true)return nil end;
task.wait(0.35)return mc end;
local function rv(k9)local H=k9;
for o=1,18 do if not H or H==k then return false end;
if H==q.uiRoot then return true end;
H=H.Parent end;
return false end;
local function rw(k9)return k9 and(k9:IsA("Frame")or k9:IsA("ScrollingFrame")or k9:IsA("CanvasGroup")or k9:IsA("ScreenGui"))end;
local function rx()local D,k8=pcall(function()return k:GetDescendants()end)if not D then return nil,nil end;
local bl,c4,ry=nil,0,nil;
local U={}local function rz(k9,bZ)if not rw(k9)or rv(k9)then return end;
local u=string.lower(tostring(k9.Name or""))if rl(u)then bZ=bZ+90 end;
if u:find("machine",1,true)then bZ=bZ+22 end;
if u:find("frame",1,true)or u:find("menu",1,true)or u:find("window",1,true)or u:find("panel",1,true)then bZ=bZ+16 end;
if u:find("button",1,true)or u:find("close",1,true)or u:find("slot",1,true)or u:find("template",1,true)or u:find("reward",1,true)or u:find("preview",1,true)or u:find("confirm",1,true)or u:find("result",1,true)then bZ=bZ-70 end;
if k9:IsA("ScrollingFrame")then bZ=bZ-12 end;
if U[k9]and U[k9]>=bZ then return end;
U[k9]=bZ;
if bZ>c4 then bl,c4=k9,bZ end end;
for ay,k9 in ipairs(k8)do if ay>8000 or not q.alive then break end;
if not rv(k9)then local u=string.lower(tostring(k9.Name or""))if rw(k9)and rl(u)then rz(k9,35)elseif k9:IsA("TextLabel")or k9:IsA("TextButton")then local mx,aF=pcall(function()return tostring(k9.Text or"")end)local a6=mx and string.lower(aF)or""if rl(a6)and(a6:find("machine",1,true)or a6:find("fusion",1,true)or u:find("title",1,true)or u:find("header",1,true))then local ab=k9.Parent;
for ac=1,4 do if not ab or ab==k then break end;
if rw(ab)then rz(ab,76-ac*5)end;
ab=ab.Parent end end;
if not ry and k9:IsA("TextButton")and db(k9,k)and rl(u.." "..a6)and(u:find("open",1,true)or u:find("menu",1,true)or u:find("machine",1,true)or a6:find("machine",1,true))and not(u:find("confirm",1,true)or u:find("craft",1,true)or u:find("submit",1,true)or u:find("buy",1,true)or u:find("purchase",1,true)or u:find("delete",1,true)or u:find("combine",1,true)or u:find("merge",1,true))then ry=k9 end elseif not ry and k9:IsA("ImageButton")and rl(u)and db(k9,k)and(u:find("open",1,true)or u:find("menu",1,true)or u:find("machine",1,true))then ry=k9 end end end;
return bl,ry end;
local function rA(k9)if not k9 or rv(k9)then return false end;
local H=k9;
for o=1,18 do if not H or H==k then break end;
if H:IsA("GuiObject")then pcall(function()H.Visible=true end)elseif H:IsA("LayerCollector")then pcall(function()H.Enabled=true end)end;
H=H.Parent end;
return db(k9,k)end;
local function rB(oh,mc)if mc and q.fuseSession==mc then mc.panel=oh end;
aP(q.language=="en"and"FUSE: OPENED"or"FUSE: окно открыто")return true end;
local function rC()local D,k8=pcall(function()return workspace:GetDescendants()end)if not D then return false end;
for ay,k9 in ipairs(k8)do if ay>14000 or not q.alive then break end;
if k9:IsA("ProximityPrompt")then local rD=rl(k9.Name)local rE,gK,bW=pcall(function()return k9.ActionText,k9.ObjectText end)if rE and rl(tostring(gK or"").." "..tostring(bW or""))then rD=true end;
local ab=k9.Parent;
for o=1,6 do if rD or not ab then break end;
rD=rl(ab.Name)ab=ab.Parent end;
if rD then if type(fireproximityprompt)=="function"then local rF=pcall(function()fireproximityprompt(k9,0)end)if rF then return true end end;
local rF=pcall(function()k9:InputHoldBegin()k9:InputHoldEnd()end)if rF then return true end end end end;
return false end;
function q.layoutUI.openFuseMachine()if q.fuseSession then rr(true)aP(q.language=="en"and"FUSE: RETURNED"or"FUSE: возвращено")return true end;
local b6=os.clock()if b6<(q.fuseNextOpenAt or 0)then aP(q.language=="en"and"FUSE: WAIT A SECOND"or"FUSE: подожди секунду")return false end;
q.fuseNextOpenAt=b6+1.1;
local oh,ry=rx()local o,rG=ro()local mc=rG and rt(rG)or nil;
if ry then local di=pcall(function()ry:Activate()end)if not di and type(firesignal)=="function"then di=pcall(function()firesignal(ry.Activated)end)end;
if di then task.wait(0.12)oh=rx()if oh and db(oh,k)then return rB(oh,mc)end end end;
if oh and rA(oh)then return rB(oh,mc)end;
if rC()then task.wait(0.18)oh=rx()if oh and rA(oh)then return rB(oh,mc)end end;
if mc and q.fuseSession==mc then aP(q.language=="en"and"FUSE: AT THE STATION • PRESS BACK"or"FUSE: у станка • нажми НАЗАД")return true end;
aP(q.language=="en"and"FUSE: MACHINE UI NOT FOUND"or"FUSE: окно машины не найдено")return false end end;
local rH,rI=oz(nL,"АНТИ-AFK",true,function(jH)q.antiAfkEnabled=jH;
aP("АНТИ-AFK: "..(jH and"включён"or"выключен"))end)rI.Size=UDim2.new(1/5,-4,1,0)rI.Position=UDim2.fromOffset(0,0)q.leverRefs.antiAfk=rH;
local rJ,rK=oz(nL,"KING GYM",false,function(jH,ox)if jH then if q.lockRock then jQ("ФИКСАЦИЯ КАМНЯ: выключена / KING")end;
if q.machineActive then jX(nil)end;
q.lockPosition=false;
q.positionCF=nil;
if q.leverRefs.lockPosition then q.leverRefs.lockPosition.Set(false,true)end;
local D,hg=ji()if not D then ox.Set(false,true)aP("KING ЗОНА: "..tostring(hg))return end;
aP("KING ЗОНА: включена")else jc()aP("KING ЗОНА: выключена")end end)rK.Size=UDim2.new(1/5,-4,1,0)rK.Position=UDim2.new(1/5,1,0,0)q.layoutUI.kingQuickButton=rK;
q.leverRefs.kingLock=rJ;
q.layoutUI.fuseQuickButton=kr(nL,"FUSE",lw.SurfaceAlt)q.layoutUI.fuseQuickButton.Name="FuseMachineQuickButton"q.layoutUI.fuseQuickButton.Size=UDim2.new(1/5,-4,1,0)q.layoutUI.fuseQuickButton.Position=UDim2.new(2/5,2,0,0)q.layoutUI.fuseQuickButton.BackgroundTransparency=0.18;
q.layoutUI.fuseQuickButton.TextSize=9;
q.layoutUI.fuseQuickButton.TextColor3=lw.Accent2;
table.insert(q.layoutUI.themeIcons,q.layoutUI.fuseQuickButton)aJ(q.layoutUI.fuseQuickButton.Activated:Connect(function()if q.fuseOpenInFlight then return end;
q.fuseOpenInFlight=true;
task.spawn(function()local D,hg=pcall(q.layoutUI.openFuseMachine)q.fuseOpenInFlight=false;
if not D then aP("FUSE: "..tostring(hg))end end)end))q.layoutUI.saveSessionQuickButton=kr(nL,"СОХРАНИТЬ",lw.SurfaceAlt)q.layoutUI.saveSessionQuickButton.Name="SaveSessionQuickButton"q.layoutUI.saveSessionQuickButton.Size=UDim2.new(1/5,-4,1,0)q.layoutUI.saveSessionQuickButton.Position=UDim2.new(3/5,3,0,0)q.layoutUI.saveSessionQuickButton.BackgroundTransparency=0.18;
q.layoutUI.saveSessionQuickButton.TextSize=7;
q.layoutUI.saveSessionQuickButton.TextColor3=lw.Success;
table.insert(q.layoutUI.themeIcons,q.layoutUI.saveSessionQuickButton)aJ(q.layoutUI.saveSessionQuickButton.Activated:Connect(function()q.layoutUI.saveLastSession()end))q.layoutUI.resumeSessionQuickButton=kr(nL,"ВОЗОБНОВИТЬ",lw.SurfaceAlt)q.layoutUI.resumeSessionQuickButton.Name="ResumeSessionQuickButton"q.layoutUI.resumeSessionQuickButton.Size=UDim2.new(1/5,-4,1,0)q.layoutUI.resumeSessionQuickButton.Position=UDim2.new(4/5,4,0,0)q.layoutUI.resumeSessionQuickButton.BackgroundTransparency=0.18;
q.layoutUI.resumeSessionQuickButton.TextSize=7;
q.layoutUI.resumeSessionQuickButton.TextColor3=lw.Accent2;
table.insert(q.layoutUI.themeIcons,q.layoutUI.resumeSessionQuickButton)aJ(q.layoutUI.resumeSessionQuickButton.Activated:Connect(function()q.layoutUI.resumeLastSession()end))local rL=oC(q.layoutUI.systemNetworkBody,"◌","ЗАЩИТА СЕТИ","автопауза при плохой сети",true,function(jH)q.netGuardEnabled=jH;
if not jH then q.manualNetworkHold=false;
if q.leverRefs.wifiHold then q.leverRefs.wifiHold.Set(false,true)end;
dL(os.clock(),"OFF")aP("ЗАЩИТА СЕТИ: выключена")else q.networkBadSamples=0;
q.networkGoodSamples=0;
aP("ЗАЩИТА СЕТИ: включена")end end)q.leverRefs.netGuard=rL;
local rM=oC(q.layoutUI.systemNetworkBody,"◌","ПАУЗА СЕТИ","удерживает клиент",false,function(jH)q.manualNetworkHold=jH;
...
local cl=math.clamp(math.floor(gV+0.5),1,1e15)q.rebirthGoal=cl;
q.rebirthGoalCompleted=false;
rX.Text=("%.0f"):format(cl)ci(q.rebirthGoalCurrent)if q.rebirthGoalEnabled then q.rebirthToken=q.rebirthToken+1;
ii()q.nextRebirth=0;
local H,o,io=iq()q.rebirthGoalCurrent=H and math.max(0,math.floor(H+0.5))or nil;
if not io then ci(nil,"Точный счётчик Rebirths не найден • запрос на паузе")aP("НОВАЯ ЦЕЛЬ: ЖДУ ТОЧНЫЙ СЧЁТЧИК REBIRTHS")elseif H~=nil and H>=cl then ik(H)else ci(H)aP("НОВАЯ ЦЕЛЬ РЕБИРТОВ: "..ch(cl))end end;
return true end;
local function s5(jH,ox)if jH then local s6=ig()if not s6 or not s6:IsA("RemoteFunction")then ox.Set(false,true)aP("ЦЕЛЬ РЕБИРТОВ: ТОЧНЫЙ REMOTEFUNCTION НЕ НАЙДЕН")return end;
if not s4()then ox.Set(false,true)return end;
local H,o,io=iq()if H==nil or not io then ox.Set(false,true)q.rebirthGoalCurrent=nil;
ci(nil,"Точный счётчик Rebirths не найден • лимит не запущен")aP("ЦЕЛЬ РЕБИРТОВ: ТОЧНЫЙ REBIRTHS.VALUE НЕ НАЙДЕН")return end;
if H>=q.rebirthGoal then ik(H)return end;
q.rebirthToken=q.rebirthToken+1;
q.rebirthGoalEnabled=true;
q.rebirthGoalCompleted=false;
q.rebirthGoalCurrent=math.max(0,math.floor(H+0.5))ii()q.autoRebirth=true;
q.nextRebirth=0;
if s2 then s2.Set(true,true)end;
...
ci(nil)s2=oC(rR,"↻","АВТОРЕБИРТ","работает без лимита",false,function(jH,ox)if jH and not ig()then ox.Set(false,true)aP("РЕБИРТ: функция игры не найдена")return end;
if jH then q.rebirthToken=q.rebirthToken+1;
q.rebirthGoalEnabled=false;
q.rebirthGoalCompleted=false;
ii()q.autoRebirth=true;
q.nextRebirth=0;
if q.leverRefs.rebirthGoal then q.leverRefs.rebirthGoal.Set(false,true)end;
ci(q.rebirthGoalCurrent)aP("АВТО РЕБИРТ: включён без лимита")else ij("АВТО РЕБИРТ: выключен",false)end end)q.leverRefs.autoRebirth=s2;
local s7=oI(rT,"РАЗМЕР ПЕРСОНАЖА","значение от 0.1 до 1000",1,function(N)q.sizeTarget=N;
if q.autoSize then q.nextSize=0 end;
aP("РАЗМЕР: "..tostring(N))end)q.layoutUI.sizeInput=s7;
local s8=oC(q.layoutUI.rebSupportBody,"◫","ФИКС. РАЗМЕР","держит нужный размер",false,function(jH,ox)if jH and not iC()then ox.Set(false,true)aP("РАЗМЕР: функция игры не найдена")return end;
q.sizeTarget=s7.Get()q.autoSize=jH;
q.nextSize=0;
aP(("АВТО РАЗМЕР: %s | %s"):format(jH and"включён"or"выключен",tostring(q.sizeTarget)))end)q.leverRefs.autoSize=s8;
local s9,sa=oj(nX,"АВТОАТАКА",106,3)s9.LayoutOrder=2;
local sb,sc=oq(nX,"КОГО АТАКОВАТЬ",141)sb.LayoutOrder=1;
q.leverRefs.kill={}local function sd(se)if q.mode then jO("ОСНОВНОЙ РЕЖИМ ОСТАНОВЛЕН / АВТОКИЛ")end;
if q.machineActive then jX(nil)end;
for gp,gy in pairs(q.leverRefs.kill)do if gp~=se and gy then gy.Set(false,true)end end end;
local sf;
sf=oC(sa,"✦","ВСЕ ИГРОКИ","без исключений",false,function(jH,ox)if jH then sd("all")if not gz("all")then ox.Set(false,true)end else if q.killMode=="all"then gw("АВТОКИЛ: выключен")end end end)q.leverRefs.kill.all=sf;
local sg;
sg=oC(sa,"♢","КРОМЕ ДРУЗЕЙ","пропускает список",false,function(jH,ox)if jH then sd("whitelist")if not gz("whitelist")then ox.Set(false,true)end else if q.killMode=="whitelist"then gw("АВТОКИЛ: выключен")end end end)q.leverRefs.kill.whitelist=sg;
local sh;
sh=oC(sa,"◎","ТОЛЬКО ЦЕЛИ","чёрный список",false,function(jH,ox)if jH then sd("blacklist")if not gz("blacklist")then ox.Set(false,true)end else if q.killMode=="blacklist"then gw("АВТОКИЛ: выключен")end end end)q.leverRefs.kill.blacklist=sh;
local function si()local pH={}for o,go in ipairs(a:GetPlayers())do if go~=j then table.insert(pH,{id=tostring(go.UserId),label=go.DisplayName,sub="@"..go.Name.."  •  ID "..tostring(go.UserId),player=go})end end;
table.sort(pH,function(az,aA)return string.lower(az.label)<string.lower(aA.label)end)return pH end;
local sj;
sj=oY(sc,"ЗАЩИЩЕНЫ","0 игроков",function()pG("ЗАЩИЩЁННЫЕ ИГРОКИ",si(),{multiple=true,selected=q.killWhitelist,onDone=function(hR)q.killWhitelist={}for e7,fF in pairs(hR)do if fF then q.killWhitelist[tonumber(e7)or e7]=true end end;
fG()aP("БЕЛЫЙ СПИСОК: "..fC(q.killWhitelist))end})end)local sk;
sk=oY(sc,"ЦЕЛИ","0 игроков",function()pG("ЦЕЛИ АВТОКИЛА",si(),{multiple=true,selected=q.killBlacklist,onDone=function(hR)q.killBlacklist={}for e7,fF in pairs(hR)do if fF then q.killBlacklist[tonumber(e7)or e7]=true end end;
fG()aP("ЦЕЛЕЙ АВТОКИЛА: "..fC(q.killBlacklist))end})end)local sl=mt(sc,"Защищённых скрипт пропускает. Цели — отдельный режим атаки.",8,Enum.Font.Gotham,lw.Muted)sl.Size=UDim2.new(1,0,0,32)sl.TextXAlignment=Enum.TextXAlignment.Center;
aH("pet")aH("aura")do local sm=fK(true)local sn=false;
for o,u in ipairs(sm)do if string.lower(u)==string.lower(tostring(q.selectedCrystal or""))then sn=true;
break end end;
if not sn and#sm>0 then q.selectedCrystal=sm[1]end end;
local so,sp=oj(nY,"ОТКРЫТИЕ / ПОКУПКА",106,3)so.LayoutOrder=2;
local sq,sr=oq(nY,"ВЫБОР ТОВАРА",258)sq.LayoutOrder=1;
q.leverRefs.crystal={}local function ss(se)for gp,gy in pairs(q.leverRefs.crystal)do if gp~=se and gy then gy.Set(false,true)end end end;
local st;
st=oC(sp,"C","АВТОКРИСТАЛЛ","режимы ×1 / ×3 / ×10",false,function(jH,ox)if jH then ss("crystal")if not i5("crystal")then ox.Set(false,true)end else if q.crystalMode=="crystal"then i4("КРИСТАЛЛЫ: выключено")end end end)q.leverRefs.crystal.crystal=st;
local su;
su=oC(sp,"D","АВТОУДАЛЕНИЕ","только отмеченные питомцы",false,function(jH,ox)if jH then local e4,h7=hY()if not e4 then ox.Set(false,true)aP("АВТОУДАЛЕНИЕ: "..tostring(h7))return end;
aP("АВТОУДАЛЕНИЕ: только отмеченные питомцы")else q.petCleanupEnabled=false;
q.petCleanupToken=(q.petCleanupToken or 0)+1;
aP("АВТОУДАЛЕНИЕ: выключено")end end)q.leverRefs.petCleanup=su;
local sv;
sv=oC(sp,"P","КУПИТЬ ПЕТА","покупка за гемы",false,function(jH,ox)if jH then ss("pet")if not i5("pet")then ox.Set(false,true)end else if q.crystalMode=="pet"then i4("АВТОПИТОМЕЦ: выключен")end end end)q.leverRefs.crystal.pet=sv;
q.leverRefs.petEvolve=oC(sp,"E","АВТОЭВОЛЮЦИЯ","5 одинаковых купленных петов",true,function(jH)q.autoEvolvePurchasedPets=jH;
q.petEvolvePending={}q.petEvolveLastError=nil;
if jH and q.crystalMode=="pet"and q.selectedPet then task.spawn(function()q.autoEvolvePurchasedPet(q.selectedPet)end)end;
aP(jH and"АВТОЭВОЛЮЦИЯ: включена"or"АВТОЭВОЛЮЦИЯ: выключена")end)local sw;
sw=oC(sp,"A","КУПИТЬ АУРУ","покупка за гемы",false,function(jH,ox)if jH then ss("aura")if not i5("aura")then ox.Set(false,true)end else if q.crystalMode=="aura"then i4("АВТОАУРА: выключена")end end end)q.leverRefs.crystal.aura=sw;
local sx;
local sy;
local sz;
local sA;
sA=oY(sr,"КРИСТАЛЛ",q.selectedCrystal,function()local pH={}for o,u in ipairs(fK(true))do table.insert(pH,{id=u,label=q.layoutUI.officialName(u,q.layoutUI.gameObjectContext(u)),sub=q.language=="en"and("game crystal • %.2f sec delay"):format(q.purchaseDelay)or("кристалл из игры • КД %.2f сек"):format(q.purchaseDelay)})end;
pG("КРИСТАЛЛЫ ИЗ ИГРЫ",pH,{selected={[q.selectedCrystal]=true},onDone=function(pw)i4(nil)q.selectedCrystal=pw.id;
sz()fG()aP("КРИСТАЛЛ: "..q.selectedCrystal)end})end)local sB=Instance.new("Frame")sB.Parent=sr;
sB.Size=UDim2.new(1,0,0,32)sB.BackgroundColor3=lw.Surface;
sB.BackgroundTransparency=0.20;
sB.BorderSizePixel=0;
mg(sB,7)mq(sB,1,0.66)local sC=mt(sB,"ОТКРЫТЬ",8,Enum.Font.GothamBlack,lw.Accent2)sC.Size=UDim2.new(0.38,-7,1,0)sC.Position=UDim2.fromOffset(7,0)local sD={}local function sE()local hR=tonumber(q.crystalAmount)or 1;
for gI,mf in pairs(sD)do local ht=gI==hR;
mf.BackgroundColor3=ht and lw.Success or lw.SurfaceAlt;
mf.BackgroundTransparency=ht and 0.04 or 0.12;
mf.TextColor3=ht and lw.Bg or lw.Text;
local ms=mf:FindFirstChild("NeonEdge")if ms then ms.Color=ht and lw.Success or lw.Border;
ms.Transparency=ht and 0.15 or 0.72 end end end;
for ay,gI in ipairs({1,3,10})do local mf=kr(sB,"×"..tostring(gI),lw.SurfaceAlt)mf.Size=UDim2.new(0.19,-3,0,26)mf.Position=UDim2.new(0.39+(ay-1)*0.20,0,0,3)mf.TextSize=11;
sD[gI]=mf;
aJ(mf.Activated:Connect(function()q.crystalAmount=gI;
sE()aP("КРИСТАЛЛЫ: открытие ×"..tostring(gI))end))end;
sE()q.layoutUI.purchaseDelayInput=oI(sr,"КД ПОКУПКИ","от 0.05 до 5 секунд",q.purchaseDelay,function(N)q.purchaseDelay=N;
aP(("КД ПОКУПКИ: %.2f сек"):format(N))end,0.05,5)sy=function()local fE=fC(q.petCleanupTargets)if fE==0 then return q.layoutUI.staticText("ВЫБРАТЬ")end;
if fE==1 then for u,fF in pairs(q.petCleanupTargets or{})do if fF then return q.layoutUI.officialName(u,q.layoutUI.gameObjectContext(u))end end end;
return tostring(fE)..(q.language=="en"and" selected"or" выбрано")end;
sz=function()local sF={}for o,u in ipairs(g4(q.selectedCrystal))do sF[P(u)]=u end;
local pz={}for u,fF in pairs(q.petCleanupTargets or{})do local cD=sF[P(u)]if fF and cD and not hO(cD)then pz[cD]=true end end;
q.petCleanupTargets=pz;
if fC(pz)==0 and q.petCleanupEnabled then q.petCleanupEnabled=false;
q.petCleanupToken=(q.petCleanupToken or 0)+1;
if q.leverRefs.petCleanup then q.leverRefs.petCleanup.Set(false,true)end end end;
sz()sx=oY(sr,"УДАЛЯТЬ",sy(),function()local pH={}local eL=g4(q.selectedCrystal)if#eL==0 then aP("ПИТОМЦЫ КРИСТАЛЛА НЕ НАЙДЕНЫ: "..tostring(q.selectedCrystal))return end;
for o,u in ipairs(eL)do local m6=hO(u)local g5=q.layoutUI.officialName(q.selectedCrystal,q.layoutUI.gameObjectContext(q.selectedCrystal))table.insert(pH,{id=u,label=q.layoutUI.officialName(u,q.layoutUI.gameObjectContext(u)),sub=m6 and(q.language=="en"and"Apex is protected from deletion"or"Apex защищён от удаления")or(q.language=="en"and"from "..g5 .." • will be deleted"or"из "..g5 .." • будет удаляться"),disabled=m6})end;
pG((q.language=="en"and"PETS • "or"ПИТОМЦЫ • ")..q.layoutUI.officialName(q.selectedCrystal,q.layoutUI.gameObjectContext(q.selectedCrystal)),pH,{multiple=true,selected=q.petCleanupTargets,onDone=function(hR)local sF={}for o,u in ipairs(g4(q.selectedCrystal))do sF[P(u)]=u end;
local pz={}for u,fF in pairs(hR or{})do local cD=sF[P(u)]if fF and cD and not hO(cD)then pz[cD]=true end end;
local function sG(kD)q.petCleanupTargets=kD;
if fC(kD)==0 and q.petCleanupEnabled then q.petCleanupEnabled=false;
q.petCleanupToken=(q.petCleanupToken or 0)+1;
if q.leverRefs.petCleanup then q.leverRefs.petCleanup.Set(false,true)end end;
fG()aP("АВТОУДАЛЕНИЕ: "..sy())end;
if fC(pz)==0 then sG(pz)return end;
pa(pz,sG)end})end)local sH;
local sI;
function q.layoutUI.openShopPicker(ai)if q.shopPickerBusy then aP(q.language=="en"and"SHOP: loading..."or"МАГАЗИН: список загружается...")return end;
q.shopPickerBusy=true;
aP(q.language=="en"and"SHOP: loading list..."or"МАГАЗИН: загружаю список...")task.spawn(function()task.wait()local D,hg=pcall(function()local pH={}for o,av in ipairs(ah(ai))do local ag=av.name;
if q.language~="en"then local y=q.layoutUI.localizationKey(av.name)ag=q.gameLocalizedNames[y]or ag end;
table.insert(pH,{id=av.name,label=ag,sub=(ai=="pet"and av.isNew and(q.language=="en"and"NEW • "or"НОВЫЙ • ")or"")..(q.language=="en"and"Direct purchase • "or"Прямая покупка • ")..aE(av.price)})end;
if not q.alive then return end;
local hR=ai=="aura"and q.selectedAura or q.selectedPet;
pG(ai=="aura"and"АУРА ЗА ГЕМЫ • БЕЗ РУЛЕТКИ"or"ПЕТ ЗА ГЕМЫ • БЕЗ РУЛЕТКИ",pH,{selected={[tostring(hR or"")]=true},onDone=function(pw)i4(nil)if ai=="aura"then q.selectedAura=pw.id else q.selectedPet=pw.id end;
fG()aP((ai=="aura"and"АУРА ИЗ МАГАЗИНА: "or"ПЕТ ИЗ МАГАЗИНА: ")..tostring(pw.id))end})end)q.shopPickerBusy=false;
if not D then aP("МАГАЗИН: "..tostring(hg):sub(1,90))end end)end;
sH=oY(sr,"ПИТОМЕЦ",q.selectedPet or"ВЫБРАТЬ",function()q.layoutUI.openShopPicker("pet")end)sI=oY(sr,"АУРА",q.selectedAura or"ВЫБРАТЬ",function()q.layoutUI.openShopPicker("aura")end)q.layoutUI.crystalSelection=sA;
q.layoutUI.cleanupSelection=sx;
q.layoutUI.petSelection=sH;
q.layoutUI.auraSelection=sI;
...
oc()if q.hologramContent then q.hologramContent:Layout()end end;
local function sQ(nl,ht)nl.BackgroundColor3=ht and lw.SurfaceAlt or lw.Surface;
nl.BackgroundTransparency=ht and 0.34 or 1;
nl.TextColor3=ht and lw.Text or lw.Muted;
local nn=nl:FindFirstChild("ActiveMark")if nn then nn.Visible=ht end;
local nm=nl:FindFirstChild("TabStroke")if nm then nm.Color=ht and lw.Accent or lw.Border;
nm.Transparency=ht and 0.68 or 1 end end;
local function sR(u)local sS=sK;
sK=u;
if q.layoutUI.appearance.rememberTab then q.layoutUI.lastActiveTab=u;
if sS~=u then q.layoutUI.queueMemoryWrite()end end;
local sT=u=="bug"local sU=u=="farm"local sV=u=="train"local sW=u=="reb"local sX=u=="kill"local k5=u=="crystal"local ku=u=="egg"local r4=u=="teleport"local hq=u=="quest"local sY=u=="system"local sZ=u=="interface"nh.Visible=not sL;
nI.Visible=not sL;
q.layoutUI.interfaceSideTab.Visible=not sL;
nb.Text="RB"nb.TextSize=12;
nT.Visible=not sL and sT;
nU.Visible=not sL and sU;
nV.Visible=not sL and sV;
...
nG.Visible=sT or sU;
if q.language=="en"then nG.Text=sU and"↻ GYMS"or"↻ ROCKS"else nG.Text=sU and"↻ ЗАЛЫ"or"↻ КАМНИ"end;
if sU and not q.machineScanned and not q.machineScanInFlight then q.refreshMachineCatalog(false)end;
if r4 and not q.machineScanned and not q.machineScanInFlight then q.refreshMachineCatalog(false)end;
...
for ay=#q.layoutUI.localizationNodes,1,-1 do local av=q.layoutUI.localizationNodes[ay]if av.node and av.node.Parent then av.node.Text=q.layoutUI.staticText(av.source)else table.remove(q.layoutUI.localizationNodes,ay)end end;
if q.layoutUI.refreshFuseQuickButton then q.layoutUI.refreshFuseQuickButton()end;
if q.layoutUI.saveSessionQuickButton then q.layoutUI.saveSessionQuickButton.Text=q.layoutUI.staticText("СОХРАНИТЬ")end;
...
if q.refreshRockList then q.refreshRockList()end;
if q.ui and q.ui.autoRockName and q.selectedRock then local q3=q.layoutUI.rockName(q.selectedRock)if q.autoRockSelection then q.ui.autoRockTitle.Text=q.layoutUI.staticText("АВТОПОДБОР ПО РЕБЁРТАМ")q.ui.autoRockName.Text=q3 ..(cJ=="en"and"  •  pet "or"  •  питомец ")..tostring(q.autoRockCalc and q.autoRockCalc.grade or"-")else q.ui.autoRockTitle.Text=q.layoutUI.staticText("РУЧНАЯ НАСТРОЙКА")q.ui.autoRockName.Text=q3 ..(cJ=="en"and"  •  multiplier "or"  •  множитель ")..(q.selectedRock.multVerified==false and"?"or tostring(q.selectedRock.mult))end end;
ci(q.rebirthGoalCurrent)sR(sK)if mC and q.ui and q.ui.status then q.ui.status.Text=tostring(q.status or"")end;
if not mC then aP(cJ=="en"and"LANGUAGE: ENGLISH"or"ЯЗЫК: РУССКИЙ")end;
if cJ=="ru"and h and not q.gameLocalizationReady and not q.gameLocalizationLoading then q.gameLocalizationLoading=true;
task.spawn(function()local c7=pcall(q.layoutUI.scanOfficialGameNames)local D,lp=pcall(function()return h:GetTranslatorForLocaleAsync("ru")end)if D and lp then q.gameTranslators.ru=lp end;
if not q.gameTranslators.ru then local t1,t2=pcall(function()return h:GetTranslatorForPlayerAsync(j)end)if t1 and t2 then local t3,ln=pcall(function()return t2.LocaleId or h.RobloxLocaleId end)if t3 and string.lower(tostring(ln or"")):sub(1,2)=="ru"then q.gameTranslators.ru=t2 end end end;
q.gameLocalizationReady=c7;
q.gameLocalizationLoading=false;
if q.alive and q.language=="ru"and q.translationToken==fd then q.layoutUI.applyLanguage("ru",true)end end)end end;
function q.layoutUI.showHome()sR("bug")end;
local function t4(eN,eD)local mT=mQ()local nA=eD.X.Offset;
local nB=eD.Y.Offset;
local t5=math.max(4,mT.X-nA-4)local t6=math.max(4,mT.Y-nB-4)return UDim2.fromOffset(math.clamp(eN.X.Offset,4,t5),math.clamp(eN.Y.Offset,4,t6))end;
local function t7(jH)if sL==(jH and true or false)then return end;
sL=jH and true or false;
if sL then sM=n7.Size;
n_.Position=t4(n7.Position,n_.Size)q.layoutUI.animateWindow(false,function()if not q.alive or not sL then return end;
n7.Visible=false;
n_.Visible=true;
nT.Visible=false;
nU.Visible=false;
...
nX.Visible=false;
nY.Visible=false;
q.layoutUI.eggPage.Visible=false;
q.layoutUI.teleportPage.Visible=false;
q.layoutUI.questPage.Visible=false;
q.layoutUI.systemPage.Visible=false;
q.layoutUI.interfacePage.Visible=false;
q.layoutUI.interfaceSideTab.Visible=false end)else n7.Size=sM;
...
-- HOLOGRAM_CONTENT_END
local t8=nil;
local function t9(ta)local mT=mQ()local mX,mY,tb,tc,td,te=mS(mT)local tf=sL and sM or n7.Size;
local nA=ta and td or math.clamp(tf.X.Offset,mX,tb)local nB=ta and te or math.clamp(tf.Y.Offset,mY,tc)local tg=UDim2.fromOffset(nA,nB)sM=tg;
if sL then n_.Position=t4(n_.Position,n_.Size)else n7.Size=tg;
n7.Position=t4(n7.Position,tg)end;
sN()end;
local function th()if t8 then B(function()t8:Disconnect()end)t8=nil end;
local mR=workspace.CurrentCamera;
if mR then t8=mR:GetPropertyChangedSignal("ViewportSize"):Connect(function()task.defer(function()if q.alive then t9(false)end end)end)table.insert(q.connections,t8)end end;
aJ(n7:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()task.defer(sN)end))aJ(workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()th()t9(false)end))th()sN()for o,av in ipairs(q.layoutUI.navigationTabs)do aJ(av.node.Activated:Connect(function()if not q.layoutUI.consumeDraggedTab(av.id)then sR(av.id)end end))end;
aJ(q.layoutUI.interfaceSideTab.Activated:Connect(function()sR("interface")end))aJ(nb.Activated:Connect(function()if q.closePicker then q.closePicker()end;
q.layoutUI.showHome()end))aJ(ng.Activated:Connect(function()if q.closePicker then q.closePicker()end;
...
local tn=nil;
local to=nil;
local function tp(f9)return f9.UserInputType==Enum.UserInputType.MouseButton1 or f9.UserInputType==Enum.UserInputType.Touch end;
aJ(n9.InputBegan:Connect(function(f9)if tp(f9)then local tq=f9.Position.X-n7.AbsolutePosition.X;
if tq<n7.AbsoluteSize.X-112 then ti=true;
tm=f9.Position;
tn=n7.Position end end end))aJ(n_.InputBegan:Connect(function(f9)if tp(f9)then tj=true;
tl=false;
tm=f9.Position;
tn=n_.Position end end))aJ(nZ.InputBegan:Connect(function(f9)if tp(f9)then tk=true;
tm=f9.Position;
to=n7.Size end end))aJ(f.InputEnded:Connect(function(f9)if tp(f9)then ti=false;
tj=false;
tk=false end end))aJ(f.InputChanged:Connect(function(f9)if tm and(f9.UserInputType==Enum.UserInputType.MouseMovement or f9.UserInputType==Enum.UserInputType.Touch)then local tr=f9.Position-tm;
...
if self.hologram then pcall(function()self.hologram:Destroy()end)end;
...
q.layoutUI.closing=true;
if q.closePicker then q.closePicker()end;
q.layoutUI.animateWindow(false,function()q:Stop("closed")end)end))aJ(j.CharacterAdded:Connect(function(tw)local tx=q.bugActive;
local ty=q.lockRock;
local tz=q.machineActive and q.selectedMachine or nil;
local tA={}for o,e7 in ipairs(q.trainOrder)do if q.activeTrains[e7]then table.insert(tA,q.activeTrains[e7])end end;
local tB=#tA>0;
local tC=q.lockPosition;
local tD=q.positionCF;
q.respawnGeneration=q.respawnGeneration+1;
local tE=q.respawnGeneration;
q.activeTool=nil;
q.lockCF=nil;
q.positionCF=nil;
q.lockRock=false;
q.lockPosition=false;
if tz then jX(nil)end;
jO((tx or tB or ty or tz)and"RESPAWN: жду персонажа для восстановления"or"RESPAWN: режимы остановлены")local tF=q.modeToken;
if q.leverRefs.lockPosition then q.leverRefs.lockPosition.Set(false,true)end;
if not q.autoResumeAfterRespawn or not tx and not tB and not ty and not tC and not tz then return end;
task.spawn(function()local d1=os.clock()+15;
local tG=nil;
local tH=nil;
repeat if not q.alive or q.respawnGeneration~=tE or q.modeToken~=tF then return end;
tG=tw and tw:FindFirstChild("HumanoidRootPart")tH=tw and tw:FindFirstChildWhichIsA("Humanoid")if tG and tH then break end;
task.wait(0.2)until os.clock()>=d1;
if not tG or not tH then aP("RESPAWN RECOVERY: персонаж не загрузился за 15с")return end;
task.wait(0.8)if not q.alive or q.respawnGeneration~=tE or q.modeToken~=tF then return end;
local tI=not ty;
local tJ=not tx;
local tK=not tB;
local tL=not tz;
if ty then tI=jR()end;
if tx then tJ=jU()end;
if tB then tK=true;
for o,ma in ipairs(tA)do if not jW(ma)then tK=false end end end;
if tz then tL=jZ(tz)end;
local tM=tI and tJ and tK and tL;
local tN=false;
if tC and tD and not q.kingLock and not q.lockPosition then if not q.networkPaused then tG.Anchored=false end;
q.lockPosition=true;
q.positionCF=tD;
q.nextPosTick=0;
tN=true;
if q.leverRefs.lockPosition then q.leverRefs.lockPosition.Set(true,true)end end;
if tM or tN then aP("RESPAWN RECOVERY: режим восстановлен")else aP("RESPAWN RECOVERY: не удалось восстановить предмет")end end)end))q.layoutUI.applyAppearance(true)bE()cH(true)if q.layoutUI.appearance.rememberTab and q.layoutUI.sectionInfo[q.layoutUI.storedLastTab or""]then sR(q.layoutUI.storedLastTab)else q.layoutUI.showHome()end;
q.layoutUI.applyLanguage(q.language or"ru",true)oc()aP((q.language=="en"and"READY • "or"ГОТОВО • ")..tostring(q.selectedRock and q.layoutUI.rockName(q.selectedRock)or(q.language=="en"and"rock not found"or"камень не найден")))q.layoutUI.animateWindow(true)end;
local tO,tP=xpcall(l6,function(hg)local kQ=""if debug and type(debug.traceback)=="function"then kQ="\n"..tostring(debug.traceback())end;
...
-- RockBugHub T38. Bundled into the existing launcher by scripts/build_hologram.py.
...
function HUD.mount(runtime, options)
    ...
    local binding = "RockBugHologramT38_" .. tostring(player.UserId)
    ...
    self.world = create("Model", {Name="RockBugHubHologramT38"})
    ...
    local function panel(id, title, x, y, width, height, canvasHeight)
        ...
        p.frame = create("Frame", {Size=UDim2.fromOffset(p.cw,p.ch), Active=false, BackgroundColor3=Color3.fromRGB(255,255,255), BackgroundTransparency=0, BorderSizePixel=0, ClipsDescendants=true}, p.screenRoot)
    ...
    self.window=create("Frame",{AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-12,0.5,0),Size=UDim2.fromOffset(380,460),BackgroundColor3=Color3.fromRGB(31,65,82),BorderSizePixel=0,Active=true,ClipsDescendants=true,ZIndex=21},self.sheet)
...
q.sessionCommandVersion=0;
q.sessionOnlineCount=0;
q.sessionRemotePaused=false;
q.sessionLeaveSent=false;
function q.sessionExecutorName()local bq,u=pcall(function()if type(identifyexecutor)=="function"then return identifyexecutor()end;
if type(getexecutorname)=="function"then return getexecutorname()end;
return""end)return bq and tostring(u or""):sub(1,80)or""end;
function q.sessionRequest(lU)local m5=nil;
local tQ=pcall(function()m5=q.sessionHttpService:JSONEncode(lU)end)if not tQ or type(m5)~="string"then return nil end;
local tR=nil;
pcall(function()if type(request)=="function"then tR=request elseif type(http_request)=="function"then tR=http_request elseif syn and type(syn.request)=="function"then tR=syn.request elseif fluxus and type(fluxus.request)=="function"then tR=fluxus.request end end)local gL=nil;
if tR then local hz,W=pcall(tR,{Url=q.sessionApiUrl,Method="POST",Headers={["Content-Type"]="application/json",["x-rockbug-key"]=q.sessionApiKey},Body=m5})if hz then gL=type(W)=="table"and(W.Body or W.body)or W end else local tS=q.sessionApiUrl.."?key="..q.sessionApiKey;
local hz,W=pcall(function()return game:HttpPost(tS,m5,"application/json")end)if not hz then hz,W=pcall(function()return q.sessionHttpService:PostAsync(tS,m5,Enum.HttpContentType.ApplicationJson,false)end)end;
if hz then gL=W end end;
if type(gL)~="string"or#gL>65536 then return nil end;
local lV,m0=pcall(function()return q.sessionHttpService:JSONDecode(gL)end)return lV and type(m0)=="table"and m0 or nil end;
function q.sessionState()if q.sessionRemotePaused then return"paused"end;
return q.alive and"running"or"stopped"end;
function q.sessionAck(tT,jl,gx)return q.sessionRequest({action="ack",session_id=q.sessionId,command_version=tonumber(tT)or q.sessionCommandVersion or 0,state=jl or q.sessionState(),error=tostring(gx or""):sub(1,300)})end;
function q.sessionApplyCommand(tU,tT)tT=math.max(0,math.floor(tonumber(tT)or 0))if tT<=q.sessionCommandVersion then return end;
q.sessionCommandVersion=tT;
if tU=="pause"then if not q.sessionRemotePaused then if q.layoutUI and type(q.layoutUI.captureLastSession)=="function"then q.sessionRemoteSnapshot=q.layoutUI.captureLastSession()end;
jT()q.sessionRemotePaused=true;
aP("REMOTE: сессия поставлена на паузу")end;
q.sessionAck(tT,"paused")elseif tU=="resume"then q.sessionRemotePaused=false;
local tV=q.sessionRemoteSnapshot;
q.sessionRemoteSnapshot=nil;
if type(tV)=="table"and q.layoutUI and type(q.layoutUI.resumeLastSession)=="function"then local cf=q.layoutUI.lastSavedSession;
q.layoutUI.lastSavedSession=tV;
q.layoutUI.resumeLastSession()q.layoutUI.lastSavedSession=cf end;
aP("REMOTE: сессия возобновлена")q.sessionAck(tT,"running")elseif tU=="stop"then q.sessionAck(tT,"stopped")task.wait(0.15)q:Stop("remote stop")end end;
function q.sessionHeartbeat()local gL=q.sessionRequest({action="heartbeat",session_id=q.sessionId,player_user_id=j.UserId,player_name=j.Name,display_name=j.DisplayName,version=m,executor_name=q.sessionExecutorName(),place_id=game.PlaceId,job_id=game.JobId,state=q.sessionState(),error=tostring(q.lastError or""):sub(1,300),details={device=f.TouchEnabled and"mobile"or"desktop",locale=tostring(q.localeId or"")}})if type(gL)~="table"then return false end;
q.sessionOnlineCount=math.max(0,math.floor(tonumber(gL.online)or 0))if type(gL.command)=="string"then q.sessionApplyCommand(gL.command,gL.command_version)end;
return true end;
function q.sessionLeave()q.sessionRequest({action="leave",session_id=q.sessionId,state="stopped"})end;
q.sessionHeartbeatToken=(q.sessionHeartbeatToken or 0)+1;
task.spawn(function()local fd=q.sessionHeartbeatToken;
while q.alive and q.sessionHeartbeatToken==fd do pcall(q.sessionHeartbeat)for o=1,12 do if not q.alive or q.sessionHeartbeatToken~=fd then return end;
task.wait(1)end end end)task.spawn(function()local tW={}while q.alive do local D,hg=xpcall(kP,function(j1)local kQ=""if debug and type(debug.traceback)=="function"then kQ="\n"..tostring(debug.traceback())end;
return tostring(j1)..kQ end)if D or not q.alive then return end;
q.lastError=hg;
q.schedulerRestarts=q.schedulerRestarts+1;
local b6=os.clock()local tX={}for o,tY in ipairs(tW)do if b6-tY<=30 then table.insert(tX,tY)end end;
