-- PetAlignHub v42_TREADMILL_CLAMP
...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v42 rock scan")
...
local function runOnTreadmill(part,targetTotal,rarity)
	...
	-- v42: стартуем не на краю, а почти в центре дорожки.
	local startLong=_G.PetAlignTreadStartLong or 0
	local y=part.Position.Y+part.Size.Y/2+heightAbove
	local startPos=Vector3.new(part.Position.X,y,part.Position.Z)+dir*startLong
	...
	local maxLong=math.max(2,length/2-1.8)
	local maxSide=math.max(1,width/2-1.1)
	...
	while not _G.PetAlignStop do
		...
		-- Бежим обычным Humanoid, без Velocity. Дорожка сопротивляется.
		hum:Move(dir,false)
		...
		-- Анти-слёт: не толкаем вперёд, а только держим персонажа ВНУТРИ полотна дорожки.
		-- Если лента уносит за край — возвращаем в границы.
		if os.clock()>=nextClamp then
			nextClamp=os.clock()+0.12
			...
			if math.abs(long)>maxLong or math.abs(sideOff)>maxSide then
				...
				local fixed=Vector3.new(part.Position.X,y,part.Position.Z)+dir*fixedLong+side*fixedSide
...
title.Text="Pet Align Comfort v42"
...
if okMath then
	statusText("v42: бег с удержанием на дорожке, без слёта.")
else
