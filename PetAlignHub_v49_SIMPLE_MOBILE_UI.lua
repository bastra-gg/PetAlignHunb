-- PetAlignHub v49_SIMPLE_MOBILE_UI
...
local function makeRockReport()
	...
	local lines={}
	table.insert(lines,"PetAlign v49 rock scan")
...
mkLabel(top,"Pet Align v49",12,0,190,42,16,Color3.fromRGB(255,255,255),true)
...
function render()
	...
	for i=1,count do
		...
		card.Text=("#%d %s | %s\n%s lvl, %s XP | hit +%s | +%s"):format(
			i,row.rarity,row.rockLabel,fmt(row.setLvl),fmt(row.setXp),fmt(row.hit),fmt(row.bonus)
		)
		if i==selectedCard then
			card.BackgroundColor3=Color3.fromRGB(72,58,145)
		else
...
statusText("v49: простой дизайн, без обрезания и центра.")
render()
