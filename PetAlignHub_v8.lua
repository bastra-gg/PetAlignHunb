...
local function useTreadmill(gain,count)
	...
	if not part then status.Text="Не нашёл дорожку +"..gain..". Нужны реальные названия объектов/Explorer или PetAPI.UseTreadmill." return false end
	for i=1,count do
...
local function hitRock(rockId)
	...
	status.Text="Камень не найден. Нужны реальные названия объектов/Explorer или PetAPI.HitRock."
	return false
end
